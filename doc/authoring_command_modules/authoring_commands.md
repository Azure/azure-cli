...
**Below is a recommended pattern for suppressing specific extensions:**
...
class MyCommandsLoader(AzCommandsLoader):
    ...
    def module_suppress_extension(self, ext):
        """ Return True for the extension that should be suppressed. An optional reason can be provided. """
        from pkg_resources import parse_version
        if ext.name == 'myextension' and ext.version and parse_version(ext.version) <= parse_version('0.5.0'):
            return True, "The commands for myextension have moved inside the CLI. Consider removing the extension."
```
