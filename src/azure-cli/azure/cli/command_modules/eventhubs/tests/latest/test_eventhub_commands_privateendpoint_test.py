...
class EHNetworkCURDScenarioTest(ScenarioTest):
    ...
    def test_eh_privateendpoint(self, resource_group):
        ...
        self.cmd('network vnet subnet update -n {subnet} --vnet-name {vnet} -g test-migration '
                 '--disable-private-endpoint-network-policies true',
                 checks=self.check('privateEndpointNetworkPolicies', 'Disabled'))

        # Create Namespace
        self.cmd(
            'eventhubs namespace create --resource-group test-migration --name {namespacename} --location {loc} --tags {tags}'
            ' --sku {sku}', checks=[self.check('sku.name', '{sku}')])

        # Get Created Namespace
        getnamespace = self.cmd(
            'eventhubs namespace show --resource-group test-migration --name {namespacename}').get_output_in_json()

        # Create a private endpoint connection
        pr = self.cmd('eventhubs namespace private-link-resource show --namespace-name {namespacename} -g test-migration').get_output_in_json()
        self.kwargs['group_id'] = pr[0]['groupId']

        getnamesapce = self.cmd('eventhubs namespace show -n {namespacename} -g test-migration').get_output_in_json()
        ...
        self.cmd('eventhubs namespace private-endpoint-connection show --namespace-name {namespacename} -g test-migration --name {ehn_pec_name}',
                 checks=self.check('id', '{sa_pec_id}'))

        getstatus = self.cmd('eventhubs namespace private-endpoint-connection approve --namespace-name {namespacename} -g test-migration --name {ehn_pec_name}').get_output_in_json()
        self.assertEqual(getstatus['privateLinkServiceConnectionState']['status'], 'Approved')

        getstatus = self.cmd(
            'eventhubs namespace private-endpoint-connection reject --namespace-name {namespacename} -g test-migration --name {ehn_pec_name}').get_output_in_json()
        self.assertEqual(getstatus['privateLinkServiceConnectionState']['status'], 'Rejected')

        getstatus = self.cmd(
            'eventhubs namespace private-endpoint-connection show --namespace-name {namespacename} -g test-migration --name {ehn_pec_name}').get_output_in_json()
        self.assertEqual(getstatus['privateLinkServiceConnectionState']['status'], 'Rejected')
