...
def load_arguments(self, _):
    ...
    with self.argument_context('batch') as c:
        c.argument('resource_group_name', resource_group_name_type, help='Name of the resource group', completer=None, validator=None)
    ...
    with self.argument_context('batch account create') as c:
        c.argument('location', location_type, help='The region in which to create the account.')
    ...
    with self.argument_context('batch location quotas show') as c:
        c.argument('location_name', location_type, help='The region from which to display the Batch service quotas.')
    ...
    for self.argument_context('batch pool create') as c:
    ...
    for self.argument_context('batch certificate') as c:
    ...
    for self.argument_context('batch certificate show') as c:
        c.argument('thumbprint', help='The certificate thumbprint.', validator=validate_cert_settings)

    for self.argument_context('batch task create') as c:
    ...
    for item in ['batch certificate delete', 'batch certificate create', 'batch pool resize', 'batch pool reset', 'batch job list', 'batch task create']:
        for self.argument_context(item):
            c.argument('account_name', arg_group='Batch Account', validator=validate_client_parameters,
                       help='The Batch account name. Alternatively, set by environment variable: AZURE_BATCH_ACCOUNT')
            c.argument('account_key', arg_group='Batch Account',
                       help='The Batch account key. Alternatively, set by environment variable: AZURE_BATCH_ACCESS_KEY')
            c.argument('account_endpoint', arg_group='Batch Account',
