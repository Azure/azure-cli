...
def load_arguments(self, _):
    ...
    stacks_deny_settings_mode = CLIArgumentType(arg_type=get_enum_type(['none', 'denyDelete', 'denyWriteAndDelete']), help='Define which operations are denied on resources managed by the stack.')
    ...
    with self.argument_context('policy exemption', min_api='2020-09-01', resource_type=ResourceType.MGMT_RESOURCE_POLICY) as c:
        ...
        c.argument('exemption_category', options_list=['--exemption-category', '-e'], help='The policy exemption category of the policy exemption')
        ...
        c.argument('expires_on', help='The expiration date and time (in UTC ISO 8601 format yyyy-MM-ddTHH:mm:ssZ) of the polteicy exemption.')
        c.argument('metadata', nargs='+', validator=validate_metadata, help='Metadata in space-separated key=value pairs.')
