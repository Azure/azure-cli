...
class Patch(object):
    ...
    def apply(self, base_dir):
        ...
        dest = os.path.join(base_dir, self.path_to_patch)
...
PATCHES = [
    ...
    Patch(os.path.join('packaged_releases', 'patches', 'patch_component_custom.py'),
          os.path.join('src', 'command_modules', 'azure-cli-component', 'azure', 'cli', 'command_modules', 'component', 'custom.py'))
    ]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--components', '-c', required=True, nargs='+',
                        help="Space separated list in 'component=version' format. (e.g. azure-cli=2.0.0 vm=2.0.0)",
                        type=_type_components_list)
    parser.add_argument('--version', '-v', required=True,
                        help="The version to name the packaged release.")
    parser.add_argument('--dest', '-d',
                        help="The destination directory to place the archive. Defaults to current directory.")
    args = parser.parse_args()
    create_packaged_archive(args.version, args.components, args.dest)
