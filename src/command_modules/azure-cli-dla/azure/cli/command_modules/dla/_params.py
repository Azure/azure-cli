...
register_cli_argument('dla', 'skip', help='The number of items to skip over before returning elements. Optional.', type=int)
register_cli_argument('dla', 'count', help='The Boolean value of true or false to request a count of the matching resources included with the resources in the response, e.g. Categories?$count=true. Optional.', type=bool)
...
register_cli_argument('dla account update', 'firewall_state', help='Optionally enable/disable existing firewall rules.', **enum_choice_list(FirewallState))
register_cli_argument('dla account update', 'allow_azure_ips', help='Optionally allow/block Azure originating IPs through the firewall', **enum_choice_list(FirewallAllowAzureIpsState))
...
# Job params
# pylint: disable=line-too-long
register_cli_argument('dla job submit', 'compile_mode', help='Optionally indicates the type of compilation to be done on this job. Valid values are: \'Semantic\' (Only performs semantic checks and necessary sanity checks), \'Full\' (full compilation) and \'SingleBox\' (Full compilation performed locally)', **enum_choice_list(CompileMode))
...
register_cli_argument('dla job list', 'submitted_after', help='An optional filter which returns jobs only submitted after the specified time, in ISO-8601 format.', type=datetime_format)
register_cli_argument('dla job list', 'submitted_before', help='An optional filter which returns jobs only submitted before the specified time, in ISO-8601 format.', type=datetime_format)
register_cli_argument('dla job list', 'state', help='An optional filter which returns jobs with only the specified state(s).', nargs='*', **enum_choice_list(JobState))
register_cli_argument('dla job list', 'result', help='An optional filter which returns jobs with only the specified result(s).', nargs='*', **enum_choice_list(JobResult))
register_cli_argument('dla job list', 'submitter', help='An optional filter which returns jobs only by the specified submitter.')
register_cli_argument('dla job list', 'name', help='An optional filter which returns jobs only by the specified friendly name.')
