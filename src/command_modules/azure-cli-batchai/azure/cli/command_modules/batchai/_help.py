...
examples:
-   name: Make a cluster to auto scale between 0 and 10 nodes depending on number
        of queued and running jobs.
    text: az batchai cluster auto-scale -g MyResourceGroup -w MyWorkspace -n MyCluster
        --min 0 --max 10
"""
...
-   name: Create a cluster with a setup command which installs unzip on every node,
        the command output will be stored on auto storage account Azure File Share.
    text: |
        az batchai cluster create -g MyResourceGroup -w MyWorkspace -n MyCluster \\
            --use-auto-storage \\
...
-   name: Create a cluster using a configuration file.
    text: >
        az batchai cluster create -g MyResourceGroup -w MyWorkspace -n MyCluster -f
        cluster.json
"""
...
long-summary: List files generated by the cluster's node setup task under $AZ_BATCHAI_STDOUTERR_DIR
    path. This functionality is available only if the node setup task output directory
    is located on mounted Azure File Share or Azure Blob Container.
examples:
...
-   name: List names, sizes and download URLs for files and directories inside of
        $AZ_BATCHAI_STDOUTERR_DIR.
    text: |
        az batchai cluster file list -g MyResourceGroup -w MyWorkspace -c MyCluster
-   name: List names, sizes and download URLs for files and directories inside of
        $AZ_BATCHAI_STDOUTERR_DIR/folder/subfolder.
    text: |
        az batchai cluster file list -g MyResourceGroup -w MyWorkspace -c MyCluster \\
            -p folder/subfolder
-   name: List names, sizes and download URLs for files and directories inside of
        $AZ_BATCHAI_STDOUTERR_DIR making download URLs to remain valid for one hour.
    text: |
        az batchai cluster file list -g MyResourceGroup -w MyWorkspace -c MyCluster \\
            --expiry 60
"""
...
long-summary: "List remote login information for cluster nodes. You can ssh to a particular\
    \ node using the provided public IP address and the port number.\nE.g. ssh <admin\
    \ user name>@<public ip> -p <node's SSH port number>"
examples:
-   name: List remote login information for a cluster.
    text: az batchai cluster node list -g MyResourceGroup -w MyWorkspace -c MyCluster
        -o table
"""
...
-   name: Resize a cluster to zero size to stop paying for it.
    text: az batchai cluster resize -g MyResourceGroup -w MyWorkspace -n MyCluster
        -t 0
...
-   name: Show cluster's summary.
    text: az batchai cluster show -g MyResourceGroup -w MyWorkspace -n MyCluster -o
        table
-   name: Show information about a cluster.
    text: az batchai cluster show --output json --workspace MyWorkspace --name MyCluster
...
-   name: Create an experiment.
    text: az batchai experiment create --workspace MyWorkspace --name MyExperiment
        --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Delete an experiment without asking for confirmation (for non-interactive
        scenarios).
    text: az batchai experiment delete -g MyResourceGroup -w MyWorkspace -e MyExperiment
        -y
...
-   name: Show information about an experiment.
    text: az batchai experiment show -g MyResourceGroup -w MyWorkspace -n MyExperiment
        -o table
"""
...
-   name: Create a NFS file server using a configuration file.
    text: az batchai file-server create -g MyResourceGroup -w MyWorkspace -n MyNFS
        -c nfs.json
...
-   name: Delete file server without asking for confirmation (for non-interactive
        scenarios).
    text: az batchai file-server delete -g MyResourceGroup -w MyWorkspace -n MyNFS
        -y
...
-   name: Show file server summary.
    text: az batchai file-server show -g MyResourceGroup -w MyWorkspace -n MyNFS -o
        table
"""
...
-   name: Create a job to run on a cluster in the same resource group.
    text: |
        az batchai job create -g MyResourceGroup -w MyWorkspace -e MyExperiment -n MyJob     -r MyCluster -f job.json
-   name: Create a job to run on a cluster in a different workspace.
    text: |
        az batchai job create -g MyJobResourceGroup -w MyJobWorkspace -e MyExperiment -n MyJob     -f job.json     -r "/subscriptions/00000000-0000-0000-0000-000000000000/"    "resourceGroups/MyClusterResourceGroup"    "/providers/Microsoft.BatchAI/workspaces/MyClusterWorkspace/clusters/MyCluster"
-   name: Create a job.
    text: az batchai job create --config-file {config-file} --cluster {cluster} --storage-account-name
        MyStorageAccount --name MyJob --resource-group MyResourceGroup --workspace
        {workspace} --experiment {experiment}
    crafted: 'True'
"""
...
-   name: Delete a job. The job will be terminated if it's currently running.
    text: az batchai job delete -g MyResourceGroup -w MyWorkspace -e MyExperiment
        -n MyJob
...
long-summary: List job's output files in a directory with given id if the output directory
    is located on mounted Azure File Share or Blob Container.
examples:
...
-   name: List files in the standard output directory of the job. Generates output
        in a table format.
    text: |
        az batchai job file list -g MyResourceGroup -w MyWorkspace -e MyExperiment -j MyJob -o table
-   name: List files in a folder 'MyFolder/MySubfolder' of an output directory with
        id 'MODELS'.
    text: |
        az batchai job file list -g MyResourceGroup -w MyWorkspace -e MyExperiment -j MyJob \\
            -d MODELS -p MyFolder/MySubfolder
-   name: List files in the standard output directory of the job making download URLs
        to remain valid for 15 minutes.
    text: |
        az batchai job file list -g MyResourceGroup -w MyWorkspace -e MyExperiment -j MyJob \\
            --expiry 15
"""
...
long-summary: Waits for the job to create the given file and starts streaming it similar
    to 'tail -f' command. The command completes when the job finished execution.
examples:
-   name: Stream standard output file of the job.
    text: |
        az batchai job file stream -g MyResourceGroup -w MyWorkspace -e MyExperiment -j MyJob     -f stdout.txt
-   name: Stream a file 'log.txt' from a folder 'logs' of an output directory with
        id 'OUTPUTS'.
    text: |
        az batchai job file stream -g MyResourceGroup -w MyWorkspace -e MyExperiment -j MyJob     -d OUTPUTS -p logs -f log.txt
-   name: Stream the content of a file (similar to 'tail -f').
    text: az batchai job file stream --file-name MyFile --job {job} --workspace {workspace}
        --experiment {experiment} --resource-group MyResourceGroup
    crafted: 'True'
"""
...
examples:
-   name: List jobs.
    text: az batchai job list -g MyResourceGroup -w MyWorkspace -e MyExperiment -o
        table
-   name: List jobs.
    text: az batchai job list --resource-group MyResourceGroup --output json --experiment
        MyExperiment --workspace MyWorkspace
    crafted: 'True'
"""
...
type: command
short-summary: Executes a command line on a cluster's node used to execute the job
    with optional ports forwarding.
examples:
...
long-summary: "List remote login information for currently existing (not deallocated)\
    \ nodes on which the job was executed. You can ssh to a particular node using\
    \ the provided public IP address and the port number.\nE.g. ssh <admin user name>@<public\
    \ ip> -p <node's SSH port number>"
examples:
-   name: List remote login information for a job nodes.
    text: az batchai job node list -g MyResourceGroup -w MyWorkspace -e MyExperiment
        -j MyJob -o table
"""
...
-   name: Show full information about a job.
    text: az batchai job show -g MyResourceGroup -w MyWorkspace -e MyExperiment -n
        MyJob
...
-   name: Terminate a job and wait for the job to be terminated.
    text: az batchai job terminate -g MyResourceGroup -w MyWorkspace -e MyExperiment
        -n MyJob
...
type: command
short-summary: Waits for specified job completion and setups the exit code to the
    job's exit code.
examples:
-   name: Wait for the job completion.
    text: |
        az batchai job wait -g MyResourceGroup -w MyWorkspace -n MyJob
-   name: Waits for specified job completion and setups the exit code to the job's
        exit code.
    text: az batchai job wait --resource-group MyResourceGroup --workspace MyWorkspace
        --experiment {experiment} --name MyJob
    crafted: 'True'
"""

helps["batchai list-usages"] = """
type: command
short-summary: Gets the current usage information as well as limits for Batch AI resources
    for given location.
examples:
