...
class AzureSearchQueryKeysTests(ScenarioTest):
    ...
    def test_query_key_create_delete_list(self, resource_group):
        ...
        _querykeys = self.cmd('az search query-key list --search-service-name {name} -g {rg}').get_output_in_json()
        self.assertTrue(len(_querykeys) == 1)
        self.assertTrue(_querykeys[0]['name'] is None)
        self.assertTrue(len(_querykeys[0]['key']) > 1)

        _querykey_1 = self.cmd('az search query-key create --search-service-name {name} -g {rg} -n {key_name_1}',
        ...
        _querykey_2 = self.cmd('az search query-key create --search-service-name {name} -g {rg} -n {key_name_2}',
        ...
        self.cmd('az search query-key delete --search-service-name {name} -g {rg} --key {key_value_1}')

        _querykeys = self.cmd('az search query-key list --search-service-name {name} -g {rg}').get_output_in_json()
        self.assertFalse(_querykey_1['name'] in [x['key'] for x in _querykeys])

        self.cmd('az search query-key delete --search-service-name {name} -g {rg} --key {key_value_2}')
