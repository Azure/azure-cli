...
class Create(AAZCommand):
    ...
    :example: Create an authorization certificate.
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        _args_schema.name = AAZStrArg(
            options=["-n", "--name"],
            help="Name of the authentication certificate.",
            required=True,
        )

        # define Arg Group "Parameters.properties.authenticationCertificates[]"
    ...
    class SubresourceSelector(AAZJsonSelector):
        ...
        def _get(self):
            ...
            result = result.properties.authenticationCertificates
...
@register_command(
    "network application-gateway auth-cert delete",
)
class Delete(AAZCommand):
    """Delete an authorization certificate.

    :example: Delete an authorization certificate.
...
@register_command(
    "network application-gateway auth-cert list",
)
class List(AAZCommand):
    """List authorization certificates.

    :example: List authorization certificates.
    ...
    class SubresourceSelector(AAZJsonSelector):
        ...
        def _get(self):
            result = self.ctx.vars.instance
            return result.properties.authenticationCertificates

        def _set(self, value):
            result = self.ctx.vars.instance
            result.properties.authenticationCertificates = value
            return
...
@register_command(
    "network application-gateway auth-cert show",
)
class Show(AAZCommand):
    """Show an authorization certificate.

    :example: Show an authorization certificate.
        az network application-gateway auth-cert show -g MyResourceGroup --gateway-name MyAppGateway -n MyAuthCert

    :example: View expiry date of an authorization certificate. It is in Base-64 encoded X.509 (.CER) format.
...
class Update(AAZCommand):
    ...
    :example: Update authorization certificates to use a new cert file.
...
@register_command(
    "network application-gateway auth-cert wait",
)
