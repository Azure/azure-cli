        ...
        for scope in ['create', 'show', 'list', 'update', 'check-name-availability']:
            ...
            with self.argument_context(argument_context_string) as c:
                ...
                if scope == "create":
                    ...
                    c.argument('tags', tags_type)
                    c.argument('location', arg_type=get_location_type(self.cli_ctx))
                    c.argument('migrationInstanceResourceId', options_list=['--migrationRuntimeResourceId'], required=False,
                            help='For private endpoint migrations, the ID will be responsible for migrating data between source and target server.')
                elif scope == "show":
                    c.argument('migration_name', arg_type=migration_id_arg_type, options_list=['--migration-name'],
                               help='Name of the migration.')
                elif scope == "list":
