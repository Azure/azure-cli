...
def load_command_table(self, _):
    ...
    with self.command_group('network lb', network_lb_sdk) as g:
        if self.cli_ctx.cloud.profile == 'latest':
            g.custom_command('create', 'create_load_balancer_latest', supports_no_wait=True, validator=process_lb_create_namespace)
            g.custom_command('list-mapping', 'list_load_balancer_mapping')
        else:
            g.show_command('show', 'get')
            g.custom_command('create', 'create_load_balancer',
                             transform=DeploymentOutputLongRunningOperation(self.cli_ctx), supports_no_wait=True,
                             table_transformer=deployment_validate_table_format, validator=process_lb_create_namespace,
                             exception_handler=handle_template_based_exception)
            g.command('delete', 'begin_delete')
