...
class DeploymentStacksTest(ScenarioTest):
    ...
    def test_create_deployment_stack_subscription(self, resource_group):
        ...
        self.cmd('stack sub create --name {name} --location {location} --deployment-resource-group {resource-group-two} --deny-settings-mode "none" --template-file "{template-file-spec}" --action-on-unmanage detachAll --parameters "name={resource-two}" --yes', checks=self.check('provisioningState', 'succeeded'))
        ...
        # update stack with resource3 set to delete
        self.cmd('stack sub create --name {name} --location {location} --template-file "{template-file-rg}" --parameters "name={resource-three}" --deny-settings-mode "none" --aou deleteResources --aou deleteResources --yes', checks=self.check('provisioningState', 'succeeded'))
    ...
    def test_delete_deployment_stack_resource_group(self, resource_group):
        ...
        # delete stack with resource2 set to delete
        self.cmd('stack group delete -g {resource-group-two} --name {name} --action-on-unmanage deleteResources --yes')
    ...
    def test_create_deployment_stack_management_group(self, resource_group):
        ...
        self.cmd('stack mg create --name {name} --management-group-id {mg} --location {location} --template-spec "{template-spec-id}" --deny-settings-mode "none" --action-on-unmanage detachAll --parameters "{parameter-file}" --description "MG stack deployment" --deployment-subscription {subscription}', checks=self.check('provisioningState', 'succeeded'))
        ...
        # test delete flag --delete-resource-groups - create stack  with resource1
        self.cmd('stack mg create --name {name} --management-group-id {mg} --location {location} --template-file "{template-file-rg}" --deployment-subscription {subscription} --deny-settings-mode "none" --parameters "name={resource-one}" --aou deleteResources --tags "tag1 tag2"', checks=self.check('provisioningState', 'succeeded'))
    ...
    def test_delete_deployment_stack_management_group(self):
        ...
        # create stack
        self.cmd('stack mg create --name {name} --management-group-id {mg} --location {location} --template-file "{template-file}" --deny-settings-mode "none" --action-on-unmanage detachAll --parameters "{parameter-file}" --yes', checks=self.check('provisioningState', 'succeeded')).get_output_in_json()
