...
def validate_fileupload_notification_ttl(ns):
    if (ns.fileupload_notification_ttl and
        ...
        raise ArgumentError(None, 'Please specify the period of time from 1 to 48 for which a file upload'
...
def validate_fileupload_sas_ttl(ns):
    if (ns.fileupload_sas_ttl and
        ...
        raise ArgumentError(None, 'Please specify the period of time from 1 to 24 for which the SAS URI'
...
def validate_feedback_ttl(ns):
    if (ns.feedback_ttl and
        ...
        raise ArgumentError(None, 'Please specify the period of time from 1 to 48 for which a message is available'
                                  ' to consume before it is expired by the IoT hub.')


def validate_feedback_lock_duration(ns):
    if (ns.feedback_lock_duration and
            ns.feedback_lock_duration not in range(5, 301, 1)):
        raise ArgumentError(None, 'Please specify the duration from 5 to 300 only.')
...
def validate_c2d_ttl(ns):
    if (ns.c2d_ttl and
        ...
        raise ArgumentError(None, 'Please specify the default time from 1 to 48 to live for cloud-to-device messages'
                                  ' in the device queue.')
