...
class ManagedIdentityAuth:
    ...
    def parse_ids(client_id=None, object_id=None, resource_id=None):
        ...
        if not any((client_id, object_id, resource_id)):
            return ManagedIdentityAuth.ID_TYPE_SYSTEM_ASSIGNED, ManagedIdentityAuth.IDENTITY_TYPE_SYSTEM_ASSIGNED

        # User-assigned - if one ID is provided
        id_type = None
        id_value = None
        if client_id:
            id_type = ManagedIdentityAuth.ID_TYPE_USER_ASSIGNED_CLIENT_ID
            id_value = client_id
        elif object_id:
            id_type = ManagedIdentityAuth.ID_TYPE_USER_ASSIGNED_OBJECT_ID
            id_value = object_id
        elif resource_id:
            id_type = ManagedIdentityAuth.ID_TYPE_USER_ASSIGNED_RESOURCE_ID
            id_value = resource_id
        return '{}-{}'.format(id_type, id_value), ManagedIdentityAuth.IDENTITY_TYPE_USER_ASSIGNED
    ...
    def credential_factory(id_string):
        ...
        if id_parts[0] == ManagedIdentityAuth.ID_TYPE_SYSTEM_ASSIGNED:
            return ManagedIdentityCredential()

        id_type, id_value = id_parts
        if id_type == ManagedIdentityAuth.ID_TYPE_USER_ASSIGNED_CLIENT_ID:
            return ManagedIdentityCredential(client_id=id_value)
        if id_type == ManagedIdentityAuth.ID_TYPE_USER_ASSIGNED_OBJECT_ID:
            return ManagedIdentityCredential(object_id=id_value)
        if id_type == ManagedIdentityAuth.ID_TYPE_USER_ASSIGNED_RESOURCE_ID:
