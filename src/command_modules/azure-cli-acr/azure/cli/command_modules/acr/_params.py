...
def load_arguments(self, _):  # pylint: disable=too-many-statements
    with self.argument_context('acr') as c:
        ...
        c.argument('password_name', help='The name of password to regenerate', arg_type=get_enum_type([PasswordName.password.value, PasswordName.password2.value]))
    ...
    with self.argument_context('acr webhook') as c:
        ...
        c.argument('actions', nargs='+', help='Space-separated list of actions that trigger the webhook to post notifications.', arg_type=get_enum_type([WebhookAction.push.value, WebhookAction.delete.value]))
        c.argument('status', help='Indicates whether the webhook is enabled.', arg_type=get_enum_type([WebhookStatus.enabled.value, WebhookStatus.disabled.value]))
    ...
    with self.argument_context('acr build-task') as c:
        ...
        c.argument('status', help='The current status of build task.', arg_type=get_enum_type([BuildTaskStatus.enabled.value, BuildTaskStatus.disabled.value]))
        c.argument('os_type', options_list=['--os'], help='The operating system type required for the build.', arg_type=get_enum_type([OsType.linux.value, OsType.windows.value]))
        ...
        c.argument('base_image_trigger', help="The type of the auto trigger for base image dependency updates.", arg_type=get_enum_type([BaseImageTriggerType.all.value,
        ...
        c.argument('build_status', help='The current status of build.', arg_type=get_enum_type([BuildStatus.queued.value,
