...
def cli_main(cli, args):
    ...
    try:
        ...
        try:
            ...
            if exit_code == 0 and cli.config.getboolean('auto-upgrade', 'enable', False) and \
                ...
                if VERSIONS[_VERSION_UPDATE_TIME]:
                    ...
                    if parse(VERSIONS['versions']['core']['local']) < parse(VERSIONS['versions']['core']['pypi']):  # pylint: disable=line-too-long
                        ...
                        update_all = cli.config.getboolean('auto-upgrade', 'all', True)
                        prompt = cli.config.getboolean('auto-upgrade', 'prompt', True)
