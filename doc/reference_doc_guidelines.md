...
- Provide **\```output** code blocks. When developing scripts, customers will likely combine two or more create, read, update and delete operations. With no documentation about the format of the returned object, customers have no choice but to run each command one by one and analyze the response.

## Conceptual content

Minimum conceptual content for the Azure CLI is one conceptual article for every command group and subgroup that creates an Azure resource. For example:

- The `az vm [create, list, auto-shutdown...]` group also contains commands that create an Azure resource.  This command group must have at least one conceptual article.
- The `az vm aem [delete, set, verify]` subgroup helps to manage Azure resources, but it doesn't create anything.  A conceptual article is encouraged but not required.
...
- **Provide at least one example for every parameter.** A single example can use multiple parameters, but a parameter without an example is often a parameter that shows no usage statistics in Azure CLI reporting.
