...
def load_arguments(self, _):
    with self.argument_context('appconfig create') as c:
        c.argument('resource_group_name', options_list=['--resource-group', '-g'], required=True)
        c.argument('name', options_list=['--name', '-n'], required=True, help='Name of the App Configuration to be created. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')
        c.argument('location', options_list=['--location', '-l'], arg_type=get_location_type(self.cli_ctx), validator=get_default_location_from_resource_group, required=True)

    with self.argument_context('appconfig delete') as c:
        c.argument('name', options_list=['--name', '-n'], required=True, help='Name of the App Configuration to be deleted. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')
    ...
    with self.argument_context('appconfig update') as c:
        c.argument('name', options_list=['--name', '-n'], required=True, help='Name of the App Configuration to be updated. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')
        c.argument('resource_group_name', options_list=['--resource-group', '-g'])
        c.argument('tags', arg_type=tags_type)

    with self.argument_context('appconfig list') as c:
        c.argument('resource_group_name', options_list=['--resource-group', '-g'])

    with self.argument_context('appconfig show') as c:
        c.argument('resource_group_name', options_list=['--resource-group', '-g'])
        c.argument('name', options_list=['--name', '-n'], required=True, help='Name of the App Configuration to be showed. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')

    with self.argument_context('appconfig credential list') as c:
        c.argument('resource_group_name', options_list=['--resource-group', '-g'])
        c.argument('name', options_list=['--name', '-n'], required=True, help='Name of the App Configuration to list credentials. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')

    with self.argument_context('appconfig credential regenerate') as c:
        c.argument('resource_group_name', options_list=['--resource-group', '-g'])
        c.argument('name', options_list=['--name', '-n'], required=True, help='Name of the App Configuration to be regenerate credentials. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')
        c.argument('id_', options_list=['--id'], required=True, help='Id of the key to be regenerated. Can be found using az appconfig credential list command.')

    with self.argument_context('appconfig kv') as c:
        c.argument('name', options_list=['--name', '-n'], help='The name of the App Configuration. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')
    ...
    with self.argument_context('appconfig kv import') as c:
        ...
        c.argument('source', options_list=['--source', '-s'], required=True, choices=['file', 'appconfig', 'appservice'], validator=validate_import, help="The source of importing.")
        c.argument('yes', options_list=['--yes', '-y'], action='store_true', help="Do not prompt for preview.")
    ...
    with self.argument_context('appconfig kv import', arg_group='File') as c:
        ...
        c.argument('separator', validator=validate_separator, help="Delimiter for flatterning the json or yaml file to key-value pairs. Required for importing hierarchical structure. Not applicable for property files. Allowed values: '.', ',', ';', '-', '_', '__', '/', ':'")
    ...
    with self.argument_context('appconfig kv export') as c:
        ...
        c.argument('destination', options_list=['--destination', '-d'], required=True, choices=['file', 'appconfig', 'appservice'], validator=validate_export, help="The destination of exporting.")
        c.argument('yes', options_list=['--yes', '-y'], action='store_true', help="Do not prompt for preview.")
    ...
    with self.argument_context('appconfig kv set') as c:
        c.argument('key', required=True, help='Key to be set.')
    ...
    with self.argument_context('appconfig kv delete') as c:
        c.argument('key', help='Support star sign as filters, for instance * means all key and abc* means keys with abc as prefix. Similarly, *abc and *abc* are also supported.', required=True)
    ...
    with self.argument_context('appconfig kv show') as c:
        c.argument('key', required=True, help='Key to be showed.')
        c.argument('label', help="If no label specified, show entry with null label. Does NOT support filters like other commands")
        c.argument('datetime', validator=validate_datetime, help='Datetime to show keylue entry. Format: "YYYY-MM-DDThh:mm:ssZ". If no time zone specified, use UTC by default.')

    with self.argument_context('appconfig kv list') as c:
        c.argument('key', help='If no key specified, return all keys by default. Support star sign as filters, for instance abc* means keys with abc as prefix. Similarly, *abc and *abc* are also supported.')
        c.argument('datetime', validator=validate_datetime, help='Datetime to list key-value entries. Format: "YYYY-MM-DDThh:mm:ssZ". If no time zone specified, use UTC by default.')
        ...
        c.argument('fields', nargs='+', help='Customize output fields.', validator=validate_query_fields, arg_type=get_enum_type(['key', 'value', 'label', 'content_type', 'etag', 'locked', 'last_modified']))
        c.argument('top', options_list=['--top', '-t'], type=int, help='Maximum number of items to return. Default to 100.')
        c.argument('all_', options_list=['--all'], action='store_true', help="List all items.")

    with self.argument_context('appconfig kv lock') as c:
        c.argument('key', required=True, help='Key to be locked.')
    ...
    with self.argument_context('appconfig kv unlock') as c:
        c.argument('key', required=True, help='Key to be unlocked.')
        c.argument('label', help="If no label specified, unlock entry with null label. Does NOT support filters like other commands")
        c.argument('yes', options_list=['--yes', '-y'], action='store_true', help="Do not prompt for confirmation.")

    with self.argument_context('appconfig revision') as c:
        c.argument('name', options_list=['--name', '-n'], help='The name of the App Configuration. You can configure the default name using `az configure --defaults app_configuration_store=<name>`', configured_default='app_configuration_store')
        c.argument('connection_string', validator=validate_connection_string, configured_default='appconfig_connection_string',
                   help="Connections of access key and endpoint of the App Configuration. Can be found using 'az appconfig credential list'. Users can preset it using `az configure --defaults appconfig_connection_string=<connection_string>` or environment variable with the name AZURE_APPCONFIG_CONNECTION_STRING.")

    with self.argument_context('appconfig revision list') as c:
        c.argument('key', help='If no key specified, return all keys by default. Support star sign as filters, for instance abc* means keys with abc as prefix. Similarly, *abc and *abc* are also supported.')
        c.argument('label', help="If no label specified, list all labels. Support star sign as filters, for instance abc* means labels with abc as prefix. Similarly, *abc and *abc* are also supported.")
        c.argument('datetime', validator=validate_datetime, help='Datetime to show history. Format: "YYYY-MM-DDThh:mm:ssZ". If no time zone specified, use UTC by default.')
        c.argument('fields', nargs='+', help='Customize output fields.', validator=validate_query_fields, arg_type=get_enum_type(['key', 'value', 'label', 'content_type', 'etag', 'locked', 'last_modified']))
        c.argument('top', options_list=['--top', '-t'], type=int, help='Maximum number of items to return. Default to 100.')
        c.argument('all_', options_list=['--all'], action='store_true', help="List all revisions.")
