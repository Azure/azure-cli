...
class SBNamespacePrivateEndpointCRUDScenarioTest(ScenarioTest):
    ...
    def test_sb_privateendpoint(self, resource_group):
        ...
        # Prepare network
        self.cmd('network vnet create -n {vnet} -g test-migration1 -l {loc} --subnet-name {subnet}',
                 checks=self.check('length(newVNet.subnets)', 1))
        self.cmd('network vnet subnet update -n {subnet} --vnet-name {vnet} -g test-migration1 '
                 '--disable-private-endpoint-network-policies true',
                 checks=self.check('privateEndpointNetworkPolicies', 'Disabled'))

        # Create Namespace
        self.cmd(
            'servicebus namespace create --resource-group test-migration1 --name {namespacename} --location {loc} --tags {tags}'
            ' --sku {sku}', checks=[self.check('sku.name', '{sku}')])

        # Get Created Namespace
        getnamespace = self.cmd(
            'servicebus namespace show --resource-group test-migration1 --name {namespacename}').get_output_in_json()

        # Create a private endpoint connection
        pr = self.cmd('servicebus namespace private-link-resource show --namespace-name {namespacename} -g test-migration1').get_output_in_json()
        self.kwargs['group_id'] = pr[0]['groupId']

        getnamesapce = self.cmd('servicebus namespace show -n {namespacename} -g test-migration1').get_output_in_json()
        self.kwargs['ehn_id'] = getnamesapce['id']
        private_endpoint = self.cmd(
            'network private-endpoint create -g test-migration1 -n {pe} --vnet-name {vnet} --subnet {subnet} -l {loc} '
        ...
        self.cmd('servicebus namespace private-endpoint-connection show --namespace-name {namespacename} -g test-migration1 --name {ehn_pec_name}',
                 checks=self.check('id', '{sa_pec_id}'))

        getstatus = self.cmd('servicebus namespace private-endpoint-connection approve --namespace-name {namespacename} -g test-migration1 --name {ehn_pec_name}').get_output_in_json()
        self.assertEqual(getstatus['privateLinkServiceConnectionState']['status'], 'Approved')

        getstatus = self.cmd(
            'servicebus namespace private-endpoint-connection reject --namespace-name {namespacename} -g test-migration1 --name {ehn_pec_name}').get_output_in_json()
        self.assertEqual(getstatus['privateLinkServiceConnectionState']['status'], 'Rejected')

        getstatus = self.cmd(
            'servicebus namespace private-endpoint-connection show --namespace-name {namespacename} -g test-migration1 --name {ehn_pec_name}').get_output_in_json()
        self.assertEqual(getstatus['privateLinkServiceConnectionState']['status'], 'Rejected')
