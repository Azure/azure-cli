...
def get_index(index_url=None, cli_ctx=None):
    ...
    for try_number in range(TRIES):
        try:
            ...
            raise CLIError(msg)
        except (requests.exceptions.ConnectionError, requests.exceptions.HTTPError, ValueError, OSError) as err:
            if try_number == TRIES - 1:
                # ValueError indicates that shortlink url is not redirecting properly to intended index url
                # OSError covers DNS resolution failures (e.g., socket.gaierror, urllib3 NameResolutionError)
