...
### **This is an autogenerated template. Please review.**
...
class CommandLogFile(object):
    ...
    def get_command_name_str(self):
        ...
        if self.metadata_tup.cmd != self.UNKNOWN_CMD:
            ...
            if args:
                command_args = args if len(args) < 20 else args[:15] + " ..."
    ...
    def get_command_duration_str(self):
        if not self.command_data_dict:
            return ""

        duration = float(self.command_data_dict.get("duration", -1))

        if duration < 0:
            return ""

        if duration > _ONE_HR_IN_SECS:
            hrs, secs = divmod(duration, _ONE_HR_IN_SECS)
            hrs = math.floor(hrs)
            mins = math.floor(secs / _ONE_MIN_IN_SECS)
            duration_str = "Duration: {} hrs {:02} mins".format(hrs, mins)
        elif duration > _ONE_MIN_IN_SECS:
            mins, secs = divmod(duration, _ONE_MIN_IN_SECS)
            mins = math.floor(mins)
            secs = math.floor(secs)
            duration_str = "Duration: {} mins {:02} secs".format(mins, secs)
        else:
            duration_str = "Duration: {:.2f} secs".format(duration)
    ...
    def _get_command_data_from_metadata(self):  # pylint: disable=too-many-statements
        ...
        # 2. Get the duration of the command. Duration is printed before last log record if any
        try:
            command_duration_msg = log_record_list[-2].log_msg.strip()
            if command_duration_msg.startswith("command ran in"):
                words = command_duration_msg.split()
                for word in words:
                    try:
                        log_data["duration"] = float(word)
                        break
                    except ValueError:
                        continue
        except IndexError:
            logger.debug("Couldn't extract duration from command log %s.", file_name)

        # 3. If there are any errors, this is a failed command. Log the errors
        # 4. Also get extension information.
        ...
        # 5. Get command args. from first record
        try:
...
def _build_issue_info_tup(command_log_file=None):
    ...
    if command_log_file:
        ...
        if command_log_file.command_data_dict:
            ...
            format_dict["command_duration"] = command_log_file.get_command_duration_str()
    ...
    return _ISSUES_TEMPLATE_PREFIX.format(issues_url), _ISSUES_TEMPLATE.format(**format_dict), issues_url
...
def _display_recent_commands(cmd):
    ...
    for i, log_info in enumerate(command_log_files):
        if log_info is None:
            ...
            print("   [{}] {}: {} {} {} ".format(i, cmd_name, success_msg, time_msg, duration_msg))
