...
from azure.cli.command_modules.vm.azure_stack._vm_utils import (check_existence, get_storage_blob_uri, list_sku_info,
                                                                import_aaz_by_profile)
from azure.cli.command_modules.vm.azure_stack._template_builder import StorageProfile
...
def _validate_vm_create_storage_profile(cmd, namespace, for_scale_set=False):
    ...
    if not namespace.storage_sku and namespace.storage_profile in [StorageProfile.SAPirImage,
                                                                   StorageProfile.SACustomImage]:  # pylint: disable=line-too-long
    ...
    if namespace.storage_profile == StorageProfile.ManagedCustomImage:
        ...
        if res['type'].lower() == 'images':
            ...
            if gallery_image_version.lower() in ['latest', '']:
                ...
                image_version_infos = [x for x in image_version_infos if
                                       not x.get("publishingProfile", {}).get("excludeFromLatest", None)]
                if not image_version_infos:
                    raise CLIError('There is no latest image version exists for "{}"'.format(namespace.image))
                image_version_info = sorted(image_version_infos, key=lambda x: x["publishingProfile"]["publishedDate"])[
                    -1]
    ...
    namespace.disk_info = normalize_disk_info(size=vm_size,
                                              ...
                                              ephemeral_os_disk_placement=getattr(namespace,
                                                                                  'ephemeral_os_disk_placement', None),
                                              data_disk_delete_option=getattr(
                                                  namespace, 'data_disk_delete_option', None),
                                              source_snapshots_or_disks=getattr(namespace, 'source_snapshots_or_disks',
                                                                                None),
                                              source_snapshots_or_disks_size_gb=getattr(namespace,
                                                                                        'source_snapshots_or_disks_size_gb',
                                                                                        None),
                                              source_disk_restore_point=getattr(namespace, 'source_disk_restore_point',
                                                                                None),
                                              source_disk_restore_point_size_gb=getattr(namespace,
...
def _validate_vm_nic_delete_option(namespace):
    if not namespace.nics and namespace.nic_delete_option:
        if len(namespace.nic_delete_option) == 1 and len(
                namespace.nic_delete_option[0].split('=')) == 1:  # pylint: disable=line-too-long
            namespace.nic_delete_option = namespace.nic_delete_option[0]
        elif len(namespace.nic_delete_option) > 1 or any((len(delete_option.split('=')) > 1 for delete_option in
                                                          namespace.nic_delete_option)):  # pylint: disable=line-too-long
            from azure.cli.core.parser import InvalidArgumentValueError
...
def _validate_admin_username(username, os_type):
    ...
    if username.lower() in DISALLOWED_USER_NAMES:
        raise CLIError(
            "This user name '{}' meets the general requirements, but is specifically disallowed for this image. Please try a different value.".format(
                username))
    return username
...
def _validate_admin_password(password, os_type):
    ...
    error_msg = ("The password length must be between {} and {}. Password must have the 3 of the following: "
                 "1 lower case character, 1 upper case character, 1 number and 1 special character.").format(min_length,
                                                                                                             max_length)
    ...
    if os.path.exists(string_or_file):
        ...
        if should_generate_ssh_keys:
            ...
            if ssh_key_type == "Ed25519":
                from azure.cli.command_modules.vm.azure_stack._vm_utils import generate_ssh_keys_ed25519
            ...
            if lb:
                ...
                if not namespace.nat_pool_name:
                    if len(lb['inboundNatPools']) > 1:
                        raise CLIError(
                            "Multiple possible values found for '{0}': {1}\nSpecify '{0}' explicitly.".format(
                                # pylint: disable=line-too-long
...
def _validate_gallery_image_reference(cmd, namespace):
    ...
    from azure.cli.command_modules.vm.azure_stack._image_builder import GalleryImageReferenceType
