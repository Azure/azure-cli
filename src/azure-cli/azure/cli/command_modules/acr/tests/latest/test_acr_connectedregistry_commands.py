...
class AcrConnectedRegistryCommandsTests(ScenarioTest):
    ...
    def test_acr_connectedregistry(self, resource_group):
        ...
        self.cmd('acr connected-registry update -n {root_name} -r {registry_name} --log-level Information -s "{defaultSyncSchedule}"',
                 + '--remove-client-tokens {clientToken} --add-client-tokens {clientToken2} --add-notifications {notificationStr2} --remove-notifications {notificationStr} --garbage-collection-schedule {garbage_collection_schedule}',
                 checks=[self.check('name', '{root_name}'),
                         self.check('logging.logLevel', 'Information'),
