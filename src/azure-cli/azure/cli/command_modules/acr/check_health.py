...
def _get_helm_version(ignore_errors):
    ...
    if match_obj and Version(output) < Version(MIN_HELM_VERSION):
...
def _get_notary_version(ignore_errors):
    ...
    if match_obj and Version(output) != Version(RECOMMENDED_NOTARY_VERSION):
        version_msg = "upgrade"
        if Version(output) > Version(RECOMMENDED_NOTARY_VERSION):
