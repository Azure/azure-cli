...
def load_arguments(self, _):  # pylint: disable=too-many-statements
    with self.argument_context('sf') as c:
        ...
        c.argument('secret_identifier', options_list=['--secret-identifier'], help='The existing Azure key vault secret URL')
        c.argument('certificate_file', options_list=['--certificate-file'], help='The existing certificate file path for the primary cluster certificate.')
        c.argument('parameter_file', options_list=['--parameter-file'], help='The path to the template parameter file.')
        c.argument('template_file', options_list=['--template-file'], help='The path to the template file.')
        c.argument('vm_password', options_list=['--vm-password'], help='The password of the Vm')
        c.argument('certificate_output_folder', options_list=['--certificate-output-folder'], help='The folder of the new certificate file to be created.')
        c.argument('certificate_password', options_list=['--certificate-password'], help='The password of the certificate file.')
        c.argument('certificate_subject_name', options_list=['--certificate-subject-name'], help='The subject name of the certificate to be created.')
        ...
        c.argument('vault_name', options_list=['--vault-name'], help='Azure key vault name, it not given it will be the cluster resource group name')
        c.argument('cluster_size', options_list=['--cluster-size', '-s'], help='The number of nodes in the cluster. Default are 5 nodes')
        c.argument('vm_sku', options_list=['--vm-sku'], help='The Vm Sku')
        c.argument('vm_user_name', options_list=['--vm-user-name'], help='The user name for logging to Vm. Default will be adminuser')
