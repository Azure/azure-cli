...
def _validate_vm_create_storage_profile(cmd, namespace, for_scale_set=False):
    ...
    if (getattr(namespace, 'os_disk_security_encryption_type', None) and
            namespace.os_disk_security_encryption_type.lower() == 'nonpersistedtpm'):
        if (getattr(namespace, 'security_type', None) != 'ConfidentialVM' or
                getattr(namespace, 'enable_vtpm', None) is not True):
            raise ArgumentUsageError(
