...
class VMCreateAndStateModificationsScenarioTest(ScenarioTest):
    ...
    def test_vm_create_state_modifications(self, resource_group):
        ...
        self.cmd('vm create --resource-group {rg} --name {vm} --admin-username azureuser --image OpenLogic:CentOS:7.5:latest --admin-password testPassword0 --authentication-type password --tags firsttag=1 secondtag=2 thirdtag --nsg {nsg} --public-ip-address {ip} --subnet {subnet} --vnet-name {vnet} --storage-account {sa} --use-unmanaged-disk --nsg-rule NONE --size Standard_B2s')
