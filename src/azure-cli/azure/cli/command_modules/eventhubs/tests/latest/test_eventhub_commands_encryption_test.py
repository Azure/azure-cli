...
class EHNamespaceMSITesting(ScenarioTest):
    ...
    def test_eh_namespace_encryption(self, resource_group):
        self.kwargs.update({
            ...
            'namespacename4': self.create_random_name(prefix='eventhubs-nscli', length=20),
            ...
            'key4': 'key4',
            'key5': 'key5',
            'kv_name': self.create_random_name(prefix='eventhubs-kg', length=20)
        })
        ...
        assert len(n) == 2
        n = [i for i in namespace['keyVaultProperties']]
        assert len(n) == 3
        self.assertEqual(False, namespace['requireInfrastructureEncryption'])
        ...
        n = [i for i in namespace['keyVaultProperties']]
        assert len(n) == 3
        self.assertEqual(True, namespace['requireInfrastructureEncryption'])
