...
examples:
-   name: Create a Kubernetes container service and generate SSH keys to connect to
        it.
    text: >
        az acs create -g MyResourceGroup -n MyContainerService --orchestrator-type
        kubernetes --generate-ssh-keys
-   name: Create a container service.
    text: az acs create --generate-ssh-keys  --name MyContainerService --orchestrator-type
        kubernetes --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Create a managed disk.
    text: az disk create --source https://vhd1234.blob.core.windows.net/vhds/osdisk1234.vhd
        --resource-group MyResourceGroup --name MyDisk
    crafted: 'True'
"""
...
examples:
-   name: Delete a managed disk.
    text: az disk delete --resource-group MyResourceGroup --yes  --name MyManagedDisk
    crafted: 'True'
"""
...
examples:
-   name: Update a managed disk.
    text: az disk update --size-gb {size-gb} --name MyManagedDisk --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
helps["disk wait"] = """
type: command
short-summary: Place the CLI in a waiting state until a condition of a managed disk
    is met.
"""
...
-   name: Create an image by capturing an existing generalized virtual machine in
        the same resource group.
    text: az image create -g MyResourceGroup -n image1 --source MyVm1
-   name: Create a custom Virtual Machine Image from managed disks or snapshots.
    text: az image create --source MyVm1 --name image1 --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Create a gallery image definition.
    text: az sig image-definition create --os-type linux --sku GreatSku --gallery-image-definition
        MyImage --resource-group MyResourceGroup --offer GreatOffer --gallery-name
        MyGallery --publisher GreatPublisher
    crafted: 'True'
"""
...
long-summary: this operation might take a long time depending on the replicate region
    number. Use "--no-wait" is advised.
examples:
...
-   name: Add a new image version replicated across multiple regions with different
        replication counts each. Eastus2 will have it's replica count set to the default
        replica count.
    text: |
...
-   name: Add a new image version and don't wait on it. Later you can invoke "az sig
        image-version wait" command when ready to create a vm from the gallery image
        version
    text: |
...
helps["snapshot"] = """
type: group
short-summary: Manage point-in-time copies of managed disks, native blobs, or other
    snapshots.
"""
...
-   name: Create a snapshot.
    text: az snapshot create --tags key[=value] --source https://vhd1234.blob.core.windows.net/vhds/osdisk1234.vhd
        --name MySnapshot --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Grant read access to a snapshot.
    text: az snapshot grant-access --output json --name MySnapshot --query [0] --duration-in-seconds
        {duration-in-seconds} --resource-group MyResourceGroup
    crafted: 'True'
"""
...
examples:
-   name: List snapshots.
    text: az snapshot list --output json
    crafted: 'True'
"""
...
examples:
-   name: Revoke read access to a snapshot.
    text: az snapshot revoke-access --output json --resource-group MyResourceGroup
        --name MySnapshot
    crafted: 'True'
"""
...
helps["snapshot wait"] = """
type: command
short-summary: Place the CLI in a waiting state until a condition of a snapshot is
    met.
"""
...
long-summary: >
    To provide redundancy to an application, it is recommended to group two or more
    virtual machines in an availability set.
    This configuration ensures that during either a planned or unplanned maintenance
    event, at least one virtual machine
    will be available.
"""
...
-   name: Create an availability set.
    text: az vm availability-set create -n MyAvSet -g MyResourceGroup --platform-fault-domain-count
        2 --platform-update-domain-count 2
-   name: Create an Azure Availability Set.
    text: az vm availability-set create --name MyAvSet --location westus2 --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
-   name: List availability sets.
    text: az vm availability-set list --output json --query [0] --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Get information for an availability set.
    text: az vm availability-set show --output json --name MyAvSet --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
long-summary: Use this feature to troubleshoot boot failures for custom or platform
    images.
"""
...
-   name: Disable boot diagnostics on all VMs in a resource group.
    text: >
        az vm boot-diagnostics disable --ids $(az vm list -g MyResourceGroup --query
        "[].id" -o tsv)
"""
...
-   name: Enable boot diagnostics on all VMs in a resource group.
    text: >
        az vm boot-diagnostics enable --storage https://mystor.blob.core.windows.net/
        --ids $(az vm list -g MyResourceGroup --query "[].id" -o tsv)

-   name: Enable the boot diagnostics on a VM.
    text: az vm boot-diagnostics enable --storage https://mystor.blob.core.windows.net/
...
-   name: Get diagnostics logs for all VMs in a resource group.
    text: >
        az vm boot-diagnostics get-boot-log --ids $(az vm list -g MyResourceGroup
        --query "[].id" -o tsv)
"""
...
-   name: Convert all VMs with unmanaged disks in a resource group to use managed
        disks.
    text: >
...
-   name: --image
    type: string
    short-summary: >
        The name of the operating system image as a URN alias, URN, custom image name
        or ID, or VHD blob URI.
        This parameter is required unless using `--attach-os-disk.` Valid URN format:
...
-   name: Create a default RedHat VM with automatic SSH authentication using an image
        URN.
    text: >
...
-   name: Create a VM by attaching to a managed operating system disk.
    text: >
        az vm create -g MyResourceGroup -n MyVm --attach-os-disk MyOsDisk --os-type
        linux
-   name: 'Create an Ubuntu Linux VM using a cloud-init script for configuration.
        See: https://docs.microsoft.com/azure/virtual-machines/virtual-machines-linux-using-cloud-init.'
    text: >
        az vm create -g MyResourceGroup -n MyVm --image debian --custom-data MyCloudInitScript.yml
-   name: Create a Debian VM with SSH key authentication and a public DNS entry, located
        on an existing virtual network and availability set.
    text: |
...
-   name: Create a simple Ubuntu Linux VM with a public IP address, DNS entry, two
        data disks (10GB and 20GB), and then generate ssh key pairs.
    text: |
...
-   name: Create a Debian VM using Key Vault secrets.
    text: >
        az keyvault certificate create --vault-name vaultname -n cert1   -p "$(az
        keyvault certificate get-default-policy)"


        secrets=$(az keyvault secret list-versions --vault-name vaultname   -n cert1
        ...
        az vm create -g group-name -n vm-name --admin-username deploy    --image debian
        --secrets "$vm_secrets"
-   name: Create a CentOS VM with a system assigned identity. The VM will have a 'Contributor'
...
-   name: Create an Azure Virtual Machine.
    text: az vm create --public-ip-address "" --size {size} --resource-group MyResourceGroup
        --image UbuntuLTS --name MyVm
    crafted: 'True'
"""
...
-   name: Deallocate a VM.
    text: az vm deallocate --name MyVirtualMachine --no-wait  --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Delete a VM.
    text: az vm delete --resource-group MyResourceGroup --yes  --name MyVm
    crafted: 'True'
"""
...
examples:
-   name: Get the default diagnostics for a Linux VM and override the storage account
        name and the VM resource ID.
    text: |
...
examples:
-   name: Set up default diagnostics on a Linux VM for Azure Portal VM metrics graphs
        and syslog collection.
    text: |
...
long-summary: >4
    ...
    Just like any other computer, virtual machines in Azure use disks as a place to
    store an operating system, applications, and data.
    All Azure virtual machines have at least two disks: An operating system disk,
    and a temporary disk.
    The operating system disk is created from an image, and both the operating system
    disk and the image are actually virtual hard disks (VHDs)
    stored in an Azure storage account. Virtual machines also can have one or more
    data disks, that are also stored as VHDs.

    Operating System Disk
    Every virtual machine has one attached operating system disk. It's registered
    as a SATA drive and is labeled /dev/sda by default.
    This disk has a maximum capacity of 1023 gigabytes (GB).

    Temporary disk
    The temporary disk is automatically created for you. On Linux virtual machines,
    the disk is typically /dev/sdb and is formatted and
    mounted to /mnt/resource by the Azure Linux Agent. The size of the temporary disk
    varies, based on the size of the virtual machine.

    Data disk
    A data disk is a VHD that's attached to a virtual machine to store application
    data, or other data you need to keep. Data disks are
    registered as SCSI drives and are labeled by the creator. Each data disk has a
    maximum capacity of 1023 GB. The size of the virtual
    machine determines how many data disks can be attached and the type of storage
    that can be used to host the disks.
"""

helps["vm disk attach"] = """
type: command
short-summary: Attach a managed persistent disk to a VM.
long-summary: This allows for the preservation of data, even if the VM is reprovisioned
    due to maintenance or resizing.
examples:
...
-   name: Attach a managed persistent disk to a VM.
    text: az vm disk attach --disk disk_name --size-gb {size-gb} --vm-name MyVm --new  --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
-   name: Detach a managed disk from a VM.
    text: az vm disk detach --name disk_name --vm-name MyVm --resource-group MyResourceGroup
    crafted: 'True'
"""
...
parameters:
-   name: --aad-client-id
    short-summary: Client ID of an AAD app with permissions to write secrets to the
        key vault.
-   name: --aad-client-secret
    short-summary: Client secret of the AAD app with permissions to write secrets
        to the key vault.
-   name: --aad-client-cert-thumbprint
    short-summary: Thumbprint of the AAD app certificate with permissions to write
        secrets to the key vault.
examples:
-   name: Enable disk encryption on the OS disk and/or data disks.
    text: az vm encryption enable --disk-encryption-keyvault {disk-encryption-keyvault}
        --name MyVirtualMachine --volume-type ALL --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Show encryption status.
    text: az vm encryption show --name MyVirtualMachine --query [0] --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
long-summary: >
    ...
    automation tasks on Azure virtual machines.
    For example, if a virtual machine requires software installation, anti-virus protection,
    or Docker configuration, a VM extension
    can be used to complete these tasks. Extensions can be bundled with a new virtual
    machine deployment or run against any existing system.
"""
...
-   name: Delete extensions with IDs containing the string "MyExtension" from a VM.
    text: >
        az vm extension delete --ids     $(az resource list --query "[?contains(name,
        'MyExtension')].id" -o tsv)
-   name: Remove an extension attached to a VM.
    text: az vm extension delete --resource-group MyResourceGroup --vm-name MyVm --name
        extension_name
    crafted: 'True'
"""
...
-   name: List the information on available extensions.
    text: az vm extension image list --output json --location westus2
    crafted: 'True'
"""
...
-   name: List attached extensions with IDs containing the string "MyExtension".
    text: >
        az vm extension list --ids     $(az resource list --query "[?contains(name,
        'MyExtension')].id" -o tsv)
-   name: List the extensions attached to a VM.
    text: az vm extension list --query [0] --vm-name MyVm --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Set extensions for a VM.
    text: az vm extension set --version {version} --resource-group MyResourceGroup
        --name MyExtension --vm-name MyVm --publisher {publisher} --settings {settings}
    crafted: 'True'
parameters:
-   name: --name -n
    populator-commands:
...
-   name: Display information about extensions attached to a VM.
    text: az vm extension show --output json --name extension_name --vm-name MyVm
...
type: command
short-summary: Place the CLI in a waiting state until a condition of a virtual machine
    extension is met.
"""
...
-   name: Mark a VM as generalized, allowing it to be imaged for multiple deployments.
    text: az vm generalize --name MyVirtualMachine --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Get instance views for all VMs in a resource group.
    text: >
        az vm get-instance-view --ids $(az vm list -g MyResourceGroup --query "[].id"
        -o tsv)

-   name: Get instance information about a VM.
    text: az vm get-instance-view --output json --name MyVm --resource-group MyResourceGroup
    crafted: 'True'
"""
...
long-summary: This is required to authenticate and interact with other Azure services
    using bearer tokens.
examples:
...
-   name: Enable the system assigned identity and a user assigned identity on a VM.
    text: az vm identity assign -g MyResourceGroup -n MyVm --role Reader --identities
        [system] myAssignedId
-   name: Enable managed service identity on a VM.
    text: az vm identity assign --role Reader --scope /subscriptions/db5eb68e-73e2-4fa8-b18a-0123456789999/resourceGroups/MyResourceGroup
...
examples:
-   name: Display VM's managed identity info.
...
type: command
short-summary: Accept Azure Marketplace term so that the image can be used to create
    VMs
examples:
-   name: Accept Azure Marketplace term so that the image can be used to create VMs.
...
parameters:
-   name: --all
    short-summary: Retrieve image list from live Azure service rather using an offline
        image list
-   name: --offer -f
...
-   name: List the VM/VMSS images available in the Azure Marketplace.
    text: az vm image list --all  --publisher {publisher}
    crafted: 'True'
"""
...
-   name: List the VM image offers available in the Azure Marketplace.
    text: az vm image list-offers --publisher Microsoft --location westus
    crafted: 'True'
"""
...
-   name: List the VM image publishers available in the Azure Marketplace.
...
-   name: --publisher -p
    ...
    -   az vm list-publishers
examples:
-   name: List all skus available for CentOS published by OpenLogic in the West US
        region.
...
-   name: Show information for the latest available CentOS image from OpenLogic.
    text: >
        latest=$(az vm image list -p OpenLogic -s 7.3 --all --query     "[?offer=='CentOS'].version"
        -o tsv | sort -u | tail -n 1)

        az vm image show -l westus -f CentOS -p OpenLogic --sku 7.3 --version {latest}
-   name: Get the details for a VM image available in the Azure Marketplace.
    text: az vm image show --output json --query [0] --urn publisher:offer:sku:version
    crafted: 'True'
"""
...
long-summary: For more information on querying information about Virtual Machines,
    see https://docs.microsoft.com/en-us/cli/azure/query-az-cli2
examples:
-   name: List all VMs.
    text: az vm list
...
-   name: Get IP addresses for all VMs in a resource group.
    text: >
        az vm list-ip-addresses --ids $(az vm list -g MyResourceGroup --query "[].id"
        -o tsv)

-   name: List IP addresses associated with a VM.
    text: az vm list-ip-addresses --output json
    crafted: 'True'
"""
...
-   name: List available sizes for VMs.
    text: az vm list-sizes --location westus
    crafted: 'True'
"""
...
long-summary: This command incorporates subscription level restriction, offering the
    most accurate information.
examples:
-   name: List all SKUs in the West US region.
    text: az vm list-skus -l westus
-   name: List all available vm sizes in the East US2 region which support availability
        zone.
    text: az vm list-skus -l eastus2 --zone
-   name: List all available vm sizes in the East US2 region which support availability
        zone with name like "standard_ds1...".
...
-   name: Get details for compute-related resource SKUs.
    text: az vm list-skus --location westus
    crafted: 'True'
"""
...
-   name: List available usage resources for VMs.
    text: az vm list-usage --location westus
    crafted: 'True'
"""
...
-   name: List available sizes for all VMs in a resource group.
    text: >
        az vm list-vm-resize-options --ids $(az vm list -g MyResourceGroup --query
        "[].id" -o tsv)
"""
...
long-summary: >
    A network interface (NIC) is the interconnection between a VM and the underlying
    software
...
-   name: List the NICs available on a VM.
    text: az vm nic list --vm-name MyVm --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Set a NIC on a VM to be the primary interface.
    text: az vm nic set -g MyResourceGroup --vm-name MyVm --nic nic_name1 nic_name2
        --primary-nic nic_name2
"""
...
-   name: Display information for a NIC attached to a VM.
    text: az vm nic show --vm-name MyVm --nic nic_name1 --resource-group MyResourceGroup
    crafted: 'True'
"""
...
long-summary: >
    Adds a security rule to the network security group (NSG) that is attached to the
    VM's
    network interface (NIC) or subnet. The existing NSG will be used or a new one
    will be
    created. The rule name is 'open-port-{port}' and will overwrite an existing rule
    with
...
-   name: Opens a VM to inbound traffic on specified ports.
    text: az vm open-port --port '*' --name MyVm --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Resize all VMs in a resource group.
    text: >
        az vm resize --size Standard_DS3_v2 --ids $(az vm list -g MyResourceGroup
        --query "[].id" -o tsv)

-   name: Update a VM's size.
    text: az vm resize --size Standard_DS3_v2 --name MyVm --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Restart VMs.
    text: az vm restart --name MyVm --no-wait  --resource-group MyResourceGroup
    crafted: 'True'
"""
...
long-summary: For more information, see https://docs.microsoft.com/en-us/azure/virtual-machines/windows/run-command
    or https://docs.microsoft.com/en-us/azure/virtual-machines/linux/run-command.
"""
...
-   name: install nginx on a vm
    text: az vm run-command invoke -g MyResourceGroup -n MyVm --command-id RunShellScript
        --scripts "sudo apt-get update && sudo apt-get install -y nginx"
-   name: invoke command with parameters
    text: az vm run-command invoke -g MyResourceGroup -n MyVm --command-id RunShellScript
        --scripts 'echo $1 $2' --parameters hello world
-   name: Execute a specific run command on a vm.
    text: az vm run-command invoke --scripts "sudo apt-get update && sudo apt-get
        install -y nginx" --name MyVm --command-id RunShellScript --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
-   name: --secrets -s
    long-summary: >
        The command will attempt to resolve the vault ID for each secret. If it is
        unable to do so,
        specify the vault ID to use for *all* secrets using: --keyvault NAME --resource-group
        NAME | --keyvault ID.
examples:
-   name: Create a self-signed certificate with the default policy, and add it to
        a virtual machine.
    text: >
        az keyvault certificate create --vault-name vaultname -n cert1   -p "$(az
        keyvault certificate get-default-policy)"
        ...
        az vm create -g group-name -n vm-name --admin-username deploy    --image debian
        --secrets "$vm_secrets"
-   name: Transform secrets into a form that can be used by VMs and VMSSes.
    text: az vm secret format --secrets {secrets}
    crafted: 'True'
"""
...
-   name: Get the details of a VM.
    text: az vm show --show-details  --name MyVm --query [0] --resource-group MyResourceGroup
    crafted: 'True'
"""
...
long-summary: The VM will continue to be billed. To avoid this, you can deallocate
    the VM through "az vm deallocate"
examples:
...
-   name: Attach an existing data disk to a VM as unmanaged.
    text: >
        az vm unmanaged-disk attach -g MyResourceGroup --vm-name MyVm     --vhd-uri
        https://mystorage.blob.core.windows.net/vhds/d1.vhd
-   name: Attach an unmanaged persistent disk to a VM.
    text: az vm unmanaged-disk attach --size-gb {size-gb} --new  --name MyDataDiskName(optionalWhenCreateNewDisk)
...
-   name: List unmanaged disks with IDs containing the string "data_disk".
    text: >
        az vm unmanaged-disk list --ids     $(az resource list --query "[?contains(name,
        'data_disk')].id" -o tsv)
-   name: List unmanaged disks of a VM.
    text: az vm unmanaged-disk list --output json --resource-group MyResourceGroup
...
long-summary: Update VM objects and properties using paths that correspond to 'az
    vm show'.
examples:
...
-   name: Set the primary NIC of a VM.
    text: az vm update -n name -g group --set networkProfile.networkInterfaces[1].primary=false
        networkProfile.networkInterfaces[0].primary=true
-   name: Add a new non-primary NIC to a VM.
    text: az vm update -n name -g group --add networkProfile.networkInterfaces primary=false
        id={NIC_ID}
-   name: Remove the fourth NIC from a VM.
    text: az vm update -n name -g group --remove networkProfile.networkInterfaces
        3
-   name: Update the properties of a VM.
    text: az vm update --set tags.tagName=tagValue --resource-group group --name name
    crafted: 'True'
"""
...
-   name: Delete a user on all VMs in a resource group.
    text: >
        az az vm user delete -u username --ids $(az vm list -g MyResourceGroup --query
        "[].id" -o tsv)
"""
...
long-summary: >
    The extension will restart the SSH service, open the SSH port on your VM, and
    reset the SSH configuration to default values. The user account (name, password,
    and SSH keys) are not changed.
examples:
...
-   name: Reset the SSH server on all VMs in a resource group.
    text: >
        az vm user reset-ssh --ids $(az vm list -g MyResourceGroup --query "[].id"
        -o tsv)
"""
...
-   name: Update a Linux user account.
    text: az vm user update -u username --ssh-key-value "$({ ~/.ssh/id_rsa.pub)" -n
        MyVm -g MyResourceGroup
-   name: Update a user on all VMs in a resource group.
    text: >
        az vm user update -u username --ssh-key-value "$({ ~/.ssh/id_rsa.pub)" --ids
        $(az vm list -g MyResourceGroup --query "[].id" -o tsv)

-   name: Update a user account.
    text: az vm user update --username username --name MyVm --ssh-key-value "$({ ~/.ssh/id_rsa.pub)"
        --resource-group MyResourceGroup
    crafted: 'True'
"""
...
-   name: Wait until all VMs in a resource group are deleted.
    text: >
        az vm wait --deleted --ids $(az vm list -g MyResourceGroup --query "[].id"
        -o tsv)

-   name: Place the CLI in a waiting state until a condition of the VM is met.
    text: az vm wait --resource-group MyResourceGroup --custom {custom} --name MyVm
    crafted: 'True'
"""

helps["vmss"] = """
type: group
short-summary: Manage groupings of virtual machines in an Azure Virtual Machine Scale
    Set (VMSS).
"""
...
-   name: --image
    ...
    -   az vm image show
examples:
-   name: Create a Windows VM scale set with 5 instances, a load balancer, a public
        IP address, and a 2GB data disk.
    text: >
        az vmss create -n MyVmss -g MyResourceGroup --instance-count 5 --image Win2016Datacenter
        --data-disk-sizes-gb 2
-   name: Create a Linux VM scale set with an auto-generated ssh key pair, a public
        IP address, a DNS entry, an existing load balancer, and an existing virtual
        network.
    text: |
        az vmss create -n MyVmss -g MyResourceGroup --public-ip-address-dns-name my-globally-dns-name \\
            ...
            --generate-ssh-keys
-   name: Create a Linux VM scale set from a custom image using the default existing
        public SSH key.
    text: >
        az vmss create -n MyVmss -g MyResourceGroup --image MyImage
-   name: Create a Linux VM scale set with a load balancer and custom DNS servers.
        Each VM has a public-ip address and a custom domain name.
    text: >
...
-   name: 'Create a Linux VM scale set using a cloud-init script for configuration.
        See: https://docs.microsoft.com/azure/virtual-machines/virtual-machines-linux-using-cloud-init'
    text: >
...
-   name: Create a VM scaleset with system assigned identity. The VM will have a 'Contributor'
        Role with access to a storage account.
    text: >
        az vmss create -n MyVmss -g MyResourceGroup --image centos --assign-identity
...
examples:
-   name: Delete VMs within a VMSS.
    text: az vmss delete-instances --no-wait  --resource-group MyResourceGroup --instance-ids
        {instance-ids} --name MyScaleSet
    crafted: 'True'
"""
...
-   name: encrypt a VM scale set using a key vault in the same resource group
    text: >
        az vmss encryption enable -g MyResourceGroup -n MyVm --disk-encryption-keyvault
        myvault
"""
...
examples:
-   name: >
        Set an extension which depends on two previously set extensions. That is,
        When a VMSS instance is created or reimaged, the customScript extension will
        be provisioned only after all extensions that it depends on have been provisioned.
        The extension need not depend on the other extensions for pre-requisite configurations.
    text: >
        az vmss extension set --vmss-name my-vmss --name customScript --resource-group
        my-group     --version 2.0 --publisher Microsoft.Azure.Extensions     --provision-after-extensions
        NetworkWatcherAgentLinux VMAccessForLinux      --settings '{"commandToExecute":
        "echo testing"}'
-   name: Add an extension to a VMSS or update an existing extension.
    text: az vmss extension set --version {version} --publisher {publisher} --resource-group
        MyResourceGroup --name MyExtension --vmss-name MyVmss --settings {settings}
    crafted: 'True'
"""
...
parameters:
-   name: --instance-id
    short-summary: A VM instance ID or "*" to list instance view for all VMs in a
        scale set.

examples:
-   name: View an instance of a VMSS.
...
-   name: Enable system assigned identity on a VMSS with the 'Owner' role.
    text: az vmss identity assign -g MyResourceGroup -n MyVmss --role Owner --scope
        /subscriptions/db5eb68e-73e2-4fa8-b18a-0123456789999/resourceGroups/MyResourceGroup
-   name: Enable managed service identity on a VMSS.
...
-   name: Remove 2 identities which are in the same resource group with the VM scaleset
    text: az vmss identity remove -g MyResourceGroup -n MyVmss --identities readerId
        writerId
-   name: Remove system assigned identity and a user identity
    text: az vmss identity remove -g MyResourceGroup -n MyVmss --identities [system]
        readerId
"""
...
examples:
-   name: Gets a list of all virtual machines in a VM scale sets.
    text: az vmss list-instances --output json --name MyScaleSet --resource-group
        MyResourceGroup
    crafted: 'True'
-   name: List VMSS.
    text: az vmss list --output json --query [0]
    crafted: 'True'
"""

helps["vmss list-instance-connection-info"] = """
type: command
short-summary: Get the IP address and port number used to connect to individual VM
    instances within a set.
examples:
-   name: Get the IP address and port number used to connect to individual VM instances
        within a set.
    text: az vmss list-instance-connection-info --output json --name MyScaleSet --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
examples:
-   name: List public IP addresses of VM instances within a set.
    text: az vmss list-instance-public-ips --output json --name MyScaleSet --query
        [0] --resource-group MyResourceGroup
    crafted: 'True'
"""
...
examples:
-   name: Restart VMs within a VMSS.
...
examples:
-   name: Change the number of VMs within a VMSS.
    text: az vmss scale --name MyScaleSet --new-capacity {new-capacity} --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
examples:
-   name: Get details on VMs within a VMSS.
    text: az vmss show --output json --resource-group MyResourceGroup --query [0]
        --name MyScaleSet
    crafted: 'True'
"""
...
long-summary: The VMs will continue to be billed. To avoid this, you can deallocate
    VM instances within a VMSS through "az vmss deallocate"
"""
...
examples:
-   name: Update a VMSS.
    text: az vmss update --set {set} --name MyScaleSet --resource-group MyResourceGroup
    crafted: 'True'
"""
...
examples:
-   name: Upgrade VMs within a VMSS.
    text: az vmss update-instances --name MyScaleSet --instance-ids {instance-ids}
        --resource-group MyResourceGroup
    crafted: 'True'
"""
...
helps["vmss wait"] = """
type: command
short-summary: Place the CLI in a waiting state until a condition of a scale set is
    met.
"""
