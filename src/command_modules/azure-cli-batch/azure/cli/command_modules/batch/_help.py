...
type: group
short-summary: Manage the access keys for the auto storage account configured for
    a Batch account.
"""
...
type: command
short-summary: List the Batch accounts associated with a subscription or resource
    group.
"""

helps["batch account login"] = """
type: command
short-summary: Log in to a Batch account through Azure Active Directory or Shared
    Key authentication.
examples:
-   name: Log in to a Batch account through Azure Active Directory or Shared Key authentication.
    text: az batch account login --resource-group MyResourceGroup --name MyBatchAccount
    crafted: 'True'
"""
...
long-summary: This step is unnecessary if the package has already been successfully
    activated by the `create` command.
"""
...
-   name: Create a Batch application package record and activate it.
    text: az batch application package create --resource-group MyResourceGroup --application-id
        {application-id} --package-file {package-file} --version {version} --name
        MyBatchAccount
    crafted: 'True'
"""
...
examples:
-   name: Update properties for a Batch application.
    text: az batch application set --default-version {default-version} --application-id
        {application-id} --name MyBatchAccount --resource-group MyResourceGroup
    crafted: 'True'
"""
...
long-summary: This operation returns only applications and versions that are available
    for use on compute nodes; that is, that can be used in an application package
    reference. For administrator information about applications and versions that
    are not yet available to compute nodes, use the Azure portal or the 'az batch
    application list' command.
examples:
-   name: Lists all of the applications available in the specified account.
    text: az batch application summary list --account-endpoint {account-endpoint}
...
long-summary: Statistics are aggregated across all jobs that have ever existed in
    the account, from account creation to the last update time of the statistics.
"""
...
examples:
-   name: Add a job and associated task(s) to a Batch account.
    text: az batch job create --template {template}
    crafted: 'True'
"""
...
type: command
short-summary: Update the properties of a Batch job. Unspecified properties which
    can be updated are reset to their defaults.
"""

helps["batch job set"] = """
type: command
short-summary: Update the properties of a Batch job. Updating a property in a subgroup
...
type: command
short-summary: Reset the properties of a job schedule.  An updated job specification
    only applies to new jobs.
"""

helps["batch job-schedule set"] = """
type: command
short-summary: Update the properties of a job schedule.
long-summary: You can independently update the schedule and the job specification,
    but any change to either of these entities will reset all properties in that entity.
"""
...
type: command
short-summary: Update the properties of a user account on a Batch compute node. Unspecified
    properties which can be updated are reset to their defaults.
"""
...
long-summary: Statistics are aggregated across all pools that have ever existed in
    the account, from account creation to the last update time of the statistics.
"""
...
type: command
short-summary: Create a Batch pool in an account. When creating a pool, choose arguments
    from either Cloud Services Configuration or Virtual Machine Configuration.
examples:
-   name: Create a Batch pool in an account. When creating a pool, choose arguments
        from either Cloud Services Configuration or Virtual Machine Configuration.
    text: az batch pool create --target-dedicated-nodes {target-dedicated-nodes} --node-agent-sku-id
        {node-agent-sku-id} --id {id} --vm-size {vm-size} --image MyImage
    crafted: 'True'
"""
...
type: command
short-summary: Update the properties of a Batch pool. Unspecified properties which
    can be updated are reset to their defaults.
"""
...
type: command
short-summary: Update the properties of a Batch pool. Updating a property in a subgroup
    will reset the unspecified properties of that group.
"""
...
-   name: Create Batch tasks.
    text: az batch task create --json-file {json-file} --job-id {job-id}
    crafted: 'True'
"""
