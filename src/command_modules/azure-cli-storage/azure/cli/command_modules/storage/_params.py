...
from azure.cli.core.commands.parameters import \
    (tags_type, file_type, get_resource_name_completion_list, get_location_type, get_enum_type, get_three_state_flag)
...
def _get_client(cli_ctx, service, parsed_args):
    ...
    return get_storage_data_service_client(
        cli_ctx, service, account_name, account_key, connection_string, sas_token)
...
def get_storage_name_completion_list(service, func, parent=None):
    ...
    def completer(cmd, prefix, namespace, **kwargs):  # pylint: disable=unused-argument
        client = _get_client(cmd.cli_ctx, service, parsed_args)
        if parent:
...
#    c.argument('marker', ignore_type)  # https://github.com/Azure/azure-cli/issues/3745
...
#    c.argument('services', validator=get_char_options_validator('bfqt', 'services'))


#with CommandContext('storage cors list') as c:
#    c.argument('services', validator=get_char_options_validator('bfqt', 'services'), default='bqft', required=False)


#with CommandContext('storage cors add') as c:
#    c.argument('max_age')
#    c.argument('origins', nargs='+')
#    c.argument('methods', nargs='+', **enum_choice_list(['DELETE', 'GET', 'HEAD', 'MERGE', 'POST', 'OPTIONS', 'PUT']))
...
#    c.argument('services', validator=get_char_options_validator('bqt', 'services'), required=False, default='bqt',
...
#    c.argument('services', validator=get_char_options_validator('bqt', 'services'),
...
#    c.argument('retention', type=int, help='Number of days for which to retain logs. 0 to disable.')
...
#    c.argument('services', validator=get_char_options_validator('bfqt', 'services'), required=False, default='bfqt',
...
def load_arguments(self, _):
    AccountType, AccessTier, Bypass, DefaultAction, Kind, SkuName = self.get_models(
        'AccountType', 'AccessTier', 'Bypass', 'DefaultAction', 'Kind', 'SkuName',
        resource_type=ResourceType.MGMT_STORAGE)

    AccountPermissions = self.get_sdk('common.models#AccountPermissions')
    DeleteSnapshot, BlockBlobService, PageBlobService, AppendBlobService = self.get_sdk(
        'DeleteSnapshot', 'BlockBlobService', 'PageBlobService', 'AppendBlobService', mod='blob')
    BaseBlobService, FileService, QueueService, QueuePermissions = self.get_sdk(
        'blob.baseblobservice#BaseBlobService', 'file#FileService', 'queue#QueueService',
        'queue.models#QueuePermissions')
    BlobContentSettings, ContainerPermissions, BlobPermissions, PublicAccess = self.get_sdk(
        'ContentSettings', 'ContainerPermissions', 'BlobPermissions', 'PublicAccess', mod='blob.models')

    FileContentSettings, SharePermissions, FilePermissions = self.get_sdk(
        'ContentSettings', 'SharePermissions', 'FilePermissions', mod='file.models')

    TableService, TablePayloadFormat = get_table_data_type(self.cli_ctx, 'table', 'TableService', 'TablePayloadFormat')
...
# region Storage
    ...
    with self.argument_context('storage', arg_group='Precondition') as c:
        c.argument('if_modified_since', help='Alter only if modified since supplied UTC datetime (Y-m-d\'T\'H:M\'Z\')', type=get_datetime_type(False))
        c.argument('if_unmodified_since', help='Alter only if unmodified since supplied UTC datetime (Y-m-d\'T\'H:M\'Z\')', type=get_datetime_type(False))
        c.argument('if_match')
...
# region StorageAccount
    ...
    def register_common_storage_account_options(context):
        context.argument('https_only', help='Allows https traffic only to storage service.', arg_type=get_three_state_flag())
        context.argument('sku', help='The storage account SKU.', arg_type=get_enum_type(SkuName))
        context.argument('assign_identity', action='store_true', resource_type=ResourceType.MGMT_STORAGE, min_api='2017-06-01', help='Generate and assign a new Storage Account Identity for this storage account for use with key management services like Azure KeyVault.')
        context.argument('access_tier', arg_type=get_enum_type(AccessTier), help='The access tier used for billing StandardBlob accounts. Cannot be set for StandardLRS, StandardGRS, StandardRAGRS, or PremiumLRS account types. It is required for StandardBlob accounts during creation')

        encryption_services_model = self.get_models('EncryptionServices', resource_type=ResourceType.MGMT_STORAGE)
        if encryption_services_model:
            ...
            context.argument('encryption_services', arg_type=get_enum_type(encryption_choices), resource_type=ResourceType.MGMT_STORAGE, min_api='2016-12-01', nargs='+', validator=validate_encryption_services, help='Specifies which service(s) to encrypt.')

    with self.argument_context('storage account create') as c:
        register_common_storage_account_options(c)
        c.argument('location', get_location_type(self.cli_ctx), validator=get_default_location_from_resource_group)
        c.argument('account_type', help='The storage account type', arg_type=get_enum_type(AccountType))
        c.argument('account_name', account_name_type, options_list=['--name', '-n'], completer=None)
        c.argument('kind', help='Indicates the type of storage account.', arg_type=get_enum_type(Kind, default='storage'))
    ...
    with self.argument_context('storage account update') as c:
        register_common_storage_account_options(c)
        c.argument('custom_domain', help='User domain assigned to the storage account. Name is the CNAME source. Use "" to clear existing value.', validator=validate_custom_domain)
        c.argument('use_subdomain', help='Specify whether to use indirect CNAME validation.', arg_type=get_enum_type(['true', 'false']))
        c.argument('tags', tags_type, default=None)

    for scope in ['storage account create', 'storage account update']:
        with self.argument_context(scope, resource_type=ResourceType.MGMT_STORAGE, min_api='2017-06-01', arg_group='Network Rule') as c:
            c.argument('bypass', nargs='+', validator=validate_bypass, arg_type=get_enum_type(Bypass), help='Bypass traffic for space-separated uses.')
            c.argument('default_action', arg_type=get_enum_type(DefaultAction), help='Default action to apply when no rule matches.')
    ...
    with self.argument_context('storage account show-connection-string') as c:
        ...
        c.argument('key_name', options_list=['--key'], help='The key to use.', arg_type=get_enum_type(list(storage_account_key_options.keys())))
    ...
    with self.argument_context('storage account key renew') as c:
        c.argument('key_name', options_list=['--key'], help='The key to regenerate.', validator=validate_key, arg_type=get_enum_type(list(storage_account_key_options.keys())))
