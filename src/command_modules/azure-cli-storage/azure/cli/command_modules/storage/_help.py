...
examples:
-   name: Create a storage account 'MyStorageAccount' in resource group 'MyResourceGroup'
        in the West US region with locally redundant storage.
    text: az storage account create -n MyStorageAccount -g MyResourceGroup -l westus
        --sku Standard_LRS
    min_profile: latest
-   name: Create a storage account 'MyStorageAccount' in resource group 'MyResourceGroup'
        in the West US region with locally redundant storage.
...
-   name: Create a storage account.
    text: az storage account create --kind BlobStorage --resource-group MyResourceGroup
        --sku Standard_LRS --location westus --name MyStorageAccount
    crafted: 'True'
"""
...
-   name: Delete a storage account.
    text: az storage account delete --resource-group MyResourceGroup --yes  --name
        MyStorageAccount
    crafted: 'True'
"""
...
parameters:
-   name: --services
    short-summary: 'The storage services the SAS is applicable for. Allowed values:
        (b)lob (f)ile (q)ueue (t)able. Can be combined.'
-   name: --resource-types
    short-summary: 'The resource types the SAS is applicable for. Allowed values:
        (s)ervice (c)ontainer (o)bject. Can be combined.'
-   name: --expiry
    short-summary: Specifies the UTC datetime (Y-m-d'T'H:M'Z') at which the SAS becomes
        invalid.
-   name: --start
    short-summary: Specifies the UTC datetime (Y-m-d'T'H:M'Z') at which the SAS becomes
        valid. Defaults to the time of the request.
-   name: --account-name
    short-summary: 'Storage account name. Must be used in conjunction with either
        storage account key or a SAS token. Environment Variable: AZURE_STORAGE_ACCOUNT'
examples:
...
-   name: Generate a sas token for the account that is valid for queue and table services
    ...
    text: |
        ...
        az storage account generate-sas --permissions cdlruwap --account-name MyStorageAccount --services qt --resource-types sco --expiry $end -otsv
-   name: Generates a shared access signature for the account.
    text: az storage account generate-sas --output json --expiry 2018-02-01't'12:20'z'
        --permissions {permissions} --services {services} --account-key {account-key}
        --resource-types {resource-types} --account-name MyAccount --https-only {https-only}
    crafted: 'True'
"""
...
-   name: List the primary and secondary keys for a storage account.
    text: az storage account keys list --output json --query [0] --account-name MyStorageAccount
...
-   name: List storage accounts.
    text: az storage account list --query [0]
    crafted: 'True'
"""
...
long-summary: >
    Rules can be created for an IPv4 address, address range (CIDR format), or a virtual
    network subnet.
examples:
-   name: Create a rule to allow a specific address-range.
    text: az storage account network-rule add -g myRg --account-name mystorageaccount
        --ip-address 23.45.1.0/24
-   name: Create a rule to allow access for a subnet.
    text: az storage account network-rule add -g myRg --account-name mystorageaccount
        --vnet myvnet --subnet mysubnet
-   name: Add a network rule.
    text: az storage account network-rule add --subnet mysubnet --account-name mystorageaccount
        --resource-group myRg
    crafted: 'True'
"""
...
-   name: List network rules.
    text: az storage account network-rule list --account-name MyAccount
    crafted: 'True'
"""
...
-   name: Show properties for a storage account using an account name and resource
        group.
    text: az storage account show -g MyResourceGroup -n MyStorageAccount
-   name: Show storage account properties.
    text: az storage account show --resource-group MyResourceGroup --name MyStorageAccount
    crafted: 'True'
"""
...
-   name: Get the connection string for a storage account.
    text: az storage account show-connection-string --resource-group MyResourceGroup
...
type: command
short-summary: Show the current count and limit of the storage accounts under the
    subscription.
"""
...
examples:
-   name: Update the properties of a storage account.
    text: az storage account update --default-action Allow --resource-group MyResourceGroup
        --name MyStorageAccount
    crafted: 'True'
"""
...
type: group
short-summary: Manage blob copy operations. Use `az storage blob show` to check the
    status of the blobs.
"""

helps["storage blob copy start"] = """
type: command
short-summary: Copies a blob asynchronously. Use `az storage blob show` to check the
    status of the blobs.
examples:
-   name: Copies a blob asynchronously.
    text: az storage blob copy start --destination-blob {destination-blob} --destination-container
        {destination-container} --source-container {source-container} --source-blob
        {source-blob}
    crafted: 'True'
"""
...
type: command
short-summary: Copy multiple blobs or files to a blob container. Use `az storage blob
    show` to check the status of the blobs.
parameters:
-   name: --destination-container -c
    type: string
    short-summary: The blob container where the selected source files or blobs will
        be copied to.
-   name: --pattern
    type: string
    short-summary: The pattern used for globbing files or blobs in the source. The
        supported patterns are '*', '?', '[seq', and '[!seq]'.
-   name: --dryrun
    type: bool
    short-summary: List the files or blobs to be uploaded. No actual data transfer
        will occur.
-   name: --source-account-name
    type: string
    short-summary: The source storage account from which the files or blobs are copied
...
-   name: --source-uri
    type: string
    short-summary: A URI specifying a file share or blob container from which the
        files or blobs are copied.
    long-summary: If the source is in another account, the source must either be public
        or be authenticated by using a shared access signature.
-   name: --source-sas
    type: string
    short-summary: The shared access signature for the source storage account.
examples:
-   name: Copy multiple blobs or files to a blob container.
    text: az storage blob copy start-batch --destination-container {destination-container}
        --source-account-name MySourceAccount --source-container {source-container}
...
long-summary: >
    The blob is marked for later deletion during garbage collection.  In order to
    delete a blob, all of its snapshots must also be deleted.
    Both can be removed at the same time.
examples:
-   name: Delete a blob.
    text: az storage blob delete -c MyContainer -n MyBlob
-   name: Mark a blob or snapshot for deletion.
    text: az storage blob delete --container-name MyContainer --name MyBlob
    crafted: 'True'
"""
...
-   name: --source -s
    ...
    long-summary: The source can be the container URL or the container name. When
        the source is the container URL, the storage account name will be parsed from
        the URL.
-   name: --pattern
    type: string
    short-summary: The pattern used for globbing files or blobs in the source. The
...
-   name: --dryrun
    type: bool
    short-summary: Show the summary of the operations to be taken instead of actually
        deleting the file(s).
-   name: --if-match
    type: string
    short-summary: An ETag value, or the wildcard character (*). Specify this header
...
-   name: --if-none-match
    ...
    long-summary: Specify this header to perform the operation only if the resource's
        ETag does not match the value specified. Specify the wildcard character (*)
        to perform the operation only if the resource does not exist, and fail the
        operation if it does exist.
examples:
-   name: Delete all blobs ending with ".py" in a container that have not been modified
        for 10 days.
    text: |
...
-   name: Delete blobs from a blob container recursively.
    text: az storage blob delete-batch --source {source} --pattern {pattern} --if-unmodified-since
        2018-02-01't'12:20'z'
    crafted: 'True'
"""
...
-   name: Download all blobs that end with .py
    text: az storage blob download-batch -d . --pattern *.py -s MyContainer --account-name
        MyStorageAccount
-   name: Download blobs from a blob container recursively.
    text: az storage blob download-batch --no-progress  --source MyContainer --account-name
        MyStorageAccount --destination . --pattern *.py
    crafted: 'True'
"""
...
examples:
-   name: Check for the existence of a blob in a container.
    text: az storage blob exists --account-key {account-key} --container-name MyContainer
        --account-name MyAccount --name MyBlob
    crafted: 'True'
"""
...
-   name: Generates a shared access signature for the blob.
    text: az storage blob generate-sas --container-name MyContainer --expiry 2018-02-01't'12:20'z'
        --permissions {permissions} --name MyBlob --output json --https-only {https-only}
        --start 2018-01-01't'12:20'z'
    crafted: 'True'
"""
...
long-summary: This operation returns a copy operation properties object, including
    a copy ID you can use to check or abort the copy operation. The Blob service copies
    blobs on a best-effort basis. The source blob for an incremental copy operation
    must be a page blob. Call get_blob_properties on the destination blob to check
    the status of the copy operation. The final blob will be committed when the copy
    completes.
examples:
-   name: Upload all files that end with .py unless blob exists and has been modified
        since given date.
    text: az storage blob incremental-copy start --source-container MySourceContainer
        --source-blob MyBlob --source-account-name MySourceAccount --source-account-key
        MySourceKey --source-snapshot MySnapshot --destination-container MyDestinationContainer
...
parameters:
-   name: --include
    short-summary: 'Specifies additional datasets to include: (c)opy-info, (m)etadata,
        (s)napshots, (d)eleted-soft. Can be combined.'
examples:
-   name: List all storage blobs in a container whose names start with 'foo'; will
        match names such as 'foo', 'foobar', and 'foo/bar'
    text: az storage blob list -c MyContainer --prefix foo
-   name: List blobs in a given container.
    text: az storage blob list --container-name MyContainer --include  --connection-string
        {connection-string} --query [0]
    crafted: 'True'
"""
...
examples:
-   name: Update storage blob service properties.
    text: az storage blob service-properties update --404-document {404-document}
        --index-document {index-document} --account-name MyAccount --static-website
        false
    crafted: 'True'
"""
...
-   name: --timeout
    short-summary: The timeout parameter is expressed in seconds. This method may
        make multiple calls to the Azure service and the timeout will apply to each
        call individually.
long-summary: >
...
examples:
-   name: Set the block or page tiers on the blob.
    text: az storage blob set-tier --account-key {account-key} --container-name MyContainer
        --account-name MyAccount --tier {tier} --name MyBlob
    crafted: 'True'
"""
...
-   name: Get the details of a blob.
    text: az storage blob show --account-key {account-key} --container-name MyContainer
        --account-name MyAccount --query [0] --name MyBlob
    crafted: 'True'
"""
...
long-summary: Creates a new blob from a file path, or updates the content of an existing
...
-   name: --validate-content
    short-summary: Specifies that an MD5 hash shall be calculated for each chunk of
        the blob and verified by the service when the chunk has arrived.
-   name: --tier
    short-summary: A page blob tier value to set the blob to. The tier correlates
        to the size of the blob and number of allowed IOPS. This is only applicable
        to page blobs on premium storage accounts.
examples:
-   name: Upload to a blob.
    text: az storage blob upload -f /path/to/file -c MyContainer -n MyBlob
-   name: Upload a file to a storage blob.
    text: az storage blob upload --no-progress  --file /path/to/file --container-name
        MyContainer --name MyBlob
    crafted: 'True'
"""
...
-   name: --destination -d
    ...
    long-summary: The destination can be the container URL or the container name.
        When the destination is the container URL, the storage account name will be
        parsed from the URL.
-   name: --pattern
    type: string
    short-summary: The pattern used for globbing files or blobs in the source. The
...
-   name: --validate-content
    short-summary: Specifies that an MD5 hash shall be calculated for each chunk of
        the blob and verified by the service when the chunk has arrived.
-   name: --type -t
    short-summary: Defaults to 'page' for *.vhd files, or 'block' otherwise. The setting
        will override blob types for every file.
-   name: --maxsize-condition
    short-summary: The max length in bytes permitted for an append blob.
-   name: --lease-id
...
-   name: Upload all files that end with .py unless blob exists and has been modified
        since given date.
    text: az storage blob upload-batch -d MyContainer --account-name MyStorageAccount
        -s directory_path --pattern *.py --if-unmodified-since 2018-08-27T20:51Z
-   name: Upload files from a local directory to a blob container.
    text: az storage blob upload-batch --source directory_path --destination MyContainer
    crafted: 'True'
"""
...
examples:
-   name: Creates the url to access a blob.
    text: az storage blob url --container-name MyContainer --name MyBlob
    crafted: 'True'
"""
...
-   name: Create a storage container in a storage account and return an error if the
        container already exists.
    text: az storage container create -n MyStorageContainer --fail-on-exist
-   name: Create a container in a storage account.
    text: az storage container create --name MyStorageContainer
    crafted: 'True'
"""
...
long-summary: >
    ...
    collection.
examples:
-   name: Marks the specified container for deletion.
    text: az storage container delete --account-key {account-key} --account-name MyAccount
        --name MyContainer
    crafted: 'True'
"""
...
examples:
-   name: Check for the existence of a storage container.
    text: az storage container exists --account-key {account-key} --account-name MyAccount
        --name MyContainer
    crafted: 'True'
"""
...
-   name: Generates a shared access signature for the container.
    text: az storage container generate-sas --connection-string {connection-string}
        --expiry 2018-02-01't'12:20'z' --permissions {permissions} --name MyContainer
    crafted: 'True'
"""
...
-   name: List containers in a storage account.
    text: az storage container list --output json --account-name MyAccount
    crafted: 'True'
"""
...
-   name: --max-age
    short-summary: The maximum number of seconds the client/browser should cache a
        preflight response.
-   name: --origins
    short-summary: Space-separated list of origin domains that will be allowed via
        CORS, or '*' to allow all domains.
-   name: --methods
    short-summary: Space-separated list of HTTP methods allowed to be executed by
        the origin.
-   name: --allowed-headers
    short-summary: Space-separated list of response headers allowed to be part of
...
-   name: --services
    short-summary: >
        The storage service(s) to remove rules from. Allowed options are: (b)lob,
        (f)ile,
        (q)ueue, (t)able. Can be combined.
"""
...
-   name: Check for the existence of a storage directory.
    text: az storage directory exists --share-name MyShare --account-key {account-key}
        --account-name MyAccount --name MyDirectory
    crafted: 'True'
"""
...
-   name: --entity -e
    type: list
    short-summary: Space-separated list of key=value pairs. Must contain a PartitionKey
        and a RowKey.
    long-summary: The PartitionKey and RowKey must be unique within the table, and
        may be up to 64Kb in size. If using an integer value as a key, convert it
        to a fixed-width string which can be canonically sorted. For example, convert
        the integer value 1 to the string value "0000001" to ensure proper sorting.
-   name: --if-exists
    type: string
    short-summary: Behavior when an entity already exists for the specified PartitionKey
        and RowKey.
-   name: --timeout
    short-summary: The server timeout, expressed in seconds.
examples:
-   name: Insert an entity into a table.
    text: az storage entity insert --sas-token {sas-token} --entity {entity} --table-name
        MyTable --account-name MyAccount --if-exists fail
    crafted: 'True'
"""
...
-   name: --marker
    type: list
    short-summary: Space-separated list of key=value pairs. Must contain a nextpartitionkey
        and a nextrowkey.
    long-summary: This value can be retrieved from the next_marker field of a previous
        generator object if max_results was specified and that generator has finished
        enumerating results. If specified, this generator will begin returning results
        from the point where the previous generator stopped.
examples:
-   name: List entities which satisfy a query.
    text: az storage entity query --filter {filter} --account-key {account-key} --table-name
        MyTable --account-name MyAccount --select {select}
    crafted: 'True'
"""
...
-   name: --destination-path
    type: string
    short-summary: The directory where the source data is copied to. If omitted, data
        is copied to the root directory.
-   name: --pattern
    type: string
    short-summary: The pattern used for globbing files and blobs. The supported patterns
        are '*', '?', '[seq', and '[!seq]'.
-   name: --dryrun
    type: bool
    short-summary: List the files and blobs to be copied. No actual data transfer
        will occur.
-   name: --source-account-name
    type: string
    short-summary: The source storage account to copy the data from. If omitted, the
        destination account is used.
-   name: --source-account-key
    type: string
    short-summary: The account key for the source storage account. If omitted, the
        active login is used to determine the account key.
-   name: --source-container
    type: string
...
-   name: --source-uri
    ...
    long-summary: If the source is in another account, the source must either be public
        or authenticated via a shared access signature.
-   name: --source-sas
    type: string
    short-summary: The shared access signature for the source storage account.
"""
...
-   name: --source -s
    type: string
    short-summary: The source of the file delete operation. The source can be the
        file share URL or the share name.
-   name: --pattern
    type: string
    short-summary: The pattern used for file globbing. The supported patterns are
        '*', '?', '[seq]', and '[!seq]'.
-   name: --dryrun
    type: bool
    short-summary: List the files and blobs to be deleted. No actual data deletion
        will occur.
examples:
-   name: Delete files from an Azure Storage File Share.
    text: az storage file delete-batch --account-key {account-key} --source {source}
        --account-name MyAccount
    crafted: 'True'
"""
...
-   name: --source -s
    type: string
    short-summary: The source of the file download operation. The source can be the
        file share URL or the share name.
-   name: --destination -d
    type: string
    short-summary: The local directory where the files are downloaded to. This directory
        must already exist.
-   name: --pattern
    type: string
    short-summary: The pattern used for file globbing. The supported patterns are
...
-   name: --dryrun
    type: bool
    short-summary: List the files and blobs to be downloaded. No actual data transfer
        will occur.
-   name: --max-connections
    type: integer
    short-summary: The maximum number of parallel connections to use. Default value
        is 1.
-   name: --validate-content
    type: bool
    short-summary: If set, calculates an MD5 hash for each range of the file for validation.
    long-summary: >
        The storage service checks the hash of the content that has arrived is identical
        to the hash that was sent.
        This is mostly valuable for detecting bitflips during transfer if using HTTP
        instead of HTTPS. This hash is not stored.
examples:
-   name: Download files from an Azure Storage File Share to a local directory in
        a batch operation.
    text: az storage file download-batch --account-key {account-key} --source {source}
        --account-name MyAccount --destination {destination} --pattern {pattern}
    crafted: 'True'
"""
...
-   name: Check for the existence of a file.
    text: az storage file exists --share-name MyShare --timeout {timeout} --path {path}
        --connection-string {connection-string} --output json
    crafted: 'True'
"""
...
-   name: Generates a shared access signature for the file.
    text: az storage file generate-sas --share-name MyShare --path {path} --expiry
        2018-02-01't'12:20'z' --permissions {permissions} --https-only {https-only}
        --account-name MyAccount
    crafted: 'True'
"""
...
examples:
-   name: List files and directories in a share.
    text: az storage file list --share-name MyShare --connection-string {connection-string}
    crafted: 'True'
"""
...
long-summary: Creates or updates an Azure file from a source path with automatic chunking
...
-   name: Upload a file to a share that uses the SMB 3.0 protocol.
    text: az storage file upload --share-name MyShare --source {source} --metadata  --connection-string
        {connection-string}
    crafted: 'True'
"""
...
type: command
short-summary: Upload files from a local directory to an Azure Storage File Share
    in a batch operation.
parameters:
-   name: --source -s
    type: string
...
-   name: --destination -d
    ...
    long-summary: The destination can be the file share URL or the share name. When
        the destination is the share URL, the storage account name is parsed from
        the URL.
-   name: --destination-path
    type: string
    short-summary: The directory where the source data is copied to. If omitted, data
...
-   name: --dryrun
    type: bool
    short-summary: List the files and blobs to be uploaded. No actual data transfer
        will occur.
-   name: --max-connections
    type: integer
    short-summary: The maximum number of parallel connections to use. Default value
...
examples:
-   name: Creates the url to access a file.
    text: az storage file url --share-name MyShare --connection-string {connection-string}
        --path {path}
    crafted: 'True'
"""
...
parameters:
-   name: --services
    short-summary: 'The storage services from which to retrieve logging info: (b)lob
        (q)ueue (t)able. Can be combined.'
"""
...
parameters:
-   name: --services
    short-summary: 'The storage service(s) for which to update logging info: (b)lob
        (q)ueue (t)able. Can be combined.'
-   name: --log
    short-summary: 'The operations for which to enable logging: (r)ead (w)rite (d)elete.
        Can be combined.'
-   name: --retention
...
parameters:
-   name: --services
    short-summary: 'The storage services from which to retrieve metrics info: (b)lob
        (q)ueue (t)able. Can be combined.'
-   name: --interval
    short-summary: Filter the set of metrics to retrieve by time interval
examples:
-   name: Show metrics settings for a storage account.
    text: az storage metrics show --services {services} --account-name MyAccount
    crafted: 'True'
"""
...
-   name: --api
    short-summary: Specify whether to include API in metrics. Applies to both hour
        and minute metrics if both are specified. Must be specified if hour or minute
        metrics are enabled and being updated.
-   name: --retention
    short-summary: Number of days for which to retain metrics. 0 to disable. Applies
        to both hour and minute metrics if both are specified.
examples:
-   name: Update metrics settings for a storage account.
    text: az storage metrics update --retention {retention} --account-name MyAccount
        --hour false --api false --services {services} --minute false
    crafted: 'True'
"""
...
examples:
-   name: Creates a new share under the specified account.
    text: az storage share create --quota {quota} --connection-string {connection-string}
        --name MyFileShare
    crafted: 'True'
"""
...
-   name: Generates a shared access signature for the share.
    text: az storage share generate-sas --https-only {https-only} --connection-string
        {connection-string} --expiry 2018-02-01't'12:20'z' --permissions {permissions}
        --name MyFileShare
    crafted: 'True'
"""
...
examples:
-   name: List the file shares in a storage account.
    text: az storage share list --output json --account-name MyAccount
    crafted: 'True'
"""
...
examples:
-   name: List tables in a storage account.
    text: az storage table list --query [0] --account-name MyAccount
    crafted: 'True'
"""
