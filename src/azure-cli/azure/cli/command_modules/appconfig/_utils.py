...
def construct_connection_string(cmd, config_store_name):
    ...
    try:
        ...
        raise CLIError('Cannot find the App Configuration store {}.'.format(config_store_name))

    raise CLIError('Cannot find a read write access key for the App Configuration store {}'.format(config_store_name))
...
def resolve_resource_group(cmd, config_store_name):
    ...
    raise CLIError("App Configuration store: {} does not exist".format(config_store_name))
...
def resolve_connection_string(cmd, config_store_name=None, connection_string=None):
    connection_string_env = cmd.cli_ctx.config.get('appconfig', 'connection_string', None)

    if not is_valid_connection_string(connection_string_env):
        raise CLIError("The environment variavle connection string is invalid.Correct format should be Endpoint=https://example.appconfig.io;Id=xxxxx;Secret=xxxx")
...
5 preset connection in environment variable like set AZURE_APPCONFIG_CONNECTION_STRING=xxxx'''
    ...
    if connection_string is not None:
        ...
        elif string != connection_string:
            raise CLIError(error_message)
    if connection_string_env is not None:
        if string == '':
            return connection_string_env
        if string != connection_string_env:
            raise CLIError(error_message)

    if string == '':
        raise CLIError('Please specify config store name or connection string(suggested).')
    return string
