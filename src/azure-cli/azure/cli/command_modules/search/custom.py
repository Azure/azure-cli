...
def setup_search_auth(cmd, instance, disable_local_auth, auth_options, aad_auth_failure_mode):
    ...
    if (aad_auth_failure_mode is not None and aad_auth_failure_mode not in ["http401WithBearerChallenge", "http403"]):
        raise UnrecognizedArgumentError(
            "SearchService.AuthOptions.AadAuthFailureMode: only [""http401WithBearerChallenge"", ""http403""] are allowed")

    if disable_local_auth and auth_options:
        raise MutuallyExclusiveArgumentError("Both the DisableLocalAuth and AuthOptions parameters can't be given at the same time")
    if disable_local_auth and aad_auth_failure_mode:
        raise MutuallyExclusiveArgumentError("Both the DisableLocalAuth and AadAuthFailureMode parameters can't be given at the same time")
    if auth_options == "apiKeyOnly" and aad_auth_failure_mode:
        raise MutuallyExclusiveArgumentError("Both an AuthOptions value of ""apiKeyOnly"" and an AadAuthFailureMode can't be given at the same time")
    if auth_options == "aadOrApiKey" and not(aad_auth_failure_mode):
        raise RequiredArgumentMissingError("An AuthOptions value of ""aadOrApiKey"" requires an AadAuthFailureMode parameter")
