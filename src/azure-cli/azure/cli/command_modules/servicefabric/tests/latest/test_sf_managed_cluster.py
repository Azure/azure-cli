...
class ServiceFabricManagedClustersTests(ScenarioTest):
    ...
    def test_node_type_operation_2(self):
        ...
        while True:
            try:
                self.cmd('az sf managed-node-type node restart -g {rg} -c {cluster_name} -n pnt --update-type ByUpgradeDomain ')
                break
            except HttpResponseError:
                if time.time() > timeout:
                    raise
        ...
        self.cmd('az sf managed-node-type node reimage -g {rg} -c {cluster_name} -n pnt --update-type ByUpgradeDomain ')
