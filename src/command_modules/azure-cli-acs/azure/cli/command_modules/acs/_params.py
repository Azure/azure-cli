...
register_cli_argument('acs', 'orchestrator_type', options_list=('--orchestrator-type', '-t'), help='DockerCE is in preview, only in ukwest and uksouth', **enum_choice_list(orchestratorTypes))
...
register_cli_argument('acs', 'api_version', options_list=('--api-version',), required=False, help='Feature in preview, only in ukwest and uksouth. Use API version of ACS to perform az acs operations. Available options: 2017-01-31, 2017-07-01. Default to use the latest version for the location')
...
register_cli_argument('acs create', 'master_profile', options_list=('--master-profile', '-m'), type=validate_file_or_dict, help='Feature in preview, only in ukwest and uksouth. The file or dictionary representation of the master profile. Note it will override any master settings once set')
register_cli_argument('acs create', 'master_vm_size', completer=get_vm_size_completion_list, help='Feature in preview, only in ukwest and uksouth')
register_cli_argument('acs create', 'master_osdisk_size', type=int, help='Feature in preview, only in ukwest and uksouth. The disk size for master pool vms. Unit in GB. If not specified, the corresponding vmsize disk size will apply')
register_cli_argument('acs create', 'master_vnet_subnet_id', type=str, help='Feature in preview, only in ukwest and uksouth. The custom vnet subnet id. Note agent need to used the same vnet if master set')
register_cli_argument('acs create', 'master_first_consecutive_static_ip', type=str, help='Feature in preview, only in ukwest and uksouth. The first consecutive ip used to specify static ip block')
register_cli_argument('acs create', 'master_storage_profile', help='Feature in preview, only in ukwest and uksouth.', **enum_choice_list(storageProfileTypes))
register_cli_argument('acs create', 'agent_profiles', options_list=('--agent-profiles', '-a'), type=validate_file_or_dict, help='Feature in preview, only in ukwest and uksouth. The file or dictionary representation of the agent profiles. Note it will override any agent settings once set')
register_cli_argument('acs create', 'agent_vm_size', completer=get_vm_size_completion_list)
register_cli_argument('acs create', 'agent_osdisk_size', type=int, help='Feature in preview, only in ukwest and uksouth. The disk size for agent pool vms. Unit in GB. If not specified, the corresponding vmsize disk size will apply')
register_cli_argument('acs create', 'agent_vnet_subnet_id', type=str, help='Feature in preview, only in ukwest and uksouth. The custom vnet subnet id. Note agent need to used the same vnet if master set')
register_cli_argument('acs create', 'agent_ports', type=validate_list_of_integers, help='Feature in preview, only in ukwest and uksouth. The ports exposed on the agent pool, such as 8080,4000,80')
register_cli_argument('acs create', 'agent_storage_profile', help='Feature in preview, only in ukwest and uksouth.', **enum_choice_list(storageProfileTypes))
