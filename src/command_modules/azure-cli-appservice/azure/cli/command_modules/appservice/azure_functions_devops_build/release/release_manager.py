...
class ReleaseManager(BaseManager):
    ...
    def create_release_definition(self, build_name, artifact_name, pool_name, service_endpoint_name,  # pylint: disable=too-many-locals
        ...
        retention_policy_environment = _get_retention_policy()
        artifact = self._get_artifact(build, project, artifact_name)
        pre_release_approvals, post_release_approvals = _get_pre_post_approvals()
        ...
        triggers = _get_triggers(artifact_name)
        deployment_input = _get_deployment_input(pool.id)

        phase_inputs = _get_phase_inputs(artifact_name)

        workflowtasks = []

        if app_type == LINUX_CONSUMPTION:
            workflowtasks.append(_blob_task(service_endpoint.id, storage_name))
            workflowtasks.append(_sas_token_task(service_endpoint.id, storage_name))
            workflowtasks.append(_app_settings_task(service_endpoint.id, functionapp_name, resource_name))
        elif app_type == LINUX_DEDICATED:
            workflowtasks.append(_app_service_deploy_task_linux(service_endpoint.id, functionapp_name))
        elif app_type == WINDOWS:
            workflowtasks.append(_app_service_deploy_task_windows(service_endpoint.id, functionapp_name))
        else:
            logging.error("Invalid app type provided. Correct types are: Linux Consumption: %s, Linux Dedicated: %s, Windows: %s",  # pylint: disable=line-too-long
        ...
        if settings is not None:
            ...
            workflowtasks.append(_app_settings_task_customized(service_endpoint.id, functionapp_name,
                                                               resource_name, settings_str))

        deploy_phases = _get_deploy_phases(deployment_input, workflowtasks, phase_inputs)
...
def _get_triggers(artifact_name):
    trigger = {}
...
def _get_deployment_input(pool_id):
    deployment_input = {}
...
def _get_phase_inputs(artifact_name):
    phase_inputs = {}
    download_input = {}
...
def _get_deploy_phases(deployment_input, workflowtasks, phase_inputs):
    deploy_phase = {}
...
def _get_retention_policy():
    return models.environment_retention_policy.EnvironmentRetentionPolicy(days_to_keep=300,
                                                                          releases_to_keep=3,
                                                                          retain_build=True)


def _get_pre_post_approvals():
    pre_approval = models.release_definition_approval_step.ReleaseDefinitionApprovalStep(id=0,
                                                                                         rank=1,
                                                                                         is_automated=True,
                                                                                         is_notification_on=False)
    post_approval = models.release_definition_approval_step.ReleaseDefinitionApprovalStep(id=0,
                                                                                          rank=1,
                                                                                          is_automated=True,
                                                                                          is_notification_on=False)
...
def _blob_task(connectedServiceNameARM, storage_name):
    blobtask = {}
...
def _sas_token_task(connectedServiceNameARM, storage_name):
    sastokentask = {}
...
def _app_settings_task(connectedServiceNameARM, functionapp_name, resource_name):
    appsetttingstask = {}
    ...
    appsetttingstask_inputs["AppSettings"] = "WEBSITE_RUN_FROM_PACKAGE='$(storageUri)/build$(Build.BuildId).zip$(storageToken)'"  # pylint: disable=line-too-long
    appsetttingstask["inputs"] = appsetttingstask_inputs
    return appsetttingstask


def _app_settings_task_customized(connectedServiceNameARM, functionapp_name, resource_name, settings):
    appsetttingstask = {}
    appsetttingstask["name"] = "Set App Settings: "
    appsetttingstask["enabled"] = True
    appsetttingstask["taskId"] = "9d2e4cf0-f3bb-11e6-978b-770d284f4f2d"
...
def _app_service_deploy_task_linux(connectedServiceNameARM, functionapp_name):
    appservicetask = {}
...
def _app_service_deploy_task_windows(connectedServiceNameARM, functionapp_name):
    appservicetask = {}
    appservicetask["name"] = "Azure App Service Deploy: " + functionapp_name
    appservicetask["enabled"] = True
    appservicetask["taskId"] = "497d490f-eea7-4f2b-ab94-48d9c1acdcb1"
