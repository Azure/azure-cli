...
class AcrTaskCommandsTests(ScenarioTest):
    ...
    def test_acr_task(self, resource_group):
        self.kwargs.update({
            ...
            'artifactContext': 'oci://myregistry.azurecr.io/hello-world:v1',
            'file': 'Dockerfile',
            'image': 'testtask:v1',
            'existing_image': 'bash',
        ...
        # Create a docker build task with artifact context
        self.cmd('acr task create -n {task_artifact} -r {registry_name} --context {artifactContext} --image {image} -f {file}',
                 checks=[self.check('name', '{task_artifact}'),
                         self.check('location', '{rg_loc}'),
                         self.check('platform.os', 'linux'),
                         self.check('agentConfiguration.cpu', 2),
                         self.check('provisioningState', 'Succeeded'),
