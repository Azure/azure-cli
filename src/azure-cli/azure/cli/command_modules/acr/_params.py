...
def load_arguments(self, _):  # pylint: disable=too-many-statements
    ...
    with self.argument_context('acr connected-acr') as c:
        ...
        c.argument('mode', options_list=['--mode', '-m'], type=int, help='Can be one of the two operating modes: registry or mirror(pull-only mode).', required=False, default="registry")
        ...
        c.argument('sync_schedule', options_list=['--sync-schedule', '-s'], type=int, help='Optional parameter to define the sync schedule. Uses cron expression to determine the schedule. If not specified, the instance is considered always online and attempts to sync every minute.', required=False, default="* * * * *")
        ...
        c.argument('next_update', options_list=['--next-update'], type=int, help='Optional ISO 8601 timestamp parameter used to specify when the next update for the instance will happen.', required=False)
        c.argument('log_level', options_list=['--log-level'], type=int, help='Sets the log level for logging on the instance. Accepted log levels are Debug, Information, Warning, Error, and None. ', required=False, default="Information")
        ...
        c.argument('sync_audit_logs_enabled', options_list=['--sync-audit-logs-enabled'], type=int, help='Indicates whether audit log synchronization is enabled. It\'s enabled by default.', required=False, arg_type=get_three_state_flag())
