...
def load_command_table(self, _):
    ...
    with self.command_group('consumption pricesheet') as p:
        p.generic_show_command('show', 'cli_consumption_list_pricesheet_show', transform=transform_pricesheet_show_output,
                               exception_handler=consumption_exception_handler, client_factory=pricesheet_mgmt_client_factory, custom_command=True)
    ...
    with self.command_group('consumption budget', exception_handler=consumption_exception_handler, client_factory=budget_mgmt_client_factory) as p:
        ...
        p.generic_show_command('show', 'cli_consumption_show_budget', transform=transform_budget_show_output, custom_command=True)
