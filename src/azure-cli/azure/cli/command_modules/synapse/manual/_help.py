...
examples:
  ...
  - name: When you are a user with permission to manage Azure RBAC role assignment on the workspace but not a Synapse Administrator, please create Synapse Administrator role by -role "6e4bf58a-b8e1-4cc3-bbf9-d73143322b78". The reason for this is , when you try to add a "Synapse Administrator" role, the cmdlet needs to get the role ID from the role name which requires workspace read permission, which the current user does not have.
    text: |-
