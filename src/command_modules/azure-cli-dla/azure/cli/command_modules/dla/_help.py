...
type: group
short-summary: (PREVIEW) Manage links between Data Lake Analytics accounts and Azure
    Storage.
"""

helps["dla account blob-storage add"] = """
type: command
short-summary: Links an Azure Storage account to the specified Data Lake Analytics
    account.
"""

helps["dla account blob-storage update"] = """
type: command
short-summary: Updates an Azure Storage account linked to the specified Data Lake
    Analytics account.
"""
...
-   name: --max-dop-per-job
    type: int
    short-summary: The maximum degree of parallelism allowed per job for this policy.
        At least one of `--min-priority-per-job` and `--max-dop-per-job` must be specified.
-   name: --min-priority-per-job
    type: int
    short-summary: The minimum priority allowed per job for this policy. At least
        one of `--min-priority-per-job` and `--max-dop-per-job` must be specified.
-   name: --compute-policy-name
    type: string
    short-summary: The name of the compute policy to create.
-   name: --object-id
    type: string
    short-summary: The Azure Active Directory object ID of the user, group, or service
        principal to apply the policy to.
-   name: --object-type
    type: string
    short-summary: The Azure Active Directory object type associated with the supplied
        object ID.
"""
...
-   name: --default-data-lake-store
    type: string
    short-summary: The default Data Lake Store account to associate with the created
        account.
...
type: group
short-summary: (PREVIEW) Manage links between Data Lake Analytics and Data Lake Store
    accounts.
"""
...
-   name: --user-name
    type: string
    short-summary: The user name associated with the credential that will have its
        password updated.
"""
...
-   name: --table-name
    type: string
    short-summary: The table to list statistics for. `--schema-name` must also be
        specified.
"""
...
-   name: --schema-name
    type: string
    short-summary: The name of the schema assocated with table valued functions to
        list.
"""
...
examples:
-   name: Get information for a Data Lake Analytics job.
    text: az dla job show --account {account} --output json --job-identity {job-identity}
    crafted: 'True'
-   name: Get information for a Data Lake Analytics job.
    text: az dla job show --account {account} --output json --job-identity {job-identity}
    crafted: true
...
-   name: --runtime-version
    ...
    long-summary: This parameter is used for explicitly overwriting the default runtime.
        It should only be done if you know what you are doing.
-   name: --degree-of-parallelism
    short-summary: The degree of parallelism for the job.
    long-summary: Higher values equate to more parallelism and will usually yield
        faster running jobs, at the cost of more AUs.
-   name: --priority
    short-summary: The priority of the job.
    long-summary: Lower values increase the priority, with the lowest value being
        1. This determines the order jobs are run in.
"""
