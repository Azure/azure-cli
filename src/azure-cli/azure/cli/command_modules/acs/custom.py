...
from azure.mgmt.containerservice.v2020_09_01.models import (
...
from azure.mgmt.containerservice.v2019_09_30_preview.models import (
...
def _acs_browse_internal(
    cmd, client, acs_info, resource_group_name, name, disable_browser, ssh_key_file
):
    orchestrator_type = (
        acs_info.orchestrator_profile.orchestrator_type
    )  # pylint: disable=no-member

    if (
        str(orchestrator_type).lower() == "kubernetes"
        or orchestrator_type == ContainerServiceOrchestratorTypes.kubernetes
        or (
            acs_info.custom_profile
            and acs_info.custom_profile.orchestrator == "kubernetes"
        )
    ):  # pylint: disable=no-member
        return k8s_browse(
            cmd,
    ...
    if (
        str(orchestrator_type).lower() == "dcos"
        or orchestrator_type == ContainerServiceOrchestratorTypes.dcos
    ):
...
def k8s_browse(
    cmd, client, name, resource_group_name, disable_browser=False, ssh_key_file=None
):
    """
...
def _dcos_browse_internal(acs_info, disable_browser, ssh_key_file):
    ...
    if not acs.connect(
        _get_host_name(acs_info), _get_username(acs_info), key_filename=ssh_key_file
    ):
    ...
    try:
        acs.create_tunnel(
            remote_host="127.0.0.1", remote_port=remote_port, local_port=local_port
        )
    finally:
...
def acs_install_cli(
    cmd, client, resource_group_name, name, install_location=None, client_version=None
):
    acs_info = _get_acs_info(cmd.cli_ctx, name, resource_group_name)
    orchestrator_type = (
        acs_info.orchestrator_profile.orchestrator_type
...
def k8s_install_cli(
    ...
    base_src_url=None,
    kubelogin_version="latest",
    kubelogin_install_location=None,
    kubelogin_base_src_url=None,
):
    k8s_install_kubectl(cmd, client_version, install_location, base_src_url)
    k8s_install_kubelogin(
        cmd, kubelogin_version, kubelogin_install_location, kubelogin_base_src_url
    )


def k8s_install_kubectl(
...
def k8s_install_kubelogin(
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        try:
            ...
            logger.warning(
                'Downloading client to "%s" from "%s"', download_path, file_url
            )
...
def _add_role_assignment(
    ...
    for x in range(0, 10):
        hook.add(
            message="Waiting for AAD role to propagate", value=0.1 * x, total_val=1.0
        )
        try:
...
def delete_role_assignments(
    ...
    assignments = _search_role_assignments(
        cli_ctx,
        assignments_client,
        definitions_client,
        scope,
    ...
    for x in range(0, 10):
        ...
        try:
            delete_role_assignments(
                cli_ctx, role=role, assignee=service_principal, scope=scope
            )
            break
...
def _search_role_assignments(
    ...
    if scope:
        assignments = list(
            assignments_client.list_for_scope(scope=scope, filter="atScope()")
        )
    ...
    if assignments:
        assignments = [
            ...
            if (
                ...
                and re.match(_get_role_property(a, "scope"), scope, re.I)
                or _get_role_property(a, "scope").lower() == scope.lower()
            )
        ]
        ...
        if role:
            ...
            assignments = [
                ...
                if _get_role_property(i, "role_definition_id") == role_id
            ]
        ...
        if assignee_object_id:
            assignments = [
                i
                for i in assignments
                if _get_role_property(i, "principal_id") == assignee_object_id
            ]
...
def list_acs_locations(cmd, client):
    return {"productionRegions": regions_in_prod, "previewRegions": regions_in_preview}
...
def _generate_master_pool_profile(
    api_version,
    master_profile,
    master_count,
    dns_name_prefix,
    ...
    if api_version == "2017-07-01":
        default_master_pool_profile = _update_dict(
            default_master_pool_profile,
            {
                "count": int(master_count),
                "dnsPrefix": dns_name_prefix + "mgmt",
...
def _generate_agent_pool_profiles(
    api_version,
    agent_profiles,
    agent_count,
    dns_name_prefix,
    ...
    if api_version == "2017-07-01":
        default_agent_pool_profile = _update_dict(
            default_agent_pool_profile,
            {
                "count": int(agent_count),
                "vmSize": agent_vm_size,
    ...
    if agent_profiles is None:
        agent_pool_profiles.append(
            _update_dict(default_agent_pool_profile, {"name": "agentpool0"})
        )
    else:
...
def _generate_outputs(name, orchestrator_type, admin_username):
    ...
    outputs = {
        "masterFQDN": {
            "type": "string",
            "value": "[reference(concat('Microsoft.ContainerService/containerServices/', '{}')).masterProfile.fqdn]".format(
                name
            ),  # pylint: disable=line-too-long
        },
        "sshMaster0": {
            "type": "string",
            "value": "[concat('ssh ', '{0}', '@', reference(concat('Microsoft.ContainerService/containerServices/', '{1}')).masterProfile.fqdn, ' -A -p 22')]".format(
                admin_username, name
            ),  # pylint: disable=line-too-long
        },
    }
    if orchestrator_type.lower() != "kubernetes":
        outputs["agentFQDN"] = {
            "type": "string",
            "value": "[reference(concat('Microsoft.ContainerService/containerServices/', '{}')).agentPoolProfiles[0].fqdn]".format(
                name
            ),  # pylint: disable=line-too-long
        }
        ...
        outputs["sshMaster0"] = {
            "type": "string",
            "value": "[concat('ssh ', '{0}', '@', reference(concat('Microsoft.ContainerService/containerServices/', '{1}')).masterProfile.fqdn, ' -A -p 2200')]".format(
                admin_username, name
            ),  # pylint: disable=line-too-long
        }
    return outputs
...
def _generate_properties(
    ...
    properties = {
        ...
        "linuxProfile": {
            "ssh": {"publicKeys": [{"keyData": ssh_key_value}]},
            "adminUsername": admin_username,
        },
    }
    if api_version == "2017-07-01":
...
def acs_create(
    ...
    if orchestrator_type.lower() == "kubernetes":
        principal_obj = _ensure_service_principal(
            cmd.cli_ctx,
            service_principal,
            client_secret,
            subscription_id,
    ...
    master_pool_profile = _generate_master_pool_profile(
        api_version,
        master_profile,
        master_count,
        dns_name_prefix,
    ...
    agent_pool_profiles = _generate_agent_pool_profiles(
        api_version,
        agent_profiles,
        agent_count,
        dns_name_prefix,
    ...
    if service_principal is not None and client_secret is not None:
        ...
        params = {"clientSecret": {"value": client_secret}}
    ...
    for _ in range(0, max_retry):
        try:
            ...
            retry_exception = ex
            if (
                "is not valid according to the validation procedure" in ex.message
                or "The credentials in ServicePrincipalProfile were invalid"
                in ex.message
                or "not found in Active Directory tenant" in ex.message
            ):
                time.sleep(3)
            else:
                raise ex
...
def _invoke_deployment(
    ...
    if cmd.supported_api_version(
        min_api="2019-10-01", resource_type=ResourceType.MGMT_RESOURCE_RESOURCES
    ):
    ...
    return sdk_no_wait(
        no_wait, smc.create_or_update, resource_group_name, deployment_name, properties
    )
...
def _k8s_get_credentials_internal(
    name, acs_info, path, ssh_key_file, overwrite_existing
):
    ...
    acs_client.secure_copy(
        user,
        "{}.{}.cloudapp.azure.com".format(dns_prefix, location),
        ".kube/config",
        path_candidate,
...
def update_acs(
    cmd, client, resource_group_name, container_service_name, new_agent_count
):
    ...
    if (
        instance.orchestrator_profile.orchestrator_type
    ...
    return client.create_or_update(
        resource_group_name, container_service_name, instance
    )
    ...
    try:
        ...
        if "insufficient privileges" in str(ex).lower():
            ...
            raise CLIError(
                ...
                "For how to configure, please refer '{}'. Original error: {}".format(
                    link, ex
                )
            )
        raise
...
def create_service_principal(cli_ctx, identifier, resolve_app=True, rbac_client=None):
    ...
    if resolve_app:
        try:
            uuid.UUID(identifier)
            result = list(
                rbac_client.applications.list(filter="appId eq '{}'".format(identifier))
            )
        except ValueError:
            result = list(
...
def create_role_assignment(
    cli_ctx, role, assignee, is_service_principal, resource_group_name=None, scope=None
):
...
def _create_role_assignment(
    cli_ctx, role, assignee, resource_group_name=None, scope=None, resolve_assignee=True
):
    ...
    custom_headers = None
    return assignments_client.create(
        scope, assignment_name, parameters, custom_headers=custom_headers
    )
...
def _get_object_stubs(graph_client, assignees):
    params = GetObjectsParameters(
        include_directory_object_references=True, object_ids=assignees
    )
...
def subnet_role_assignment_exists(cli_ctx, scope):
    ...
    for i in assignments_client.list_for_scope(scope=scope, filter="atScope()"):
        if i.scope == scope and i.role_definition_id.endswith(
...
def aks_browse(
    ...
    if StrictVersion(instance.kubernetes_version) >= StrictVersion("1.19.0") or (
        not addon_profile.enabled
    ):
        subscription_id = get_subscription_id(cmd.cli_ctx)
        dashboardURL = cmd.cli_ctx.cloud.endpoints.portal + (  # Azure Portal URL (https://portal.azure.com for public cloud)
            "/#resource/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.ContainerService"
            "/managedClusters/{2}/workloads"
        ).format(
        ...
        if in_cloud_console():
            logger.warning(
                "To view the Kubernetes resources view, please open %s in a new tab",
                dashboardURL,
            )
        else:
...
def _add_monitoring_role_assignment(result, cluster_resource_id, cmd):
    ...
    if (
        hasattr(result, "service_principal_profile")
        and hasattr(result.service_principal_profile, "client_id")
        and result.service_principal_profile.client_id.lower() != "msi"
    ):
        logger.info("valid service principal exists, using it")
        service_principal_msi_id = result.service_principal_profile.client_id
        is_service_principal = True
    elif (
        (hasattr(result, "addon_profiles"))
        and (CONST_MONITORING_ADDON_NAME in result.addon_profiles)
        and (hasattr(result.addon_profiles[CONST_MONITORING_ADDON_NAME], "identity"))
        and (
            hasattr(
...
def _add_ingress_appgw_addon_role_assignment(result, cmd):
    ...
    if (
        ...
        and (hasattr(result.addon_profiles[CONST_INGRESS_APPGW_ADDON_NAME], "identity"))
        and (
            hasattr(
...
def _add_virtual_node_role_assignment(cmd, result, vnet_subnet_id):
    ...
    if (
        ...
        (hasattr(result, "addon_profiles"))
        and (addon_name in result.addon_profiles)
        and (hasattr(result.addon_profiles[addon_name], "identity"))
        and (hasattr(result.addon_profiles[addon_name].identity, "object_id"))
    ):
...
def aks_create(
    ...
    if (
        vnet_subnet_id
        and not skip_subnet_role_assignment
        and not subnet_role_assignment_exists(cmd.cli_ctx, vnet_subnet_id)
    ):
        ...
        if service_principal_profile is None and not assign_identity:
            ...
            if not _add_role_assignment(
                cmd.cli_ctx, "Network Contributor", identity_client_id, scope=scope
            ):
                logger.warning(
    ...
    if enable_aad:
        if any([aad_client_app_id, aad_server_app_id, aad_server_app_secret]):
            raise CLIError(
                '"--enable-aad" cannot be used together with '
        ...
        if any(
            [aad_client_app_id, aad_server_app_id, aad_server_app_secret, aad_tenant_id]
        ):
    ...
    if uptime_sla:
        mc.sku = ManagedClusterSKU(name="Basic", tier="Paid")
    ...
    for _ in range(0, max_retry):
        try:
            need_pull_for_result = (
                monitoring
                or (enable_managed_identity and attach_acr)
                or ingress_appgw_addon_enabled
                or enable_virtual_node
                or need_post_creation_vnet_permission_granting
            )
            ...
            if enable_managed_identity and attach_acr:
                if (
                    result.identity_profile is None
                    or result.identity_profile["kubeletidentity"] is None
                ):
                    logger.warning(
                        "Your cluster is successfully created, but we failed to attach acr to it, "
                    ...
                    kubelet_identity_client_id = result.identity_profile[
                        "kubeletidentity"
                    ].client_id
                    _ensure_aks_acr(
                        cmd.cli_ctx,
            ...
            if need_post_creation_vnet_permission_granting:
                if not _create_role_assignment(
                    cmd.cli_ctx,
                    "Network Contributor",
...
def aks_disable_addons(cmd, client, resource_group_name, name, addons, no_wait=False):
    ...
    return sdk_no_wait(
        no_wait, client.create_or_update, resource_group_name, name, instance
    )
...
def aks_enable_addons(
    ...
    enable_virtual_node = (
        virtual_node_addon_name in instance.addon_profiles
        and instance.addon_profiles[virtual_node_addon_name].enabled
    )

    need_pull_for_result = (
        enable_monitoring or ingress_appgw_addon_enabled or enable_virtual_node
    )
...
def aks_update_credentials(
    ...
    if not all([aad_client_app_id, aad_server_app_id, aad_server_app_secret]):
        raise CLIError(
            "usage error: --reset-aad --aad-client-app-id ID --aad-server-app-id ID "
    ...
    return sdk_no_wait(
        no_wait, client.reset_aad_profile, resource_group_name, name, parameters
    )


def aks_scale(
    cmd, client, resource_group_name, name, node_count, nodepool_name="", no_wait=False
):
    instance = client.get(resource_group_name, name)
...
def aks_update(
    ...
    if (
        update_autoscaler != 1
        and cluster_autoscaler_profile is None
        and not update_lb_profile
        and not attach_acr
        and not detach_acr
        and not uptime_sla
        and api_server_authorized_ip_ranges is None
        and not enable_aad
        and not update_aad_profile
        and not enable_ahub
        and not disable_ahub
    ):
        raise CLIError(
    ...
    _validate_autoscaler_update_counts(
        min_count, max_count, enable_cluster_autoscaler or update_cluster_autoscaler
    )
    ...
    # else profile is provided, update instance profile if it exists
        ...
        instance.auto_scaler_profile = (
            _update_dict(
                instance.auto_scaler_profile.__dict__,
                dict(
                    (key.replace("-", "_"), value)
            ...
            else cluster_autoscaler_profile
        )
    ...
    if instance.identity is not None and instance.identity.type == "SystemAssigned":
        if (
            instance.identity_profile is None
            or instance.identity_profile["kubeletidentity"] is None
        ):
            raise CLIError(
    ...
    if uptime_sla:
        instance.sku = ManagedClusterSKU(name="Basic", tier="Paid")
    ...
    if api_server_authorized_ip_ranges is not None:
        instance.api_server_access_profile = _populate_api_server_access_profile(
    ...
    if enable_aad:
        ...
        instance.aad_profile = ManagedClusterAADProfile(managed=True)
    ...
    if node_image_only:
        ...
        for agent_pool_profile in instance.agent_pool_profiles:
            ...
            _upgrade_single_nodepool_image_version(
                True, client, resource_group_name, name, agent_pool_profile.name
            )
        mc = client.get(resource_group_name, name)
        return _remove_nulls([mc])[0]
    ...
    if instance.max_agent_pools < 8 or vmas_cluster:
        if control_plane_only:
            msg = (
                ...
                "upgraded to {} as well. Continue?"
            ).format(instance.kubernetes_version)
            if not yes and not prompt_y_n(msg, default="n"):
                return None
        upgrade_all = True
    else:
        if not control_plane_only:
            msg = (
                ...
                "AND all nodepools to version {}. Continue?"
            ).format(instance.kubernetes_version)
            if not yes and not prompt_y_n(msg, default="n"):
                return None
            upgrade_all = True
            ...
            msg = (
                ...
                "Node pool will not change. Continue?"
            ).format(instance.kubernetes_version)
            if not yes and not prompt_y_n(msg, default="n"):
                return None
...
def _upgrade_single_nodepool_image_version(
    no_wait, client, resource_group_name, cluster_name, nodepool_name
):
    return sdk_no_wait(
        no_wait,
        client.upgrade_node_image_version,
        resource_group_name,
        cluster_name,
        nodepool_name,
    )
...
def aks_use_dev_spaces(
    ...
    if _get_or_add_extension(
        cmd, DEV_SPACES_EXTENSION_NAME, DEV_SPACES_EXTENSION_MODULE, update
    ):
...
def aks_remove_dev_spaces(cmd, client, name, resource_group_name, prompt=False):
    ...
    if _get_or_add_extension(
        cmd, DEV_SPACES_EXTENSION_NAME, DEV_SPACES_EXTENSION_MODULE
    ):
        azext_custom = _get_azext_module(
            DEV_SPACES_EXTENSION_NAME, DEV_SPACES_EXTENSION_MODULE
        )
...
def aks_rotate_certs(cmd, client, resource_group_name, name, no_wait=True):
    return sdk_no_wait(
        no_wait, client.rotate_cluster_certificates, resource_group_name, name
    )
...
def _handle_addons_args(
    ...
    if "virtual-node" in addons:
        ...
        addon_profiles[
            ...
            enabled=True, config={CONST_VIRTUAL_NODE_SUBNET_NAME: aci_subnet_name}
        )
...
def _ensure_default_log_analytics_workspace_for_monitoring(
    ...
    if (
        cloud_name.lower() == "azurecloud"
        and AzureChinaRegionToOmsRegionMap.get(rg_location, False)
    ) or (
        cloud_name.lower() == "azurecloud"
        and AzureFairfaxRegionToOmsRegionMap.get(rg_location, False)
    ):
        raise CLIError(
            'Wrong cloud (azurecloud) setting for region {}, please use "az cloud set ..."'.format(
                rg_location
            )
        )

    if (
        cloud_name.lower() == "azurechinacloud"
        and AzureCloudRegionToOmsRegionMap.get(rg_location, False)
    ) or (
        cloud_name.lower() == "azurechinacloud"
        and AzureFairfaxRegionToOmsRegionMap.get(rg_location, False)
    ):
        raise CLIError(
            'Wrong cloud (azurechinacloud) setting for region {}, please use "az cloud set ..."'.format(
                rg_location
            )
        )

    if (
        cloud_name.lower() == "azureusgovernment"
        and AzureCloudRegionToOmsRegionMap.get(rg_location, False)
    ) or (
        cloud_name.lower() == "azureusgovernment"
        and AzureChinaRegionToOmsRegionMap.get(rg_location, False)
        ...
        raise CLIError(
            'Wrong cloud (azureusgovernment) setting for region {}, please use "az cloud set ..."'.format(
                rg_location
            )
        )
    ...
    default_workspace_resource_id = (
        "/subscriptions/{0}/resourceGroups/{1}/providers/Microsoft.OperationalInsights"
        "/workspaces/{2}".format(
            subscription_id, default_workspace_resource_group, default_workspace_name
        )
    )
    ...
    default_workspace_params = {
        ...
        "properties": {"sku": {"name": "standalone"}},
    }
    async_poller = resources.create_or_update_by_id(
        default_workspace_resource_id, "2015-11-01-preview", default_workspace_params
    )
...
def _ensure_container_insights_for_monitoring(cmd, addon):
    ...
    template = {
        ...
        "parameters": {
            "workspaceResourceId": {
                "type": "string",
                "metadata": {"description": "Azure Monitor Log Analytics Resource ID"},
            },
            "workspaceRegion": {
                "type": "string",
                "metadata": {
            ...
            "solutionDeploymentName": {
                "type": "string",
                "metadata": {"description": "Name of the solution deployment"},
            },
        },
        "resources": [
            {
...
def _ensure_aks_acr(cli_ctx, client_id, acr_name_or_id, subscription_id, detach=False):
    ...
    try:
        ...
        if "was not found" in ex.message:
            raise CLIError(
                "ACR {} not found. Have you provided the right ACR name?".format(
                    registry_name
                )
            )
        raise CLIError(ex.message)
...
def aks_agentpool_update(
    ...
    if update_autoscaler > 1:
        raise CLIError(
            'Please specify one of "--enable-cluster-autoscaler" or '
            '"--disable-cluster-autoscaler" or '
    ...
    if enable_cluster_autoscaler:
        if instance.enable_auto_scaling:
            logger.warning(
                "Autoscaler is already enabled for this node pool.\n"
    ...
    if update_cluster_autoscaler:
        if not instance.enable_auto_scaling:
            raise CLIError(
                "Autoscaler is not enabled for this node pool.\n"
                'Run "az aks nodepool update --enable-cluster-autoscaler" '
                "to enable cluster with min-count and max-count."
            )
        instance.min_count = int(min_count)
...
def aks_agentpool_delete(
    cmd, client, resource_group_name, cluster_name, nodepool_name, no_wait=False
):
    ...
    if not agentpool_exists:
        raise CLIError(
            "Node pool {} doesnt exist, "
            "use 'aks nodepool list' to get current node pool list".format(
                nodepool_name
            )
        )

    return sdk_no_wait(
        no_wait, client.delete, resource_group_name, cluster_name, nodepool_name
    )


def aks_agentpool_get_upgrade_profile(
    cmd, client, resource_group_name, cluster_name, nodepool_name
):
    return client.get_upgrade_profile(resource_group_name, cluster_name, nodepool_name)


def _ensure_aks_acr_role_assignment(cli_ctx, client_id, registry_id, detach=False):
    if detach:
        if not _delete_role_assignments(
            cli_ctx, "acrpull", client_id, scope=registry_id
        ):
            raise CLIError(
                "Could not delete role assignments for ACR. "
                "Are you an Owner on this subscription?"
            )
        return

    if not _add_role_assignment(cli_ctx, "acrpull", client_id, scope=registry_id):
        raise CLIError(
...
def _ensure_aks_service_principal(
    cli_ctx,
    service_principal=None,
    client_secret=None,
    subscription_id=None,
...
def _ensure_osa_aad(
    ...
    if create:
        ...
        directory_access = ResourceAccess(
            id="5778995a-e1bf-45b8-affa-663a9f3f4d04",
            additional_properties=None,
            type="Role",
        )

        required_osa_aad_access = RequiredResourceAccess(
            resource_access=[resource_access, directory_access],
            additional_properties=None,
            resource_app_id="00000002-0000-0000-c000-000000000000",
        )

        list_aad_filtered = list(
            rbac_client.applications.list(
                filter="identifierUris/any(s:s eq '{}')".format(app_id_name)
            )
        )
        if list_aad_filtered:
            ...
            update_application(
                client=rbac_client.applications,
            ...
            result, _aad_session_key = create_application(
                client=rbac_client.applications,
                display_name=name,
                identifier_uris=[app_id_name],
                homepage=app_id_name,
...
def _check_cluster_autoscaler_flag(
    enable_cluster_autoscaler, min_count, max_count, node_count, agent_pool_profile
):
    if enable_cluster_autoscaler:
...
def _validate_autoscaler_update_counts(min_count, max_count, is_enable_or_update):
    ...
    if min_count is None or max_count is None:
        if is_enable_or_update:
            raise CLIError(
                "Please specify both min-count and max-count when --enable-cluster-autoscaler or "
...
def _remove_osa_nulls(managed_clusters):
    ...
    for managed_cluster in managed_clusters:
        for attr in attrs:
            if (
                hasattr(managed_cluster, attr)
                and getattr(managed_cluster, attr) is None
            ):
                delattr(managed_cluster, attr)
...
def _validate_aci_location(norm_location):
    ...
    if norm_location not in aci_locations:
        raise CLIError(
            'Azure Container Instance is not available at location "{}".'.format(
                norm_location
            )
...
def openshift_create(
    ...
    try:
        ...
        if (
            aad_client_app_id is None
            and aad_client_app_secret is None
            and aad_tenant_id is None
        ):
            create_aad = True

    osa_aad_identity = _ensure_osa_aad(
        cmd.cli_ctx,
    ...
    identity_providers.append(
        OpenShiftManagedClusterIdentityProvider(
            name="Azure AD", provider=osa_aad_identity
        )
    )
    ...
    if workspace_id is not None:
        ...
        monitor_profile = OpenShiftManagedClusterMonitorProfile(
            enabled=True, workspace_resource_id=workspace_id
        )  # pylint: disable=line-too-long
    else:
    ...
    try:
        ...
        _ensure_osa_aad(
            ...
            aad_tenant_id=osa_aad_identity.tenant_id,
            identifier=instance.public_hostname,
            name=name,
            create=create_aad,
        )
    except CloudError as ex:
        if (
            "The resource type could not be found in the namespace 'Microsoft.ContainerService"
            in ex.message
        ):
            raise CLIError(
                "Please make sure your subscription is whitelisted to use this service. https://aka.ms/openshift/managed"
            )  # pylint: disable=line-too-long
...
def openshift_scale(
    cmd, client, resource_group_name, name, compute_count, no_wait=False
):
    logger.warning(
        "Support for existing ARO 3.11 clusters ends June 2022. Please see aka.ms/aro/4 for information on switching to ARO 4."
    )  # pylint: disable=line-too-long
...
def openshift_monitor_enable(
    cmd, client, resource_group_name, name, workspace_id, no_wait=False
):
    logger.warning(
        "Support for existing ARO 3.11 clusters ends June 2022. Please see aka.ms/aro/4 for information on switching to ARO 4."
    )  # pylint: disable=line-too-long
...
def openshift_monitor_disable(cmd, client, resource_group_name, name, no_wait=False):
    ...
    instance = client.get(resource_group_name, name)
    monitor_profile = OpenShiftManagedClusterMonitorProfile(
        enabled=False, workspace_resource_id=None
    )  # pylint: disable=line-too-long
    instance.monitor_profile = monitor_profile
    return sdk_no_wait(
        no_wait, client.create_or_update, resource_group_name, name, instance
