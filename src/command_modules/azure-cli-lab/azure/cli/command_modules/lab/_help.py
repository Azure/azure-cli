...
-   name: --os-type
    short-summary: 'Type of the OS on which the custom image is based. Allowed values
        are: Windows, Linux'
-   name: --os-state
...
examples:
-   name: Create a custom image in the lab from a running Windows virtual machine
        without applying sysprep.
    text: |
...
-   name: --artifacts
    short-summary: JSON encoded array of artifacts to be applied. Use '@{file}' to
        load from a file.
"""
...
-   name: --image
    short-summary: The name of the operating system image (gallery image name or custom
        image name/ID).
    long-summary: Use `az lab gallery-image list` for available gallery images or
...
-   name: --formula
    ...
    long-summary: >
        Use `az lab formula` with the `--export-artifacts` flag to export and update
        artifacts, then pass
        the results via the `--artifacts` argument.
-   name: --size
    short-summary: The size of the VM to be created. See https://azure.microsoft.com/en-us/pricing/details/virtual-machines/
        for size info.
-   name: --admin-username
...
-   name: --ssh-key
    short-summary: The SSH public key or public key file path. Use `--generate-ssh-keys`
        to generate SSH keys.
-   name: --authentication-type
    short-summary: 'Type of authentication allowed for the VM. Allowed values are:
        password, ssh.'
-   name: --saved-secret
    short-summary: Name of the saved secret to be used for authentication.
    long-summary: When this value is provided, it is used in the place of other authentication
        methods.
-   name: --vnet-name
...
-   name: --ip-configuration
    short-summary: 'Type of IP configuration to use for the VM. Allowed values are:
...
-   name: Create a VM in the lab from a gallery image.
    text: >
        ...
        "Ubuntu Server 16.04 LTS" --image-type gallery --size Standard_DS1_v2
-   name: Create a VM in the lab from a gallery image with SSH authentication.
    text: >
        az lab vm create --lab-name {LabName} -g {ResourceGroup} --name {VMName} --image
        "Ubuntu Server 16.04 LTS" --image-type gallery --size Standard_DS1_v2 --authentication-type
        ssh
-   name: Create a claimable VM in the lab from a gallery image with password authentication.
    text: >
        az lab vm create --lab-name {LabName} -g {ResourceGroup} --name {VMName} --image
        "Ubuntu Server 16.04 LTS" --image-type gallery --size Standard_DS1_v2 --allow-claim
-   name: Create a windows VM in the lab from a gallery image with password authentication.
    text: >
        az lab vm create --lab-name {LabName} -g {ResourceGroup} --name {VMName} --image
        "Windows Server 2008 R2 SP1" --image-type gallery --size Standard_DS1_v2
...
-   name: Create a VM in the lab with a public IP.
    text: >
        az lab vm create --lab-name {LabName} -g {ResourceGroup} --name {VMName} --image
        "Ubuntu Server 16.04 LTS" --image-type gallery --size Standard_DS1_v2 --ip-configuration
        public
-   name: Create a VM from a formula.
    text: >
        az lab vm create --lab-name {LabName} -g {ResourceGroup} --name {VMName} --formula
        MyFormula --artifacts '@artifacts.json'
-   name: Create a VM in a lab.
    text: az lab vm create --size Standard_DS1_v2 --image-type gallery --image "Ubuntu
        Server 16.04 LTS" --name {VMName} --resource-group {ResourceGroup} --lab-name
        {LabName} --admin-password {admin-password}
    crafted: 'True'
"""
...
-   name: --claimable
    short-summary: List only claimable virtual machines in the lab. Cannot be used
        with `--filters`.
-   name: --all
    short-summary: List all virtual machines in the lab. Cannot be used with `--filters`
-   name: --environment
    short-summary: Name or ID of the environment to list virtual machines in. Cannot
        be used with `--filters`.
-   name: --object-id
    short-summary: Object ID of the owner to list VMs for.
examples:
-   name: List the VMs in an Azure DevTest Lab.
    text: az lab vm list --all  --lab-name MyLab --resource-group MyResourceGroup
    crafted: 'True'
"""
