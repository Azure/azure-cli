...
class BlobServicePropertiesTests(StorageScenarioMixin, ScenarioTest):
    ...
    @StorageAccountPreparer(name_prefix='filesoftdelete')
    def test_storage_account_update_delete_retention_policy(self, resource_group, storage_account):
        self.kwargs.update({
            'sa': storage_account,
            'rg': resource_group,
        ...
        with self.assertRaises(SystemExit):
            self.cmd('storage account file-service-properties update --enable-delete-retention true -n {sa} -g {rg}')

        with self.assertRaises(SystemExit):
            self.cmd('storage account file-service-properties update --enable-delete-retention false --delete-retention-days 365 -n {sa} -g {rg}').get_output_in_json()
