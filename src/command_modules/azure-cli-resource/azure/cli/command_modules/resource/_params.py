...
from ._validators import validate_deployment_name, validate_lock_parameters, validate_deployment_parameters
...
register_cli_argument('group deployment create', 'parameters', action='append', nargs='+', completer=FilesCompleter(), help="provide deployment parameter values, either json string, or use `@<file path>` to load from a file. Can be repeated. If a the same parameter is present in multiple arguments, the last value wins.", validator=validate_deployment_parameters)
