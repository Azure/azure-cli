...
class ApimSubscriptionScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_test_apim_subscription-')
    @ApiManagementPreparer(sku_name='Consumption')
    def test_apim_subscription(self):
    ...
    def _create_subscription(self):
        self.cmd('apim subscription create -n {apim} -g {rg} --sid {subscription_id} --scope {scope} -d {display_name}', checks=[
            self.check('name', '{subscription_id}')
        ])

    def _show_subscription(self):
        self.subscription = self.cmd('apim subscription show -n {apim} -g {rg} --sid {subscription_id}').get_output_in_json()

    def _update_subscription(self):
        updated_subscription = self.cmd('apim subscription update -n {apim} -g {rg} --sid {subscription_id} -d {updated_display_name}').get_output_in_json()
        assert updated_subscription['displayName'] != self.subscription['displayName']

    def _regenerate_keys(self):
        original_keys = self.cmd('apim subscription keys list -n {apim} -g {rg} --sid {subscription_id}').get_output_in_json()
        self.cmd('apim subscription keys regenerate --key-kind primary -n {apim} -g {rg} --sid {subscription_id}')
        self.cmd('apim subscription keys regenerate --key-kind secondary -n {apim} -g {rg} --sid {subscription_id}')
        modified_keys = self.cmd('apim subscription keys list -n {apim} -g {rg} --sid {subscription_id}').get_output_in_json()
    ...
    def _delete_subscription(self):
        self.cmd('apim subscription delete -n {apim} -g {rg} --sid {subscription_id} --yes')
    ...
    def _get_subscription_count(self):
        return len(self.cmd('apim subscription list -n {apim} -g {rg}').get_output_in_json())
