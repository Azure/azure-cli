...
subscription than the app service environment, please use the resource ID for --app-service-environment parameter. ",
    ...
    with self.argument_context('webapp deploy') as c:
        ...
        c.argument('is_async', options_list=['--async'], help='If true, the artifact is deployed asynchronously. (The command will exit once the artifact is pushed to the web app.). Synchronous deployments are not yet supported when using "--src-url"', arg_type=get_three_state_flag(return_label=True))
        c.argument('restart', options_list=['--restart'], help='If true, the web app will be restarted following the deployment. Set this to false if you are deploying multiple artifacts and do not want to restart the site on the earlier deployments.', arg_type=get_three_state_flag(return_label=True))
        c.argument('clean', options_list=['--clean'], help='If true, cleans the target directory prior to deploying the file(s). Default value is determined based on artifact type.', arg_type=get_three_state_flag(return_label=True))
        c.argument('ignore_stack', options_list=['--ignore-stack'], help='If true, any stack-specific defaults are ignored.', arg_type=get_three_state_flag(return_label=True))
        c.argument('resest', options_list=['--reset'], help='Reset Java apps to the default parking page if set to true with no type specified.', arg_type=get_three_state_flag(return_label=True))
        c.argument('timeout', options_list=['--timeout'], help='Timeout for the deployment operation in milliseconds. Ignored when using "--src-url" since synchronous deployments are not yet supported when using "--src-url"')
        c.argument('slot', help="The name of the slot. Default to the productions slot if not specified.")
