...
class AzArgumentContext(ArgumentsContext):
    ...
    def _merge_kwargs(self, kwargs):
        return _merge_kwargs(self.group_kwargs, kwargs)
    ...
    def extra(self, dest, arg_type=None, **kwargs):
        ...
        merged_kwargs = _merge_kwargs(self.group_kwargs, kwargs)
        resource_type = _resolve_kwarg(merged_kwargs, 'resource_type', 'arg_type')
        min_api = _resolve_kwarg(merged_kwargs, 'min_api', 'arg_type')
        max_api = _resolve_kwarg(merged_kwargs, 'max_api', 'arg_type')
        if self.command_loader.supported_api_version(resource_type=resource_type,
                                                     min_api=min_api,
                                                     max_api=max_api):
