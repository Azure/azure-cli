...
def configure_load_balancer_profile(managed_outbound_ip_count, outbound_ips, outbound_ip_prefixes, outbound_ports,
    ...
    if managed_outbound_ip_count or outbound_ip_resources or outbound_ip_prefix_resources:
        profile.managed_outbound_i_ps = None        # ips -> i_ps due to track 2 naming issue
        profile.outbound_i_ps = None                # ips -> i_ps due to track 2 naming issue
        profile.outbound_ip_prefixes = None
        if managed_outbound_ip_count:
            profile.managed_outbound_i_ps = ManagedClusterLoadBalancerProfileManagedOutboundIPs(    # ips -> i_ps due to track 2 naming issue
                count=managed_outbound_ip_count
            )
        if outbound_ip_resources:
            profile.outbound_i_ps = ManagedClusterLoadBalancerProfileOutboundIPs(                   # ips -> i_ps due to track 2 naming issue
                public_i_ps=outbound_ip_resources                                                   # ips -> i_ps due to track 2 naming issue
            )
