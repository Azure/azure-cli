...
def set_encryption(cmd, client, resource_group_name, account_name, key_source=None, key_version=None, key_vault_id=None):
    try:
        ...
        if ex.message == '(BadRequest) Access to the Customer Key was forbidden.':
            recommendation = 'Please use the Azure Portal to grant the key vault access to the media account. For more information please visit https://aka.ms/keyvaultaccess'
