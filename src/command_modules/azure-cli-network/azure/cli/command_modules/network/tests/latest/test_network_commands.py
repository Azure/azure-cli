...
class NetworkAppGatewayRewriteRuleset(ScenarioTest):
    ...
    def test_network_app_gateway_rewrite_rulesets(self, resource_group):
        ...
        self.cmd('network application-gateway rewrite-rule create -g {rg} --gateway-name {gw} --rule-set-name {set} -n {rule} --sequence 100 --request-headers foo=bar --response-headers cat=hat --no-wait')
        ...
        self.cmd('network application-gateway rewrite-rule condition create -g {rg} --gateway-name {gw} --rule-set-name {set} -n {rule} --variable {var} --pattern "^Bearer" --ignore-case false --negate --no-wait')
        self.cmd('network application-gateway rewrite-rule condition create -g {rg} --gateway-name {gw} --rule-set-name {set} -n {rule}')
        # list
        # delete
        # update
