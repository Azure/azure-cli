        ...
        for command in self.module_help_docs:
            ...
            if help_dict['type'] == 'command':
                ...
                if len(crafted_examples) != 0:
                    ...
                    help_dict['examples'] = filter(lambda example_dict: 'crafted' not in example_dict or example_dict['crafted'] != True or example_dict['text'] not in examples_to_remove, help_dict['examples'])

                    # add new crafted examples
                    for crafted_example_text in examples_to_add:
                        crafted_example_name = self.command_examples_dict[crafted_example_text]
                        help_dict['examples'].append(CommentedMap([('name', crafted_example_name),
                                                                   ('text', crafted_example_text),
                                                                   ('crafted', True)]))
                        self.command_examples_dict.pop(crafted_example_text)

                # add escape slash to filepaths in examples for future loads
