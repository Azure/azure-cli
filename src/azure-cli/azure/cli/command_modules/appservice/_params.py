...
subscription than the app service environment, please use the resource ID for --app-service-environment parameter. ",
    ...
    with self.argument_context('webapp create') as c:
        ...
        c.argument('basic_auth', help='Enable or disable basic auth for both SCM and FTP Basic Auth Publishing Credentials. Defaults to Enabled if not specified. Allowed values: Disabled, Enabled. See https://aka.ms/app-service-basic-auth to learn more.', arg_type=get_enum_type(BASIC_AUTH_TYPES))
        c.ignore('language')
