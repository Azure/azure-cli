...
helps["acr build"] = """
type: command
short-summary: Queues a quick build, providing streaming logs for an Azure Container
    Registry.
examples:
...
-   name: Queue a remote GitHub context as a Windows build, tag it, and push it to
        the registry.
    text: >
...
type: group
short-summary: Manage build definitions, which can be triggered by git commits or
    base image updates for OS & Framework Patching.
"""

helps["acr build-task create"] = """
type: command
short-summary: Creates a new build definition which can be triggered by git commits
    or base image updates for an Azure Container Registry.
examples:
-   name: Create a build definition without git commits and base image updates.
    text: >
        az acr build-task create -t hello-world:{{.Build.ID}} -n hello-world -r MyRegistry
        -c https://github.com/Azure-Samples/acr-build-helloworld-node.git --commit-trigger-enabled
        false
-   name: Create a build definition which updates on git commits and base image updates
        (--git-access-token must have permissions to create github webhooks).
    text: >
        az acr build-task create -t hello-world:{{.Build.ID}} -n hello-world -r MyRegistry
        -c https://github.com/Azure-Samples/acr-build-helloworld-node.git --git-access-token
...
-   name: List the last 10 successful builds for a registry displaying the results
        in a table.
    text: >
        az acr build-task list-builds -r MyRegistry --build-status Succeeded --top
        10 -o table
-   name: List all of the builds that built the image 'hello-world' for an Azure Container
...
type: command
short-summary: Show logs for a particular build. If no build-id is supplied, display
    the logs for the last created build.
examples:
-   name: Show logs for the last created build in the registry.
    text: >
        az acr build-task logs -r MyRegistry
-   name: Show logs for the last created build in the registry, filtered by build
        task.
    text: >
...
-   name: Show logs for the last created build in the registry that built the image
        'hello-world'.
    text: >
...
type: command
short-summary: Trigger a build task that might otherwise be waiting for git commits
    or base image update triggers for an Azure Container Registry.
examples:
...
type: command
short-summary: Get the properties of a specified build task for an Azure Container
    Registry.
examples:
...
-   name: Get the details of a build, displaying the results in a table.
    text: >
        az acr build-task show-build -n MyBuildTask -r MyRegistry --build-id aab1
        -o table
"""
...
examples:
-   name: Update the git access token for a build definition in an Azure Container
        Registry.
    text: >
...
-   name: Update an existing build to be archived.
    text: >
        az acr build-task update-build -r MyRegistry --build-id MyBuild --no-archive
        false
"""

helps["acr check-name"] = """
type: command
short-summary: Checks if an Azure Container Registry name is valid and available for
    use.
examples:
...
-   name: Create an Azure Container Registry with a new storage account with the Classic
        SKU (Classic registries are being deprecated by March 2019).
    text: >
...
-   name: Push a chart package to an Azure Container Registry, overwriting the existing
        one.
    text: >
...
type: command
short-summary: Add a helm chart repository from an Azure Container Registry through
    the Helm CLI.
long-summary: Helm must be installed on your machine.
examples:
-   name: Add a helm chart repository from an Azure Container Registry to manage helm
        charts.
    text: >
...
helps["acr import"] = """
type: command
short-summary: Imports an image to an Azure Container Registry from another Container
    Registry. Import removes the need to docker pull, docker tag, docker push.
examples:
-   name: Import an image to the target registry and inherits sourcerepository:sourcetag
...
-   name: Import an image from a public repository in Docker Hub
    text: >
        az acr import -n MyRegistry --source docker.io/sourcerepository:sourcetag
        -t targetrepository:targettag
-   name: Imports an image to an Azure Container Registry from another Container Registry.
        Import removes the need to docker pull, docker tag, docker push.
    text: az acr import --source sourceregistry.azurecr.io/sourcerepository:sourcetag
        --name MyRegistry
    crafted: 'True'
-   name: Imports an image to an Azure Container Registry from another Container Registry.
        Import removes the need to docker pull, docker tag, docker push.
...
examples:
-   name: List container registries and show the results in a table, across multiple
        resource groups.
    text: >
        az acr list -o table
-   name: List container registries in a resource group and show the results in a
        table.
    text: >
...
-   name: Update tags for a replication
    text: >
        az acr replication update -n MyReplication -r MyRegistry --tags key1=value1
        key2=value2
"""
...
long-summary: This command deletes all associated layer data that are not referenced
    by any other manifest in the container registry.
examples:
-   name: Delete a repository from an Azure Container Registry. This deletes all manifests
        and tags under 'hello-world'.
    text: az acr repository delete -n MyRegistry --repository hello-world
-   name: Delete an image by tag. This deletes the manifest referenced by 'hello-world:latest'
        and all other tags referencing the manifest.
    text: az acr repository delete -n MyRegistry --image hello-world:latest
-   name: Delete an image by sha256-based manifest digest. This deletes the manifest
        identified by 'hello-world@sha256:abc123' and all tags referencing the manifest.
...
-   name: Show the latest 10 manifests ordered by timestamp of a repository in an
        Azure Container Registry.
    text: az acr repository show-manifests -n MyRegistry --repository MyRepository
        --top 10 --orderby time_desc
-   name: Show the detailed information of the latest 10 manifests ordered by timestamp
        of a repository in an Azure Container Registry.
    text: az acr repository show-manifests -n MyRegistry --repository MyRepository
        --top 10 --orderby time_desc --detail
-   name: Show manifests of a repository in an Azure Container Registry.
...
-   name: Show the detailed information of tags of a repository in an Azure Container
        Registry.
    text: az acr repository show-tags -n MyRegistry --repository MyRepository --detail
-   name: Show the detailed information of the latest 10 tags ordered by timestamp
        of a repository in an Azure Container Registry.
    text: az acr repository show-tags -n MyRegistry --repository MyRepository --top
        10 --orderby time_desc --detail
-   name: Show tags for a repository in an Azure Container Registry.
    text: az acr repository show-tags --output json --repository MyRepository --name
        MyRegistry
    crafted: 'True'
-   name: Show tags for a repository in an Azure Container Registry.
    text: az acr repository show-tags --output json --repository MyRepository --name
...
long-summary: This command does not delete the manifest referenced by the tag or any
    associated layer data.
examples:
...
-   name: Update the attributes of the repository 'hello-world' to disable write operation.
    text: az acr repository update -n MyRegistry --repository hello-world --write-enabled
        false
-   name: Update the attributes of the image referenced by tag 'hello-world:latest'
        to disable write operation.
    text: az acr repository update -n MyRegistry --image hello-world:latest --write-enabled
        false
-   name: Update the attributes of the image referenced by digest 'hello-world@sha256:abc123'
        to disable write operation.
    text: az acr repository update -n MyRegistry --image hello-world@sha256:abc123
        --write-enabled false
"""
...
-   name: Queue a remote git context with streaming logs.
    text: >
        az acr run -r MyRegistry https://github.com/Azure-Samples/acr-tasks.git -f
        hello-world.yaml
"""
...
helps["acr task"] = """
type: group
short-summary: Manage a collection of steps for building, testing and OS & Framework
    patching container images using Azure Container Registries.
"""
...
type: command
short-summary: Creates a series of steps for building, testing and OS & Framework
    patching containers. Tasks support triggers from git commits and base image updates.
examples:
-   name: Create a Linux task from a public GitHub repository which builds the hello-world
        image without triggers
    text: >
...
-   name: Create a Linux task using a private GitHub repository which builds the hello-world
        image without triggers
    text: >
        az acr task create -t hello-world:{{.Run.ID}} -n hello-world -r MyRegistry
        -c https://github.com/Azure-Samples/acr-build-helloworld-node.git -f Dockerfile
        --commit-trigger-enabled false --pull-request-trigger-enabled false --git-access-token
        0000000000000000000000000000000000000000
-   name: Create a Linux task from a public GitHub repository which builds the hello-world
        image with a git commit trigger
    text: >
        az acr task create -t hello-world:{{.Run.ID}} -n hello-world -r MyRegistry
        -c https://github.com/Azure-Samples/acr-build-helloworld-node.git -f Dockerfile
        --git-access-token 0000000000000000000000000000000000000000
-   name: Create a Windows task from a public GitHub repository which builds the Azure
        Container Builder image.
    text: >
        az acr task create -t acb:{{.Run.ID}} -n acb-win -r MyRegistry -c https://github.com/Azure/acr-builder.git
        -f Windows.Dockerfile --commit-trigger-enabled false --pull-request-trigger-enabled
        false --os Windows
"""
...
type: command
short-summary: List all of the executed runs for an Azure Container Registry, with
    the ability to filter by a specific Task.
examples:
...
-   name: List the last 10 successful runs for a registry and show the results in
        a table.
    text: >
        az acr task list-runs -r MyRegistry --run-status Succeeded --top 10 -o table
-   name: List all of the runs that built the image 'hello-world' for a registry and
        show the results in a table.
    text: >
        az acr task list-runs -r MyRegistry --image hello-world -o table
-   name: List all of the executed runs for an Azure Container Registry, with the
...
helps["acr task logs"] = """
type: command
short-summary: Show logs for a particular run. If no run-id is supplied, show logs
    for the last created run.
examples:
...
-   name: Show logs for the last created run in the registry that built the image
        'hello-world'.
    text: >
...
helps["acr task run"] = """
type: command
short-summary: Manually trigger a task that might otherwise be waiting for git commits
    or base image update triggers.
examples:
-   name: Trigger a task.
    text: >
        az acr task run -n MyTask -r MyRegistry
-   name: Manually trigger a task that might otherwise be waiting for git commits
        or base image update triggers.
...
type: command
short-summary: Get the properties of a specified run of an Azure Container Registry
    Task.
examples:
...
examples:
-   name: Update base image updates to trigger on all dependent images of a multi-stage
        dockerfile, and status of a task in an Azure Container Registry.
    text: >
        az acr task update -n MyTask -r MyRegistry --base-image-trigger All --status
        Disabled
"""
...
-   name: Update the storage account for an Azure Container Registry (Classic Registries
        are being deprecated as of March 2019).
    text: >
...
examples:
-   name: Create a webhook for an Azure Container Registry that will deliver docker
        push and delete events to a service URI.
    text: >
        az acr webhook create -n MyWebhook -r MyRegistry --uri http://myservice.com
        --actions push delete
-   name: Create a webhook for an Azure Container Registry that will deliver docker
        push events to a service URI with a basic authentication header.
    text: >
        az acr webhook create -n MyWebhook -r MyRegistry --uri http://myservice.com
        --actions push --headers "Authorization=Basic 000000"
-   name: Create a webhook for an Azure Container Registry that will deliver helm
        chart push and delete events to a service URI.
    text: >
        az acr webhook create -n MyWebhook -r MyRegistry --uri http://myservice.com
        --actions chart_push chart_delete
"""
...
-   name: Update headers for a webhook.
    text: >
        az acr webhook update -n MyWebhook -r MyRegistry --headers "Authorization=Basic
        000000"
-   name: Update the service URI and actions for a webhook.
    text: >
        az acr webhook update -n MyWebhook -r MyRegistry --uri http://myservice.com
        --actions push delete
