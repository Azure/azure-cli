...
class AppConfigSnapshotRefScenarioTest(ScenarioTest):
    ...
    @AllowLargeResponse()
    def test_azconfig_kv_set_snapshot_ref(self, resource_group, location):
        ...
        self.cmd('appconfig kv set-snapshot-ref -n {config_store_name} --key {key} --label {label} --snapshot-name {snapshot_name} -y',
        ...
        self.cmd('appconfig kv set-snapshot-ref -n {config_store_name} --key {key} --snapshot-name {snapshot_name} --tags env=prod team=config -y',
                 checks=[self.check('contentType', SnapshotReferenceConstants.SNAPSHOT_REFERENCE_CONTENT_TYPE),
    ...
    @ResourceGroupPreparer(parameter_name_for_location='location')
    @AllowLargeResponse()
    def test_azconfig_kv_list_resolve_snapshot_ref(self, resource_group, location):
        ...
        # Create a snapshot containing those keys
        snapshot_name = "TestSnapshot"
        self.kwargs.update({'snapshot_name': snapshot_name})
        self.cmd('appconfig snapshot create -n {config_store_name} -s {snapshot_name} --filters {{\"key\":\"*\"}} --retention-period 86400')

        # Create a snapshot reference pointing to the snapshot
        self.kwargs.update({'key': 'MySnapshotRef'})
        self.cmd('appconfig kv set-snapshot-ref -n {config_store_name} --key {key} --snapshot-name {snapshot_name} -y')

        # List without resolve - should return the snapshot reference itself
        result = self.cmd('appconfig kv list -n {config_store_name} --key MySnapshotRef').get_output_in_json()
        assert len(result) == 1
        assert result[0]['contentType'] == SnapshotReferenceConstants.SNAPSHOT_REFERENCE_CONTENT_TYPE

        # List with resolve - should return key-values from the referenced snapshot
        result = self.cmd('appconfig kv list -n {config_store_name} --key MySnapshotRef --resolve-snapshot-ref').get_output_in_json()
        keys = [kv['key'] for kv in result]
        assert 'Color' in keys, "Resolved snapshot should contain 'Color' key"
        assert 'Size' in keys, "Resolved snapshot should contain 'Size' key"
