...
def load_arguments(self, _):
    ...
    for item in ['recover', 'purge']:
        with self.argument_context('keyvault ' + item) as c:
            c.argument('vault_name', help='Name of the deleted vault', required=True, completer=None, validator=validate_deleted_vault_name)
            c.argument('resource_group_name', resource_group_name_type, id_part=None, required=False, help='Resource group of the deleted vault.')
            c.argument('location', help='Location of the deleted vault to be restored.', required=False, validator=get_default_location_from_resource_group)
