...
def load_arguments(self, _):
    ...
    with self.argument_context('consumption pricesheet show') as cps:
        ...
        cps.argument('billing_period_name', options_list=['--billing-period-name', '-p'], help='Name of the specific billing period to get the price sheet')

    with self.argument_context('consumption marketplace list') as cmp:
        cmp.argument('billing_period_name', options_list=['--billing-period-name', '-p'], help='Name of the specific billing period to get the marketplace')
        cmp.argument('top', options_list=['--top', '-t'], type=int, help='Maximum number of items to return. Range: 1-1000')
        cmp.argument('start_date', options_list=['--start-date', '-s'], type=get_datetime_type(), help='Start date (YYYY-MM-DD in UTC). Requires --end-date')
        cmp.argument('end_date', options_list=['--end-date', '-e'], type=get_datetime_type(), help='End date (YYYY-MM-DD in UTC). Requires --start-date')
