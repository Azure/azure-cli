    ...
    def get_op_handler(self, operation, operation_group=None):
        ...
        for rt in AZURE_API_PROFILES[self.cli_ctx.cloud.profile]:
            if operation.startswith(rt.import_prefix + ".operations."):
                ...
                deduced_operation_group = subs[:subs.index('_operations')]
                operation = operation.replace(
                    rt.import_prefix,
                    get_versioned_sdk_path(self.cli_ctx.cloud.profile, rt, operation_group=deduced_operation_group))
            # That's Autorest.Python v4, operation_group is not guessed anymore
            elif operation.startswith(rt.import_prefix + ".operations#"):
                operation = operation.replace(
                    rt.import_prefix,
