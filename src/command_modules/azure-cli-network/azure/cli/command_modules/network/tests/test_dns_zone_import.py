...
class TestDnsZoneImport(unittest.TestCase):
    ...
    def _check_soa(self, zone, zone_name, ttl, serial_number, refresh, retry, expire, min_ttl):
        self.assertEqual(zone[zone_name]['soa'][0]['ttl'], ttl)
        self.assertEqual(zone[zone_name]['soa'][0]['serial'], serial_number)
        self.assertEqual(zone[zone_name]['soa'][0]['refresh'], refresh)
        self.assertEqual(zone[zone_name]['soa'][0]['retry'], retry)
        self.assertEqual(zone[zone_name]['soa'][0]['expire'], expire)
        self.assertEqual(zone[zone_name]['soa'][0]['minimum'], min_ttl)
    ...
    def _get_zone_object(self, file_name, zone_name):
    ...
    def test_zone_file_2(self):
        ...
        self._check_a(zone, 'a2.' + zn, [(3600, '1.2.3.4'), (3600, '2.3.4.5')])
        self._check_aaaa(zone, 'aaaa2.' + zn, [(3600, '2001:cafe:130::100'), (3600, '2001:cafe:130::101')])
        self._check_txt(zone, 'doozie.' + zn, [(3600, 108, None)])
        self._check_cname(zone, 'fee2.' + zn, [(3600, 'bar.com.'), (3600, 'bar.')])  # TODO: CNAME should only have ONE record...
        self._check_mx(zone, 'mail.' + zn, [(3600, 10, 'mail1.mymail.com.'), (3600, 11, 'flooble.')])
        ...
        #self._check_a(zone, '200.' + zn, [(300, '7.8.9.0')])  # TODO: Fix Name/TTL parsing...
        ...
        self._check_txt(zone, 'even.' + zn, [(3600, None, 'v=spf1 mx include:_spf4.xgn.de include:_spf6.xgn.de -all')])

    def test_zone_file_3(self):
        zn = 'example.com.'
        zone = self._get_zone_object('zone3.txt', zn)
        self._check_soa(zone, zn, 86400, 2003080800, 43200, 900, 1814400, 10800)  # TODO: should 3600 override 1d or not??
        ...
        self._check_a(zone, 'd1.' + zn, [(3600, '12.1.2.3'), (3600, '12.2.3.4'), (3600, '12.3.4.5'), (3600, '12.4.5.6')])
        self._check_ns(zone, 'd1.' + zn, [(3600, 'hood.com.')])
        self._check_txt(zone, 'd1.' + zn, [(3600, None, 'fishfishfish')])
        self._check_a(zone, 'f1.' + zn, [(3600, '11.1.2.3'), (3600, '11.2.3.3')])
        self._check_a(zone, 'f2.' + zn, [(3600, '11.2.3.4'), (3600, '11.5.6.7')])
