...
ENV_DEBIAN_REPO_ID = os.environ.get('DEBIAN_REPO_ID')
ENV_DEBIAN_REPO_URL = os.environ.get('DEBIAN_REPO_URL')
ENV_DEBIAN_REPO_USERNAME = os.environ.get('DEBIAN_REPO_USERNAME')
ENV_DEBIAN_REPO_PASSWORD = os.environ.get('DEBIAN_REPO_PASSWORD')

assert (ENV_REPO_NAME and ENV_CLI_VERSION and ENV_CLI_DOWNLOAD_SHA256 and ENV_STORAGE_CONNECTION_STRING and ENV_DEBIAN_REPO_ID and ENV_DEBIAN_REPO_URL and ENV_DEBIAN_REPO_USERNAME and ENV_DEBIAN_REPO_PASSWORD),\
        "Not all required environment variables have been set. "\
        "Set REPO_NAME, CLI_VERSION, CLI_DOWNLOAD_SHA256, AZURE_STORAGE_CONNECTION_STRING, DEBIAN_REPO_ID, DEBIAN_REPO_URL, DEBIAN_REPO_USERNAME, DEBIAN_REPO_PASSWORD"
...
def print_env_vars():
    ...
    print('DEBIAN_REPO_ID = {}'.format(ENV_DEBIAN_REPO_ID))
    print('DEBIAN_REPO_URL = {}'.format(ENV_DEBIAN_REPO_URL))
    print('DEBIAN_REPO_USERNAME = {}'.format(ENV_DEBIAN_REPO_USERNAME))
    print('DEBIAN_REPO_PASSWORD = {}'.format(ENV_DEBIAN_REPO_PASSWORD))
...
def main():
    ...
    my_vm(['export', 'CLI_VERSION={}'.format(ENV_CLI_VERSION), '&&',
           'export', 'CLI_DOWNLOAD_SHA256={}'.format(ENV_CLI_DOWNLOAD_SHA256), '&&',
    ...
    my_vm(['az', 'storage', 'container', 'create', '--name', 'repos', '--public-access', 'blob',
           '--connection-string', '"{}"'.format(ENV_STORAGE_CONNECTION_STRING)],
          _out=sys.stdout, _err=sys.stdout)
    my_vm(['az', 'storage', 'blob', 'upload', '-f', deb_file_path,
           '-n', artifact_name, '-c', 'repos', '--connection-string', '"{}"'.format(ENV_STORAGE_CONNECTION_STRING)],
          _out=sys.stdout, _err=sys.stdout)
    io = StringIO()
    my_vm(['az', 'storage', 'blob', 'url', '-n', artifact_name, '-c', 'repos', '--output', 'tsv',
           '--connection-string', '"{}"'.format(ENV_STORAGE_CONNECTION_STRING)], _out=io)
    ...
    upload_script = REPO_UPLOAD_SCRIPT_TMPL.format(cli_version=ENV_CLI_VERSION,
                                                   repo_id=ENV_DEBIAN_REPO_ID,
                                                   source_url=deb_url,
                                                   repo_package_url=ENV_DEBIAN_REPO_URL,
                                                   repo_user=ENV_DEBIAN_REPO_USERNAME,
                                                   repo_pass=ENV_DEBIAN_REPO_PASSWORD)
