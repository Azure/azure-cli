...
def cli_consumption_list_usage(cmd, client, billing_period_name=None, top=None, include_additional_properties=False, include_meter_details=False, start_date=None, end_date=None):
    ...
    if billing_period_name:
        scope = "/subscriptions/{}/providers/Microsoft.Billing/billingPeriods/{}".format(
            cf_consumption(cmd.cli_ctx).config.subscription_id, billing_period_name)
    else:
        scope = "/subscriptions/{}".format(
            cf_consumption(cmd.cli_ctx).config.subscription_id)
    ...
    if top:
        pages = client.list(scope, expand=expand,
                            filter=filter_expression, top=top)
        return list(pages.advance_page())
    return list(client.list(scope, expand=expand, filter=filter_expression))


def cli_consumption_list_reservations_summaries(cmd, client, grain, reservationorderid, reservationid=None, start_date=None, end_date=None):
    """List all the reservation summaries """
    if reservationid:
        scope = "providers/Microsoft.Capacity/reservationorders/{}/reservations/{}".format(
            reservationorderid, reservationid)
    else:
        scope = "providers/Microsoft.Capacity/reservationorders/{}".format(
            reservationorderid)
    ...
    filter_expression = None
    if start_date and end_date:
        filter_from = "properties/UsageDate ge {}".format(
            start_date.strftime("%Y-%m-%dT%H:%M:%SZ"))
        filter_to = "properties/UsageDate le {}".format(
            end_date.strftime("%Y-%m-%dT%H:%M:%SZ"))
        filter_expression = "{} and {}".format(filter_from, filter_to)
...
def cli_consumption_list_reservations_details(cmd, client, reservationorderid, start_date, end_date, reservationid=None):
