...
examples:
  - name: Create a federated identity credential using subject identifier
    text: |
        az identity federated-credential create --federated-credential-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --subject 'system:serviceaccount:ns:svcaccount' --audiences 'api://AzureADTokenExchange'
  - name: Create a federated identity credential using claims matching expression (long form)
    text: |
        az identity federated-credential create --fed-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --claims-matching-expression-value "claims['sub'] startswith 'repo:contoso-org/contoso-repo:ref:refs/heads/'" --claims-matching-expression-version 1 --audiences 'api://AzureADTokenExchange'
...
examples:
  - name: Update a federated identity credential using subject identifier
    text: |
        az identity federated-credential update --federated-credential-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --subject 'system:serviceaccount:ns:svcaccount' --audiences 'api://AzureADTokenExchange'
  - name: Update a federated identity credential using claims matching expression (long form)
    text: |
        az identity federated-credential update --fed-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --claims-matching-expression-value "claims['sub'] startswith 'repo:contoso-org/contoso-repo:ref:refs/heads/'" --claims-matching-expression-version 1 --audiences 'api://AzureADTokenExchange'
...
examples:
  - name: Delete a federated identity credential using long form
    text: |
        az identity federated-credential delete --federated-credential-name myFicName --identity-name myIdentityName --resource-group myResourceGroup
...
examples:
  - name: Show a federated identity credential using long form
    text: |
        az identity federated-credential show --federated-credential-name myFicName --identity-name myIdentityName --resource-group myResourceGroup
