...
class JsonCTemplatePolicy(SansIOHTTPPolicy):
    ...
    def on_request(self, request):
        ...
        if modified_data.get('properties', {}).get('template'):
            ...
            # Example format: {"properties": {"parameters": {...}, "mode": "Incremental", "template":{<raw template content>}}}
            # The template content is spliced in raw (not re-serialized) to preserve JSONC comments and avoid double-escaping.
            # If not do this splicing, the request data generated by default serialization cannot be deserialized on the service side.
            # Because the service cannot deserialize the template element: "template": "{\"$schema\": \"...\",\"contentVersion\": \"...\",\"parameters\": {...}}"
