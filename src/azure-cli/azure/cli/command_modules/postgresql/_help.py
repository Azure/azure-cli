...
examples:
  ...
  - name: >
      Create a PostgreSQL flexible server with private access using existing virtual network, subnet and private DNS zone names.
    text: >
  ...
  - name: >
      Create a PostgreSQL flexible server using existing subnet and private DNS zone resource identifiers. The subnet and DNS zone can be in different subscriptions or resource groups.
    text: |
      az postgres flexible-server create \\
  ...
  - name: >
    ...
    text: >
      ...
      az postgres flexible-server create --resource-group testGroup --name testserver \\
        ...
        --private-dns-zone /subscriptions/{subscriptionId}/resourceGroups/{resourceGroup}/providers/Microsoft.Network/privateDnsZones/testdns.postgres.database.azure.com
  - name: >
      Create a PostgreSQL flexible server using existing network resources in a different resource group / subscription.
    text: >
      az postgres flexible-server create --resource-group testgroup \\
         --name testserver --location testLocation \\
...
examples:
  ...
  - name: Update a flexible server to update private DNS zone for a VNET enabled server, using private DNS zone in the different resource group and subscription. Private DNS zone will be linked to the VNET if not already linked.
    text: az postgres flexible-server update --resource-group testgroup --name testserver --private-dns-zone /subscriptions/{subscriptionId2}/resourceGroups/{resourceGroup2}/providers/Microsoft.Network/privateDnsZones/testDNS.postgres.database.azure.com
...
examples:
  - name: Restore 'testserver' to a specific point-in-time as a new server 'testserverNew'.
    text: az postgres flexible-server restore --resource-group testgroup --name testserverNew --source-server testserver --restore-time "2026-03-22T18:20:22Z"
...
examples:
  - name: Create a read replica 'testreplica' for 'testserver' with public or private access in the specified zone and location if available.
    text: az postgres flexible-server replica create --name testreplica -g testGroup --source-server testserver --zone 3 --location testLocation
  - name: Create a read replica 'testreplica' for 'testserver' with private access using existing network resources.
    text: >
      az postgres flexible-server replica create --name testreplica -g testGroup \\
        ...
        --tags "key=value"
  - name: >
      Create a read replica 'testreplica' for 'testserver' with public or private access \
    ...
    text: az postgres flexible-server replica create --name testreplica -g testgroup --source-server testserver --location testLocation
  - name: Create a read replica 'testreplica' for 'testserver' with custom --storage-size and --sku.
    text: az postgres flexible-server replica create --name testreplica -g testgroup --source-server testserver --sku-name Standard_D4ds_v5 --storage-size 256
  - name: Create a read replica 'testreplica' for 'testserver', where 'testreplica' is in a different resource group 'newTestGroup'. \
    ...
    text: >
      az postgres flexible-server replica create --name testreplicaserver -g newTestGroup \
        --source-server /subscriptions/{sourceSubscriptionId}/resourceGroups/{sourceResourceGroup}/providers/Microsoft.DBforPostgreSQL/flexibleServers/{sourceServer} --location testLocation
"""
...
examples:
  - name: Stop replication to 'testreplica' and promote it a standalone read/write server.
    text: az postgres flexible-server replica promote -g testgroup -n testreplica
  - name: Stop replication to 'testreplica' and promote it a standalone read/write server with forced data sync.
    text: az postgres flexible-server replica promote -g testgroup -n testreplica --promote-mode standalone --promote-option forced
  - name: >
      Stop replication to 'testreplica' and promote it to primary server with planned data sync. \
      The replica you are promoting must have the reader virtual endpoint assigned, or you will receive an error on promotion.
    text: az postgres flexible-server replica promote -g testgroup -n testreplica --promote-mode switchover --promote-option planned
"""
...
examples:
  - name: >
    ...
    text: az postgres flexible-server geo-restore --resource-group testgroup --name testservernew --source-server testserver --location testlocationnew
  - name: >
...
examples:
  - name: >
    ...
    text: az postgres flexible-server revive-dropped --resource-group testgroup --name testservernew --source-server /subscriptions/{subscriptionId}/resourceGroups/{resourceGroup}/providers/Microsoft.DBforPostgreSQL/flexibleServers/testserver --location testlocationnew
  - name: >
...
examples:
  - name: Enable system assigned managed identity on the server.
    text: az postgres flexible-server identity update -g testgroup -s testserver --system-assigned Enabled
"""
...
examples:
  - name: Add identities 'test-identity' and 'test-identity-2' to server 'testserver'.
    text: az postgres flexible-server identity assign -g testgroup -s testserver --identity test-identity test-identity-2
"""
...
examples:
  - name: Remove identity 'testidentity' from server 'testserver'.
    text: az postgres flexible-server identity remove -g testgroup -s testserver --identity testidentity
"""
...
examples:
  - name: Get identity 'testidentity' from server 'testserver'.
    text: az postgres flexible-server identity show -g testgroup -s testserver --identity testidentity
"""
...
examples:
  - name: List all identities from server 'testserver'.
    text: az postgres flexible-server identity list -g testgroup -s testserver
"""
...
examples:
  - name: Create Microsoft Entra administrator with user 'john@contoso.com', administrator identifier '00000000-0000-0000-0000-000000000000' and type User.
    text: az postgres flexible-server microsoft-entra-admin create -g testgroup -s testserver -u john@contoso.com -i 00000000-0000-0000-0000-000000000000 -t User
"""
...
examples:
  - name: Delete Microsoft Entra administrator with identifier '00000000-0000-0000-0000-000000000000'.
    text: az postgres flexible-server microsoft-entra-admin delete -g testgroup -s testserver -i 00000000-0000-0000-0000-000000000000
"""
...
examples:
  - name: List Microsoft Entra administrators.
    text: az postgres flexible-server microsoft-entra-admin list -g testgroup -s testserver
"""
...
examples:
  - name: Get Microsoft Entra administrator with identifier '00000000-0000-0000-0000-000000000000'.
    text: az postgres flexible-server microsoft-entra-admin show -g testgroup -s testserver -i 00000000-0000-0000-0000-000000000000
"""
...
examples:
  - name: Wait until a Microsoft Entra administrator exists.
    text: az postgres flexible-server microsoft-entra-admin wait -g testgroup -s testserver -i 00000000-0000-0000-0000-000000000000 --exists
  - name: Wait for a Microsoft Entra administrator to be deleted.
    text: az postgres flexible-server microsoft-entra-admin wait -g testgroup -s testserver -i 00000000-0000-0000-0000-000000000000 --deleted
"""
...
examples:
  - name: >
      Downloads log files f1 and f2 to the current directory from the server 'testserver'. Please note that f1 and f2 should match the log file name including the foldername, for instance serverlogs/f1.log
    text: >
      az postgres flexible-server server-logs download -g testgroup -s testserver -n serverlogs/f1.log serverlogs/f2.log
"""
...
examples:
  - name: List log files for 'testserver' modified in the last 72 hours (default value).
    text: az postgres flexible-server server-logs list -g testgroup -s testserver
  - name: List log files for 'testserver' modified in the last 10 hours.
    text: az postgres flexible-server server-logs list -g testgroup -s testserver --file-last-written 10
  - name: List log files for 'testserver' less than 30Kb in size.
    text: az postgres flexible-server server-logs list -g testgroup -s testserver --max-file-size 30
  - name: List log files for 'testserver' containing name 'serverlogs'.
    text: az postgres flexible-server server-logs list -g testgroup -s testserver --subscription testSubscription --filename-contains serverlogs
"""
...
examples:
  - name: Delete a private endpoint connection associated with a PostgreSQL flexible server.
    text: >
      az postgres flexible-server private-endpoint-connection delete -g testgroup -s testserver \
        --id /subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/testgroup/providers/Microsoft.DBforPostgreSQL/flexibleServers/testserver/privateEndpointConnections/pec-connection.40e3d3a8-7d8f-41eb-8462-1cd05bc3e33b
"""
...
examples:
  - name: Enable bringing your PostgreSQL data into Microsoft Fabric.
    text: az postgres flexible-server fabric-mirroring start -g testgroup -s testserver --database-names testdatabase
"""
...
examples:
  - name: Stop bringing your PostgreSQL data into Microsoft Fabric.
    text: az postgres flexible-server fabric-mirroring stop -g testgroup -s testserver
"""
...
examples:
  - name: Update allowed mirrored databases.
    text: az postgres flexible-server fabric-mirroring update-databases -g testgroup -s testserver --database-names testdatabase1 testdatabase2
"""
...
examples:
  - name: Update index tuning to be enabled/disabled for a PostgreSQL flexible server.
    text: az postgres flexible-server index-tuning update -g testgroup -s testserver --enabled True
"""
...
examples:
  - name: Show state of index tuning for a PostgreSQL flexible server.
    text: az postgres flexible-server index-tuning show -g testgroup -s testserver
"""
...
examples:
  - name: Get tuning settings for a PostgreSQL flexible server.
    text: az postgres flexible-server index-tuning list-settings -g testgroup -s testserver
"""
...
examples:
  - name: Get a tuning setting for a PostgreSQL flexible server.
    text: az postgres flexible-server index-tuning show-settings -g testgroup -s testserver --name setting-name
"""
...
examples:
  - name: Update a tuning setting for a PostgreSQL flexible server.
    text: az postgres flexible-server index-tuning set-settings -g testgroup -s testserver --name setting-name --value setting-value
"""
...
examples:
  - name: Get tuning index recommendations for a PostgreSQL flexible server. Filter by selected type.
    text: az postgres flexible-server index-tuning list-recommendations -g testgroup -s testserver --recommendation-type CreateIndex
"""
...
examples:
  - name: Update autonomous tuning to be enabled for a PostgreSQL flexible server.
    text: az postgres flexible-server autonomous-tuning update -g testgroup -s testserver --enabled True
  - name: Update autonomous tuning to be disabled for a PostgreSQL flexible server.
    text: az postgres flexible-server autonomous-tuning update -g testgroup -s testserver --enabled False
"""
...
examples:
  - name: Show state of autonomous tuning for a PostgreSQL flexible server.
    text: az postgres flexible-server autonomous-tuning show -g testgroup -s testserver
"""
...
examples:
  - name: Get autonomous tuning settings for a PostgreSQL flexible server.
    text: az postgres flexible-server autonomous-tuning list-settings -g testgroup -s testserver
"""
...
examples:
  - name: Get an autonomous tuning setting for a PostgreSQL flexible server.
    text: az postgres flexible-server autonomous-tuning show-settings -g testgroup -s testserver --name setting-name
"""
...
examples:
  - name: Update an autonomous tuning setting for a PostgreSQL flexible server.
    text: az postgres flexible-server autonomous-tuning set-settings -g testgroup -s testserver --name setting-name --value setting-value
"""
...
examples:
  - name: Get autonomous tuning index recommendations for a PostgreSQL flexible server. Filter by selected type.
    text: az postgres flexible-server autonomous-tuning list-index-recommendations -g testgroup -s testserver --recommendation-type CreateIndex
"""
...
examples:
  - name: Get autonomous tuning table recommendations for a PostgreSQL flexible server. Filter by selected type.
    text: az postgres flexible-server autonomous-tuning list-table-recommendations -g testgroup -s testserver --recommendation-type AnalyzeTable
"""
