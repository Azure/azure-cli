...
from azure.cli.command_modules.appconfig._kv_helpers import __flatten_json_key_value, __is_key_vault_ref, __read_kv_from_config_store, __write_configuration_setting_to_config_store
import chardet
...
from azure.cli.core.azclierror import (AzureInternalError, FileOperationError,
                                       InvalidArgumentValueError, RequiredArgumentMissingError,
                                       ValidationError)

from ._constants import (FeatureFlagConstants, KeyVaultConstants, SearchFilterOptions, KVSetConstants, ImportExportProfiles, AppServiceConstants, JsonDiff, CompareFieldsMap, ImportMode)
...
def __read_features_from_file(file_path, format_):
    ...
    try:
        ...
        for keywordset in (FeatureManagementReservedKeywords.PASCAL, FeatureManagementReservedKeywords.CAMEL, FeatureManagementReservedKeywords.HYPHEN):
        ...
        msFeatureManagementKeyword = FeatureManagementReservedKeywords.UNDERSCORE.feature_management
        if msFeatureManagementKeyword in config_data:
           if (foundLegacySchema and any([key != FeatureFlagConstants.FEATURE_FLAGS_KEY for key in config_data[msFeatureManagementKeyword].keys()])):
               raise FileOperationError('Data contains an already defined section with the key %s.' % (msFeatureManagementKeyword))

           if (FeatureFlagConstants.FEATURE_FLAGS_KEY in config_data[msFeatureManagementKeyword]):
                foundMsFmSchema = True

           if (any([key != FeatureFlagConstants.FEATURE_FLAGS_KEY for key in config_data[msFeatureManagementKeyword].keys()])):
                foundLegacySchema = True
                legacySchemaKeyWord = msFeatureManagementKeyword

           features_dict[msFeatureManagementKeyword] = config_data[msFeatureManagementKeyword]
    ...
    return __convert_feature_dict_to_keyvalue_list(legacySchemaKeyWord, foundMsFmSchema, features_dict)

def __convert_feature_dict_to_keyvalue_list(legacySchemaKeyWord, foundMsFmSchema, features_dict):
    ...
    try:
        if foundMsFmSchema:
            ...
            feature_flags.extend(__read_features_from_msfm_schema(featureManagementSection[FeatureFlagConstants.FEATURE_FLAGS_KEY]))
        ...
        if legacySchemaKeyWord:
            for keywordset in FeatureManagementReservedKeywords.ALL:
                if keywordset.feature_management == legacySchemaKeyWord:
                    ...
                    break
            legacy_feature_management_section = __get_legacy_feature_management_schema(legacySchemaKeyWord, features_dict)
            legacy_feature_flags = __read_features_from_legacy_schema(legacy_feature_management_section, feature_management_keywords)
            feature_flags.extend([flag for flag in legacy_feature_flags if not any(flag.id == existingflag.id for existingflag in feature_flags)])
...
def __get_ms_feature_management_schema(features_dict):
    ...
    if feature_management_section and type(feature_management_section) != dict:
        raise ValidationError("Feature management section must be a dictionary.")
    
    if type(feature_management_section[FeatureFlagConstants.FEATURE_FLAGS_KEY]) != list:
        raise ValidationError("Feature flags must be a list.")
    
    return feature_management_section


def __get_legacy_feature_management_schema(legacySchemaFeatureManagementKeyWord, features_dict):
    ms_fm_keyword = FeatureManagementReservedKeywords.UNDERSCORE.feature_management
    feature_management_section = features_dict.get(legacySchemaFeatureManagementKeyWord, None)

    if feature_management_section and type(feature_management_section) != dict:
        raise ValidationError("Feature management section must be a dictionary.")
    
    if legacySchemaFeatureManagementKeyWord == ms_fm_keyword:
        return dict((key, feature_management_section[key]) for key in feature_management_section.keys()
...
def  __read_features_from_msfm_schema(feature_flags_list):
    ...
    for feature in feature_flags_list:
        if validate_import_feature(feature[FeatureFlagConstants.ID]):
            ...
            if any(existing_feature_flag.id == feature_id for existing_feature_flag in feature_flags):
                raise ValidationError(f"Duplicate feature flag with id '{feature['id']}' is found.")

            new_feature = FeatureFlagValue(
                id_=str(feature.get(FeatureFlagConstants.ID)),
                enabled=_convert_boolean_value(feature.get(FeatureFlagConstants.ENABLED, None), feature_id),
                ...
                if feature_flag_value.conditions[FeatureFlagConstants.CLIENT_FILTERS]:
                    ...
                    for idx, val in enumerate(feature_flag_value.conditions[FeatureFlagConstants.CLIENT_FILTERS]):
                        ...
                        if not val.get(FeatureFlagConstants.NAME, None):
                            raise ValidationError("Feature filter for feature '%s' must contain a 'Name' attribute.", str(k))
                        ...
                        new_val = {FeatureFlagConstants.NAME: val[FeatureFlagConstants.NAME]}
                        if filter_param:
