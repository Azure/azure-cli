...
#   ------------------------------ My Tests --------------------------------------------
    ...
    #self._prepare_connection_monitor_v2_env(resource_group, resource_group_location)
        ...
        try:
            self.cmd('network watcher connection-monitor create '
                    '--name cmv2-01 '
                    '--network-watcher-name kumaamtestnw '
                    '--resource-group kumaamtestrg '
                    '--location eastus2 '
                    '--test-groups  "[{tg1}]" ').get_output_in_json()
            print("cm created")
        except Exception as e:
            print("error creating cm",e)
            raise
        # time.sleep(10)

        # cm=self.cmd('network watcher connection-monitor show '
        #         '--name cmv2-01 '
        #         '--network-watcher-name kumamtestnw '
        #         '--resource-group kumamtestrg ').get_output_in_json()
    ...
    #self._prepare_connection_monitor_v2_env(resource_group, resource_group_location)
        ...
        endpoint1_3=self.cmd('network watcher connection-monitor endpoint add '
                '--name aks-private-vnet '
                '--resource-id /subscriptions/9cece3e3-0f7d-47ca-af0e-9772773f90b7/resourceGroups/aks-private-rg/providers/Microsoft.Network/virtualNetworks/aks-private-vnet '
                '--type AzureVNet ').get_output_in_json()
        
        endpoint2_3=self.cmd('network watcher connection-monitor endpoint add '
                '--name Bing '
                '--address bing.com '
                '--type ExternalAddress ').get_output_in_json()
        
        endpoint3_3=self.cmd('network watcher connection-monitor endpoint add '
                '--name aks-azure-cni-vnet '
                '--resource-id /subscriptions/9cece3e3-0f7d-47ca-af0e-9772773f90b7/resourceGroups/aks-azure-cni-rg/providers/Microsoft.Network/virtualNetworks/aks-azure-cni-vnet '
                '--type AzureVNet ').get_output_in_json()
        
        print("e1=",endpoint1_3)
        print("e2=",endpoint2_3)
        ...
        self.kwargs.update({
            'endpoint1_3': endpoint1_3,
            'endpoint2_3': endpoint2_3,
            'endpoint3_3': endpoint3_3,
            'tc13': tc13,
            'tc23': tc23
        })
        ...
        sys.argv = ['network watcher connection-monitor test-group add', '--sources', [endpoint1_3,endpoint2_3], '--destinations', [endpoint2_3,endpoint3_3], '--test-configurations', [tc13,tc23]]
        
        tg1_3=self.cmd("network watcher connection-monitor test-group add "
        '--name tg1_3 '
        '--sources "[{endpoint1_3},{endpoint3_3}]" '
        '--destinations "[{endpoint2_3}]" '
        '--test-configurations "[{tc13},{tc23}]" ').get_output_in_json()

        tg2_3=self.cmd("network watcher connection-monitor test-group add "
        '--name tg2_3 '
        '--sources "[{endpoint1_3},{endpoint3_3}]" '
        '--destinations "[{endpoint2_3}]" '
        '--test-configurations "[{tc13},{tc23}]" ').get_output_in_json()
        ...
        original_argv = sys.argv
        sys.argv = ['network watcher connection-monitor create', '--test-groups', [tg1_3,tg2_3]]
        ...
        cm=self.cmd('network watcher connection-monitor create '
                ...
                '--resource-group kumaamtestrg '
                '--location eastus2 '
                '--test-groups  "[{tg1_3},{tg2_3}]" '
                '--debug ').get_output_in_json()
