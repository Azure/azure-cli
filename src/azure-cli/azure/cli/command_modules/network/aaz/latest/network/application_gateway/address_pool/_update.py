...
@register_command(
    "network application-gateway address-pool update",
)
class Update(AAZCommand):
    """Update an address pool.

    :example: Update backend address pool.
    ...
    :example: Add to the backend address pool by using backend server IP address.
    ...
    :example: Remove an existing IP of the backend address pool ("0" is the index).
