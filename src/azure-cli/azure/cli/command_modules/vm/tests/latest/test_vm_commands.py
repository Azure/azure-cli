...
TEST_SSH_KEY_PUB_ED25519 = (
    "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIE+N4unGvv6pXPYebWiQ6Ak618DjYm5g6d2ECnq/1F0x generated-by-azure"
)
...
def _write_config_file(user_name):
    ...
    public_key = (
        "ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAABAQC8InHIPLAu6lMc0d+5voyXqigZfT5r6fAM1+FQAi+mkPDdk2hNq1BG0Bwfc88G"
    ...
    config = {"username": user_name, "ssh_key": public_key}
...
class VmReimageTest(ScenarioTest):
    ...
    def test_vm_reimage(self, resource_group):
        ...
        self.cmd(
            'vm create -g {rg} -n {vm} --image "OpenLogic:CentOS:7.5:latest" --admin-username centosadmin --admin-password testPassword0 '
        ...
        self.kwargs.update({"os_disk_before_reimage": vm_json_before_reimage["storageProfile"]["osDisk"]["name"]})

        self.cmd(
            "vm reimage --name {vm} --resource-group {rg} --temp-disk false "
            '--admin-password password --custom-data "dGVzdA=="'
        )
        vm_json_after_reimage = self.cmd("vm show -n {vm} -g {rg}").get_output_in_json()
        self.kwargs.update({"os_disk_after_reimage": vm_json_after_reimage["storageProfile"]["osDisk"]["name"]})
...
class VMUsageScenarioTest(ScenarioTest):
    ...
    def test_vm_usage(self):
        self.cmd("vm list-usage --location westus", checks=self.check("type(@)", "array"))
...
class VMImageListThruServiceScenarioTest(ScenarioTest):
    ...
    def test_vm_images_list_thru_services_edge_zone(self):
        result = self.cmd(
            "vm image list --edge-zone losangeles --offer CentOs --publisher OpenLogic --sku 7.7 -o tsv --all"
        ).output
    ...
    def test_vm_image_list_thru_services_edge_zone_by_arch(self):
        result = self.cmd(
            "vm image list --edge-zone losangeles --offer CentOs --publisher OpenLogic --sku 7.7 --architecture x64 -o tsv --all"
        ).output
    ...
    def test_vm_image_list_thru_services_deprecation_status(self):
        self.cmd(
            "vm image list --offer CentOs --publisher OpenLogic --sku 7.7 --architecture x64 --all ",
            checks=[
        ...
        self.cmd(
            "vm image list --offer CentOs --publisher OpenLogic --sku 7.7 --architecture x64 --edge-zone losangeles",
            checks=[
                self.check("[0].imageDeprecationStatus.imageState", "Active"),
                self.check("[0].imageDeprecationStatus.scheduledDeprecationTime", None),
            ],
...
class VMOpenPortTest(ScenarioTest):
    ...
    def test_vm_open_port(self, resource_group):
        ...
        self.kwargs["nsg_list_id"] = self.cmd(
        ...
        self.cmd(
            "network nsg show -g {rg} -n {nsg}", checks=self.check("length(securityRules[?name == 'open-port-all'])", 1)
        )
...
class VMShowListSizesListIPAddressesScenarioTest(ScenarioTest):
    ...
    def test_vm_show_list_sizes_list_ip_addresses(self, resource_group):
        ...
        self.cmd(
            "vm create --resource-group {rg} --location {loc} -n {vm} --admin-username ubuntu --image Canonical:0001-com-ubuntu-server-jammy:22_04-lts-gen2:latest --public-ip-address {pubip} "
        ...
        self.cmd("vm list-vm-resize-options --resource-group {rg} --name {vm}", checks=self.check("type(@)", "array"))

        # Expecting the one we just added
        self.kwargs["rg_caps"] = resource_group.upper()  # test the command handles name with casing diff.
        self.cmd(
            "vm list-ip-addresses --resource-group {rg_caps} -n {vm}",
            checks=[
                self.check("length(@)", 1),
...
class VMSizeListScenarioTest(ScenarioTest):
    ...
    def test_vm_size_list(self):
        self.cmd("vm list-sizes --location westus", checks=self.check("type(@)", "array"))


class VMImageListOffersScenarioTest(ScenarioTest):

    def test_vm_image_list_offers(self):
        self.kwargs.update({"loc": "westus", "pub": "Canonical"})
    ...
    def test_vm_image_list_offers_edge_zone(self):
        self.kwargs.update({"loc": "westus", "pub": "OpenLogic", "edge_zone": "microsoftlosangeles1"})
...
class VMImageListPublishersScenarioTest(ScenarioTest):
    ...
    def test_vm_image_list_publishers_edge_zone(self):
        self.kwargs.update({"loc": "westus", "edge_zone": "microsoftlosangeles1"})
...
class VMImageListSkusScenarioTest(ScenarioTest):
    ...
    def test_vm_image_list_skus(self):
        ...
        self.kwargs.update({"loc": "westus", "pub": "Canonical", "offer": "UbuntuServer"})
    ...
    def test_vm_image_list_skus_edge_zone(self):
        ...
        self.kwargs.update(
            {"loc": "westus", "pub": "OpenLogic", "offer": "CentOs", "edge_zone": "microsoftlosangeles1"}
        )
...
class VMImageShowScenarioTest(ScenarioTest):
    ...
    def test_vm_image_show(self):
        ...
        self.kwargs.update(
            {"loc": "westus", "pub": "Canonical", "offer": "UbuntuServer", "sku": "18.04-LTS", "ver": "18.04.202002180"}
        )
...
class VMGeneralizeScenarioTest(ScenarioTest):
    ...
    def test_vm_generalize(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --admin-username ubuntu --image OpenLogic:CentOS:7.5:latest --admin-password testPassword0 "
    ...
    def test_vm_generalize_debian(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --admin-username debian --image Debian:debian-10:10:latest --admin-password testPassword0 "
            "--authentication-type password --use-unmanaged-disk --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip} --nsg-rule NONE"
        )
    ...
    def test_vm_capture_zone_resilient_image(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} --location {loc} -n {vm} --admin-username ubuntu --image OpenLogic:CentOS:7.5:latest --public-ip-address {pubip} "
...
class VMWindowsLicenseTest(ScenarioTest):
    ...
    def test_vm_windows_license_type(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Win2022Datacenter --admin-username clitest1234 --admin-password Test123456789# "
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=[self.check("licenseType", "Windows_Server")])
        self.cmd("vm update -g {rg} -n {vm} --license-type None", checks=[self.check("licenseType", "None")])
...
class VMCustomImageTest(ScenarioTest):
    ...
    def test_vm_custom_image(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --image OpenLogic:CentOS:7.5:latest --use-unmanaged-disk --admin-username sdk-test-admin "
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --image OpenLogic:CentOS:7.5:latest --storage-sku standard_lrs --data-disk-sizes-gb 1 1 1 1 "
        ...
        self.cmd(
            "vmss create -g {rg} -n vmss1 --image {image1} --admin-username sdk-test-admin --admin-password testPassword0 "
            "--authentication-type password --public-ip-address {pubip4} --orchestration-mode Flexible --nsg {nsg}",
            checks=[
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "storageProfile.dataDisks[?lun == `0`] | [0].managedDisk.storageAccountType", "StandardSSD_LRS"
                ),
                self.check(
                    "storageProfile.dataDisks[?lun == `1`] | [0].managedDisk.storageAccountType", "Standard_LRS"
                ),
                self.check(
                    "storageProfile.dataDisks[?lun == `3`] | [0].managedDisk.storageAccountType", "Standard_LRS"
                ),
            ],
        )
        ...
        self.cmd(
            "vmss create -g {rg} -n vmss2 --image {image2} --admin-username sdk-test-admin --admin-password testPassword0 "
            "--authentication-type password --orchestration-mode Flexible --public-ip-address {pubip7} --nsg {nsg}",
            checks=[
                self.check("vmss.virtualMachineProfile.storageProfile.imageReference.resourceGroup", "{rg}"),
                self.check("vmss.virtualMachineProfile.storageProfile.osDisk.createOption", "FromImage"),
                self.check("length(vmss.virtualMachineProfile.storageProfile.dataDisks)", 2),
                self.check(
                    "vmss.virtualMachineProfile.storageProfile.dataDisks[?lun == `1`] | [0].createOption", "FromImage"
                ),
                self.check(
                ...
                self.check(
                    "vmss.virtualMachineProfile.storageProfile.dataDisks[?lun == `3`] | [0].createOption", "FromImage"
                ),
                self.check(
    ...
    def test_vm_custom_image_debian(self, resource_group):
        ...
        self.cmd("network public-ip create --name {pubip2} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm2} --image Debian:debian-10:10:latest --storage-sku standard_lrs --data-disk-sizes-gb 1 1 1 1 "
            "--admin-username sdk-test-admin --admin-password testPassword0 --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip2} --nsg-rule NONE"
        )
        data_disks = self.cmd("vm show -g {rg} -n {vm2}").get_output_in_json()["storageProfile"]["dataDisks"]
        self.kwargs["disk_0_name"] = data_disks[0]["name"]
        ...
        self.cmd("network public-ip create --name {pubip4} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vmss create -g {rg} -n vmss1 --image {image1} --admin-username sdk-test-admin --admin-password testPassword0 --public-ip-address {pubip4} "
            "--authentication-type password --orchestration-mode Uniform --lb-sku Standard --nsg {nsg}",
            checks=[
                self.check("vmss.virtualMachineProfile.storageProfile.imageReference.resourceGroup", "{rg}"),
                self.check("vmss.virtualMachineProfile.storageProfile.osDisk.createOption", "FromImage"),
            ],
        ...
        self.cmd("network public-ip create --name {pubip5} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {newvm2} --image {image2} --admin-username sdk-test-admin --admin-password testPassword0 --authentication-type password "
        ...
        self.cmd("network public-ip create --name {pubip7} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vmss create -g {rg} -n vmss2 --image {image2} --admin-username sdk-test-admin --admin-password testPassword0 --public-ip-address {pubip7} "
            "--authentication-type password --orchestration-mode Uniform --lb-sku Standard --nsg {nsg}",
            checks=[
                self.check("vmss.virtualMachineProfile.storageProfile.imageReference.resourceGroup", "{rg}"),
                self.check("vmss.virtualMachineProfile.storageProfile.osDisk.createOption", "FromImage"),
    ...
    def test_vm_custom_image_name_conflict(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --size Standard_B2ms --use-unmanaged-disk --admin-username ubuntu "
    ...
    def test_vm_custom_image_management(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --admin-username theuser --image OpenLogic:CentOS:7.5:latest --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip} --nsg-rule NONE"
        )
...
class VMCreateFromUnmanagedDiskTest(ScenarioTest):
    ...
    def test_vm_create_from_unmanaged_disk(self, resource_group):
        ...
        self.cmd("network public-ip create --name {pubip1} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --use-unmanaged-disk --admin-username ubuntu --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip1} --nsg-rule NONE"
        )
        ...
        vm1_info = self.cmd(
            "vm show -g {rg} -n {vm}", checks=[self.check("name", "{vm}"), self.check("licenseType", None)]
        ).get_output_in_json()
        self.cmd("vm stop -g {rg} -n {vm}")

        # import the unmanaged os disk into a specialized managed disk
        self.kwargs.update(
            {"os_disk_vhd_uri": vm1_info["storageProfile"]["osDisk"]["vhd"]["uri"], "vm": "vm2", "os_disk": "os1"}
        )
        self.cmd(
            "disk create -g {rg} -n {os_disk} --source {os_disk_vhd_uri} --os-type linux",
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --attach-os-disk {os_disk} --os-type linux --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip2} --nsg-rule NONE",
...
class VMCreateWithSpecializedUnmanagedDiskTest(ScenarioTest):
    ...
    def test_vm_create_with_specialized_unmanaged_disk(self, resource_group):
        ...
        # create a vm with unmanaged os disk
        self.cmd(
            "vm create -g {rg} -n vm1 --image Debian:debian-10:10:latest --use-unmanaged-disk --admin-username ubuntu --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip1} --nsg-rule NONE"
        )
        ...
        self.cmd(
            "vm create -g {rg} -n vm2 --attach-os-disk {disk_uri} --os-type linux --use-unmanaged-disk --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip2} --nsg-rule NONE",
            checks=self.check("powerState", "VM running"),
        )
    ...
    def test_vm_create_with_unmanaged_data_disks(self, resource_group):
        ...
        vm_create_cmd = (
            "vm create -g {rg} -n vm1 --image Debian:debian-10:10:latest --use-unmanaged-disk --admin-username ubuntu --admin-password testPassword0 "
        ...
        # create a vm by attaching the OS disk from the deleted VM
        vm_create_cmd = (
            "vm create -g {rg} -n {vm2} --attach-os-disk {disk_uri} --os-type linux --use-unmanaged-disk --public-ip-address {pubip2} "
...
class VMRedeployTest(ScenarioTest):
    ...
    def test_vm_redeploy(self, resource_group):
        ...
        self.cmd("network public-ip create --name {pubip} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --use-unmanaged-disk --admin-username ubuntu --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip} --nsg-rule NONE"
        )
...
class VMAttachDisksOnCreate(ScenarioTest):
    ...
    def test_vm_create_by_attach_os_and_data_disks(self, resource_group):
        ...
        # creating a vm
        self.cmd(
            "vm create -g {rg} -n vm1 --image OpenLogic:CentOS:7.5:latest --size Standard_B2ms --admin-username centosadmin --admin-password testPassword0 --authentication-type password --data-disk-sizes-gb 2 --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip1} --nsg-rule NONE"
        )
        ...
        self.cmd(
            "vm create -g {rg} -n vm2 --attach-os-disk {os_disk} --os-disk-delete-option Delete "
    ...
    def test_vm_create_by_attach_unmanaged_os_and_data_disks(self, resource_group):
        ...
        # creating a vm
        self.cmd(
            "vm create -g {rg} -n vm1 --use-unmanaged-disk --image OpenLogic:CentOS:7.5:latest --admin-username centosadmin --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip1} --nsg-rule NONE"
        )
        ...
        # rebuild a new vm
        self.cmd(
            "vm create -g {rg} -n vm2 --attach-os-disk {os_disk_vhd} --attach-data-disks {data_disk_vhd} --public-ip-address {pubip2} "
    ...
    def test_vm_create_data_disk_delete_option(self, resource_group):
        ...
        self.cmd(
            "vm create -n Delete_CLI1 -g {rg} --image RedHat:RHEL:8-lvm-gen2:latest -l northeurope "
...
class VMOSDiskSize(ScenarioTest):
    ...
    def test_vm_set_os_disk_size(self, resource_group):
        self.kwargs.update({"subnet": "subnet1", "vnet": "vnet1"})
        # test unmanaged disk
        self.kwargs.update({"sa": self.create_random_name(prefix="cli", length=12)})
        self.cmd(
            "vm create -g {rg} -n vm --image OpenLogic:CentOS:7.5:latest --admin-username centosadmin "
        ...
        self.cmd("vm show -g {rg} -n vm1", checks=self.check("storageProfile.osDisk.diskSizeGb", 75))
...
class VMManagedDiskScenarioTest(ScenarioTest):
    ...
    def test_vm_managed_disk(self, resource_group):
        ...
        data_disk = self.cmd(
            ...
            checks=[self.check("sku.name", "Premium_LRS"), self.check("diskSizeGB", 1), self.check("tags.tag1", "d1")],
        ).get_output_in_json()
        self.cmd(
            "disk update -g {rg} -n {disk1} --size-gb 10 --sku Standard_LRS",
            checks=[self.check("sku.name", "Standard_LRS"), self.check("diskSizeGB", 10)],
        )
        ...
        # create a snpashot
        os_snapshot = self.cmd(
            "snapshot create -g {rg} -n {snapshot1} --size-gb 1 --sku Premium_LRS --tags tag1=s1",
            checks=[self.check("sku.name", "Premium_LRS"), self.check("diskSizeGB", 1), self.check("tags.tag1", "s1")],
        ).get_output_in_json()
        # update the sku
        self.cmd(
            "snapshot update -g {rg} -n {snapshot1} --sku Standard_LRS",
            checks=[self.check("sku.name", "Standard_LRS"), self.check("diskSizeGB", 1)],
        )
        ...
        self.kwargs.update(
            {"snapshot1_id": os_snapshot["id"], "snapshot2_id": data_snapshot["id"], "disk2_id": data_disk2["id"]}
        )
    ...
    def test_vm_disk_from_restore_point(self, resource_group):
        ...
        # create a disk and update
        data_disk = self.cmd(
            "disk create -g {rg} -n {disk_name1} --size-gb 1",
            checks=[self.check("sku.name", "Premium_LRS"), self.check("diskSizeGB", 1)],
        ).get_output_in_json()
        # create a vm
        vm = self.cmd(
            "vm create -n {vm_name} -g {rg} --image {vm_image} --attach-data-disks {disk_name1} "
            "--admin-username rp_disk_test --size Standard_B2ms --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        ).get_output_in_json()
        ...
        collection = self.cmd(
            "restore-point collection create -g {rg} --collection-name {collection_name} --source-id {vm_id}"
        ).get_output_in_json()
        self.kwargs.update(
            {
        ...
        point = self.cmd(
            "restore-point create -g {rg} -n {point_name} --collection-name {collection_name}"
        ).get_output_in_json()
        self.kwargs.update({"point_id": point["id"]})
        disk_restore_point_id = self.cmd(
            'restore-point show --resource-group {rg} --collection-name {collection_name} --name {point_name} --query "sourceMetadata.storageProfile.dataDisks[0].diskRestorePoint.id"'
        ).get_output_in_json()
    ...
    def test_vm_data_disk_creation_from_copy_and_restore(self):
        ...
        vm = self.cmd(
            "vm create -n {vm_name} -g {rg} --image Canonical:ubuntu-24_04-lts:server:latest --attach-data-disks {disk_name1} "
            "--admin-username rp_disk_test --size Standard_B2ms --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        ).get_output_in_json()
        ...
        self.kwargs["disk_restore_point_id"] = disk_restore_point["sourceMetadata"]["storageProfile"]["dataDisks"][0][
            "diskRestorePoint"
        ]["id"]

        restore_disk = self.cmd(
            "disk create --resource-group {rg} --name {disk_name2} --sku Standard_LRS --size-gb 5 --source {disk_restore_point_id}"
        ).get_output_in_json()
        ...
        self.kwargs.update({"copy_resource1_id": copy_disk["id"], "copy_resource2_id": copy_snapshot["id"]})
        self.cmd(
            "vm create -g {rg} -n {vm_name2} --image ubuntu2204 --size Standard_B2ms --generate-ssh-keys --source-resource {copy_resource1_id} {copy_resource2_id} "
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message1):
            self.cmd(
                "vm create -g {rg} -n {vm_name2} --image ubuntu2204 --generate-ssh-keys --source-resource {copy_resource1_id} {copy_resource2_id} --source-resource-size 5 "
                "--source-disk-rp {disk_restore_point_id} --source-rp-size 5 --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
            )

        message2 = (
            "usage error: --source-snapshots-or-disks and --source-snapshots-or-disks-size-gb must be used together"
        )
        with self.assertRaisesRegex(ArgumentUsageError, message2):
            self.cmd(
                "vm create -g {rg} -n {vm_name2} --image ubuntu2204 --generate-ssh-keys --source-resource {copy_resource1_id} {copy_resource2_id} "
                "--subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
            )
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message3):
            self.cmd(
                "vm create -g {rg} -n {vm_name2} --image ubuntu2204 --generate-ssh-keys --source-disk-rp {disk_restore_point_id} --source-rp-size 5 5 "
                "--subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
            )

        message4 = (
            "usage error: --source-disk-restore-point and --source-disk-restore-point-size-gb must be used together"
        )
        with self.assertRaisesRegex(ArgumentUsageError, message4):
            self.cmd(
                "vm create -g {rg} -n {vm_name2} --image ubuntu2204 --generate-ssh-keys --source-rp-size 5 "
                "--subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
            )
    ...
    def test_vm_disk_attach_from_copy_and_restore(self):
        ...
        self.cmd("disk create -g {rg} -n {disk_name1} --size-gb 1 --sku Standard_LRS")
        vm = self.cmd(
            "vm create -n {vm_name} -g {rg} --image Canonical:0001-com-ubuntu-server-jammy:22_04-lts:latest --attach-data-disks {disk_name1} "
            "--admin-username rp_disk_test --subnet {subnet} --vnet-name {vnet} --size Standard_B2ms --nsg-rule NONE"
        ).get_output_in_json()
        ...
        self.kwargs.update({"copy_resource1_id": copy_disk["id"], "copy_resource2_id": copy_snapshot["id"]})
        self.cmd(
            "vm disk attach -g {rg} --size 20 --sku Standard_LRS --vm-name {vm_name} --source-resource {copy_resource1_id} {copy_resource2_id} --new-names-of-sr {disk_name6} {disk_name7}"
            " --source-disk-rp {disk_restore_point_id} --new-names-of-rp {disk_name8}"
        )
        self.cmd(
            "vm show -g {rg} -n {vm_name}",
            checks=[
        ...
        with self.assertRaisesRegex(
            ...
            self.cmd(
                "vm disk attach -g {rg} --vm-name {vm_name} --source-resource {copy_resource1_id} --new-names-of-sr {disk_name6} {disk_name7}"
            )
        with self.assertRaisesRegex(
            ArgumentUsageError,
            ...
            self.cmd(
                "vm disk attach -g {rg} --vm-name {vm_name} --source-disk-rp {disk_restore_point_id} --new-names-of-rp {disk_name7} {disk_name8}"
            )
    ...
    def test_vm_snapshot_enable_instant_access(self, resource_group):
        ...
        snapshot = self.cmd(
            "snapshot create -g {rg} -n {snapshot} --ia-duration 300 --incremental true --source {disk}",
            checks=[self.check("incremental", True), self.check("creationData.instantAccessDurationMinutes", 300)],
        ).get_output_in_json()
        self.kwargs.update({"snapshot_id": snapshot["id"]})
        self.cmd(
            "disk create -g {rg} -n {iadisk1} --source {snapshot_id} --sku UltraSSD_LRS",
            checks=[self.check("creationData.sourceResourceId", "{snapshot_id}")],
        )
    ...
    def test_vm_disk_max_shares_etc(self, resource_group):
        ...
        self.kwargs.update(
            {
                ...
                "image": "/Subscriptions/"
                + subs_id
                + "/Providers/Microsoft.Compute/Locations/westus/Publishers/Canonical/ArtifactTypes/VMImage/Offers/0001-com-ubuntu-server-jammy/Skus/22_04-lts-gen2/Versions/22.04.202507300",
        ...
        self.cmd(
            ...
            checks=[self.check("diskIOPSReadOnly", 200), self.check("diskMBpsReadOnly", 30)],
        )

        self.cmd(
            "disk update -g {rg} -n {disk1} --disk-iops-read-only 250 --disk-mbps-read-only 40",
            checks=[self.check("diskIOPSReadOnly", 250), self.check("diskMBpsReadOnly", 40)],
        )

        self.cmd(
            "disk create -g {rg} -n {disk2} --image-reference {image}",
            checks=[self.check("creationData.imageReference.id", "{image}")],
        )

        self.cmd(
            "disk create -g {rg} -n {disk3} --image-reference Canonical:0001-com-ubuntu-server-jammy:22_04-lts-gen2:latest",
            checks=[self.check("creationData.imageReference.id", "{image}")],
        )
        ...
        self.kwargs.update({"gallery_image": gallery_image})
        self.cmd(
            "disk create -g {rg} -n {disk4} --gallery-image-reference {gallery_image}",
            checks=[self.check("creationData.galleryImageReference.id", "{gallery_image}")],
        )

        self.cmd(
            "disk create -g {rg} -n {disk6} --size-gb 256 --max-shares 2 -l centraluseuap",
            checks=[self.check("maxShares", 2)],
        )

        self.cmd("disk update -g {rg} -n {disk6} --max-shares 1", checks=[self.check("maxShares", 1)])
    ...
    def test_create_disk_from_diff_gallery_image_version(self):
        ...
        self.kwargs["vm_id"] = self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --data-disk-sizes-gb 10 --admin-username clitest1 "
        ...
        self.cmd(
            "sig create -g {rg} --gallery-name {gallery1} --permissions community --publisher-uri publisher1 "
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery1} --gallery-image-definition {image1} "
            "--os-type linux --os-state Specialized -p publisher1 -f offer1 -s sku1 --hyper-v-generation v1"
        )
        self.cmd(
            "sig image-version create -g {rg} --gallery-name {gallery1} --gallery-image-definition {image1} "
            "--gallery-image-version {version} --virtual-machine {vm_id}"
        )

        self.kwargs["public_name"] = self.cmd(
            "sig show --gallery-name {gallery1} --resource-group {rg} --select Permissions",
            checks=[
        ...
        community_gallery_image_version = self.cmd(
            ...
            "--location {location} --gallery-image-version {version}"
        ...
        self.cmd(
            "disk create -g {rg} -n {disk1} --gallery-image-reference {community_gallery_image_version}",
            checks=[
                self.check("provisioningState", "Succeeded"),
                self.check(
                    "creationData.galleryImageReference.communityGalleryImageId", "{community_gallery_image_version}"
                ),
            ],
        )
        ...
        self.cmd(
            "sig show --gallery-name {gallery1} --resource-group {rg} --select Permissions",
            checks=[self.check("sharingProfile.permissions", "Private")],
        )

        self.cmd("sig create -g {rg} --gallery-name {gallery2} --permissions groups")
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery2} --gallery-image-definition {image2} "
            "--os-type linux --os-state Specialized -p publisher1 -f offer1 -s sku1 --hyper-v-generation v1"
        )
        ...
        self.cmd(
            "disk create -g {rg} -n {disk2} --gallery-image-reference {compute_gallery_image_version}",
            checks=[
                self.check("provisioningState", "Succeeded"),
        ...
        unique_name = self.cmd(
            "sig show --gallery-name {gallery2} --resource-group {rg} --select Permissions"
        ...
        shared_gallery_image_version = self.cmd(
            "sig image-version show-shared --gallery-image-definition {image2} --gallery-unique-name "
        ...
        self.cmd(
            "disk create -g {rg} -n {disk3} --gallery-image-reference {shared_gallery_image_version}",
            checks=[
                self.check("provisioningState", "Succeeded"),
    ...
    @unittest.skip(
        "No match for the request (<Request (GET) https://azcliprod.blob.core.windows.net/cli-extensions/elastic_san-1.1.0-py3-none-any.whl>) was found."
    )
    ...
    def test_disk_and_snapshot_with_elastic_san(self, resource_group):
        ...
        self.cmd(
            "elastic-san create -n {elastic_san} -g {rg} -l westus2 --base-size-tib 23 --extended-size 14 --sku {{name:Premium_LRS,tier:Premium}}"
        )
        self.cmd("elastic-san volume-group create -e {elastic_san} -g {rg} -n {volume_group}")
        volume = self.cmd(
            "elastic-san volume create -e {elastic_san} -g {rg} -v {volume_group} -n {volume_name} --size-gib 2"
        ).get_output_in_json()
        self.kwargs.update({"volume_id": volume["id"]})
        snapshot = self.cmd(
            "elastic-san volume snapshot create -g {rg} -e {elastic_san} -v {volume_group} -n {snapshot_name} --creation-data {{source-id:{volume_id}}}"
        ).get_output_in_json()
        self.kwargs.update({"snapshot_id": snapshot["id"]})
        ...
        # ])
        self.cmd(
            "snapshot create -n {snapshot} -g {rg} --elastic-san-resource-id {snapshot_id}",
            checks=[
                self.check("creationData.createOption", "CopyFromSanSnapshot"),
    ...
    def test_disk_with_optimized_for_frequent_attach(self, resource_group):
        ...
        self.cmd(
            "disk create -g {rg} -n {disk1} --size-gb 10 -l westus --optimized-for-frequent-attach true",
            self.check("optimizedForFrequentAttach", True),
        )
        self.cmd(
            "disk create -g {rg} -n {disk2} --size-gb 10 -l westus --optimized-for-frequent-attach false",
    ...
    def test_disk_config_update(self, resource_group):
        self.kwargs.update({"disk": self.create_random_name("disk", length=10)})

        self.cmd("disk create -g {rg} -n {disk} --size-gb 10 ", checks=[self.check("diskSizeGB", 10)])

        self.cmd("disk config update -g {rg} -n {disk} --size-gb 20 ", checks=[self.check("diskSizeGB", 20)])
...
class VMCreateAndStateModificationsScenarioTest(ScenarioTest):
    ...
    def test_vm_create_state_modifications(self, resource_group):
        ...
        self.cmd(
            "vm create --resource-group {rg} --location {loc} --name {vm} --admin-username azureuser --image OpenLogic:CentOS:7.5:latest --admin-password testPassword0 --authentication-type password --tags firsttag=1 secondtag=2 thirdtag --nsg {nsg} --public-ip-address {ip} --subnet {subnet} --vnet-name {vnet} --storage-account {sa} --use-unmanaged-disk --nsg-rule NONE --size Standard_B2s"
        )
        ...
        self.cmd(
            "vm resize -g {rg} -n {vm} --size Standard_B2ms",
    ...
    def test_vm_user_update_win(self, resource_group):
        self.kwargs.update({"subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n vm --image Win2022Datacenter --admin-username AzureUser "
    ...
    def test_vm_size_properties(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --size Standard_D2s_v3 --v-cpus-available 1 --v-cpus-per-core 1 --admin-username vmtest "
        ...
        self.cmd(
            "vm update -g {rg} -n {vm} --v-cpus-available 2 --v-cpus-per-core 2",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --vm-sku Standard_D2s_v3 --v-cpus-available 1 "
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss} --v-cpus-available 2 --v-cpus-per-core 2",
            checks=[
...
class VMSimulateEvictionScenarioTest(ScenarioTest):
    ...
    def test_vm_simulate_eviction(self, resource_group):
        ...
        self.cmd(
            "vm create --resource-group {rg} --name {vm1} --admin-username azureuser --admin-password testPassword0 --authentication-type password"
        ...
        self.cmd(
            "vm create --resource-group {rg} --name {vm2} --admin-username azureuser --admin-password testPassword0 "
        ...
        self.cmd(
            "vm create --resource-group {rg} --name {vm3} --admin-username azureuser --admin-password testPassword0 "
...
class VMNoWaitScenarioTest(ScenarioTest):
    ...
    def test_vm_create_no_wait(self, resource_group):
        ...
        self.kwargs.update({"loc": "westus", "vm": "vmnowait2", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --admin-username user12 --admin-password testPassword0 --authentication-type password "
        ...
        self.cmd("vm update -g {rg} -n {vm} --set tags.mytag=tagvalue1 --no-wait", checks=self.is_empty())
        self.cmd("vm wait -g {rg} -n {vm} --updated", checks=self.is_empty())
        self.cmd("vm show -g {rg} -n {vm}", checks=self.check("tags.mytag", "tagvalue1"))
...
class VMAvailSetScenarioTest(ScenarioTest):
    ...
    def test_vm_availset(self, resource_group):
        ...
        self.kwargs.update({"availset": "availset-test"})
        self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("length(@)", 1), self.check("[0].name", "{availset}")],
        )
        result = self.cmd("vm availability-set list --query \"[?name=='availset-test']\"").get_output_in_json()
        self.assertEqual(1, len(result))
        self.cmd("vm availability-set list-sizes -g {rg} -n {availset}", checks=self.check("type(@)", "array"))
        self.cmd("vm availability-set show -g {rg} -n {availset}", checks=[self.check("name", "{availset}")])
        self.cmd("vm availability-set delete -g {rg} -n {availset}")
        self.cmd("vm availability-set list -g {rg}", checks=self.check("length(@)", 0))
    ...
    def test_vm_availset_scheduled_events_policy(self, resource_group):
        self.kwargs.update({"availset1": "availset-test1", "availset2": "availset-test2"})

        self.cmd(
            "vm availability-set create -g {rg} -n {availset1} --additional-events True --enable-reboot True --enable-redeploy True --platform-fault-domain-count 1 --platform-update-domain-count 1",
            checks=[
        ...
        self.cmd(
            "vm availability-set update -g {rg} -n {availset1} --additional-events False --enable-redeploy False",
            checks=[
                self.check("name", "{availset1}"),
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
        ...
        self.cmd(
            "vm availability-set create -g {rg} -n {availset2} --platform-fault-domain-count 1 --platform-update-domain-count 1",
            checks=[
        ...
        self.cmd(
            "vm availability-set update -g {rg} -n {availset2} --enable-reboot False --additional-events True",
            checks=[
                self.check("name", "{availset2}"),
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
...
class VMAvailSetLiveScenarioTest(ScenarioTest):
    ...
    def test_vm_availset_convert(self, resource_group):
        ...
        self.kwargs.update({"availset": "availset-test-conv"})
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{availset}"), self.check("sku.name", "Aligned")],
        )
    ...
    def test_vm_avset_migrate(self, resource_group):
        ...
        self.cmd(
            "vm availability-set create -g {rg} -n {avset1} --platform-fault-domain-count 2 --platform-update-domain-count 2",
            checks=[self.check("name", "{avset1}"), self.check("platformFaultDomainCount", 2)],
        )
        self.cmd(
            "vm create -g {rg} -n {vm1} --image OpenLogic:CentOS:7.5:latest --admin-username vmtest --nsg-rule NONE --size Standard_B2ms --availability-set {avset1}"
        )
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image OpenLogic:CentOS:7.5:latest --admin-username vmsstest --vm-sku Standard_B2ms"
        )
...
class ComputeListSkusScenarioTest(ScenarioTest):
    ...
    def test_list_compute_skus_filter(self):
        ...
        self.assertTrue(
            result
            and len(result)
            == len([x for x in result if x["name"] == "Standard_DS1_v2" and x["locationInfo"][0]["zones"]])
        )
...
class VMExtensionScenarioTest(ScenarioTest):
    ...
    def test_vm_extension(self, resource_group):
        ...
        self.cmd(
            "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password --admin-username user11 --admin-password testPassword0 --nsg-rule NONE"
        )

        self.cmd("vm extension list --vm-name {vm} --resource-group {rg}", checks=self.check("length([])", 0))
        self.cmd(
        ...
        self.cmd(
            "vm extension show --resource-group {rg} --vm-name {vm} --name {ext} --instance-view",
            checks=[
        ...
        self.cmd(
            "vm extension show --resource-group {rg} --vm-name {vm} --name {ext2}",
            checks=[
                self.check("type(@)", "object"),
        ...
        self.cmd(
            "vm extension show --resource-group {rg} --vm-name {vm2} --name {ext3}",
            checks=[
                self.check("type(@)", "object"),
    ...
    def test_vm_extension_debian(self, resource_group):
        ...
        self.cmd(
            "vm create -n {vm} -g {rg} --image Debian:debian-10:10:latest --authentication-type password --admin-username user11 "
    ...
    def test_vm_extension_list(self, resource_group):
        ...
        vm = self.cmd(
            "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password "
    ...
    def test_vm_extension_instance_name(self, resource_group):
        ...
        self.cmd(
            "vm extension set -n {ext_type} --publisher {pub} --version 1.2 --vm-name {vm} --resource-group {rg} "
            '--protected-settings "{config}" --extension-instance-name {ext_name}'
        )

        self.cmd(
            "vm extension show --resource-group {rg} --vm-name {vm} --name {ext_name}",
            checks=[self.check("name", "{ext_name}"), self.check("typePropertiesType", "{ext_type}")],
        )
...
class VMMachineExtensionImageScenarioTest(ScenarioTest):
    ...
    def test_vm_machine_extension_image(self):
        ...
        self.kwargs.update(
            {"loc": "westus", "pub": "Microsoft.Azure.Diagnostics", "ext": "IaaSDiagnostics", "ver": "1.6.4.0"}
        )
...
class VMExtensionImageSearchScenarioTest(LiveScenarioTest):
    ...
    def test_vm_extension_image_search(self):
        ...
        self.kwargs.update({"pub": "Test.Microsoft.VisualStudio.Services", "image": "TeamServicesAgentLinux1"})
        self.cmd(
            "vm extension image list -l westus --publisher {pub} --name {image}",
            checks=[self.check("type(@)", "array"), self.check("length([?name == '{image}']) == length(@)", True)],
        )
...
class VMCreateEphemeralOsDisk(ScenarioTest):
    ...
    def test_vm_create_ephemeral_os_disk(self, resource_group, resource_group_location):
        ...
        self.cmd(
            "vm create --resource-group {rg} --name {vm} --image {image} --ssh-key-value '{ssh_key}' --location {loc} "
        ...
        self.cmd(
            "vm create --resource-group {rg} --name {vm_2} --image {image} --ssh-key-value '{ssh_key}' --location {loc} "
    ...
    def test_vm_create_ephemeral_os_disk_placement(self, resource_group, resource_group_location):
        ...
        # check base
        self.cmd(
            "vm create -n {base} -g {rg} --image {image} --size Standard_DS4_v2 --location {loc} --ssh-key-value '{ssh_key}' "
...
class VMUpdateTests(ScenarioTest):
    ...
    def test_vm_update_size(self, resource_group, resource_group_location):
        ...
        # check base
        self.cmd(
            "vm create -n {base} -g {rg} --image {image} --size Standard_DS4_v2 --location {loc} --ephemeral-os-disk "
        ...
        self.cmd(
            "vm create -n {base2} -g {rg} --image {image}  --location {loc} --admin-username vmtest "
...
class VMMultiNicScenarioTest(ScenarioTest):  # pylint: disable=too-many-instance-attributes
    ...
    def test_vm_create_multi_nics(self, resource_group):
        ...
        self.kwargs.update({"vnet": "myvnet", "subnet": "mysubnet", "vm": "multinicvm1", "ssh_key": TEST_SSH_KEY_PUB})
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --nics nic1 nic2 nic3 nic4 --nic-delete-option nic1=Delete nic3=Delete --admin-username user11 --size Standard_DS3 --ssh-key-value '{ssh_key}'"
        )
        self.cmd(
            "vm show -g {rg} -n {vm}",
            checks=[
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --nics nic1 nic2 nic3 nic4 --nic-delete-option Detach --admin-username user11 --size Standard_DS3 --ssh-key-value '{ssh_key}'"
        )
        self.cmd(
            "vm show -g {rg} -n {vm}",
            checks=[
        ...
        self.cmd(
            "vm nic list -g {rg} --vm-name {vm}", checks=[self.check("length(@)", 4), self.check("[0].primary", True)]
        )
...
class VMCreateNoneOptionsTest(ScenarioTest):  # pylint: disable=too-many-instance-attributes
    ...
    def test_vm_create_none_options(self, resource_group):
        ...
        self.cmd(
            "vm create -n {vm} -g {rg} --computer-name {quotes} --image Debian:debian-10:10:latest --availability-set {quotes} "
...
class VMMonitorTestDefault(ScenarioTest):
    ...
    def test_vm_create_with_monitor(self, resource_group):
        ...
        with mock.patch("azure.cli.command_modules.vm.custom._gen_guid", side_effect=self.create_guid):
            self.cmd(
                "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --workspace {workspace} "
    ...
    def test_vm_metric_tail(self, resource_group):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --nsg {nsg} --admin-username azureuser "
        ...
        self.cmd("vm monitor metrics list-definitions -n {vm} -g {rg}", checks=[self.check("length(@) != '0'", True)])
...
class VMMonitorTestCreateLinux(ScenarioTest):
    ...
    def test_vm_create_with_workspace_linux(self, resource_group):
        ...
        with mock.patch("azure.cli.command_modules.vm.custom._gen_guid", side_effect=self.create_guid):
            self.cmd(
                "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --workspace {workspace} --nsg {nsg} "
        ...
        uri_template = (
            "https://management.azure.com{0}/dataSources?$filter=kind eq '{1}'&api-version=2020-03-01-preview"
        )
        uri = uri_template.format(workspace_id, "LinuxPerformanceCollection")
        self.cmd('az rest --method get --uri "{}"'.format(uri), checks=[self.check("length(value)", 1)])
        ...
        self.cmd('az rest --method get --uri "{}"'.format(uri), checks=[self.check("length(value)", 4)])
...
class VMMonitorTestCreateWindows(ScenarioTest):
    ...
    def test_vm_create_with_workspace_windows(self, resource_group):
        ...
        self.cmd('az rest --method get --uri "{}"'.format(uri), checks=[self.check("length(value)", 15)])
...
class VMMonitorTestUpdateLinux(ScenarioTest):
    ...
    def test_vm_update_with_workspace_linux(self, resource_group):
        ...
        self.cmd(
            "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --nsg {nsg} --generate-ssh-keys --admin-username azureuser"
        )
        with mock.patch("azure.cli.command_modules.vm.custom._gen_guid", side_effect=self.create_guid):
...
class VMBootDiagnostics(ScenarioTest):
    ...
    def test_vm_boot_diagnostics(self, resource_group, storage_account):
        ...
        self.kwargs.update({"vm": "myvm", "vm2": "myvm2", "subnet": "subnet1", "vnet": "vnet1"})
        self.kwargs["storage_uri"] = "https://{}.blob.core.windows.net/".format(self.kwargs["sa"])

        self.cmd(
            "vm create -n {vm} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --subnet {subnet} --vnet-name {vnet} "
        ...
        self.cmd(
            ...
            checks=[self.exists("consoleScreenshotBlobUri"), self.exists("serialConsoleLogBlobUri")],
        )
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=self.check("diagnosticsProfile.bootDiagnostics.enabled", False))

        # try enable it at the create
        self.cmd(
            "vm create -g {rg} -n {vm2} --image Debian:debian-10:10:latest --admin-username user11 --admin-password testPassword0 "
...
class VMSSExtensionInstallTest(ScenarioTest):
    ...
    def test_vmss_extension(self):
        ...
        self.kwargs.update(
            {
                "vmss": "vmss1",
                "net-pub": "Microsoft.Azure.NetworkWatcher",
                "script-pub": "Microsoft.Azure.Extensions",
                "access-pub": "Microsoft.OSTCExtensions",
                "code-pub": "Microsoft.Azure.Security.WindowsCodeIntegrity",
                "net-ext": "NetworkWatcherAgentLinux",
                "script-ext": "customScript",
                "access-ext": "VMAccessForLinux",
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password --admin-username admin123 --admin-password testPassword0 --instance-count 1 --orchestration-mode Uniform --lb-sku Standard --no-wait"
        )
        ...
        self.cmd("vmss extension list --vmss-name {vmss} -g {rg}", checks=[self.check("length(@)", 1)])
        result = self.cmd(
        ...
        self.cmd(
            "vmss extension set -g {rg} --vmss-name {vmss} -n {script-ext} --publisher {script-pub} --version 2.0 "
        ...
        self.cmd(
            "vmss extension set -g {rg} --vmss-name {vmss} -n {access-ext} --publisher {access-pub} --version 1.5 "
        ...
        self.cmd(
            "vmss extension set -g {rg} --vmss-name {vmss} -n {code-ext} --publisher {code-pub} "
            '--protected-settings "{config_file}"'
        )

        self.cmd(
            "vmss extension show -g {rg} --vmss-name {vmss} --name {code-ext}",
            checks=[self.check("enableAutomaticUpgrade", True)],
        )
    ...
    def test_vmss_extension_debian(self):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --authentication-type password --admin-username admin123 --admin-password testPassword0 --instance-count 1 --no-wait --orchestration-mode Uniform --lb-sku Standard"
        )
        self.cmd("vmss wait --created -n {vmss} -g {rg}")
    ...
    def test_vmss_extension_instance_name(self):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password --admin-username admin123 --admin-password testPassword0 --instance-count 1 --orchestration-mode Flexible"
        )
        self.cmd(
...
class VMSSExtensionImageTest(ScenarioTest):
    ...
    def test_vmss_extension_image(self, resource_group):
        ...
        self.cmd("vmss extension image list -p {pub}", checks=[self.check("[0].publisher", self.kwargs["pub"])])

        result = self.cmd(
            "vmss extension image list-names -p {pub} -l {location}",
            checks=[self.check("[0].location", self.kwargs["location"])],
        ).get_output_in_json()
        self.assertTrue([n for n in result if n["name"] == self.kwargs["name"]])

        result = self.cmd(
            "vmss extension image list-versions -n {name} -p {pub} -l {location}",
            checks=[
        ...
        self.cmd(
            "vmss extension image show -n {name} -p {pub} -l {location} --version {ver}",
            checks=[
...
class DiagnosticsExtensionInstallTest(ScenarioTest):
    ...
    def test_diagnostics_extension_install(self, resource_group, storage_account):
        ...
        self.kwargs.update({"vm": "testdiagvm", "vmss": "testdiagvmss", "subnet": "subnet1", "vnet": "vnet1"})

        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password "
            "--lb-sku Standard --admin-username user11 --admin-password TestTest12#$ --orchestration-mode Uniform"
        )
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password "
        ...
        with open(protected_settings, "w") as outfile:
            json.dump({"storageAccountName": storage_account, "storageAccountSasToken": storage_sastoken}, outfile)
...
class VMCreateExistingOptions(ScenarioTest):
    ...
    def test_vm_create_existing_options(self, resource_group, storage_account):
        ...
        self.cmd(
            "vm create --image Canonical:UbuntuServer:18.04-LTS:latest --os-disk-name {disk} --os-disk-delete-option Delete "
        ...
        self.cmd(
            ...
            checks=[
                self.check(
                    "storageProfile.osDisk.vhd.uri", "https://{sa}.blob.core.windows.net/{container}/{disk}.vhd"
                ),
                self.check("storageProfile.osDisk.deleteOption", "Delete"),
            ],
        )
    ...
    def test_vm_create_provision_vm_agent(self, resource_group):
        self.kwargs.update(
            {"vm1": "vm1", "vm2": "vm2", "pswd": "qpwWfn1qwernv#xnklwezxcvslkdfj", "subnet": "subnet1", "vnet": "vnet1"}
        )

        self.cmd(
            "vm create -g {rg} -n {vm1} --image Canonical:UbuntuServer:18.04-LTS:latest --enable-agent --admin-username azureuser "
        ...
        self.cmd("vm show -g {rg} -n {vm1}", checks=[self.check("osProfile.linuxConfiguration.provisionVmAgent", True)])

        self.cmd(
            "vm create -g {rg} -n {vm2} --image Win2022Datacenter --admin-username azureuser --admin-password {pswd} "
            "--authentication-type password --enable-agent false --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        self.cmd(
            "vm show -g {rg} -n {vm2}", checks=[self.check("osProfile.windowsConfiguration.provisionVmAgent", False)]
        )
    ...
    def test_vm_create_auth(self, resource_group):
        ...
        self.cmd(
            "vm create --image Debian:debian-10:10:latest -l westus -g {rg} -n {vm_1} --authentication-type all "
        ...
        self.cmd(
            "vm create --image Debian:debian-10:10:latest -l westus -g {rg} -n {vm_2} --authentication-type ssh "
...
class VMCreateExistingIdsOptions(ScenarioTest):
    ...
    def test_vm_create_existing_ids_options(self, resource_group, storage_account):
        ...
        self.kwargs.update(
            {
                "availset_id": resource_id(
                    subscription=subscription_id,
                    resource_group=rg,
                    namespace="Microsoft.Compute",
                    type="availabilitySets",
                    name=self.kwargs["availset"],
                ),
                "pubip_id": resource_id(
                    subscription=subscription_id,
                    resource_group=rg,
                    namespace="Microsoft.Network",
                    type="publicIpAddresses",
                    name=self.kwargs["pubip"],
                ),
        ...
        self.cmd(
            "vm create --image Canonical:UbuntuServer:18.04-LTS:latest --os-disk-name {disk} --subnet {subnet_id} "
...
class VMCreateCustomIP(ScenarioTest):
    ...
    def test_vm_create_custom_ip(self, resource_group):
        ...
        self.cmd(
            "vm create -n {vm} -g {rg} --image OpenLogic:CentOS:7.5:latest --admin-username user11 --private-ip-address 10.0.0.5 "
...
class VMDiskAttachDetachTest(ScenarioTest):
    ...
    def test_vm_disk_attach_detach(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} --location {loc} -n {vm} --admin-username admin123 --image OpenLogic:CentOS:7.5:latest "
            "--admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        ...
        self.cmd("vm disk detach -g {rg} --vm-name {vm} -n {disk1}")
        self.cmd("vm show -g {rg} -n {vm}", checks=self.check("length(storageProfile.dataDisks)", 0))
    ...
    def test_vm_disk_force_detach(self, resource_group):
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=[self.check("length(storageProfile.dataDisks)", 2)])
    ...
    def test_vm_disk_attach_detach_api(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} --location {loc} -n {vm} --admin-username admin123 --image OpenLogic:CentOS:7.5:latest --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        self.cmd("network vnet subnet update -g {rg} --vnet-name {vnet} -n {subnet} --default-outbound-access false")
        ...
        self.kwargs.update({"disk1_id": disks[0]["managedDisk"]["id"], "disk2_id": disks[1]["managedDisk"]["id"]})
    ...
    def test_vm_disk_attach_multiple_disks(self, resource_group):
        ...
        with self.assertRaisesRegex(
            RequiredArgumentMissingError,
            "Please use at least one of --name, --disks, --disk-ids, --source-snapshots-or-disks and --source-disk-restore-point",
        ):
        ...
        with self.assertRaisesRegex(
            MutuallyExclusiveArgumentError, "You can only specify one of --name and --disk-ids"
        ):
            self.cmd(
                "vm disk attach -g {rg} --name {disk1} --disk-ids {disk1} {disk2} {disk3} --vm-name {vm} --new --size-gb 1 "
            )

        with self.assertRaisesRegex(
            MutuallyExclusiveArgumentError, "You can only specify one of --disks and --disk-ids"
        ):
            self.cmd(
                "vm disk attach -g {rg} --disk-ids {disk1} --disks {disk1} {disk2} {disk3} --vm-name {vm} --new --size-gb 1 "
            )
        ...
        with self.assertRaisesRegex(
            MutuallyExclusiveArgumentError, "You cannot specify the --lun for multiple disk IDs"
        ):
    ...
    def test_vm_disk_storage_sku(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg}  -n {vm} --size Standard_B2ms --admin-username admin123 --admin-password testPassword0 --image Debian:debian-10:10:latest "
    ...
    def test_vm_disk_storage_sku2(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg}  -n {vm} --admin-username admin123 --admin-password testPassword0 --image Debian:debian-10:10:latest "
        ...
        # create a snpashot
        self.cmd(
            "snapshot create -g {rg} -n {snapshot1} --size-gb 4 --sku Standard_LRS --tags tag1=s1",
            checks=[self.check("sku.name", "Standard_LRS"), self.check("diskSizeGb", 4), self.check("tags.tag1", "s1")],
        )

        # test that images can be created with different storage skus and os disk caching settings.
        self.cmd(
            "image create -g {rg} -n {image1} --source {snapshot1} --storage-sku PremiumV2_LRS --os-type linux",
            checks=[
    ...
    def test_vmss_disk_storage_sku(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --admin-username admin123 --admin-password testPassword0 --image Debian:debian-10:10:latest "
        ...
        self.cmd(
            ...
            checks=[
                self.check(
                    "virtualMachineProfile.storageProfile.dataDisks[0].managedDisk.storageAccountType", "PremiumV2_LRS"
                ),
            ],
        )
        self.cmd("disk create -g {rg} -n {disk1} --size-gb 4 --sku PremiumV2_LRS --zone 1")
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.storageProfile.dataDisks[1].managedDisk.storageAccountType", "PremiumV2_LRS"
                ),
            ],
        )
    ...
    def test_vm_ultra_ssd_storage_sku(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --admin-username admin123 --size Standard_D2s_v3 --admin-password testPassword0 --image Debian:debian-10:10:latest "
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --admin-username admin123 --admin-password testPassword0 --image Debian:debian-10:10:latest "
    ...
    def test_vm_ultra_ssd_disk_update(self, resource_group):
        self.kwargs.update({"disk1": "d1"})
        self.cmd(
            "disk create -g {rg} -n {disk1} --size-gb 4 --sku UltraSSD_LRS --disk-iops-read-write 500 --disk-mbps-read-write 8 --zone 2"
        )
        self.cmd(
            "disk update -g {rg} -n {disk1} --disk-iops-read-write 510 --disk-mbps-read-write 10",
            checks=[self.check("diskIOPSReadWrite", 510), self.check("diskMBpsReadWrite", 10)],
        )
    ...
    def test_vm_disk_create_with_standard_zrs_sku(self, resource_group):
        self.kwargs.update({"disk1": "d1", "snapshot1": "s1"})
    ...
    def test_vmss_ultra_ssd_storage_sku(self, resource_group):
        ...
        self.kwargs.update({"vmss": "vm-ultrassd", "vmss2": "vm-ultrassd2"})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --admin-username admin123 --admin-password testPassword0 --image Debian:debian-10:10:latest --storage-sku UltraSSD_LRS "
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.storageProfile.dataDisks[0].managedDisk.storageAccountType", "UltraSSD_LRS"
                ),
            ],
        )
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --admin-username admin123 --admin-password testPassword0 --image Debian:debian-10:10:latest --ultra-ssd-enabled --zones 2 "
    ...
    def test_vm_vmss_update_ultra_ssd_enabled(self, resource_group):
        self.kwargs.update({"vm": "vm1", "vmss": "vmss1"})
        ...
        self.cmd(
            ...
            checks=[self.check("additionalCapabilities.ultraSsdEnabled", True)],
        )
    ...
    def test_vm_hibernation_enabled(self, resource_group):
        self.kwargs.update({"vm": self.create_random_name("vm-", 10), "subnet": "subnet1", "vnet": "vnet1"})

        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --enable-hibernation True --admin-username vmtest "
            "--subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=[self.check("additionalCapabilities.hibernationEnabled", True)])
        self.cmd("vm deallocate -g {rg} -n {vm}")
        self.cmd(
            "vm update -g {rg} -n {vm} --enable-hibernation False",
            checks=[self.check("additionalCapabilities.hibernationEnabled", False)],
        )
    ...
    def test_vm_disk_supported_security_option(self, resource_group):
        self.kwargs.update({"disk1": self.create_random_name("disk", 10), "disk2": self.create_random_name("disk", 10)})

        self.cmd(
            "disk create -g {rg} -n {disk1} --sku PremiumV2_LRS --size-gb 10 --supported-security-option TrustedLaunchSupported",
            checks=[self.check("supportedCapabilities.supportedSecurityOption", "TrustedLaunchSupported")],
        )
        self.cmd(
            "disk update -g {rg} -n {disk1} --supported-security-option TrustedLaunchAndConfidentialVMSupported",
            checks=[
        ...
        self.cmd(
            "disk update -g {rg} -n {disk2} --supported-security-option TrustedLaunchAndConfidentialVMSupported",
            checks=[
                self.check("supportedCapabilities.supportedSecurityOption", "TrustedLaunchAndConfidentialVMSupported")
            ],
        )
    ...
    def test_vm_disk_availability_policy(self, resource_group):
        self.kwargs.update({"disk1": self.create_random_name("disk", 10)})

        self.cmd(
            "disk create -g {rg} -n {disk1} --sku PremiumV2_LRS --size-gb 10 --action-on-disk-delay AutomaticReattach",
            checks=[self.check("availabilityPolicy.actionOnDiskDelay", "AutomaticReattach")],
        )
        self.cmd("disk create -g {rg} -n {disk1} --sku PremiumV2_LRS --size-gb 10")
        self.cmd(
            "disk update -g {rg} -n {disk1} --action-on-disk-delay AutomaticReattach",
            checks=[self.check("availabilityPolicy.actionOnDiskDelay", "AutomaticReattach")],
        )
...
class VMUnmanagedDataDiskTest(ScenarioTest):
    ...
    def test_vm_data_unmanaged_disk(self, resource_group):
        ...
        self.kwargs.update(
            {"loc": "westus", "vm": "vm-datadisk-test", "disk": "d7", "subnet": "subnet1", "vnet": "vnet1"}
        )

        self.cmd(
            "vm create -g {rg} --location {loc} -n {vm} --admin-username ubuntu --image OpenLogic:CentOS:7.5:latest "
        ...
        result = self.cmd(
            "vm show -g {rg} -n {vm}", checks=self.check("length(storageProfile.dataDisks)", 0)
        ).get_output_in_json()
    ...
    def test_vm_data_unmanaged_disk_debian(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} --location {loc} -n {vm} --admin-username debian --image Debian:debian-10:10:latest --admin-password testPassword0 "
...
class VMCreateCustomDataScenarioTest(ScenarioTest):
    ...
    def test_vm_create_custom_data(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --admin-username {username} --authentication-type {auth} --image {image} "
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=self.check("provisioningState", "Succeeded"))
...
class VMSSCreateAndModify(ScenarioTest):
    ...
    def test_vmss_create_and_modify(self):
        ...
        self.kwargs.update({"vmss": "vmss1", "count": 5, "new_count": 4, "vmss2": self.create_random_name("vmss", 10)})

        self.cmd(
            "vmss create --admin-password testPassword0 --name {vmss} -g {rg} --admin-username myadmin --image Win2022Datacenter "
        ...
        self.cmd("vmss list", checks=self.check("type(@)", "array"))
        ...
        self.cmd("vmss list-skus --resource-group {rg} --name {vmss}", checks=self.check("type(@)", "array"))
        self.cmd(
            "vmss show --resource-group {rg} --name {vmss}",
            checks=[self.check("type(@)", "object"), self.check("name", "{vmss}"), self.check("resourceGroup", "{rg}")],
        )
    ...
    def test_vmss_hibernate(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image MicrosoftWindowsServer:WindowsServer:2022-datacenter-smalldisk-g2:latest --enable-hibernation true --admin-username vmtest --admin-password Test123456789# --orchestration-mode Flexible",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image MicrosoftWindowsServer:WindowsServer:2022-datacenter-smalldisk-g2:latest --enable-hibernation true --admin-username vmtest --admin-password Test123456789# --orchestration-mode Flexible",
            checks=[
                self.check("vmss.additionalCapabilities.hibernationEnabled", True),
            ],
        )
    ...
    def test_vmss_scale_in_policy(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image OpenLogic:CentOS:7.5:latest --scale-in-policy NewestVM "
            "--admin-username azureuser --orchestration-mode Uniform --lb-sku Standard",
            checks=[self.check("vmss.scaleInPolicy.rules[0]", "NewestVM")],
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss} --scale-in-policy OldestVM",
            checks=[self.check("scaleInPolicy.rules[0]", "OldestVM")],
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss} --force-deletion", checks=[self.check("scaleInPolicy.forceDeletion", True)]
        )
    ...
    def test_vmss_scale_extended_location(self, resource_group):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image OpenLogic:CentOS:7.7:latest --admin-username clittester --lb-sku standard --public-ip-per-vm --dns-servers 10.0.0.6 10.0.0.5 --nsg {nsg} --admin-username vmsstest --admin-password Test123456789# --orchestration-mode Uniform -l westus --edge-zone losangeles"
        )
        self.cmd(
            "vmss show -n {vmss} -g {rg}",
            checks=[self.check("extendedLocation.name", "losangeles"), self.check("sku.capacity", 2)],
        )
        self.cmd(
            "vmss scale -n {vmss} -g {rg} --new-capacity 5",
            checks=[
...
class VMSSCreateOptions(ScenarioTest):
    ...
    def test_vmss_create_options(self, resource_group):
        ...
        self.kwargs.update(
            {"vmss": "vrfvmss", "count": 2, "caching": "ReadWrite", "update": "automatic", "ip": "vrfpubip"}
        )

        self.cmd("network public-ip create --name {ip} -g {rg}")

        self.cmd(
            "vmss create --image Debian:debian-10:10:latest --admin-password testPassword0 -l westus -g {rg} -n {vmss} --disable-overprovision --instance-count {count} "
        ...
        self.cmd("vmss show -g {rg} -n {vmss} --instance-id {id}", checks=self.check("instanceId", "{id}"))
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.storageProfile.dataDisks[0].managedDisk.storageAccountType", "Standard_LRS"
                ),
                self.check("virtualMachineProfile.storageProfile.dataDisks[1].diskSizeGb", 3),
                self.check(
                    "virtualMachineProfile.storageProfile.dataDisks[1].managedDisk.storageAccountType", "Standard_LRS"
                ),
            ],
        )
    ...
    def test_vmss_create_with_policy_setting(self, resource_group):
        ...
        self.kwargs.update({"vmss": "vrfvmss", "maxBIP": maxBIP, "maxUIP": maxUIP, "maxUUIP": maxUUIP, "PTB": PTB})

        self.cmd(
            "vmss create --image Debian:debian-10:10:latest --admin-password testPassword0 -g {rg} -n {vmss} --instance-count 1 --generate-ssh-keys --lb-sku Standard "
    ...
    def test_vmss_with_resiliency_policy(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image {image} --vm-sku {sku} --admin-username vmtest --admin-password Test123456789# "
    ...
    def test_vmss_with_automatic_zone_rebalancing_policy(self, resource_group):
        ...
        self.cmd(
            "network lb rule create -g {rg} --lb-name {lb1} -n {rule1} --probe-name {probe1} --protocol tcp --frontend-port 80 --backend-port 80"
        )
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image {image} --vm-sku Standard_D1_v2 --load-balancer {lb1} --health-probe {probe1} --zone-balance true --orchestration-mode Uniform --admin-username vmtest --admin-password Test123456789# --nsg {nsg} --zones 1 2 3 "
        ...
        self.cmd(
            "network lb rule create -g {rg} --lb-name {lb2} -n {rule2} --probe-name {probe2} --protocol tcp --frontend-port 80 --backend-port 80"
        )
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image {image} --vm-sku Standard_D1_v2 --load-balancer {lb2} --health-probe {probe2} --zone-balance true --orchestration-mode Uniform --admin-username vmtest --admin-password Test123456789# --nsg {nsg} --zones 1 2 3 --location eastus2"
        )

        self.cmd(
            "vmss update -g {rg} -n {vmss2} --enable-automatic-zone-balancing True --automatic-zone-balancing-strategy Recreate"
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss2}",
            checks=[
    ...
    def test_vmss_with_max_surge(self, resource_group):
        self.kwargs.update({"vmss": self.create_random_name("vmss", 10)})
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image OpenLogic:CentOS:7.5:latest --upgrade-policy-mode Manual "
            "--max-surge true --disable-overprovision --orchestration-mode Uniform --lb-sku Standard",
            checks=[
                self.check("vmss.upgradePolicy.rollingUpgradePolicy.maxSurge", True),
            ],
        )
        self.cmd(
            "vmss update -n {vmss} -g {rg} --max-surge false",
            checks=[
    ...
    def test_vmss_with_auto_os_upgrade(self, resource_group):
        ...
        self.cmd(
            "vmss create --debug -n {vmss} -g {rg} --image {image} --upgrade-policy-mode Automatic --enable-auto-os-upgrade --orchestration-mode Uniform "
    ...
    def test_vmss_create_ephemeral_os_disk(self, resource_group):
        ...
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss} --image {image} --ephemeral-os-disk --disable-overprovision --lb-sku Standard "
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.storageProfile.osDisk.managedDisk.storageAccountType", "Standard_LRS"
                ),
                self.check("length(virtualMachineProfile.storageProfile.dataDisks)", 1),
                self.check(
                    "virtualMachineProfile.storageProfile.dataDisks[0].managedDisk.storageAccountType", "Premium_LRS"
                ),
            ],
        )
        ...
        # explicitly specify os-disk-caching
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss_2} --image {image} --ephemeral-os-disk --lb-sku Standard "
    ...
    def test_vmss_update_ephemeral_os_disk(self, resource_group):
        self.kwargs.update({"vmss1": self.create_random_name("vmss", 10), "vmss2": self.create_random_name("vmss", 10)})
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss1} --image OpenLogic:CentOS:7.5:latest --ephemeral-os-disk --lb-sku Standard "
            "--disable-overprovision --instance-count 2 --data-disk-sizes-gb 2 --storage-sku os=standard_lrs 0=premium_lrs --admin-username testuser1 --admin-password testPassword01! --orchestration-mode Uniform",
            checks=[
        ...
        self.cmd(
            "vmss update --resource-group {rg} --name {vmss1} --ephemeral-os-disk false",
            checks=[self.check("virtualMachineProfile.storageProfile.osDisk.diffDiskSettings", None)],
        )
        self.cmd(
            "vmss update --resource-group {rg} --name {vmss1} --ephemeral-os-disk true --ephemeral-option local --ephemeral-placement CacheDisk ",
            checks=[
                self.check("virtualMachineProfile.storageProfile.osDisk.diffDiskSettings.option", "Local"),
        ...
        self.cmd(
            "vmss update --resource-group {rg} --name {vmss1} --ephemeral-os-disk true --ephemeral-option local --ephemeral-placement ResourceDisk --vm-sku Standard_DS4_v2",
            checks=[
                self.check("virtualMachineProfile.storageProfile.osDisk.diffDiskSettings.option", "Local"),
        ...
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss2} --image OpenLogic:CentOS:7.5:latest --ephemeral-os-disk --lb-sku Standard "
            "--disable-overprovision --instance-count 2 --data-disk-sizes-gb 2 --storage-sku os=standard_lrs 0=premium_lrs --admin-username testuser1 --admin-password testPassword01! --orchestration-mode Uniform",
            checks=[
                self.check("vmss.virtualMachineProfile.storageProfile.osDisk.diffDiskSettings.option", "Local"),
                self.check("vmss.virtualMachineProfile.storageProfile.osDisk.diffDiskSettings.placement", "CacheDisk"),
        ...
        self.cmd(
            "vmss update --resource-group {rg} --name {vmss2} --ephemeral-os-disk false",
            checks=[self.check("virtualMachineProfile.storageProfile.osDisk.diffDiskSettings", None)],
        )
        self.cmd(
            "vmss update --resource-group {rg} --name {vmss2} --ephemeral-os-disk true --ephemeral-option local --ephemeral-placement ResourceDisk --vm-sku Standard_DS4_v2",
            checks=[
                self.check("virtualMachineProfile.storageProfile.osDisk.diffDiskSettings.option", "Local"),
                self.check("virtualMachineProfile.storageProfile.osDisk.diffDiskSettings.placement", "ResourceDisk"),
            ],
    ...
    def test_vmss_update_instance_disks(self, resource_group):
        ...
        self.cmd(
            "vmss create --image Debian:debian-10:10:latest --vm-sku Standard_B2ms --admin-username clitest1 --admin-password testPassword0 "
        ...
        self.cmd(
            "vmss list-instances -g {rg} -n {vmss} --query \"[?instanceId=='{instance_id}']\"",
            checks=[self.check("length([0].storageProfile.dataDisks)", 0)],
        )
    ...
    def test_vmss_create_auth(self, resource_group):
        ...
        self.cmd(
            "vmss create --image Debian:debian-10:10:latest -l westus -g {rg} -n {vmss_1} --authentication-type all "
            " --admin-username myadmin --admin-password testPassword0 --ssh-key-value '{ssh_key}' --orchestration-mode Flexible",
            checks=[
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.disablePasswordAuthentication", False
                ),
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.ssh.publicKeys[0].keyData",
                    TEST_SSH_KEY_PUB,
                ),
            ],
        )
        ...
        self.cmd(
            "vmss create --image Debian:debian-10:10:latest -l westus -g {rg} -n {vmss_2} --authentication-type ssh "
            ' --admin-username myadmin --ssh-key-value "{ssh_key}" "{ssh_key_2}" --ssh-dest-key-path "{ssh_dest}" --orchestration-mode Flexible',
            checks=[
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.disablePasswordAuthentication", True
                ),
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.ssh.publicKeys[0].keyData",
                    TEST_SSH_KEY_PUB,
                ...
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.ssh.publicKeys[1].keyData",
                    TEST_SSH_KEY_PUB_2,
                ),
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.ssh.publicKeys[0].path", "{ssh_dest}"
                ),
                self.check(
                    "vmss.virtualMachineProfile.osProfile.linuxConfiguration.ssh.publicKeys[1].path", "{ssh_dest}"
                ),
            ],
        )
...
class VMSSCreateBalancerOptionsTest(ScenarioTest):  # pylint: disable=too-many-instance-attributes
    ...
    def test_vmss_create_none_options(self, resource_group):
        self.kwargs.update(
            {"vmss": "vmss1", "ssh_key": TEST_SSH_KEY_PUB, "quotes": '""' if platform.system() == "Windows" else "''"}
        )
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --load-balancer {quotes} --admin-username ubuntu "
        ...
        self.cmd("vmss update -g {rg} -n {vmss} --set tags.test=success", checks=self.check("tags.test", "success"))
    ...
    def test_vmss_create_with_app_gateway(self, resource_group):
        self.kwargs.update({"vmss": "vmss1", "ssh_key": TEST_SSH_KEY_PUB})
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --admin-username clittester "
    ...
    def test_vmss_create_default_app_gateway(self, resource_group):
        ...
        res = self.cmd(
            "vmss create -g {rg} --name {vmss} --validate --image Canonical:UbuntuServer:18.04-LTS:latest --disable-overprovision --instance-count 101 --single-placement-group false "
            '--admin-username ubuntuadmin --generate-ssh-keys --lb "" --orchestration-mode Uniform --lb-sku Standard'
        ).get_output_in_json()
    ...
    def test_vmss_existing_lb(self, resource_group):
        self.kwargs.update({"vmss": "vmss1", "lb": "lb1"})
        self.cmd("network lb create -g {rg} -n {lb} --backend-pool-name test")
        self.cmd(
            "vmss create -g {rg} -n {vmss} --load-balancer {lb} --image Canonical:UbuntuServer:18.04-LTS:latest "
    ...
    def test_vmss_single_placement_group_default_to_std_lb(self, resource_group):
        self.kwargs.update({"vmss": "vmss123"})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest --single-placement-group false"
        )
        self.cmd("vmss show -g {rg} -n {vmss}", checks=[self.check("singlePlacementGroup", False)])

        self.cmd("network lb list -g {rg}", checks=[self.check("[0].sku.name", "Standard")])
        self.cmd("network public-ip list -g {rg}", checks=[self.check("[0].sku.name", "Standard")])
    ...
    def test_vmss_create_with_nat_rule_v2(self, resource_group):
        ...
        self.cmd(
            "vmss create -n {vmss1} -g {rg} --image OpenLogic:CentOS:7.5:latest --lb-sku Standard --orchestration-mode Uniform"
        )
        self.cmd(
            "vmss show -n {vmss1} -g {rg}",
            checks=[
                self.check(
        ...
        self.cmd("network lb show -n {vmss1}LB -g {rg}", checks=[self.check("sku.name", "Standard")])
        self.cmd("vmss list-instance-connection-info -n {vmss1} -g {rg}")

        self.cmd(
            "vmss create -n {vmss2} -g {rg} --image OpenLogic:CentOS:7.5:latest --lb-sku Basic --orchestration-mode Uniform"
        )
        self.cmd(
            "vmss show -n {vmss2} -g {rg}",
            checks=[
                self.exists(
        ...
        self.cmd("network lb show -n {vmss2}LB -g {rg}", checks=[self.check("sku.name", "Basic")])
        self.cmd("vmss list-instance-connection-info -n {vmss2} -g {rg}")

        self.cmd(
            "vmss create -n {vmss3} -g {rg} --image OpenLogic:CentOS:7.5:latest --nat-rule-name {natrule} --lb-sku Standard --orchestration-mode Uniform"
        )
        self.cmd(
            "vmss show -n {vmss3} -g {rg}",
            checks=[
                self.check(
                    "virtualMachineProfile.networkProfile.networkInterfaceConfigurations[0].ipConfigurations[0].loadBalancerInboundNatPools",
                    None,
        ...
        self.cmd("network lb show -n {vmss3}LB -g {rg}", checks=[self.check("sku.name", "Standard")])
        self.cmd("vmss list-instance-connection-info -n {vmss3} -g {rg}")

        self.cmd(
            "vmss create -n {vmss4} -g {rg} --image OpenLogic:CentOS:7.5:latest --nat-rule-name {natrule1} --lb-sku Basic --orchestration-mode Uniform"
        )
        self.cmd(
            "vmss show -n {vmss4} -g {rg}",
            checks=[
                self.check(
                    "virtualMachineProfile.networkProfile.networkInterfaceConfigurations[0].ipConfigurations[0].loadBalancerInboundNatPools",
                    None,
        ...
        self.cmd("network lb show -n {vmss4}LB -g {rg}", checks=[self.check("sku.name", "Basic")])
        ...
        self.cmd(
            "vmss create -n {vmss6} -g {rg} --image OpenLogic:CentOS:7.5:latest --nat-pool-name {natpool} --lb-sku Standard --orchestration-mode Uniform"
        )
        self.cmd(
            "vmss show -n {vmss6} -g {rg}",
            checks=[
                self.exists(
                    "virtualMachineProfile.networkProfile.networkInterfaceConfigurations[0].ipConfigurations[0].loadBalancerInboundNatPools[0].id"
                )
        ...
        self.cmd("network lb show -n {vmss6}LB -g {rg}", checks=[self.check("sku.name", "Standard")])
    ...
    def test_vmss_zone_balance(self, resource_group):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            'vmss create -n {vmss1} -g {rg} --image Debian:debian-10:10:latest --zone-balance true --admin-username clittester --lb-sku standard --public-ip-per-vm --dns-servers 10.0.0.6 10.0.0.5 --nsg {nsg} --admin-username vmsstest --admin-password Test123456789# --orchestration-mode Uniform  --ssh-key-value "{ssh_key}"  --zones 1 2 -l eastus2',
            checks=[
                self.check("vmss.zoneBalance", True),
            ],
        )
        self.cmd("vmss update -n {vmss1} -g {rg} --zone-balance false", checks=[self.check("zoneBalance", False)])

        self.cmd(
            'vmss create -n {vmss2} -g {rg} --image Debian:debian-10:10:latest --admin-username clittester --lb-sku standard --public-ip-per-vm --dns-servers 10.0.0.6 10.0.0.5 --nsg {nsg} --admin-username vmsstest --admin-password Test123456789# --orchestration-mode Uniform  --ssh-key-value "{ssh_key}"  --zones 1 2 -l eastus2'
        )
        self.cmd("vmss update -n {vmss2} -g {rg} --zone-balance True", checks=[self.check("zoneBalance", True)])
...
class VMSSCreatePublicIpPerVm(ScenarioTest):  # pylint: disable=too-many-instance-attributes
    ...
    def test_vmss_public_ip_per_vm_custom_domain_name(self, resource_group):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --admin-username clittester --ssh-key-value '{ssh_key}' --lb-sku Standard "
        ...
        result = self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.networkProfile.networkInterfaceConfigurations[0].networkSecurityGroup.id",
        ...
        self.cmd(
            "vmss create -g {rg} -n {flex_vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username vmtest --orchestration-mode Flexible"
        )
...
class VMSSUpdateTests(ScenarioTest):
    ...
    def test_vmss_update(self, resource_group):
        self.kwargs.update({"vmss": "winvmss", "vmss2": self.create_random_name("vmss", 10)})

        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --admin-username clitest1234 --admin-password Test123456789# "
        ...
        self.cmd(
            ...
            checks=[self.check("virtualMachineProfile.licenseType", "None")],
        )

        # test generic update on the vmss
        self.cmd(
            "vmss update -g {rg} -n {vmss} --set virtualMachineProfile.licenseType=Windows_Server",
            checks=[self.check("virtualMachineProfile.licenseType", "Windows_Server")],
        )
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username vmtest --orchestration-mode Flexible"
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss2}",
            checks=[
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss2} --enable-hibernation True",
            checks=[
    ...
    def test_vmss_update_policy(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --admin-username clitest1234 --admin-password Test123456789# --license-type Windows_Server --instance-count 1 --generate-ssh-keys --orchestration-mode Uniform --lb-sku Standard"
        )
    ...
    def test_vmss_set_rolling_upgrade_policy_during_creation(self):
        self.kwargs.update(
            {"linux_vmss": self.create_random_name("vmss", 10), "windows_vmss": self.create_random_name("vmss", 10)}
        )
        self.cmd(
            "vmss create -g {rg} -n {linux_vmss} --image ubuntu2204 --upgrade-policy-mode rolling --platform-fault-domain-count 1 --instance-count 2 --orchestration-mode Uniform --admin-username clitester1 --admin-password Testqwer1234! --lb-sku Standard",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {windows_vmss} --image Win2022Datacenter --upgrade-policy-mode rolling --platform-fault-domain-count 1 --instance-count 2 --orchestration-mode Uniform --admin-username clitester1 --admin-password Testqwer1234! --lb-sku Standard",
            checks=[
                self.check("vmss.upgradePolicy.mode", "Rolling"),
                self.check(
                    "vmss.virtualMachineProfile.extensionProfile.extensions[0].name", "ApplicationHealthWindows"
                ),
            ],
        )

    @ResourceGroupPreparer(
        name_prefix="cli_test_flexible_vmss_set_automatic_upgrade_policy_during_creation", location="eastus"
    )
    def test_flexible_vmss_set_automatic_upgrade_policy_during_creation(self):
        self.kwargs.update({"vmss": self.create_random_name("vmss", 10)})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image ubuntu2204 --orchestration-mode Flexible --upgrade-policy-mode Automatic --admin-username azureuser --generate-ssh-keys",
            checks=[
                self.check("vmss.upgradePolicy.mode", "Automatic"),
    ...
    def test_vmss_update_enable_auto_os_upgrade(self):
        self.kwargs.update(
            {"uniform_vmss": self.create_random_name("vmss", 10), "flexible_vmss": self.create_random_name("vmss", 10)}
        )
        self.cmd(
            "vmss create -g {rg} -n {uniform_vmss} --image Canonical:0001-com-ubuntu-server-jammy:22_04-LTS-Gen2:latest --lb-sku Standard "
            "--orchestration-mode uniform --upgrade-policy-mode rolling --platform-fault-domain-count 1 --instance-count 2 --admin-password Test123456789#",
            checks=[self.check("vmss.upgradePolicy.mode", "Rolling"), self.check("vmss.orchestrationMode", "Uniform")],
        )
        time.sleep(150)
        self.cmd(
            "vmss update -g {rg} -n {uniform_vmss} --enable-auto-os-upgrade true",
            checks=[
        ...
        self.cmd(
            "vmss update -g {rg} -n {uniform_vmss} --upgrade-policy-mode manual",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {flexible_vmss} --image ubuntu2204 --upgrade-policy-mode rolling --platform-fault-domain-count 1 --instance-count 2 --orchestration-mode Flexible --admin-username clitester1 --admin-password Testqwer1234!",
            checks=[self.check("vmss.upgradePolicy.mode", "Rolling"), self.check("vmss.orchestrationMode", "Flexible")],
        )
        time.sleep(150)
        self.cmd(
            "vmss update -g {rg} -n {flexible_vmss} --enable-auto-os-upgrade true --upgrade-policy-mode manual",
            checks=[
                self.check("upgradePolicy.mode", "Manual"),
    ...
    def test_vmss_update_image(self):
        self.kwargs.update({"vm": "vm1", "img": "img1", "vmss": "vmss1", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --admin-username clitest1 --generate-ssh-key "
        ...
        self.kwargs.update({"image_id": res["id"]})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image {image_id} --admin-username vmtest --orchestration-mode Flexible"
        )
        self.cmd("vmss update -g {rg} -n {vmss} --set tags.foo=bar", checks=[self.check("tags.foo", "bar")])
        from azure.core.exceptions import HttpResponseError

        with self.assertRaisesRegex(
            HttpResponseError, "Managed Images are not supported for virtual machines with security type TrustedLaunch."
        ):
    ...
    def test_vmss_update_security_type(self):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -n {vmss2} -g {rg} --image {img2} --admin-username vmtest --admin-password Test123456789# --orchestration-mode Uniform --lb-sku Standard --nsg {nsg}"
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss2} --set virtualMachineProfile.storageProfile.imageReference.sku={img2_sku_gen2} --security-type TrustedLaunch --enable-secure-boot true --enable-vtpm true",
            checks=[
        ...
        self.cmd(
            "vmss create -n {vmss3} -g {rg} --image {img3} --admin-username vmtest --generate-ssh-keys --orchestration-mode Uniform --lb-sku Standard --nsg {nsg}",
            checks=[
                self.check("vmss.virtualMachineProfile.securityProfile.securityType", "TrustedLaunch"),
            ],
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss3} --security-type TrustedLaunch",
            checks=[
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss3} --security-type TrustedLaunch --enable-secure-boot true --enable-vtpm false",
            checks=[
                self.check("virtualMachineProfile.securityProfile.securityType", "TrustedLaunch"),
                self.check("virtualMachineProfile.securityProfile.uefiSettings.secureBootEnabled", True),
        ...
        self.cmd(
            "vmss create -n {vmss4} -g {rg} --image {img4} --admin-username vmtest --admin-password Test123456789# --vm-sku Standard_DC2as_v5 --nsg {nsg} "
            "--security-type ConfidentialVM --enable-vtpm true --enable-secure-boot true --os-disk-security-encryption-type VMGuestStateOnly --orchestration-mode Uniform --lb-sku Standard",
            checks=[
    ...
    def test_vmss_update_vm_sku(self, resource_group, resource_group_location):
        ...
        # check base
        self.cmd(
            "vmss create -n {base} -g {rg} --image {image} --vm-sku Standard_DS4_v2 --admin-username vmtest --orchestration-mode Flexible"
        )
        self.cmd(
            "vmss show -g {rg} -n {base}",
            checks=[
        ...
        # check create with default size
        self.cmd(
            "vmss create -n {base2} -g {rg} --image {image}  --location {loc} --admin-username vmtest --orchestration-mode Flexible"
        )
        self.cmd(
...
class AcceleratedNetworkingTest(ScenarioTest):
    ...
    def test_vmss_accelerated_networking(self, resource_group):
        ...
        self.kwargs.update({"vmss": "vmss1"})
        self.cmd(
            "vmss create -n {vmss} -g {rg} --vm-sku Standard_DS4_v2 --image Win2022Datacenter --admin-username clittester "
    ...
    def test_vm_accelerated_networking(self, resource_group):
        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "vnet": "vnet1"})
        ...
        self.cmd(
            "vm create -n {vm} -g {rg} --size Standard_DS4_v2 --image Canonical:UbuntuServer:18.04-LTS:latest "
...
class ApplicationSecurityGroup(ScenarioTest):
    ...
    def test_vmss_asg(self, resource_group):
        self.kwargs.update({"vmss": "vmss1", "vm": "vm1", "asg": "asg1"})
        asg_id = self.cmd("network asg create -g {rg} -n {asg}").get_output_in_json()["id"]
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Debian:debian-10:10:latest --instance-count 1 --asgs {asg} --admin-username clittester --generate-ssh-keys --orchestration-mode Flexible"
        ).get_output_in_json()
        self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --asgs {asg} --admin-username clittester --generate-ssh-keys --public-ip-sku Standard --nsg-rule NONE"
        ).get_output_in_json()
        ...
        self.kwargs.update(
            {
                ...
                "vm": "vm-name",
                "secrets": json.dumps(
                    [{"sourceVault": {"id": "id"}, "vaultCertificates": [{"certificateUrl": "certurl"}]}]
                ),
            }
        )

        message = (
            "Secret is missing certificateStore within vaultCertificates array at secret index 0 and "
...
class VMSSCreateLinuxSecretsScenarioTest(ScenarioTest):
    ...
    def test_vmss_create_linux_secrets(self, resource_group, key_vault):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --admin-username deploy --ssh-key-value '{ssh_key}' --secrets '{secrets}'"
        )
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.osProfile.secrets[0].vaultCertificates[0].certificateUrl", "{secret_out}"
                ),
            ],
        )
...
class VMSSCreateExistingOptions(ScenarioTest):
    ...
    def test_vmss_create_existing_options(self):
        ...
        self.cmd(
            "vmss create --image OpenLogic:CentOS:7.5:latest --os-disk-name {os_disk} --admin-username ubuntu --vnet-name {vnet} --lb-sku Standard "
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.storageProfile.osDisk.vhdContainers[0].ends_with(@, '{container}')", True
                ),
            ],
        )
        self.cmd(
        ...
        subnet_ip_id = self.cmd("network vnet show --name {vnet} -g {rg}").get_output_in_json()["subnets"][0][
            "ipConfigurations"
        ][0]["id"]
    ...
    def test_vmss_create_with_delete_option(self, resource_group):
        self.kwargs.update({"vmss": self.create_random_name("vmss", 10), "vmss1": self.create_random_name("vmss", 10)})
        ...
        with self.assertRaisesRegex(InvalidArgumentValueError, message):
            self.cmd(
                "vmss create -g {rg} -n {vmss} --image ubutults --os-disk-delete-option delete --admin-username vmtest --orchestration-mode Uniform"
            )
        with self.assertRaisesRegex(InvalidArgumentValueError, message):
            self.cmd(
                "vmss create -g {rg} -n {vmss} --image ubutults --data-disk-delete-option delete --admin-username vmtest --orchestration-mode Uniform"
            )

        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --os-disk-delete-option delete",
            checks=[
                self.check("vmss.orchestrationMode", "Flexible"),
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Canonical:UbuntuServer:18.04-LTS:latest --data-disk-sizes-gb 4 --data-disk-delete-option detach",
            checks=[
                self.check("vmss.orchestrationMode", "Flexible"),
...
class VMSSCreateExistingIdsOptions(ScenarioTest):
    ...
    def test_vmss_create_existing_ids_options(self, resource_group):
        ...
        self.cmd(
            'vmss create --image OpenLogic:CentOS:7.5:latest --os-disk-name {os_disk} --admin-username ubuntu --subnet {subnet_id} -l "West US" --lb-sku Standard '
...
class VMSSVMsScenarioTest(ScenarioTest):
    ...
    def _check_vms_power_state(self, *args):
        for iid in self.kwargs["instance_ids"]:
            result = self.cmd(
                "vmss get-instance-view --resource-group {{rg}} --name {{vmss}} --instance-id {}".format(iid)
            ).get_output_in_json()
    ...
    def test_vmss_vms(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --authentication-type password --lb-sku Standard "
            "--admin-username admin123 --admin-password TestTest12#$ --instance-count {count} --orchestration-mode Uniform"
        )

        instance_list = self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("type(@)", "object"), self.check("instanceId", "{id}")],
        )
        ...
        self.cmd(
            "vmss create -g {rg} -n {flex_vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username vmtest --lb-sku Standard"
        )
        from azure.cli.core.azclierror import ArgumentUsageError
    ...
    def test_vmss_vms_debian(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Debian:debian-10:10:latest --authentication-type password --admin-username admin123 "
        ...
        self.cmd(
            "vmss create -g {rg} -n {flex_vmss} --image Debian:debian-10:10:latest --admin-username vmtest --lb-sku Standard"
        )
        from azure.cli.core.azclierror import ArgumentUsageError
...
class VMSSSimulateEvictionScenarioTest(ScenarioTest):
    ...
    def test_vmss_simulate_eviction(self, resource_group):
        ...
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss1} --location {loc} --instance-count 2 --image OpenLogic:CentOS:7.5:latest "
        ...
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss2} --location {loc} --instance-count 2 --image OpenLogic:CentOS:7.5:latest --lb-sku Standard "
        ...
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss3} --location {loc} --instance-count 2 --image OpenLogic:CentOS:7.5:latest --priority Spot "
    ...
    def test_spot_restore_policy(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {spot_vmss_name} --location NorthEurope --instance-count 2 --image OpenLogic:CentOS:7.5:latest --priority Spot --lb-sku Standard "
            "--eviction-policy Deallocate --single-placement-group True --enable-spot-restore {enabled_1} --spot-restore-timeout {restore_timeout1} --admin-username vmtest --orchestration-mode Uniform",
            checks=[
...
class VMSSCustomDataScenarioTest(ScenarioTest):
    ...
    def test_vmss_create_custom_data(self):
        ...
        self.kwargs.update({"vmss": "vmss-custom-data", "ssh_key": TEST_SSH_KEY_PUB})

        self.cmd(
            'vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --admin-username deploy --ssh-key-value "{ssh_key}" --custom-data "#cloud-config\nhostname: myVMhostname"'
        )

        # custom data is write only, hence we have no automatic way to cross check. Here we just verify VM was provisioned
        self.cmd("vmss show -n {vmss} -g {rg}", checks=self.check("provisioningState", "Succeeded"))

    @AllowLargeResponse()
    @ResourceGroupPreparer(name_prefix="cli_test_vmss_update_custom_data")
    def test_vmss_update_custom_data(self):
        self.kwargs.update({"vmss": self.create_random_name("vmss", 10), "ssh_key": TEST_SSH_KEY_PUB})

        self.cmd(
            'vmss create -n {vmss} -g {rg} --image "OpenLogic:CentOS:7.5:latest" --admin-username deploy --ssh-key-value "{ssh_key}"'
        )
    ...
    def test_vmss_create_user_data(self):
        ...
        self.cmd(
            'vmss create -n {vmss} -g {rg} --image Debian:debian-10:10:latest --admin-username deploy --ssh-key-value "{ssh_key}" '
        ...
        self.kwargs["instance_id"] = self.cmd(
            'vmss list-instances -g {rg} -n {vmss} --query "[].instanceId"'
        ).get_output_in_json()[0]
...
class VMSSNicScenarioTest(ScenarioTest):
    ...
    def test_vmss_nics(self):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --authentication-type password --admin-username admin123 --admin-password PasswordPassword1! "
        ...
        self.cmd(
            ...
            checks=[self.check("type(@)", "object"), self.check("name", "{nic}"), self.check("resourceGroup", "{rg}")],
        )
...
class VMSSCreateIdempotentTest(ScenarioTest):
    ...
    def test_vmss_create_idempotent(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --authentication-type password --admin-username admin123 --admin-password PasswordPassword1!  --image Canonical:UbuntuServer:18.04-LTS:latest --use-unmanaged-disk"
        )
        self.cmd(
            "vmss create -g {rg} -n {vmss} --authentication-type password --admin-username admin123 --admin-password PasswordPassword1!  --image Canonical:UbuntuServer:18.04-LTS:latest --use-unmanaged-disk"
        )
...
class VMSSILBTest(ScenarioTest):
    ...
    def test_vmss_with_ilb(self, resource_group):
        ...
        self.cmd(
            'vmss create -g {rg} -n {vmss} --admin-username admin123 --admin-password PasswordPassword1! --image OpenLogic:CentOS:7.5:latest --instance-count 1 --public-ip-address ""'
        )
...
class VMSSLoadBalancerWithSku(ScenarioTest):
    ...
    def test_vmss_lb_sku(self, resource_group):
        ...
        self.kwargs.update(
            {"vmss0": "vmss0", "vmss": "vmss1", "lb": "lb1", "ip": "pubip1", "sku": "standard", "loc": "eastus2"}
        )

        # default to Basic
        self.cmd(
            "vmss create -g {rg} -l {loc} -n {vmss0} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username admin123 --admin-password PasswordPassword1!"
        )
        self.cmd("network lb list -g {rg}", checks=self.check("[0].sku.name", "Basic"))
        self.cmd(
            "network public-ip list -g {rg}",
            checks=[self.check("[0].sku.name", "Basic"), self.check("[0].publicIPAllocationMethod", "Dynamic")],
        )

        # but you can overrides the defaults
        self.cmd(
            "vmss create -g {rg} -l {loc} -n {vmss} --lb {lb} --lb-sku {sku} --public-ip-address {ip} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username admin123 --admin-password PasswordPassword1!"
        )
        self.cmd("network lb show -g {rg} -n {lb}", checks=self.check("sku.name", "Standard"))
        self.cmd(
            "network public-ip show -g {rg} -n {ip}",
            checks=[self.check("sku.name", "Standard"), self.check("publicIPAllocationMethod", "Static")],
        )
...
class MSIScenarioTest(ScenarioTest):
    ...
    def test_vm_msi(self, resource_group):
        ...
        with self.assertRaisesRegex(ArgumentUsageError, "please specify both --role and --scope"):
            self.cmd(
                "vm create -g {rg} -n {vm1} --image Debian:debian-10:10:latest --assign-identity --admin-username admin123 "
        ...
        with mock.patch("azure.cli.core.commands.arm._gen_guid", side_effect=self.create_guid):
            ...
            self.cmd(
                "vm create -g {rg} -n {vm1} --image Debian:debian-10:10:latest --assign-identity --admin-username admin123 "
                "--admin-password PasswordPassword1! --scope {scope} --role Contributor --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE",
                checks=[
            ...
            result = self.cmd(
                "vm create -g {rg} -n {vm2} --image Win2022Datacenter --assign-identity --scope {vm1_id} --role reader "
                "--admin-username admin123 --admin-password PasswordPassword1! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE",
                checks=[
            ...
            vm3_result = self.cmd(
                "vm create -g {rg} -n {vm3} --image Debian:debian-10:10:latest --admin-username admin123 "
                "--admin-password PasswordPassword1! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
            ).get_output_in_json()
            self.assertIsNone(vm3_result.get("identity"))

            with self.assertRaisesRegex(
                ArgumentUsageError,
                "please specify both --role and --scope when assigning a role to the managed identity",
            ):
                self.cmd("vm identity assign -g {rg} -n {vm3} --scope {vm1_id}")
    ...
    def test_vmss_msi(self, resource_group):
        ...
        with self.assertRaisesRegex(ArgumentUsageError, "please specify both --role and --scope"):
            self.cmd(
                "vmss create -g {rg} -n {vmss1} --image Debian:debian-10:10:latest --instance-count 1 --assign-identity --admin-username admin123 --admin-password PasswordPassword1! --scope {scope} --orchestration-mode Uniform"
            )

        with self.assertRaisesRegex(ArgumentUsageError, "please specify both --role and --scope"):
            self.cmd(
                "vmss create -g {rg} -n {vmss1} --image Debian:debian-10:10:latest --instance-count 1 --assign-identity --admin-username admin123 --admin-password PasswordPassword1! --role Contributor --orchestration-mode Uniform"
            )

        with mock.patch("azure.cli.core.commands.arm._gen_guid", side_effect=self.create_guid):
            # create linux vm with default configuration
            self.cmd(
                "vmss create -g {rg} -n {vmss1} --image Debian:debian-10:10:latest --instance-count 1 --assign-identity --admin-username admin123 --admin-password PasswordPassword1! --scope {scope} --role Contributor --orchestration-mode Uniform --lb-sku Standard",
                checks=[
            ...
            # create a windows vm with reader role on the linux vm
            result = self.cmd(
                "vmss create -g {rg} -n {vmss2} --image Win2022Datacenter --instance-count 1 --assign-identity --scope {vmss1_id} --role reader --admin-username admin123 --admin-password PasswordPassword1! --orchestration-mode Uniform --lb-sku Standard",
                checks=[
            ...
            # create a linux vm w/o identity and later enable it
            result = self.cmd(
                "vmss create -g {rg} -n {vmss3} --image Debian:debian-10:10:latest --instance-count 1 --admin-username admin123 --admin-password PasswordPassword1! --orchestration-mode Uniform --lb-sku Standard"
            ).get_output_in_json()["vmss"]
            ...
            with self.assertRaisesRegex(
                ...
                self.cmd('vmss identity assign -g {rg} -n {vmss3} --scope "{vmss1_id}"')
    ...
    def test_vm_msi_no_scope(self, resource_group):
        ...
        self.kwargs.update(
            {"vm1": "vm1", "vmss1": "vmss1", "vm2": "vm2", "vmss2": "vmss2", "subnet": "subnet1", "vnet": "vnet1"}
        )
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Debian:debian-10:10:latest --assign-identity --admin-username admin123 --admin-password PasswordPassword1! --orchestration-mode Uniform --lb-sku Standard",
            checks=[
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --image Debian:debian-10:10:latest --admin-username admin123 --admin-password PasswordPassword1! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image Debian:debian-10:10:latest --admin-username admin123 --admin-password PasswordPassword1! --orchestration-mode Uniform --lb-sku Standard"
        )
        self.cmd(
    ...
    def test_vm_explicit_msi(self, resource_group):
        ...
        emsi_result = self.cmd(
            "identity create -g {rg} -n {emsi} --tags tag1=d1",
            checks=[self.check("name", "{emsi}"), self.check("tags.tag1", "d1")],
        ).get_output_in_json()
        ...
        result = self.cmd(
            ...
            "--generate-ssh-keys --admin-username {user} --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE",
            checks=[
                self.check("identity.role", None),
                self.check("identity.scope", None),
            ],
        ...
        result = self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --assign-identity {emsi} [system] --role reader --scope {scope} --generate-ssh-keys --admin-username {user} --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        ).get_output_in_json()
        emsis = [x.lower() for x in result["identity"]["userAssignedIdentities"].keys()]
        self.assertEqual(emsis, [emsi_result["id"].lower()])
        result = self.cmd(
            "vm identity show -g {rg} -n {vm}", checks=[self.check("type", "SystemAssigned, UserAssigned")]
        ).get_output_in_json()
    ...
    def test_vmss_explicit_msi(self, resource_group):
        ...
        result = self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --assign-identity {emsi} [system] "
            "--role reader --scope {scope} --instance-count 1 --generate-ssh-keys --admin-username ubuntuadmin --orchestration-mode Uniform --lb-sku Standard"
        ).get_output_in_json()
        emsis = [x.lower() for x in result["vmss"]["identity"]["userAssignedIdentities"].keys()]
        self.assertEqual(emsis, [emsi_result["id"].lower()])

        result = self.cmd(
            "vmss identity show -g {rg} -n {vmss}", checks=[self.check("type", "SystemAssigned, UserAssigned")]
        ).get_output_in_json()
        emsis = [x.lower() for x in result["userAssignedIdentities"].keys()]
        self.assertEqual(emsis, [emsi_result["id"].lower()])
...
class VMLiveScenarioTest(LiveScenarioTest):
    ...
    def test_vm_create_progress(self, resource_group):
        ...
        with force_progress_logging() as test_io:
            self.cmd(
                "vm create -g {rg} -n {vm} --admin-username {vm} --admin-password PasswordPassword1! --authentication-type password --image Debian:debian-10:10:latest --nsg-rule NONE"
            )
...
class VMZoneScenarioTest(ScenarioTest):
    ...
    def test_vm_create_zones(self, resource_group, resource_group_location):
        ...
        self.kwargs.update({"zones": "2", "vm": "vm123", "ip": "vm123ip", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest "
        ...
        self.cmd("network public-ip show -g {rg} -n {ip}", checks=self.check("zones[0]", "{zones}"))
    ...
    def test_vm_error_on_zone_unavailable(self, resource_group, resource_group_location):
        ...
        try:
            self.cmd(
                "vm create -g {rg} -n vm1 --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest --zone 1 --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
            )
    ...
    def test_vmss_create_single_zone(self, resource_group, resource_group_location):
        ...
        self.kwargs.update({"zones": "2", "vmss": "vmss123"})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest --zones {zones}"
        )
        self.cmd("vmss show -g {rg} -n {vmss}", checks=self.check("zones[0]", "{zones}"))
        ...
        self.cmd(
            "network public-ip list -g {rg}",
            checks=[self.check("[0].sku.name", "Standard"), self.check("[0].zones", ["2"])],
        )
    ...
    def test_vmss_create_x_zones(self, resource_group, resource_group_location):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss}  --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest --zones {zones} --vm-sku Standard_D1_V2"
        )
        self.cmd("vmss show -g {rg} -n {vmss}", checks=self.check("zones", ["1", "2", "3"]))
        result = self.cmd("vmss show -g {rg} -n {vmss} -otable")
        table_output = set(result.output.splitlines()[2].split())
        self.assertTrue(
        ...
        self.cmd(
            "network public-ip list -g {rg}",
            checks=[self.check("[0].sku.name", "Standard"), self.check("[0].zones", None)],
        )
    ...
    def test_vm_disk_create_zones(self, resource_group, resource_group_location):
        ...
        self.kwargs.update({"zones": "2", "disk": "disk123", "size": 1})
        self.cmd(
            "disk create -g {rg} -n {disk} --size-gb {size} --zone {zones}", checks=[self.check("zones[0]", "{zones}")]
        )
        self.cmd("disk show -g {rg} -n {disk}", checks=self.check("zones[0]", "{zones}"))
        ...
        self.assertTrue(
            set(
                [
                    ...
                    self.kwargs["disk"],
                    self.kwargs["zones"],
                    str(self.kwargs["size"]),
                    "Premium_LRS",
                ]
            ).issubset(table_output)
        )
    ...
    def test_vmss_create_zonal_with_fd(self, resource_group, resource_group_location):
        ...
        self.kwargs.update({"zones": "2", "vmss": "vmss123"})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest --zones {zones} --platform-fault-domain-count 1"
        )
        self.cmd("vmss show -g {rg} -n {vmss}", checks=[self.check("singlePlacementGroup", False)])
    ...
    def test_vmss_update_zones(self, resource_group):
        ...
        self.cmd("vmss show -g {rg} -n {vmss1}", checks=[self.check("zones", ["1"])])
        self.cmd("vmss update -g {rg} -n {vmss1} --zones 1 2", checks=[self.check("zones", ["1", "2"])])

        self.cmd("vmss create -g {rg} -n {vmss2} --image ubuntu2204")
        self.cmd("vmss show -g {rg} -n {vmss2}", checks=[self.check("zones", None)])
        self.cmd("vmss update -g {rg} -n {vmss2} --zones 1 2", checks=[self.check("zones", ["1", "2"])])
...
class VMRunCommandScenarioTest(ScenarioTest):
    ...
    def test_vm_run_command_e2e(self, resource_group, resource_group_location):
        ...
        self.kwargs.update({"vm": "test-run-command-vm", "loc": resource_group_location})

        self.cmd("vm run-command list -l {loc}")
        self.cmd("vm run-command show --command-id RunShellScript -l {loc}")
        public_ip = self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username clitest1 --admin-password Test12345678!! --generate-ssh-keys --nsg-rule NONE"
        ).get_output_in_json()["publicIpAddress"]
    ...
    def test_vm_run_command_with_parameters(self, resource_group):
        self.kwargs.update({"vm": "test-run-command-vm2", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --admin-username clitest1 --admin-password Test12345678!! "
    ...
    def test_run_command_v2(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:ubuntu-24_04-lts:server:latest --admin-username vmtest --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --size Standard_B2ms"
        )
        ...
        self.cmd(
            'vm run-command create -g {rg} --vm-name {vm} --name {run_cmd} --script "Write-Host Hello World!"',
            checks=[
                self.check("resourceGroup", "{rg}"),
        ...
        self.cmd(
            "vm run-command update -g {rg} --vm-name {vm} --name {run_cmd} --script script1 --parameters arg1=f1 --run-as-user user1 --timeout-in-seconds 3600",
            checks=[
                self.check("resourceGroup", "{rg}"),
                self.check("name", "{run_cmd}"),
...
class VMSSRunCommandScenarioTest(ScenarioTest):
    ...
    def test_vmss_run_command_e2e(self, resource_group, resource_group_location):
        ...
        self.kwargs.update({"vmss": "test-run-command-vmss", "loc": resource_group_location})
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username clitest1 --instance-count 1 --generate-ssh-keys --disable-overprovision"
        ).get_output_in_json()
    ...
    def test_vmss_run_command_with_parameters(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Debian:debian-10:10:latest --admin-username clitest1 --generate-ssh-keys --orchestration-mode Uniform --lb-sku Standard --vm-sku Standard_B2ms"
        )
    ...
    def test_vmss_run_command_v2(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:ubuntu-24_04-lts:server:latest --security-type Standard --admin-username {user} "
        ...
        self.kwargs.update({"instance_id": instace_ids[0]})
        self.cmd(
            'vmss run-command create --name {run_cmd} -g {rg} --vmss-name {vmss} --instance-id {instance_id} --script "Write-Host Hello World!"',
            checks=[
                self.check("resourceGroup", "{rg}"),
                self.check("name", "{run_cmd}"),
...
class VMSecurityProfileTestForDiskEncryption(ScenarioTest):
    ...
    def test_vm_encryption_identity_for_disk_encryption(self, resource_group, resource_group_location):
        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
        ...
        self.kwargs.update({"encryptionIdentityId": encryptionIdentityId, "principalId": principalId})

        self.cmd(
            "vm create -g {rg} -n {vm} --image win2022datacenter --admin-username clitester1 --admin-password Test123456789! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --encryption-identity {encryptionIdentityId} --assign-identity {emsi}"
        )
        ...
        self.assertTrue(
            encryptionIdentityId.lower()
            in (k.lower() for k in virtualMachine["identity"]["userAssignedIdentities"].keys())
        )
        ...
        self.assertEqual(
            virtualMachine["securityProfile"]["encryptionIdentity"]["userAssignedIdentityResourceId"],
    ...
    def test_vm_incorrect_encryption_identity_for_disk_encryption(self, resource_group, resource_group_location):
        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "emsi": "id1", "emsi2": "id2", "vnet": "vnet1"})

        emsi_result2 = self.cmd(
            "identity create -g {rg} -n {emsi2} --tags tag1=d2",
            checks=[self.check("name", "{emsi2}"), self.check("tags.tag1", "d2")],
        ).get_output_in_json()

        encryptionIdentityId = emsi_result2["id"]

        self.kwargs.update({"encryptionIdentityId": encryptionIdentityId})
    ...
    def test_vm_null_encryption_identity_for_disk_encryption(self, resource_group, resource_group_location):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image win2022datacenter --admin-username clitester1 --admin-password Test123456789! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --assign-identity {emsi}"
        )
        virtualMachine = self.cmd("vm show -g {rg} -n {vm}").get_output_in_json()
    ...
    def test_vmss_encryption_identity_for_disk_encryption(self, resource_group, resource_group_location):
        self.kwargs.update({"vmss": "vmss1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --instance-count 1 --admin-username clitester1 --admin-password Test123456789! --encryption-identity {encryptionIdentityId} --assign-identity {emsi} --orchestration-mode Uniform --lb-sku Standard"
        )
        ...
        self.assertEqual(
            virtualMachine["virtualMachineProfile"]["securityProfile"]["encryptionIdentity"][
    ...
    def test_vmss_incorrect_encryption_identity_for_disk_encryption(self, resource_group, resource_group_location):
        self.kwargs.update({"vmss": "vm1", "subnet": "subnet1", "emsi": "id1", "emsi2": "id2", "vnet": "vnet1"})
    ...
    def test_vmss_null_encryption_identity_for_disk_encryption(self, resource_group, resource_group_location):
        self.kwargs.update({"vmss": "vm1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --instance-count 1 --admin-username clitester1 --admin-password Test123456789! --orchestration-mode Uniform --lb-sku Standard --assign-identity {emsi}"
        )
        virtualMachine = self.cmd("vmss show -g {rg} -n {vmss}").get_output_in_json()
...
class VMDiskEncryptionTest(ScenarioTest):
    ...
    @KeyVaultPreparer(
        name_prefix="vault", name_len=20, key="vault", additional_params="--enabled-for-disk-encryption true"
    )
    def test_vmss_disk_encryption_e2e(self, resource_group, resource_group_location, key_vault):
        self.kwargs.update({"vmss": "vmss1"})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --instance-count 1 --admin-username clitester1 --admin-password Test123456789! --orchestration-mode Uniform --lb-sku Standard"
        )
        ...
        self.cmd(
            ...
            checks=[
                self.check(
                    "virtualMachineProfile.extensionProfile.extensions[0].settings.EncryptionOperation",
                    "EnableEncryption",
                ),
    ...
    @KeyVaultPreparer(
        name_prefix="vault", name_len=10, key="vault", additional_params="--enabled-for-disk-encryption true "
    )
    def test_vm_disk_encryption_e2e(self, resource_group, resource_group_location, key_vault):
        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --image win2022datacenter --admin-username clitester1 --admin-password Test123456789! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --size Standard_B2ms"
        )
    ...
    @KeyVaultPreparer(
        name_prefix="vault", name_len=10, key="vault", additional_params="--enable-rbac-authorization false"
    )
    def test_vm_disk_encryption_with_encryption_identity_e2e(self, resource_group, resource_group_location, key_vault):
        ...
        self.kwargs.update(
            {"vault_id": vault_id, "encryptionIdentityId": encryptionIdentityId, "principalId": principalId}
        )
        ...
        self.cmd(
            "keyvault set-policy -n {vault} -g {rg} --object-id {principalId} --key-permissions all --secret-permissions all"
        )

        time.sleep(15)

        self.cmd(
            "vm encryption enable -g {rg} -n {vm} --disk-encryption-keyvault {vault} --encryption-identity {encryptionIdentityId}"
        )
        self.cmd(
            "vm encryption show -g {rg} -n {vm}",
            checks=[self.check("disks[0].statuses[0].code", "EncryptionState/encrypted")],
    ...
    def test_vm_encryption_identity_set_at_encryption_cmdlet(self, resource_group, resource_group_location, key_vault):
        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "emsi1": "id1", "vnet": "vnet1"})

        emsi_result1 = self.cmd(
            "identity create -g {rg} -n {emsi1} --tags tag1=d1",
            checks=[self.check("name", "{emsi1}"), self.check("tags.tag1", "d1")],
        ).get_output_in_json()
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image win2022datacenter --admin-username clitester1 --admin-password Test123456789! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --assign-identity {emsi1}"
        )
        virtualMachine = self.cmd("vm show -g {rg} -n {vm}").get_output_in_json()
        ...
        self.assertIsNotNone(virtualMachine["identity"])
        self.assertTrue(
            encryptionIdentityId1.lower()
            in (k.lower() for k in virtualMachine["identity"]["userAssignedIdentities"].keys())
        )

        self.cmd(
            "keyvault set-policy -n {vault} -g {rg} --object-id {principalId1} --key-permissions all --secret-permissions all"
        )

        time.sleep(15)

        self.cmd(
            "vm encryption enable -g {rg} -n {vm} --disk-encryption-keyvault {vault} --encryption-identity {encryptionIdentityId1}"
        )
        self.cmd(
            "vm encryption show -g {rg} -n {vm}",
            checks=[self.check("disks[0].statuses[0].code", "EncryptionState/encrypted")],
    ...
    def test_vm_disk_encryption_with_incorrect_encryption_identity_e2e(
        self, resource_group, resource_group_location, key_vault
    ):

        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "emsi1": "id1", "emsi2": "id2", "vnet": "vnet1"})
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image win2022datacenter --admin-username clitester1 --admin-password Test123456789! --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --encryption-identity {encryptionIdentityId2} --assign-identity {emsi2}"
        )
        virtualMachine = self.cmd("vm show -g {rg} -n {vm}").get_output_in_json()
    ...
    def test_vm_disk_encryption_with_encryption_identity_not_acled_in_keyvaulte2e(
        self, resource_group, resource_group_location, key_vault
    ):
        self.kwargs.update({"vm": "vm1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
    ...
    def test_vmss_disk_encryption_with_encryption_identity_e2e(
        self, resource_group, resource_group_location, key_vault
    ):
        self.kwargs.update({"vmss": "vm1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
    ...
    def test_vmss_encryption_identity_set_at_encryption_cmdlet(
        self, resource_group, resource_group_location, key_vault
    ):
        self.kwargs.update({"vmss": "vm1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --instance-count 1 --admin-username clitester1 --admin-password Test123456789! --assign-identity {emsi} --orchestration-mode Uniform --lb-sku Standard"
        )
        virtualMachine = self.cmd("vmss show -g {rg} -n {vmss}").get_output_in_json()
        ...
        self.cmd(
            "vmss encryption enable -g {rg} -n {vmss} --disk-encryption-keyvault {vault} --encryption-identity {encryptionIdentityId}"
        )
        self.cmd('vmss update-instances -g {rg} -n {vmss}  --instance-ids "*"')
        self.cmd(
            "vmss encryption show -g {rg} -n {vmss}",
    ...
    def test_vmss_disk_encryption_with_incorrect_encryption_identity_e2e(
        self, resource_group, resource_group_location, key_vault
    ):

        self.kwargs.update({"vmss": "vm1", "subnet": "subnet1", "emsi1": "id1", "emsi2": "id2", "vnet": "vnet1"})
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --instance-count 1 --admin-username clitester1 --admin-password Test123456789! --encryption-identity {encryptionIdentityId2} --assign-identity {emsi2} --orchestration-mode Uniform --lb-sku Standard"
        )
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message) as context:
            self.cmd(
                "vmss encryption enable -g {rg} -n {vmss} --disk-encryption-keyvault {vault} --encryption-identity {encryptionIdentityId1}"
            )
    ...
    def test_vmss_disk_encryption_with_encryption_identity_not_acled_in_keyvaulte2e(
        self, resource_group, resource_group_location, key_vault
    ):
        self.kwargs.update({"vmss": "vm1", "subnet": "subnet1", "emsi": "id1", "vnet": "vnet1"})
    ...
    def test_vm_disk_encryption_with_key(self, resource_group, resource_group_location, key_vault):
        self.kwargs.update({"vm": "vm1", "key": "KEK"})
        self.cmd(
    ...
    def test_vm_encryption_at_host(self, resource_group):
        self.kwargs.update({"vm": "vm1", "vmss": "vmss1", "subnet": "subnet1", "vnet": "vnet1"})

        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --generate-ssh-keys --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --encryption-at-host --admin-username exampleusername"
        )

        # Disable default outbound access
        self.cmd("network vnet subnet update -g {rg} --vnet-name {vnet} -n {subnet} --default-outbound-access false")

        self.cmd("vm show -g {rg} -n {vm}", checks=[self.check("securityProfile.encryptionAtHost", True)])
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image OpenLogic:CentOS:7.5:latest --generate-ssh-keys --encryption-at-host --admin-username exampleusername --orchestration-mode Flexible"
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss}",
            checks=[self.check("virtualMachineProfile.securityProfile.encryptionAtHost", True)],
        )
...
class VMSSRollingUpgrade(ScenarioTest):
    ...
    def test_vmss_rolling_upgrade(self, resource_group):
        ...
        self.kwargs.update({"lb": "lb1", "probe": "probe1", "vmss": "vmss1"})
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username clitester1 --admin-password Testqwer1234! --lb {lb} --health-probe {probe}"
        )
...
class VMSSPriorityTesting(ScenarioTest):
    ...
    def test_vmss_create_with_low_priority(self, resource_group):
        self.kwargs.update({"priority": "Low", "vmss": "vmss1", "vmss2": "vmss2"})
        self.cmd(
            "vmss create -g {rg} -n {vmss} --admin-username clitester --admin-password PasswordPassword1! --image Debian:debian-10:10:latest --priority {priority} --lb-sku Standard"
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss}",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --admin-username clitester --admin-password PasswordPassword1! --image OpenLogic:CentOS:7.5:latest --priority {priority} --eviction-policy delete --lb-sku Standard"
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss2}",
            checks=[
...
class VMLBIntegrationTesting(ScenarioTest):
    ...
    def test_vm_lb_integration(self, resource_group):
        ...
        self.kwargs.update({"lb": "lb1", "vm1": "vm1", "vm2": "vm2", "avset": "av1"})
        # provision 2 web servers
        self.cmd("vm availability-set create -g {rg} -n {avset}")
        self.cmd(
            'vm create -g {rg} -n {vm1} --image Canonical:UbuntuServer:18.04-LTS:latest --public-ip-address "" --availability-set {avset} --generate-ssh-keys --admin-username ubuntuadmin'
        )
        self.cmd("vm open-port -g {rg} -n {vm1} --port 80")
        self.cmd(
            'vm create -g {rg} -n {vm2} --image Canonical:UbuntuServer:18.04-LTS:latest --public-ip-address "" --availability-set {avset} --generate-ssh-keys --admin-username ubuntuadmin'
        )
...
class VMCreateWithExistingNic(ScenarioTest):
    ...
    def test_vm_create_existing_nic(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n vm1 --image OpenLogic:CentOS:7.5:latest --nics my-nic --generate-ssh-keys "
...
class VMSecretTest(ScenarioTest):
    ...
    def test_vm_secret_e2e_test(self, resource_group, resource_group_location, key_vault):
        self.kwargs.update({"vm": "vm1", "cert": "vm-secrt-cert", "loc": resource_group_location})

        self.kwargs["policy_path"] = os.path.join(TEST_DIR, "keyvault", "policy.json")

        self.cmd(
            "vm create -g {rg} -n {vm} --image Ubuntu2204 --generate-ssh-keys --admin-username rheladmin --nsg-rule NONE"
        )
        ...
        self.assertTrue(
            "https://{vault}.vault.azure.net/secrets/{cert}/".format(**self.kwargs)
            in secret_result[0]["vaultCertificates"][0]["certificateUrl"]
        )
        self.cmd("vm secret list -g {rg} -n {vm}")
        self.cmd("vm secret remove -g {rg} -n {vm} --keyvault {vault} --certificate {cert}")

        self.cmd("vm secret list -g {rg} -n {vm}", checks=self.check("length([])", 0))
...
class VMOsDiskSwap(ScenarioTest):
    ...
    def test_vm_os_disk_swap(self, resource_group):
        self.kwargs.update({"vm": "vm1", "backupDisk": "disk1", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --admin-username clitest123 "
        ...
        backup_disk_id = self.cmd(
            "disk create -g {{rg}} -n {{backupDisk}} --source {}".format(original_disk_id)
        ).get_output_in_json()["id"]
...
class VMGenericUpdate(ScenarioTest):
    ...
    def test_vm_generic_update(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Debian:debian-10:10:latest --data-disk-sizes-gb 1 2 "
        ...
        self.cmd(
            ...
            checks=[self.check("storageProfile.dataDisks[0].caching", "ReadWrite")],
        )
        self.cmd(
...
class VMGalleryImage(ScenarioTest):
    ...
    def test_shared_gallery(self, resource_group, resource_group_location):
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} "
        ...
        self.cmd("network public-ip create --name {pubip} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --data-disk-sizes-gb 10 --admin-username clitest1 "
            "--generate-ssh-key --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip} --nsg-rule NONE"
        )
        ...
        self.cmd(
            "vm run-command invoke -g {rg} -n {vm} --command-id RunShellScript --scripts "
        ...
        self.cmd(
            "sig image-version create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} "
        ...
        res = self.cmd(
            ...
            checks=[self.check("sharingProfile.permissions", "Groups")],
        ).get_output_in_json()
        ...
        self.cmd(
            "sig list-shared --location {location} --query \"[?name=='{unique_name}']\"",
            checks=[
        ...
        self.cmd(
            "sig image-definition show-shared --gallery-image-definition {image} "
        ...
        self.cmd(
            "sig image-version list-shared --gallery-image-definition {image} --gallery-unique-name {unique_name} "
            "--location {location}",
            checks=[
                self.check("[0].location", "{location}"),
        ...
        self.cmd(
            "sig image-version show-shared --gallery-image-definition {image} --gallery-unique-name {unique_name} "
            "--location {location} --gallery-image-version {version}",
            checks=[
                self.check("location", "{location}"),
    ...
    def test_image_version_create_for_diff_source(self, resource_group_location):
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image1} "
            "--os-type linux --os-state Specialized -p publisher1 -f offer1 -s sku1 --hyper-v-generation v1"
        )
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image2} "
        ...
        with self.assertRaises(InvalidArgumentValueError):
            self.cmd(
                "sig image-version create -g {rg} --gallery-name {gallery} --gallery-image-definition {image2} "
    ...
    def test_gallery_e2e(self, resource_group, resource_group_location, key_vault):
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} --os-type linux -p publisher1 -f offer1 -s sku1 --hyper-v-generation V1",
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --data-disk-sizes-gb 10 --admin-username clitest1 --generate-ssh-key --nsg-rule NONE"
        )
        self.cmd(
        ...
        vault_id = self.cmd("keyvault show -g {rg} -n {vault}").get_output_in_json()["id"]
        kid = self.cmd("keyvault key create -n {key} --vault {vault} --protection software").get_output_in_json()[
            "key"
        ]["kid"]
        self.kwargs.update({"vault_id": vault_id, "kid": kid})
        ...
        self.kwargs.update({"des1_sp_id": des1_sp_id, "des1_id": des1_id})

        self.cmd(
            "keyvault set-policy -n {vault} -g {rg} --object-id {des1_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check("publishingProfile.targetRegions[0].regionalReplicaCount", 1),
                self.check(
                    "publishingProfile.targetRegions[0].encryption.osDiskImage.diskEncryptionSetId", "{des1_id}"
                ),
                self.check("publishingProfile.targetRegions[0].encryption.dataDiskImages[0].lun", 0),
                self.check(
                    "publishingProfile.targetRegions[0].encryption.dataDiskImages[0].diskEncryptionSetId", "{des1_id}"
                ),
            ],
        )
    ...
    def test_image_update_add_set(self, resource_group: object, resource_group_location: object) -> object:
        ...
        self.cmd(
            "sig image-version create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} "
            "--gallery-image-version {version} --managed-image {captured} --replica-count 1",
            checks=[self.check("length(publishingProfile.targetRegions)", 1)],
        )

        self.cmd(
            "sig image-version update -g {rg} --gallery-name {gallery} --gallery-image-definition {image} "
        ...
        self.cmd(
            ...
            checks=[self.check("safetyProfile.allowDeletionOfReplicatedLocations", True)],
        )

        self.cmd(
            "sig image-version update -g {rg} --gallery-name {gallery} --gallery-image-definition {image} "
            "--gallery-image-version {version} --set safetyProfile.allowDeletionOfReplicatedLocations=false",
            checks=[self.check("safetyProfile.allowDeletionOfReplicatedLocations", False)],
        )
    ...
    def test_gallery_image_version_vhd(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --use-unmanaged-disk --size Standard_B2ms --nsg-rule NONE --generate-ssh-key --admin-username vmtest --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        account_id = resource_id(
            ...
            name=storage_account_os,
        )

        self.kwargs.update({"vhd": vhd_uri, "stac": storage_account_os})
    ...
    def test_image_version_create_os_vhd(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n vm1 --image OpenLogic:CentOS:7.5:latest --use-unmanaged-disk --nsg-rule NONE "
        ...
        self.kwargs.update({"vhd_uri": vhd_uri, "account_id": account_id})
        ...
        self.cmd(
            "sig image-version create --resource-group {rg} --gallery-name {gallery} "
            "--gallery-image-definition {image} --gallery-image-version 1.0.0 "
            "--os-vhd-storage-account {account_id} --os-vhd-uri {vhd_uri}",
            checks=[
                self.check("provisioningState", "Succeeded"),
    ...
    def test_create_image_version_with_allow_replicated_location_deletion(
        ...
        self.cmd("sig create -g {rg} --gallery-name {gallery}")
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} --features SecurityType=Standard "
            "--os-type linux -p publisher1 -f offer1 -s sku1 --hyper-v-generation v1",
            checks=[
                self.check("name", "{image}"),
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username clitest1 --generate-ssh-key --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
    ...
    def test_image_version_end_of_life_date(self, resource_group, resource_group_location):
        ...
        self.cmd(
            "sig image-version create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} "
            '--gallery-image-version {version} --managed-image {captured} --end-of-life-date "2025-12-31" --block-deletion-before-end-of-life true',
            checks=[self.check("safetyProfile.blockDeletionBeforeEndOfLife", True)],
        )
        self.cmd(
            "sig image-version update -g {rg} --gallery-name {gallery} --gallery-image-definition {image} --gallery-image-version {version} --block-deletion-before-end-of-life False",
            checks=[self.check("safetyProfile.blockDeletionBeforeEndOfLife", False)],
        )
    ...
    def test_image_version_with_target_extended_locations(self):
        ...
        self.kwargs.update({"disk_id": disk_id})
    ...
    def test_image_version_with_target_extended_locations_encryption(self, resource_group, key_vault):
        ...
        self.kwargs.update({"des1_sp_id": des1_sp_id})
    ...
    def test_gallery_specialized(self, resource_group):
        self.kwargs.update({"gallery": self.create_random_name(prefix="gallery_", length=20), "image": "image1"})
    ...
    def test_specialized_image(self, resource_group):
        ...
        self.cmd("sig create -g {rg} --gallery-name {gallery}")
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image} --os-type linux "
            "--os-state specialized -p publisher1 -f offer1 -s sku1 --tags tag=test --hyper-v-generation v1",
            checks=[self.check("osState", "Specialized"), self.check("tags", {"tag": "test"})],
        )
        self.cmd(
            "vm create -g {rg} -n {vm1} --image OpenLogic:CentOS:7.5:latest --nsg-rule NONE --admin-username azureuser --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        self.kwargs.update({"image_version": image_version})
        self.cmd(
            "vm create -g {rg} -n {vm2} --image {image_version} --specialized --nsg-rule NONE --admin-username azureuser --admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet}"
        )

        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image {image_version} --specialized --admin-username azureuser --admin-password testPassword0 "
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_image_version_",
        location="westus2",
        parameter_name="another_resource_group",
        subscription=AUX_SUBSCRIPTION,
    )
    def test_sig_image_version_cross_tenant(self, resource_group, another_resource_group):
        ...
        # Create a public IP resource with service tag
        self.cmd(
            "network public-ip create --name {pubip} -g {another_rg} --ip-tags FirstPartyUsage=/NonProd --subscription {aux_sub}"
        )
        ...
        # Disable default outbound access
        self.cmd(
            "network vnet subnet update -g {another_rg} --vnet-name {vnet} -n {subnet} --subscription {aux_sub} --default-outbound-access false"
        )
        ...
        res = self.cmd(
            "image create -g {another_rg} -n {image_name} --source {vm} --subscription {aux_sub}"
        ).get_output_in_json()
        self.kwargs.update({"image_id": res["id"]})

        self.cmd("sig create -g {rg} --gallery-name {sig_name}", checks=self.check("name", self.kwargs["sig_name"]))
        self.cmd(
    ...
    def test_sig_image_version_create_cross_tenant(self, resource_group, another_resource_group):
        ...
        self.cmd(
            "sig create -g {another_rg} --gallery-name {gallery1} --subscription {aux_sub}",
        ...
        res1 = self.cmd(
            ...
            checks=self.check("name", self.kwargs["image1"]),
        ).get_output_in_json()
        image_version_id = self.cmd(
            ...
            checks=self.check("name", self.kwargs["version1"]),
        ).get_output_in_json()["id"]
        self.kwargs.update({"image_version_id": image_version_id})
        ...
        self.cmd(
            "sig image-version create -g {rg} --gallery-name {gallery2} --gallery-image-definition {image2} "
            "--gallery-image-version {version2} --image-version {image_version_id}",
            checks=[self.check("name", self.kwargs["version2"]), self.check("provisioningState", "Succeeded")],
        )
    ...
    def test_create_vm_with_shared_gallery_image(self, resource_group, resource_group_location):
        ...
        self.cmd("network public-ip create --name {pubip} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --data-disk-sizes-gb 10 --admin-username clitest1 --generate-ssh-key --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip} --nsg-rule None"
        )
        ...
        self.kwargs["unique_name"] = self.cmd(
            "sig show --gallery-name {gallery} --resource-group {rg} --select Permissions"
        ).get_output_in_json()["identifier"]["uniqueName"]
        ...
        self.cmd(
            "vm create -g {rg} -n {vm_with_shared_gallery_version} --image {shared_gallery_image_version} --admin-username clitest1 --generate-ssh-key --subnet {subnet} --vnet-name {vnet} --nsg-rule None"
        )
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -g {rg} -n {vmss_with_shared_gallery_version} --image {shared_gallery_image_version} "
        ...
        self.cmd(
            ...
            checks=[
                self.check("provisioningState", "Succeeded"),
                self.check(
                    "virtualMachineProfile.storageProfile.imageReference.sharedGalleryImageId",
    ...
    def test_gallery_soft_delete(self, resource_group_location):
        ...
        self.cmd(
            "sig create -g {rg} -r {gallery} --soft-delete True",
            checks=[
        ...
        self.cmd(
            "sig show -g {rg} -r {gallery} --sharing-groups",
            checks=[
                self.check("location", "westus"),
                self.check("name", "{gallery}"),
                self.check("resourceGroup", "{rg}"),
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery} --gallery-image-definition {image_name} "
            "--os-type linux --os-state Specialized -p publisher1 -f offer1 -s sku1 --hyper-v-generation v1"
        )

        vm_id = self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --data-disk-sizes-gb 10 "
            "--admin-username clitest1 --generate-ssh-key --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        ).get_output_in_json()["id"]
        self.kwargs.update({"vm_id": vm_id})
        ...
        self.cmd(
            "sig image-version create -g {rg} --gallery-name {gallery} --gallery-image-definition {image_name} "
            "--gallery-image-version {version} --virtual-machine {vm_id}",
            checks=[
                self.check("location", resource_group_location),
                self.check("name", "{version}"),
        ...
        self.cmd(
            "sig image-version delete -g {rg} --gallery-name {gallery} --gallery-image-definition {image_name} "
        ...
        self.cmd(
            "sig image-version undelete -g {rg} --gallery-name {gallery} --gallery-image-definition {image_name} "
            "--gallery-image-version {version}",
            checks=[
                self.check("location", resource_group_location),
                self.check("name", "{version}"),
        ...
        self.cmd(
            "sig update -g {rg} -r {gallery} --soft-delete False",
            checks=[
                self.check("location", "westus"),
                self.check("name", "{gallery}"),
                self.check("resourceGroup", "{rg}"),
        ...
        self.cmd("sig update -g {rg} -r {gallery}", checks=[self.check("softDeletePolicy.isSoftDeleteEnabled", False)])
        if self.is_live:
            time.sleep(30)
        ...
        with self.assertRaises(InvalidArgumentValueError):
            self.cmd(
                "sig image-version undelete -g {rg} --gallery-name {gallery} --gallery-image-definition {image_name} --gallery-image-version {version}"
            )

        self.cmd(
            "sig show -g {rg} -r {gallery}",
            checks=[
                self.check("location", "westus"),
                self.check("name", "{gallery}"),
                self.check("resourceGroup", "{rg}"),
    ...
    def test_replication_mode(self, resource_group):
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {sig_name} --hyper-v-generation v1 "
            "--gallery-image-definition {img_def_name} --os-type linux -p {pub_name} -f {of_name} -s {sku_name}"
        )
        self.cmd(
            "vm create -g {rg} -n {vm_name} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username vmtest "
            "--subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        ...
        self.cmd(
            "sig image-version create --replication-mode Full -g {rg} -r {sig_name}"
            " -i {img_def_name} -e 1.1.1 --managed-image {img_name}",
            checks=[self.check("name", "1.1.1"), self.check("publishingProfile.replicationMode", "Full")],
        )
        self.cmd(
        ...
        self.cmd(
            "sig image-version create --replication-mode Shallow -g {rg} -r {sig_name}"
            " -i {img_def_name} -e 1.1.2 --managed-image {img_name}",
            checks=[self.check("name", "1.1.2"), self.check("publishingProfile.replicationMode", "Shallow")],
        )
        self.cmd(
    ...
    def test_community_gallery_operations(self, resource_group, resource_group_location):
        ...
        self.cmd("network public-ip create --name {pubip} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username gallerytest "
        ...
        self.cmd(
            "sig show-community --location {location} --public-gallery-name {public_name}",
            checks=[
                self.check("location", "{location}"),
        ...
        self.cmd(
            "sig list-community --location {location}",
            checks=[self.check("[0].location", "{location}", case_sensitive=False)],
        )
        ...
        self.cmd(
            "sig image-definition list-community --public-gallery-name {public_name} --location {location}",
            checks=[
                self.check("[0].location", "{location}"),
                self.check("[0].name", "{image}"),
    ...
    def test_create_vm_with_community_gallery_image(self, resource_group, resource_group_location):
        ...
        self.kwargs["community_gallery_image_version"] = self.cmd(
            "sig image-version show-community --gallery-image-definition {image} --public-gallery-name {public_name} --location eastus2 --gallery-image-version {version}"
        ).get_output_in_json()["uniqueId"]

        # Create a public IP resource with service tag
        self.cmd("network public-ip create --name {pubip2} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm_with_community_gallery} --image {community_gallery_image_version} "
        ...
        self.cmd(
            "vm show -g {rg} -n {vm_with_community_gallery}",
            checks=[
                self.check("storageProfile.imageReference.exactVersion", "{version}"),
                self.check(
                    "storageProfile.imageReference.communityGalleryImageId", "{community_gallery_image_version}"
                ),
            ],
        )
        ...
        self.cmd("network public-ip create --name {pubip3} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vmss create -g {rg} -n {vmss_with_community_gallery_version} --admin-username gallerytest --generate-ssh-keys "
            "--image {community_gallery_image_version} --accept-term --orchestration-mode Flexible --public-ip-address {pubip3} --nsg {nsg}"
        )

        self.cmd(
            "vmss show -g {rg} -n {vmss_with_community_gallery_version}",
            checks=[
                self.check(
                    "virtualMachineProfile.storageProfile.imageReference.communityGalleryImageId",
    ...
    def test_update_gallery_permissions(self, resource_group):
        self.kwargs.update(
            {"gallery1": self.create_random_name("gallery1", 15), "gallery2": self.create_random_name("gallery2", 15)}
        )
        self.cmd(
            "sig create -g {rg} --gallery-name {gallery1} --permissions Community "
            "--publisher-uri pubUri --publisher-email test@123.com --eula eula --public-name-prefix pubName",
            checks=[self.check("sharingProfile.permissions", "Community")],
        )
    ...
    def test_gallery_in_vm_access_control_profile(self, resource_group, resource_group_location):
        ...
        self.cmd(
            "sig in-vm-access-control-profile create --resource-group {rg} --gallery-name {gallery} --name {profile1} "
            "--os-type Linux --applicable-host-endpoint WireServer",
            checks=[
        ...
        self.cmd(
            "sig in-vm-access-control-profile update --resource-group {rg} --gallery-name {gallery} --name {profile1} "
            "--description test",
            checks=[
                self.check("name", "{profile1}"),
        ...
        self.cmd(
            "sig in-vm-access-control-profile list --resource-group {rg} --gallery-name {gallery}",
            checks=[
                self.check("length(@)", "1"),
            ],
        )
        ...
        self.cmd(
            "sig in-vm-access-control-profile-version create --resource-group {rg} --gallery-name {gallery} "
            "--profile-name {profile2} --profile-version {profile_version} --default-access Allow --mode Audit "
            "--exclude-from-latest true --target-regions westus eastus2 --rules {rules_file}",
            checks=[
        ...
        self.cmd(
            "sig in-vm-access-control-profile-version update --resource-group {rg} --gallery-name {gallery} "
            "--profile-name {profile2} --profile-version {profile_version} --exclude-from-latest true --target-regions eastus2 westus eastus",
            checks=[
                self.check("name", "{profile_version}"),
                self.check("excludeFromLatest", True),
        ...
        self.cmd(
            "sig in-vm-access-control-profile-version show --resource-group {rg} --gallery-name {gallery} "
            "--profile-name {profile2} --profile-version {profile_version}",
            checks=[
                self.check("name", "{profile_version}"),
                self.check("excludeFromLatest", True),
                self.check("length(targetLocations)", "3"),
        ...
        self.cmd(
            "sig in-vm-access-control-profile-version list --resource-group {rg} --gallery-name {gallery} --profile-name {profile2}",
            checks=[
                self.check("length(@)", "1"),
            ],
        )
        ...
        self.cmd(
            "sig in-vm-access-control-profile-version delete --resource-group {rg} --gallery-name {gallery} "
...
class ProximityPlacementGroupScenarioTest(ScenarioTest):
    ...
    def test_proximity_placement_group(self, resource_group, resource_group_location):
        self.kwargs.update({"ppg1": "my_ppg_1", "ppg2": "my_ppg_2", "ppg3": "my_ppg_3", "loc": resource_group_location})
        ...
        self.cmd(
            "ppg create -n {ppg2} -t Standard -g {rg}",
            checks=[
        ...
        self.cmd(
            "ppg update -n {ppg3} -g {rg} -t Standard",
            checks=[
                self.check("name", "{ppg3}"),
                self.check("location", "{loc}"),
                self.check("proximityPlacementGroupType", "Standard"),
        ...
        self.cmd("ppg list -g {rg}", checks=[self.check("length(@)", 3)])
        ...
        self.cmd('ppg update -n {ppg3} -g {rg} --set tags.foo="bar"', checks=[self.check("tags.foo", "bar")])
    ...
    def test_ppg_intent_vm_sizes_and_zone(self):
        ...
        self.cmd(
            "ppg create -n {ppg1} -g {rg} --intent-vm-sizes {vm_size1} {vm_size2} --zone {zone} --tags tag=test",
            checks=[
                self.check("name", "{ppg1}"),
        ...
        self.cmd(
            "ppg create -n {ppg2} -g {rg} --intent-vm-sizes {vm_size1} {vm_size2}",
            checks=[self.check("name", "{ppg2}"), self.check("length(intent.vmSizes)", "2")],
        )
        ...
        self.cmd(
            "ppg update -n {ppg1} -g {rg} --intent-vm-sizes {vm_size1}",
            checks=[self.check("name", "{ppg1}"), self.check("length(intent.vmSizes)", "1")],
        )

        self.cmd(
            "ppg show -n {ppg1} -g {rg}",
            checks=[self.check("name", "{ppg1}"), self.check("length(intent.vmSizes)", "1")],
        )
    ...
    def test_ppg_with_related_resources(self, resource_group):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Debian:debian-10:10:latest --admin-username debian --ssh-key-value '{ssh_key}' --ppg {ppg_id} --orchestration-mode Flexible --nsg {nsg}"
        )
        self.kwargs["vmss_id"] = self.cmd("vmss show -g {rg} -n {vmss}").get_output_in_json()["id"]

        self.kwargs["avset_id"] = self.cmd(
        ...
        self._assert_ids_equal(
            ppg_resource["availabilitySets"][0]["id"], self.kwargs["avset_id"], rg_prefix="cli_test_ppg_vm_vmss_"
        )
        self._assert_ids_equal(
            ppg_resource["virtualMachines"][0]["id"], self.kwargs["vm_id"], rg_prefix="cli_test_ppg_vm_vmss_"
        )
        self._assert_ids_equal(
            ppg_resource["virtualMachineScaleSets"][0]["id"], self.kwargs["vmss_id"], "cli_test_ppg_vm_vmss_"
        )
    ...
    def test_ppg_update(self, resource_group):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Debian:debian-10:10:latest --admin-username debian --ssh-key-value '{ssh_key}' --orchestration-mode Uniform --lb-sku Standard --nsg {nsg}"
        )
        self.kwargs["vmss_id"] = self.cmd("vmss show -g {rg} -n {vmss}").get_output_in_json()["id"]
        self.cmd("vmss deallocate -g {rg} -n {vmss}")
        time.sleep(30)
        ...
        self._assert_ids_equal(
            ppg_resource["availabilitySets"][0]["id"], self.kwargs["avset_id"], rg_prefix="cli_test_ppg_update_"
        )
        self._assert_ids_equal(
            ppg_resource["virtualMachines"][0]["id"], self.kwargs["vm_id"], rg_prefix="cli_test_ppg_update_"
        )
        self._assert_ids_equal(
            ppg_resource["virtualMachineScaleSets"][0]["id"], self.kwargs["vmss_id"], "cli_test_ppg_update_"
        )
...
class DedicatedHostScenarioTest(ScenarioTest):
    ...
    def test_vm_host_management(self, resource_group):
        ...
        self.cmd(
            "vm host group list -g {rg}", checks=[self.check("length(@)", 1), self.check("[0].name", "{host-group}")]
        )
        self.cmd("vm host create -n {host} --host-group {host-group} -d 2 -g {rg} --sku DSv3-Type1")
        self.cmd(
            "vm host list --host-group {host-group} -g {rg}",
            checks=[self.check("length(@)", 1), self.check("[0].name", "{host}")],
        )

        self.cmd(
            'vm host group update -n {host-group} -g {rg} --set tags.foo="bar"', checks=[self.check("tags.foo", "bar")]
        )
        self.cmd(
            'vm host update -n {host} --host-group {host-group} -g {rg} --set tags.foo="bar"',
    ...
    def test_vm_host_resize(self, resource_group):
        ...
        self.kwargs["resize-sku"] = self.cmd(
            "vm host list-resize-options --name {host} --host-group {host-group} -g {rg}"
        ).get_output_in_json()[0]

        self.cmd(
            "vm host resize -n {host} --host-group {host-group} -g {rg} --sku {resize-sku}",
            checks=[self.check("sku.name", "{resize-sku}")],
        )

    @ResourceGroupPreparer(name_prefix="cli_test_vm_host_ultra_ssd_")
    def test_vm_host_ultra_ssd(self, resource_group):
        self.kwargs.update({"host-group": self.create_random_name("host", 10)})
        self.cmd(
            "vm host group create -n {host-group} -g {rg} --ultra-ssd-enabled true -c 1 -l eastus2euap --zone 3",
            checks=[self.check("additionalCapabilities.ultraSsdEnabled", True)],
        )
    ...
    def test_dedicated_host_e2e(self, resource_group, resource_group_location):
        self.kwargs.update({"host-group": "my-host-group", "host-name": "my-host", "vm-name": "ded-host-vm"})
        ...
        try:
            self.cmd(
                "vm host create -n {host-name} --host-group {host-group} -d 2 -g {rg} "
                '--sku DSv3-Type1 --auto-replace false --tags "bar=baz" ',
                checks=[
        ...
        self.cmd("vm host group get-instance-view -g {rg} -n {host-group}", checks=[self.exists("instanceView")])

        host_id = self.cmd("vm host show -g {rg} -n {host-name} --host-group {host-group}").get_output_in_json()["id"]
        self.kwargs.update({"host_id": host_id})

        self.cmd(
            "vm create -n {vm-name} --image Debian:debian-10:10:latest -g {rg} --size Standard_D4s_v3 "
        ...
        vm_json = self.cmd(
            ...
            checks=[self.check("name", "{vm-name}"), self.check("provisioningState", "Succeeded")],
        ).get_output_in_json()
        ...
        self.kwargs.update({"host_id": host_id})
        self.cmd(
            "vm create -g {rg2} -n vm1 --image OpenLogic:CentOS:7.5:latest --host {host_id} --size Standard_D4s_v3 --nsg-rule NONE --generate-ssh-keys --admin-username azureuser"
        )
        self.cmd(
            "vm create -g {rg2} -n vm2 --image OpenLogic:CentOS:7.5:latest --host-group {host-group} --size Standard_D4s_v3 --nsg-rule NONE --generate-ssh-keys --admin-username azureuser"
        )
        self.cmd("vm show -g {rg2} -n vm1", checks=[self.check_pattern("host.id", ".*/{host-name}$")])
        self.cmd("vm show -g {rg2} -n vm2", checks=[self.check_pattern("hostGroup.id", ".*/{host-group}$")])
    ...
    def test_update_dedicated_host_e2e(self, resource_group, resource_group_location):
        ...
        try:
            self.cmd(
                "vm host create -n {host2-name} --host-group {host2-group} -d 2 -g {rg} "
                '--sku DSv3-Type1 --auto-replace false --tags "bar=baz" ',
                checks=[
        ...
        self.cmd("vm host group get-instance-view -g {rg} -n {host2-group}", checks=[self.exists("instanceView")])

        host_id = self.cmd("vm host show -g {rg} -n {host-name} --host-group {host-group}").get_output_in_json()["id"]
        host2_id = self.cmd("vm host show -g {rg} -n {host2-name} --host-group {host2-group}").get_output_in_json()[
            "id"
        ]
        self.kwargs.update({"host_id": host_id, "host2_id": host2_id})
        ...
        self.kwargs.update({"host_id": host_id, "host2_id": host2_id})
        self.cmd(
            "vm create -g {rg} -n vm1 --image OpenLogic:CentOS:7.5:latest --size Standard_D4s_v3 --nsg-rule NONE --generate-ssh-keys --admin-username azureuser"
        )
        self.cmd(
            "vm create -g {rg} -n vm2 --image OpenLogic:CentOS:7.5:latest --size Standard_D4s_v3 --nsg-rule NONE --generate-ssh-keys --admin-username azureuser"
        )
        self.cmd("vm show -g {rg} -n vm1", checks=[self.check("host", None)])
        self.cmd("vm show -g {rg} -n vm2", checks=[self.check("hostGroup", None)])

        # validate none host to host
        self.cmd("vm deallocate -n vm1 -g {rg}")
        self.cmd("vm update -n vm1 -g {rg} --host {host_id}")
        self.cmd("vm start -n vm1 -g {rg}")
        self.cmd("vm show -g {rg} -n vm1", checks=[self.check_pattern("host.id", ".*/{host-name}$")])

        # validate host to host2
        self.cmd("vm deallocate -n vm1 -g {rg}")
        self.cmd("vm update -n vm1 -g {rg} --host {host2_id}")
        self.cmd("vm start -n vm1 -g {rg}")
        self.cmd("vm show -g {rg} -n vm1", checks=[self.check_pattern("host.id", ".*/{host2-name}$")])
        ...
        self.cmd("vm show -g {rg} -n vm2", checks=[self.check_pattern("hostGroup.id", ".*/{host-group}$")])

        # validate group to group2
        self.cmd("vm deallocate -n vm2 -g {rg}")
        self.cmd("vm update -n vm2 -g {rg} --host-group {host2-group}")
        self.cmd("vm start -n vm2 -g {rg}")
        self.cmd("vm show -g {rg} -n vm2", checks=[self.check_pattern("hostGroup.id", ".*/{host2-group}$")])

        # validate group2 to host
        self.cmd("vm deallocate -n vm2 -g {rg}")
        self.cmd("vm update -n vm2 -g {rg} --host {host_id}")
        self.cmd("vm start -n vm2 -g {rg}")
        self.cmd("vm show -g {rg} -n vm2", checks=[self.check_pattern("host.id", ".*/{host-name}$")])
...
class VMSSTerminateNotificationScenarioTest(ScenarioTest):
    ...
    def test_vmss_terminate_notification(self, resource_group):
        ...
        self.kwargs.update({"vmss1": "vmss1", "vmss2": "vmss2"})

        # Create, enable terminate notification
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Canonical:UbuntuServer:18.04-LTS:latest --terminate-notification-time 5 --admin-username azureuser",
            checks=[self.check(create_enable_key, True), self.check(create_not_before_timeout_key, "PT5M")],
        )
        ...
        self.cmd(
            ...
            checks=[self.check(update_enable_key, True), self.check(update_not_before_timeout_key, "PT8M")],
        )

        # Update, set time
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --terminate-notification-time 9",
            checks=[self.check(update_not_before_timeout_key, "PT9M")],
        )

        # Update, disable terminate notification
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --enable-terminate-notification false",
            checks=[self.check("virtualMachineProfile.scheduledEventsProfile.terminateNotificationProfile", None)],
        )
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username azureuser"
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss2} --enable-terminate-notification true --terminate-notification-time 5",
            checks=[self.check(update_enable_key, True), self.check(update_not_before_timeout_key, "PT5M")],
        )
    ...
    def test_vmss_os_image_notification(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --enable-osimage-notification true",
            checks=[
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss} --enable-osimage-notification false",
            checks=[self.check("virtualMachineProfile.scheduledEventsProfile.osImageNotificationProfile", None)],
        )
    ...
    def test_vmss_scheduled_events_policy(self, resource_group):
        self.kwargs.update({"vmss1": self.create_random_name("vmss", 10), "vmss2": self.create_random_name("vmss", 10)})
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Canonical:UbuntuServer:18.04-LTS:latest --additional-events True --enable-reboot True --enable-redeploy True",
            checks=[
                self.check(
                    "vmss.scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
                ),
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --additional-events False --enable-reboot False --enable-redeploy False",
            checks=[self.check("scheduledEventsPolicy", None)],
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --additional-events true --enable-redeploy true --enable-reboot False ",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image Canonical:UbuntuServer:18.04-LTS:latest",
            checks=[self.check("vmss.scheduledEventsPolicy", None)],
        )

        self.cmd(
            "vmss update -g {rg} -n {vmss2} --additional-events True ",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss2} --enable-reboot True --enable-redeploy true",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
    ...
    def test_vm_scheduled_events_policy(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --image Canonical:UbuntuServer:18.04-LTS:latest --additional-events True --enable-reboot True --enable-redeploy True --nsg-rule NONE"
        )
        self.cmd(
            "vm show -g {rg} -n {vm1}",
            checks=[
        ...
        self.cmd(
            "vm update -g {rg} -n {vm1} --additional-events False --enable-reboot False --enable-redeploy False",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    False,
        ...
        self.cmd(
            "vm update -g {rg} -n {vm1} --additional-events true --enable-redeploy true --enable-reboot False ",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
        ...
        self.cmd("vm show -g {rg} -n {vm2}", checks=[self.check("scheduledEventsPolicy", None)])
        self.cmd(
            "vm update -g {rg} -n {vm2} --additional-events True ",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
        ...
        self.cmd(
            "vm update -g {rg} -n {vm2} --enable-reboot True --enable-redeploy true",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
        ...
        self.cmd(
            "vm create -g {rg} -n {vm3} --image Canonical:UbuntuServer:18.04-LTS:latest --additional-events True --nsg-rule NONE"
        )
        self.cmd(
            "vm show -g {rg} -n {vm3}",
            checks=[
        ...
        self.cmd(
            "vm update -g {rg} -n {vm3} --enable-reboot True",
            checks=[
                self.check(
                    "scheduledEventsPolicy.scheduledEventsAdditionalPublishingTargets.eventGridAndResourceGraph.enable",
                    True,
...
class VMPriorityEvictionBillingTest(ScenarioTest):
    ...
    def test_vm_priority_eviction_billing(self, resource_group):
        ...
        # vm create
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --priority Low --subnet {subnet} --vnet-name {vnet} "
        ...
        self.cmd(
            ...
            checks=[self.check("priority", "Spot"), self.check("billingProfile.maxPrice", 100)],
        )
...
class VMCreateSpecialName(ScenarioTest):
    ...
    def test_vm_create_special_name(self, resource_group):
        ...
        """
        self.kwargs.update({"vm": "vm_1", "subnet": "subnet1", "vnet": "vnet1"})

        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username azureuser "
...
class VMImageTermsTest(ScenarioTest):
    ...
    def test_vm_image_terms(self, resource_group):
        ...
        self.cmd("vm image terms accept --urn {urn}", checks=[self.check("accepted", True)])
        self.cmd("vm image terms show --urn {urn}", checks=[self.check("accepted", True)])
        self.cmd("vm image terms cancel --urn {urn}", checks=[self.check("accepted", False)])
        self.cmd("vm image terms show --urn {urn}", checks=[self.check("accepted", False)])
        self.cmd(
            "vm image terms accept --publisher {publisher} --offer {offer} --plan {plan}",
            checks=[self.check("accepted", True)],
        )
        self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("accepted", False)],
        )
        self.cmd(
            "vm image terms show --publisher {publisher} --offer {offer} --plan {plan}",
            checks=[self.check("accepted", False)],
...
class DiskEncryptionSetTest(ScenarioTest):
    ...
    def test_disk_encryption_set(self, resource_group, key_vault):
        ...
        self.kwargs.update({"des2_sp_id": des2_sp_id, "des2_id": des2_id})
        ...
        self.kwargs.update({"des3_sp_id": des3_sp_id, "des3_id": des3_id})

        self.cmd(
            "keyvault set-policy -n {vault} -g {rg} --object-id {des1_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        self.cmd(
            "keyvault set-policy -n {vault} -g {rg} --object-id {des2_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        self.cmd(
            "keyvault set-policy -n {vault} -g {rg} --object-id {des3_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --image OpenLogic:CentOS:7.5:latest --os-disk-encryption-set {des1} --data-disk-sizes-gb 10 10 --data-disk-encryption-sets {des2} {des3} --nsg-rule NONE --admin-username azureuser --admin-password testPassword0 --authentication-type password"
        )
        self.cmd(
            "vm show -g {rg} -n {vm2}",
            checks=[
                self.check_pattern(
                    "storageProfile.osDisk.managedDisk.diskEncryptionSet.id", self.kwargs["des1_pattern"]
                ),
                self.check_pattern(
                    "storageProfile.dataDisks[0].managedDisk.diskEncryptionSet.id", self.kwargs["des2_pattern"]
                ),
                self.check_pattern(
                    "storageProfile.dataDisks[1].managedDisk.diskEncryptionSet.id", self.kwargs["des3_pattern"]
                ),
            ],
        )

        self.cmd(
            'vmss create -g {rg} -n {vmss} --image OpenLogic:CentOS:7.5:latest --os-disk-encryption-set {des1} --data-disk-sizes-gb 10 10 --data-disk-encryption-sets {des2} {des3} --admin-username azureuser --admin-password testPassword0 --authentication-type password --nsg ""'
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss}",
            checks=[
                self.check_pattern(
                    "virtualMachineProfile.storageProfile.osDisk.managedDisk.diskEncryptionSet.id",
                    self.kwargs["des1_pattern"],
                ),
                self.check_pattern(
                    "virtualMachineProfile.storageProfile.dataDisks[0].managedDisk.diskEncryptionSet.id",
                    self.kwargs["des2_pattern"],
                ),
                self.check_pattern(
                    "virtualMachineProfile.storageProfile.dataDisks[1].managedDisk.diskEncryptionSet.id",
    ...
    def test_disk_encryption_set_identity(self, resource_group, key_vault):
        ...
        self.kwargs.update({"federated_client_id": federated_client_id, "vault_id": vault_id, "kid": kid})
        ...
        des3_principalId = self.cmd(
            "disk-encryption-set create -g {rg} -n {des3} --key-url {kid} --source-vault {vault} "
            "--mi-system-assigned --encryption-type EncryptionAtRestWithCustomerKey",
            checks=[
        ...
        self.kwargs.update({"des1_principalId": des1_principalId, "des3_principalId": des3_principalId})
        ...
        self.cmd(
            "disk-encryption-set update -g {rg} -n {des1} --key-url {kid} --source-vault {vault} "
            "--federated-client-id None",
            checks=[self.check("federatedClientId", "None")],
        )

        # update federated client id of disk encryption set
        self.cmd(
            "disk-encryption-set update -g {rg} -n {des1} --key-url {kid} --source-vault {vault} "
            "--federated-client-id {federated_client_id}",
            checks=[self.check("federatedClientId", "{federated_client_id}")],
        )

        # remove user assigned identity from an existing disk encryption set
        self.cmd(
            "disk-encryption-set identity remove -g {rg} -n {des1} --user-assigned {identity1_id} --yes",
            checks=[self.check("type", "SystemAssigned"), self.check("userAssignedIdentities", "None")],
        )
        ...
        self.cmd("disk-encryption-set show -g {rg} -n {des1}", checks=[self.check("identity", "None")])
        ...
        self.cmd("disk-encryption-set show -g {rg} -n {des2}", checks=[self.check("identity", "None")])

        # assign neither system nor user assigned identity (the value of --user-assigned is none)
        # to an existing disk encryption set
        self.cmd(
            "disk-encryption-set identity assign -g {rg} -n {des2} --user-assigned",
            checks=[self.check("type", "SystemAssigned")],
        )

        # assign user assigned identity to an existing disk encryption set
        self.cmd(
            "disk-encryption-set identity assign -g {rg} -n {des1} --user-assigned {identity1_id}",
            checks=[self.check("type", "UserAssigned"), self.check("length(userAssignedIdentities)", 1)],
        )

        # assign system assigned identity to an existing disk encryption set
        self.cmd(
            "disk-encryption-set identity assign -g {rg} -n {des1} --system-assigned",
            checks=[
        ...
        # remove system assigned identity from an existing disk encryption set
        self.cmd(
            "disk-encryption-set identity remove -g {rg} -n {des1} --system-assigned --yes",
            checks=[self.check("type", "UserAssigned"), self.check("length(userAssignedIdentities)", 1)],
        )

        # show managed identities of an existing disk encryption set
        self.cmd(
            "disk-encryption-set identity show -g {rg} -n {des1}",
            checks=[self.check("type", "UserAssigned"), self.check("length(userAssignedIdentities)", 1)],
        )
    ...
    def test_disk_encryption_set_update(self, resource_group, key_vault1, key_vault2):
        ...
        kid1 = self.cmd("keyvault key create -n {key1} --vault {vault1} --protection software").get_output_in_json()[
            "key"
        ]["kid"]
        ...
        kid2 = self.cmd("keyvault key create -n {key2} --vault {vault2} --protection software").get_output_in_json()[
            "key"
        ]["kid"]
        kid3 = self.cmd("keyvault key create -n {key3} --vault {vault3} --protection software").get_output_in_json()[
            "key"
        ]["kid"]

        self.kwargs.update({"kid1": kid1, "vault2_id": vault2_id, "vault3_id": vault3_id, "kid2": kid2, "kid3": kid3})
        ...
        self.kwargs.update({"des_sp_id": des_sp_id, "des_id": des_id})

        self.cmd(
            "keyvault set-policy -n {vault2} -g {rg} --object-id {des_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        time.sleep(30)
        with mock.patch("azure.cli.command_modules.role.custom._gen_guid", side_effect=self.create_guid):
        ...
        self.cmd(
            "disk-encryption-set create -g {rg} -n {des2} --key-url {kid3} --enable-auto-key-rotation true",
            checks=[self.check("activeKey.sourceVault", None)],
        )
        ...
        self.cmd(
            "keyvault set-policy -n {vault3} -g {rg} --object-id {des2_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        self.cmd(
            "disk-encryption-set update -g {rg} -n {des2} --source-vault {vault3}",
            checks=[self.check("activeKey.sourceVault.id", "{vault3_id}")],
        )
        ...
        self.cmd(
            "keyvault set-policy -n {vault2} -g {rg} --object-id {des2_sp_id} --key-permissions wrapKey unwrapKey get"
        )
        self.cmd(
            "disk-encryption-set update -g {rg} -n {des2} --key-url {kid2}",
            checks=[self.check("activeKey.keyUrl", "{kid2}"), self.check("activeKey.sourceVault", None)],
        )
        self.cmd(
            "keyvault set-policy -n {vault3} -g {rg} --object-id {des2_sp_id} --key-permissions wrapKey unwrapKey get"
        )
    ...
    def test_disk_encryption_set_disk_update(self, resource_group, key_vault):
        ...
        self.kwargs.update({"des1_pattern": ".*/{}$".format(self.kwargs["des1"])})
        ...
        self.cmd(
            "disk create -g {rg} -n {disk2} --source {disk} --encryption-type EncryptionAtRestWithPlatformKey",
            checks=[
        ...
        self.cmd(
            "disk update -g {rg} -n {disk} --encryption-type EncryptionAtRestWithPlatformKey",
            checks=[
                self.check("encryption.diskEncryptionSetId", "None"),
                self.check("encryption.type", "EncryptionAtRestWithPlatformKey"),
            ],
    ...
    def test_disk_encryption_set_snapshot(self, resource_group, key_vault):
        ...
        self.kwargs.update(
            {"des1_pattern": ".*/{}$".format(self.kwargs["des1"]), "des2_pattern": ".*/{}$".format(self.kwargs["des2"])}
        )
        ...
        self.cmd(
            ...
            checks=[self.check_pattern("encryption.diskEncryptionSetId", self.kwargs["des2_pattern"])],
        )

        self.cmd("disk-encryption-set list-associated-resources -g {rg} -n {des2}", checks=[self.check("length(@)", 1)])
    ...
    def test_disk_encryption_set_double_encryption(self, resource_group):
        ...
        self.cmd(
            "disk create -g {rg} -n {disk1} --disk-encryption-set {des1} --size-gb 10",
            checks=[self.check("encryption.type", "EncryptionAtRestWithPlatformAndCustomerKeys")],
        )

        self.cmd(
            "vm create -g {rg} -n {vm1} --image OpenLogic:CentOS:7.5:latest --os-disk-encryption-set {des1} --nsg-rule NONE --admin-username azureuser --admin-password testPassword0 --authentication-type password"
        )

        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image OpenLogic:CentOS:7.5:latest --os-disk-encryption-set {des1} --admin-username azureuser --admin-password testPassword0 --authentication-type password"
        )
    ...
    def test_confidential_disk_encryption_set(self, resource_group):
        ...
        self.cmd(
            "keyvault create --name {vault} -g {rg} --sku Premium --enable-purge-protection true --retention-days 7 --enable-rbac-authorization false"
        )
        vault_id = self.cmd("keyvault show -g {rg} -n {vault}").get_output_in_json()["id"]
        kid = self.cmd(
            'keyvault key create --vault-name {vault} --name {key} --ops wrapKey unwrapKey --kty RSA-HSM --size 3072 --exportable true --policy "{policy_path}"'
        ...
        self.cmd(
            ...
            checks=[self.check("encryptionType", "ConfidentialVmEncryptedWithCustomerKey")],
        )
        des_show_output = self.cmd("disk-encryption-set show -g {rg} -n {des}").get_output_in_json()
        des_sp_id = des_show_output["identity"]["principalId"]
        des_id = des_show_output["id"]
    ...
    def test_os_disk_security_encryption(self, resource_group):
        ...
        vault_id = self.cmd(
            "keyvault create -g {rg} -n {vault} --enable-purge-protection true --retention-days 7 --enable-rbac-authorization false"
        ).get_output_in_json()["id"]
        kid = self.cmd("keyvault key create -n {key} --vault {vault} --protection software").get_output_in_json()[
            "key"
        ]["kid"]
        ...
        self.cmd(
            ...
            checks=[
                self.check(
                    "storageProfile.osDisk.managedDisk.securityProfile.securityEncryptionType", "DiskWithVMGuestState"
                ),
    ...
    def test_os_disk_security_encryption_vmss(self, resource_group):
        ...
        vault_id = self.cmd(
            "keyvault create -g {rg} -n {vault} --enable-purge-protection true --retention-days 7"
        ).get_output_in_json()["id"]
        kid = self.cmd("keyvault key create -n {key} --vault {vault} --protection software").get_output_in_json()[
            "key"
        ]["kid"]
        ...
        self.kwargs["shared_gallery_image_version"] = self.cmd(
            "sig image-version show-shared --gallery-image-definition {image} --gallery-unique-name {unique_name} --location CentralUSEUAP --gallery-image-version {version}"
        ).get_output_in_json()["uniqueId"]
        ...
        self.cmd(
            ...
            checks=[
                self.check(
                    "virtualMachineProfile.storageProfile.osDisk.managedDisk.securityProfile.securityEncryptionType",
    ...
    def test_disk_controller_type(self, resource_group):
        ...
        self.cmd("network public-ip create --name {pubip1} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create --disk-controller-type SCSI -n {vm} -g {rg} --attach-os-disk {vm_disk} --os-type linux "
        ...
        self.cmd("vm show -n {vm} -g {rg}", checks=[self.check("storageProfile.diskControllerType", "SCSI")])

        # Create a public IP resource with service tag
        self.cmd("network public-ip create --name {pubip2} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm1} --image CANONICAL:UBUNTUSERVER:18_04-LTS-GEN2:latest --storage-sku standard_lrs "
        ...
        self.cmd("vm show -n {vm1} -g {rg}", checks=[self.check("storageProfile.diskControllerType", "SCSI")])

        # Create a public IP resource with service tag
        self.cmd("network public-ip create --name {pubip3} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm2} --image CANONICAL:UBUNTUSERVER:18_04-LTS-GEN2:latest --storage-sku standard_lrs "
        ...
        self.cmd("network nsg create -g {rg} -n {nsg}")
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image {image} --disk-controller-type scsi --admin-username sdk-test-admin --admin-password testPassword001! --vm-sku Standard_E2bs_v5 --nsg {nsg}",
            checks=[self.check("vmss.virtualMachineProfile.storageProfile.diskControllerType", "SCSI")],
        )

        # Create a public IP resource with service tag
        self.cmd("network public-ip create --name {pubip4} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image {image} --admin-username sdk-test-admin --admin-password testPassword001! --vm-sku Standard_E2bs_v5 --orchestration-mode Flexible --public-ip-address {pubip4} --nsg {nsg}"
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --disk-controller-type scsi",
            checks=[self.check("virtualMachineProfile.storageProfile.diskControllerType", "SCSI")],
        )
    ...
    def test_vm_vmss_non_persisted_tpm(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --image Canonical:0001-com-ubuntu-confidential-vm-jammy:22_04-lts-cvm:latest --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE "
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "storageProfile.osDisk.managedDisk.securityProfile.securityEncryptionType", "NonPersistedTPM"
                ),
            ],
        )
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Canonical:0001-com-ubuntu-confidential-vm-jammy:22_04-lts-cvm:latest --orchestration-mode Flexible "
            "--vm-sku Standard_DC2es_v5 --enable-vtpm --enable-secure-boot --os-disk-security-encryption-type NonPersistedTPM --security-type ConfidentialVM --admin-password azuretest01!! --storage-sku StandardSSD_LRS",
            checks=[
                self.check("vmss.virtualMachineProfile.securityProfile.securityType", "ConfidentialVM"),
                self.check("vmss.virtualMachineProfile.securityProfile.uefiSettings.vTpmEnabled", True),
                self.check(
                    "vmss.virtualMachineProfile.storageProfile.osDisk.managedDisk.securityProfile.securityEncryptionType",
                    "NonPersistedTPM",
                ),
            ],
        )
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message):
            self.cmd(
                "vm create -g {rg} -n {vm1} --image Canonical:0001-com-ubuntu-confidential-vm-jammy:22_04-lts-cvm:latest --size Standard_DC2es_v5 --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE "
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message):
            self.cmd(
                "vm create -g {rg} -n {vm1} --image Canonical:0001-com-ubuntu-confidential-vm-jammy:22_04-lts-cvm:latest --size Standard_DC2es_v5 --enable-vtpm --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE "
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message):
            self.cmd(
                "vmss create -g {rg} -n {vmss1} --image Canonical:0001-com-ubuntu-confidential-vm-jammy:22_04-lts-cvm:latest --vm-sku Standard_DC2es_v5 --orchestration-mode Flexible "
                "--os-disk-security-encryption-type NonPersistedTPM --security-type ConfidentialVM --admin-password azuretest01!! --storage-sku StandardSSD_LRS"
            )
        with self.assertRaisesRegex(ArgumentUsageError, message):
            self.cmd(
...
class DiskAccessTest(ScenarioTest):
    ...
    def test_disk_access(self, resource_group):
        self.kwargs.update(
            {"loc": "centraluseuap", "diskaccess": "mydiskaccess", "disk": "mydisk", "snapshot": "mysnapshot"}
        )
        ...
        self.kwargs["disk_access_id"] = self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{disk}"), self.check("networkAccessPolicy", "AllowPrivate")],
        )
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{snapshot}"), self.check("networkAccessPolicy", "AllowPrivate")],
        )
        ...
        self.cmd("disk-access list -g {rg}", checks=[self.check("length(@)", 0)])
...
class DiskBurstingTest(ScenarioTest):
    ...
    def test_disk_bursting(self, resource_group):
        self.kwargs.update({"disk1": "mydisk1", "disk2": "mydisk2"})

        self.cmd("disk create -g {rg} -n {disk1} --size-gb 1024 --location centraluseuap --enable-bursting")
        self.cmd(
            "disk show -g {rg} -n {disk1}", checks=[self.check("name", "{disk1}"), self.check("burstingEnabled", True)]
        )
        self.cmd("disk create -g {rg} -n {disk2} --size-gb 1024 --location centraluseuap")
        self.cmd(
            "disk show -g {rg} -n {disk2}", checks=[self.check("name", "{disk2}"), self.check("burstingEnabled", None)]
        )
        self.cmd("disk update -g {rg} -n {disk2} --enable-bursting")
        self.cmd(
            "disk show -g {rg} -n {disk2}", checks=[self.check("name", "{disk2}"), self.check("burstingEnabled", True)]
        )
    ...
    def test_performance_plus(self, resource_group):
        self.kwargs.update({"disk": self.create_random_name("disk", 10)})

        self.cmd(
            "disk create -n {disk} -g {rg} --size-gb 530 --performance-plus",
            checks=[self.check("creationData.performancePlus", True)],
        )
...
class VMSSCreateDiskOptionTest(ScenarioTest):
    ...
    def test_vmss_create_disk_iops_mbps(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Debian:debian-10:10:latest --data-disk-sizes-gb 10 10 --data-disk-iops 555 666 "
...
class VMCreateAutoCreateSubnetScenarioTest(ScenarioTest):
    ...
    def test_vm_create_auto_create_subnet(self, resource_group):
        ...
        self.kwargs.update({"loc": "eastus", "vm": "vm-subnet", "subnet": "subnet1", "vnet": "vnet1"})
        ...
        self.cmd(
            "vm create --resource-group {rg} --location {loc} --name {vm} --admin-username ubuntu --image Canonical:UbuntuServer:18.04-LTS:latest "
        ...
        self.cmd(
            "network vnet show --resource-group {rg} --name {vnet}", checks=[self.check("subnets[0].name", "{subnet}")]
        )
...
class VMSSAutomaticRepairsScenarioTest(ScenarioTest):
    ...
    def test_vmss_create_automatic_repairs_with_health_probe(self, resource_group):
        self.kwargs.update({"vmss": "vmss1", "lb": "lb1", "probe": "probe", "lbrule": "lbrule"})

        # Test raise error if not provide health probe or load balance
        with self.assertRaises(ArgumentUsageError):
            self.cmd(
                "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --automatic-repairs-grace-period 30 --admin-username azureuser"
            )
        with self.assertRaises(ArgumentUsageError):
            self.cmd(
                "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --load-balancer {lb} --automatic-repairs-grace-period 30 --admin-username azureuser"
            )
        with self.assertRaises(ArgumentUsageError):
            self.cmd(
                "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --health-probe {probe} --automatic-repairs-grace-period 30 --admin-username azureuser"
            )
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --load-balancer {lb} --health-probe {probe} --automatic-repairs-grace-period 30 "
    ...
    def test_vmss_update_automatic_repairs_with_health_probe(self, resource_group):
        ...
        # Prepare vmss
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username azureuser --orchestration-mode Uniform --lb-sku Standard"
        )
        ...
        self.cmd(
            ...
            checks=[self.check("virtualMachineProfile.networkProfile.healthProbe.id", self.kwargs["probe_id"])],
        )
        self.kwargs["instance_ids"] = " ".join(
    ...
    def test_vmss_update_automatic_repairs_with_health_extension(self, resource_group):
        ...
        _, settings_file = tempfile.mkstemp()
        with open(settings_file, "w") as outfile:
            json.dump({"port": 80, "protocol": "http", "requestPath": "/"}, outfile)
        settings_file = settings_file.replace("\\", "\\\\")
        self.kwargs["settings"] = settings_file
        self.cmd(
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_vmss_create_disbale_automatic_repairs_of_zone_balancing", location="eastus2"
    )
    def test_vmss_create_disbale_automatic_repairs_of_zone_balancing(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image {image} --enable-automatic-repairs false --vm-sku Standard_D1_v2 --load-balancer {lb1} --health-probe {probe1} --orchestration-mode Uniform --admin-username vmtest --admin-password Test123456789# --nsg {nsg} --zones 1 2 3 "
            "--enable-automatic-zone-balancing True --automatic-zone-balancing-strategy Recreate --automatic-zone-balancing-behavior CreateBeforeDelete --location eastus2"
        )
        self.cmd(
            "vmss show -g {rg} -n {vmss1}",
        ...
        with self.assertRaisesRegex(ArgumentUsageError, message):
            self.cmd(
                "vmss create -g {rg} -n {vmss2} --image {image} --enable-automatic-repairs false --automatic-repairs-grace-period 30 --vm-sku Standard_D1_v2 --load-balancer {lb1} --health-probe {probe1} --orchestration-mode Uniform --admin-username vmtest --admin-password Test123456789# --nsg {nsg} --zones 1 2 3 "
                "--enable-automatic-zone-balancing True --automatic-zone-balancing-strategy Recreate --automatic-zone-balancing-behavior CreateBeforeDelete --location eastus2"
            )
...
class SkuProfileTest(ScenarioTest):
    ...
    def test_vmss_create_sku_profile(self, resource_group):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image ubuntu2204 --vm-sku Mix "
    ...
    def test_vmss_create_sku_profile_update(self, resource_group):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image ubuntu2204 --vm-sku Mix "
            + " --skuprofile-vmsizes Standard_DS1_v2 --skuprofile-allocation-strategy CapacityOptimized",
            checks=[
                self.check("vmss.orchestrationMode", "Flexible"),
        ...
        self.cmd(
            "vmss update -n {vmss} -g {rg} "
            + " --skuprofile-vmsizes Standard_DS1_v2 Standard_D2s_v4 --skuprofile-allocation-strategy CapacityOptimized",
            checks=[
    ...
    def test_vmss_create_sku_profile_prioritized(self, resource_group):
        ...
        self.cmd(
            "vmss update -n {vmss} -g {rg} "
            + " --skuprofile-vmsizes Standard_DS1_v2 Standard_D2s_v4 --skuprofile-rank 1 0",
            checks=[
...
class VMCreateNSGRule(ScenarioTest):
    ...
    def test_vm_create_nsg_rule(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --nsg-rule NONE --admin-username azureuser "
            "--admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        self.cmd("network nsg show -g {rg} -n {vm}NSG", checks=[self.check("securityRules", "[]")])
...
class VMSSSetOrchestrationServiceStateScenarioTest(ScenarioTest):
    ...
    def test_vmss_set_orchestration_service_state(self, resource_group):
        self.kwargs.update(
            {"vmss": "vmss1", "lb": "lb1", "probe": "probe", "lbrule": "lbrule", "service_name": "AutomaticRepairs"}
        )
...
class VMSSOrchestrationModeScenarioTest(ScenarioTest):
    ...
    def test_vmss_simple_orchestration_mode(self, resource_group):
        self.kwargs.update({"ppg": "ppg1", "vmss": "vmss1", "vm": "vm1", "subnet": "subnet1", "vnet": "vnet1"})

        self.cmd("ppg create -g {rg} -n {ppg}")
        self.cmd(
            "vmss create -g {rg} -n {vmss} --single-placement-group false "
            "--ppg {ppg} --platform-fault-domain-count 3 --orchestration-mode Flexible",
            checks=[self.check("vmss.singlePlacementGroup", False), self.check("vmss.platformFaultDomainCount", 3)],
        )

        self.cmd(
            "vm create -g {rg} -n {vm} --image OpenLogic:CentOS:7.5:latest --platform-fault-domain 0 --vmss {vmss} "
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=[self.check("platformFaultDomain", 0)])
    ...
    def test_vmss_complex_orchestration_mode(self, resource_group, virtual_network):
        ...
        self.cmd(
            "vmss create -n {vmss0} -g {rg} --single-placement-group false "
        ...
        # test with password
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --single-placement-group false "
        ...
        # test with ssh
        self.cmd(
            "vmss create -n {vmss2} -g {rg} --single-placement-group false "
            "--platform-fault-domain-count 1 --vm-sku Standard_DS1_v2 --instance-count 0 --image Canonical:UbuntuServer:18.04-LTS:latest "
            "--computer-name-prefix testvmss --vnet-name {vnet_name} --subnet default --network-api-version "
        ...
        # test with ssh path
        self.cmd(
            "vmss create -n {vmss3} -g {rg} --single-placement-group false "
            "--platform-fault-domain-count 1 --vm-sku Standard_DS1_v2 --instance-count 0 --image Canonical:UbuntuServer:18.04-LTS:latest "
            "--computer-name-prefix testvmss --vnet-name {vnet_name} --subnet default --network-api-version "
            "2020-11-01 --admin-username testvmss --generate-ssh-keys "
        )
        ...
        # test with ssh value
        self.cmd(
            "vmss create -n {vmss4} -g {rg} --single-placement-group false "
            "--platform-fault-domain-count 1 --vm-sku Standard_DS1_v2 --instance-count 0 --image Canonical:UbuntuServer:18.04-LTS:latest "
            "--computer-name-prefix testvmss --vnet-name {vnet_name} --subnet default --network-api-version "
        ...
        self.cmd(
            "vmss create -n {vmss5} -g {rg} --single-placement-group false "
            "--platform-fault-domain-count 1 --vm-sku Standard_DS1_v2 --instance-count 0 --image Canonical:UbuntuServer:18.04-LTS:latest "
            "--computer-name-prefix testvmss --vnet-name {vnet_name} --subnet default --network-api-version "
    ...
    def test_quick_create_flexible_vmss(self, resource_group):
        ...
        with self.assertRaisesRegex(
            ArgumentUsageError, "please specify the --image when you want to specify the VM SKU"
        ):
            self.cmd(
                "vmss create -n {vmss} -g {rg} --platform-fault-domain-count 1 --zones 1 --instance-count 3 --vm-sku Standard_D1_v2"
            )

        self.cmd(
            "vmss create -n {vmss} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username vmtest"
        )
    ...
    def test_create_flexible_vmss_with_priority_mix_policy(self, resource_group):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image OpenLogic:CentOS:7.5:latest --regular-priority-count 4 --regular-priority-percentage 50 --instance-count 2 --image OpenLogic:CentOS:7.5:latest --priority Spot --eviction-policy Deallocate --single-placement-group False",
            checks=[
        ...
        self.cmd(
            "vmss update -n {vmss} -g {rg} --regular-priority-count 2 --regular-priority-percentage 25",
            checks=[
    ...
    def test_create_flexible_vmss_by_default(self, resource_group):
        ...
        self.cmd(
            "vmss create -n {vmss} -g {rg} --image ubuntu2204",
            checks=[self.check("vmss.orchestrationMode", "Flexible"), self.check("vmss.upgradePolicy.mode", "Manual")],
        )
        self.cmd("network lb inbound-nat-pool list -g {rg} --lb-name {vmss}LB", checks=[self.check("length(@)", 0)])
        self.cmd(
            "network lb inbound-nat-rule list -g {rg} --lb-name {vmss}LB",
            checks=[
                self.check("length(@)", 1),
...
class VMCrossTenantUpdateScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_vm_cross_tenant_",
        location="westus2",
        parameter_name="another_resource_group",
        subscription=AUX_SUBSCRIPTION,
    )
    def test_vm_cross_tenant_update(self, resource_group, another_resource_group):
        ...
        # Create a public IP resource with service tag
        self.cmd(
            "network public-ip create --name {pubip} -g {another_rg} --subscription {aux_sub} --ip-tags FirstPartyUsage=/NonProd"
        )
        # Prepare sig in another tenant
        self.cmd(
            "vm create -g {another_rg} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username clitest1 "
        ...
        self.cmd(
            "vm update -g {rg} -n {vm} --set tags.tagName=tagValue", checks=[self.check("tags.tagName", "tagValue")]
        )
        # Disable default outbound access
        self.cmd("network vnet subnet update -g {rg} --vnet-name {vnet2} -n {subnet2} --default-outbound-access false")
...
class VMSSCrossTenantUpdateScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_vmss_update_",
        location="westus2",
        parameter_name="another_resource_group",
        subscription=AUX_SUBSCRIPTION,
    )
    def test_vmss_cross_tenant_update(self, resource_group, another_resource_group):
        ...
        self.cmd(
            "sig create -g {another_rg} --gallery-name {sig_name} --subscription {aux_sub}",
        ...
        res1 = self.cmd(
            ...
            checks=self.check("name", self.kwargs["image_definition_name_1"]),
        ).get_output_in_json()
        self.cmd(
        ...
        res2 = self.cmd(
            ...
            checks=self.check("name", self.kwargs["image_definition_name_2"]),
        ).get_output_in_json()
        self.cmd(
        ...
        self.kwargs.update({"image_1_id": res1["id"], "image_2_id": res2["id"]})

        # Create a public IP resource with service tag
        self.cmd("network public-ip create --name {pubip2} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image {image_1_id} --orchestration-mode Uniform --lb-sku Standard --public-ip-address {pubip2}"
        )
        self.cmd(
            "vmss show --name {vmss} -g {rg}",
            checks=[
...
class VMCreateFromACGToOtherSubScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_vm_create_from_acg_image_to_other_sub",
        parameter_name="another_resource_group",
        subscription=AUX_SUBSCRIPTION,
    )
    def test_vm_create_from_acg_image_to_other_sub(self, resource_group, another_resource_group):
        ...
        # Create a public IP resource with service tag
        self.cmd(
            "network public-ip create --name {pubip} -g {rg2} --subscription {sub_id} --ip-tags FirstPartyUsage=/NonProd"
        )
        self.cmd(
            "vm create -g {rg2} -n {vm} --image Canonical:UbuntuServer:18.04-LTS:latest --subscription {sub_id} "
            "--public-ip-sku Standard --subnet {subnet} --vnet-name {vnet} --public-ip-address {pubip} --nsg-rule NONE"
        )
        # Disable default outbound access
        self.cmd(
            "network vnet subnet update -g {rg2} --subscription {sub_id} --vnet-name {vnet} -n {subnet} --default-outbound-access false"
        )
        ...
        self.cmd(
            "sig image-definition create -g {rg2} --gallery-name {sig_name} --gallery-image-definition {image_def} "
            "--os-type linux -p publisher1 -f offer1 -s sku1 --subscription {sub_id} --hyper-v-generation v1"
        )
        res = self.cmd(
            "sig image-version create -g {rg2} --gallery-name {sig_name} --gallery-image-definition {image_def} --gallery-image-version {version} --managed-image {image_name} --replica-count 1 --subscription {sub_id}"
        ).get_output_in_json()
        self.kwargs.update({"image_version": res["id"]})

        # Create a public IP resource with service tag
        self.cmd("network public-ip create --name {pubip2} -g {rg} --ip-tags FirstPartyUsage=/NonProd")
        self.cmd(
            "vm create -g {rg} -n {vm2} --image {image_version} --public-ip-sku Standard "
            "--public-ip-address {pubip2} --vnet-name {vnet2} --subnet {subnet2} --nsg-rule NONE"
        )
        # Disable default outbound access
        self.cmd("network vnet subnet update -g {rg} --vnet-name {vnet2} -n {subnet2} --default-outbound-access false")

        self.cmd("vm show -g {rg} -n {vm2}", checks=[self.check("storageProfile.imageReference.id", "{image_version}")])
...
class VMPlacementScenarioTest(ScenarioTest):
    ...
    def test_vm_placement(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --image ubuntu2204 --zone-placement-policy any --include-zones 1 3 --align-regional-disks-to-vm-zone True --nsg-rule NONE"
        )
        self.cmd(
            "vm show -g {rg} -n {vm1}",
            checks=[
        ...
        self.cmd(
            "vm update -g {rg} -n {vm1} --align-regional-disks-to-vm-zone False",
            checks=[self.check("storageProfile.alignRegionalDisksToVmZone", False)],
        )
        self.cmd(
            "vm create -g {rg} -n {vm2} --image ubuntu2204 --zone-placement-policy any --exclude-zones 2 --align-regional-disks-to-vm-zone False --nsg-rule NONE"
        )
        self.cmd(
            "vm show -g {rg} -n {vm2}",
            checks=[
        ...
        self.cmd(
            "vm update -g {rg} -n {vm2} --align-regional-disks-to-vm-zone True",
            checks=[self.check("storageProfile.alignRegionalDisksToVmZone", True)],
        )
...
class VMAutoUpdateScenarioTest(ScenarioTest):
    ...
    def test_vm_auto_update(self, resource_group):
        ...
        self.cmd("vm assess-patches -g {rg} -n {vm}", checks=[self.check("status", "Succeeded")])
    ...
    def test_linux_vm_patch_mode(self, resource_group):
        self.kwargs.update({"subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n vm1 --image Canonical:UbuntuServer:18.04-LTS:latest --enable-agent --patch-mode AutomaticByPlatform "
            "--generate-ssh-keys --nsg-rule NONE --admin-username vmtest --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        self.cmd(
            "vm show -g {rg} -n vm1",
            checks=[self.check("osProfile.linuxConfiguration.patchSettings.patchMode", "AutomaticByPlatform")],
        )
        self.cmd("vm assess-patches -g {rg} -n vm1", checks=[self.check("status", "Succeeded")])
...
class VMSSPatchModeScenarioTest(ScenarioTest):
    ...
    def test_vmss_windows_patch_mode(self, resource_group):
        self.kwargs.update({"vmss": self.create_random_name("clitestvmss", 20), "rg": resource_group})

        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Win2022Datacenter --enable-agent --enable-auto-update false --patch-mode Manual --admin-username azureuser --admin-password testPassword0"
        )
    ...
    def test_vmss_linux_patch_mode(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image Canonical:UbuntuServer:18.04-LTS:latest --enable-agent --patch-mode ImageDefault --generate-ssh-keys --instance-count 0 --admin-username vmtest"
        )
    ...
    def test_vmss_security_posture_reference(self, resource_group):
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.securityPostureReference.excludeExtensions[0]",
...
class VMDiskLogicalSectorSize(ScenarioTest):
    ...
    def test_vm_disk_logical_sector_size(self, resource_group):
        self.cmd(
            ...
            checks=[self.check("creationData.logicalSectorSize", 4096)],
        )
        self.cmd("disk create -g {rg} -n d2 --size-gb 10 --tier P4", checks=[self.check("tier", "P4")])
...
class VMSSReimageScenarioTest(ScenarioTest):
    ...
    def test_vmss_reimage(self, resource_group):
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss} --image OpenLogic:CentOS:7.5:latest --admin-username vmtest --instance-count 2 --orchestration-mode Uniform --lb-sku Standard"
        )
        instances = self.cmd("vmss list-instances -g {rg} -n {vmss}").get_output_in_json()
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_vmss_reimage_force_update_os_disk_for_ephemeral", location="eastus2euap"
    )
    def test_vmss_reimage_force_update_os_disk_for_ephemeral(self, resource_group):
        self.kwargs.update({"vmss": self.create_random_name("vmss", 10)})
        self.cmd(
            "vmss create --resource-group {rg} --name {vmss} --image OpenLogic:CentOS:7.5:latest --ephemeral-os-disk --disable-overprovision --lb-sku Standard "
        ...
        before_reimage_os_disk_name = self.cmd("vmss list-instances -g {rg} -n {vmss}").get_output_in_json()[0][
            "storageProfile"
        ]["osDisk"]["name"]

        self.cmd("vmss reimage -g {rg} -n {vmss}")
        after_reimage_os_disk_name = self.cmd("vmss list-instances -g {rg} -n {vmss}").get_output_in_json()[0][
            "storageProfile"
        ]["osDisk"]["name"]
        assert before_reimage_os_disk_name == after_reimage_os_disk_name

        self.cmd("vmss reimage -g {rg} -n {vmss} --update-os-disk")
        after_reimage_force_update_os_disk_name = self.cmd(
            "vmss list-instances -g {rg} -n {vmss}"
        ).get_output_in_json()[0]["storageProfile"]["osDisk"]["name"]
...
class VMSSHKeyScenarioTest(ScenarioTest):
    ...
    def test_vm_ssh_key(self, resource_group):
        ...
        self.cmd("sshkey show -g {rg} -n k1", checks=[self.check("name", "k1"), self.exists("publicKey")])
        self.cmd('sshkey create -g {rg} -n k2 --public-key "{key}"')
        self.cmd('sshkey update -g {rg} -n k2 --public-key "{updated-key}"')

        self.cmd("sshkey list -g {rg}", checks=[self.check("length(@)", 2)])
        self.cmd("sshkey delete -g {rg} -n k2 --yes")
        self.cmd("sshkey list -g {rg}", checks=[self.check("length(@)", 1)])

        # Use existing key
        self.cmd(
            "vm create -g {rg} -n vm1 --image OpenLogic:CentOS:7.5:latest --nsg-rule None --ssh-key-name k1 --admin-username vmtest --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        # Create new one
        self.cmd(
            "vm create -g {rg} -n vm3 --image OpenLogic:CentOS:7.5:latest --nsg-rule None --ssh-key-name k3 --generate-ssh-keys --admin-username vmtest --subnet {subnet} --vnet-name {vnet}"
        )
    ...
    def test_vm_ssh_key_ed25519(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --image OpenLogic:CentOS:7.5:latest --nsg-rule None --ssh-key-name {key_name} --admin-username vmtest --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --image OpenLogic:CentOS:7.5:latest --nsg-rule None --ssh-key-value '{ssh_key}' --admin-username vmtest2 --subnet {subnet} --vnet-name {vnet}"
        )

        self.cmd(
            "vm create -g {rg} -n {vm3} --image OpenLogic:CentOS:7.5:latest --nsg-rule None --generate-ssh-keys --ssh-key-type ed25519 --admin-username vmtest3 --subnet {subnet} --vnet-name {vnet}"
        )
...
class VMInstallPatchesScenarioTest(ScenarioTest):
    ...
    def test_vm_install_patches(self, resource_group):
        ...
        # Create a windows vm
        self.cmd(
            "vm create -g {rg} -n vm -l westus --image MicrosoftWindowsServer:WindowsServer:2022-Datacenter-Azure-Edition-Hotpatch:latest "
        ...
        # Create a linux vm
        self.cmd(
            "vm create -g {rg} -n vm2 --image Ubuntu2204 -l westus --admin-username azureuser --admin-password Password123! "
            "--subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        ...
        with self.assertRaisesRegex(
            HttpResponseError,
            "The entry in 'kbNumbersToInclude' with value '1111' is not valid. It must be a KBID such as",
        ):
            self.cmd(
                "vm install-patches -g {rg} -n vm --maximum-duration PT4H --reboot-setting Never --classifications-to-include-win Critical Security "
...
class VMTrustedLaunchScenarioTest(ScenarioTest):
    ...
    def test_vm_trusted_launch(self, resource_group):
        self.kwargs.update({"vm1": "vm1", "vm2": "vm2", "vm3": "vm3", "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm1} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --security-type TrustedLaunch "
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --image OpenLogic:CentOS:7_6-gen2:latest --admin-username azureuser --admin-password testPassword0 --subnet {subnet} --vnet-name {vnet} --nsg-rule None"
        )
        self.cmd("vm show -g {rg} -n {vm2}", checks=[self.check("securityProfile", "None")])
        ...
        self.cmd(
            "vm create -g {rg} -n {vm3} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest "
            "--admin-username clitest1 --generate-ssh-key --security-type Standard --subnet {subnet} --vnet-name {vnet} --nsg-rule None"
        )
        self.cmd("vm show -g {rg} -n {vm3}", checks=[self.check("securityProfile", "None")])
    ...
    def test_vm_trusted_launch_update(self, resource_group):
        self.kwargs.update({"subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n vm --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --security-type TrustedLaunch "
    ...
    def test_disk_trusted_launch(self):
        ...
        self.kwargs["disk_id"] = self.cmd(
            "disk create -g {rg} -n {disk} --image-reference Canonical:0001-com-ubuntu-server-jammy:22_04-lts-gen2:latest --hyper-v-generation V2 --security-type TrustedLaunch",
            checks=[self.check("securityProfile.securityType", "TrustedLaunch")],
        ).get_output_in_json()["id"]

        self.cmd("disk show -g {rg} -n {disk}", checks=[self.check("securityProfile.securityType", "TrustedLaunch")])

        self.cmd(
            "snapshot create -g {rg} -n {snapshot} --source {disk_id}",
            checks=[self.check("securityProfile.securityType", "TrustedLaunch")],
        )

        self.cmd(
            "snapshot show -g {rg} -n {snapshot}", checks=[self.check("securityProfile.securityType", "TrustedLaunch")]
        )

        self.cmd(
            "vm create -g {rg} -n {vm} --size Standard_B2ms --attach-os-disk {disk} --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE --os-type linux"
        )
    ...
    def test_vm_trusted_launch_os_disk_secure_import(self):
        ...
        blob_uri = self.cmd(
            "vm show -g {rg} -n {vm}", checks=self.check("length(storageProfile.dataDisks)", 0)
        ).get_output_in_json()["storageProfile"]["osDisk"]["vhd"]["uri"]
        self.kwargs.update(
            {"blob_uri": blob_uri, "vhd_uri": blob_uri[0 : blob_uri.rindex("/") + 1] + "seccuredisk.vhd"}
        )
    ...
    def test_vm_trusted_launch_os_disk_secure_upload(self):
        ...
        self.cmd(
            "disk grant-access -n {disk} -g {rg} --access-level Write --duration-in-seconds 86400 --secure-vm-guest-state-sas",
            checks=[
        ...
        self.cmd(
            "disk grant-access -n {disk2} -g {rg} --access-level Write --duration-in-seconds 86400",
            checks=[
                self.exists("accessSAS"),
            ],
        )
    ...
    def test_vmss_trusted(self, resource_group):
        self.cmd(
            "vmss create -g {rg} -n vm --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --security-type TrustedLaunch --admin-username azureuser --admin-password testPassword0"
        )
    ...
    def test_guest_attestation_extension_and_msi(self, resource_group):
        ...
        self.cmd(
            "vm create --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --security-type TrustedLaunch "
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --admin-username azureuser "
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "virtualMachineProfile.extensionProfile.extensions[0].publisher",
        ...
        self.cmd(
            "vmss list-instances -n {vmss1} -g {rg}",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --admin-username azureuser "
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss3} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest "
    ...
    def test_enable_trusted_launch_on_v1(self, resource_group):
        self.kwargs.update({"vm": self.create_random_name("vm1", 10), "subnet": "subnet1", "vnet": "vnet1"})
        self.cmd(
            "vm create -g {rg} -n {vm} --image MicrosoftWindowsServer:WindowsServer:2022-datacenter-smalldisk:latest "
            "--admin-password testPassword0 --authentication-type password --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        )
        ...
        self.cmd("vm show -g {rg} -n {vm}", checks=[self.check("securityProfile", None)])
        self.cmd("disk list -g {rg}", checks=[self.check("[0].hyperVGeneration", "V1")])
        self.cmd("vm deallocate -g {rg} -n {vm}")
        self.cmd(
            "vm update -g {rg} -n {vm} --security-type TrustedLaunch",
            checks=[
                self.check("securityProfile.securityType", "TrustedLaunch"),
                self.check("securityProfile.uefiSettings.secureBootEnabled", True),
                self.check("securityProfile.uefiSettings.vTpmEnabled", True),
    ...
    def test_enable_trusted_launch_on_v2(self, resource_group):
        ...
        self.cmd(
            ...
            checks=[self.check("hyperVGeneration", "V2")],
        )
        self.cmd(
            "vm create -g {rg} -n {vm1} --size Standard_B2ms --attach-os-disk {disk1} --nsg-rule NONE --os-type windows --subnet {subnet} --vnet-name {vnet}"
        )
        ...
        self.cmd(
            "vm update -g {rg} -n {vm1} --security-type TrustedLaunch",
            checks=[
                self.check("securityProfile.securityType", "TrustedLaunch"),
                self.check("securityProfile.uefiSettings.secureBootEnabled", True),
                self.check("securityProfile.uefiSettings.vTpmEnabled", True),
        ...
        self.cmd("vm show -g {rg} -n {vm2}", checks=[self.check("securityProfile.securityType", "ConfidentialVM")])
    ...
    def test_vm_vmss_extension_autoupgrade(self, resource_group):
        ...
        self.cmd(
            "vm create --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --security-type TrustedLaunch --subnet {subnet} --vnet-name {vnet} "
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --orchestration-mode Flexible "
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image canonical:0001-com-ubuntu-server-focal:20_04-lts-gen2:latest --admin-username azureuser --security-type TrustedLaunch "
    ...
    @AllowLargeResponse(size_kb=99999)
    @ResourceGroupPreparer(name_prefix="cli_vm_vmss_proxy_agent_", location="eastus2")
    def test_vm_vmss_proxy_agent(self, resource_group):
        ...
        self.cmd(
            "vm create -g {rg} -n {vm1} --image Ubuntu2204 --enable-proxy-agent --wire-server-mode Audit --imds-mode Audit --key-incarnation-id 1 --size Standard_D2s_v3 --subnet {subnet} --vnet-name {vnet} --admin-username azureuser --generate-ssh-keys --nsg-rule NONE --add-proxy-agent-extension true"
        )
        ...
        self.cmd(
            "vm update -g {rg} -n {vm1} --enable-proxy-agent False --wire-server-mode Enforce --imds-mode Enforce --key-incarnation-id 2",
            checks=[
        ...
        self.cmd(
            "vm create -g {rg} -n {vm2} --image Win2022Datacenter --size Standard_D2s_v3 --subnet {subnet} --vnet-name {vnet} --admin-password Password001! --nsg-rule NONE"
        )
        self.cmd(
            "vm update -g {rg} -n {vm2} --enable-proxy-agent True --wire-server-mode Enforce --imds-mode Enforce --key-incarnation-id 1"
        )
        self.cmd(
            "vm show -g {rg} -n {vm2}",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Ubuntu2204 --nsg {nsg1} --enable-proxy-agent --wire-server-mode Audit --imds-mode Audit --vm-sku Standard_D2s_v3 --orchestration-mode Flexible --admin-username azureuser --generate-ssh-keys --add-proxy-agent-extension true",
            checks=[
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --enable-proxy-agent False --wire-server-mode Enforce --imds-mode Enforce",
            checks=[
        ...
        self.cmd(
            "vmss create -g {rg} -n {vmss2} --image Win2022Datacenter --nsg {nsg2} --vm-sku Standard_D2s_v3 --orchestration-mode Flexible --admin-password Password001!"
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss2} --enable-proxy-agent True --wire-server-mode Audit --imds-mode Audit",
            checks=[
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss2} --enable-proxy-agent False",
            checks=[
                self.check("virtualMachineProfile.securityProfile.proxyAgentSettings.enabled", False),
                self.check("virtualMachineProfile.securityProfile.proxyAgentSettings.wireServer.mode", "Audit"),
                self.check("virtualMachineProfile.securityProfile.proxyAgentSettings.imds.mode", "Audit"),
    ...
    def test_vm_vmss_proxy_agent_control_profile_reference(self, resource_group):
        ...
        self.cmd("network nsg create -g {rg} -n {nsg1}")
        self.cmd(
            "vm create -g {rg} -n {vm1} --image Win2022Datacenter --enable-proxy-agent --wire-server-mode Audit --imds-mode Audit --key-incarnation-id 1 --size Standard_D2s_v3 --subnet {subnet} --vnet-name {vnet} --admin-password Password001! --nsg-rule NONE"
        )
        # Disable default outbound access
        self.cmd("network vnet subnet update -g {rg} --vnet-name {vnet} -n {subnet} --default-outbound-access false")
        ...
        self.cmd("sig create -g {rg} --gallery-name {gallery}")
        self.cmd(
            "sig in-vm-access-control-profile create -g {rg} --gallery-name {gallery} --name {profile1} --os-type Windows --applicable-host-endpoint WireServer"
        )
        wireseriver_prifile = self.cmd(
            "sig in-vm-access-control-profile-version create -g {rg}  --gallery-name {gallery} --profile-name {profile1} --profile-version 1.0.0 --default-access Allow --mode Audit"
        ).get_output_in_json()
        self.cmd(
            "az sig in-vm-access-control-profile create -g {rg} --gallery-name {gallery} --name {profile2} --os-type Windows --applicable-host-endpoint IMDS"
        )
        imds_prifile = self.cmd(
            "sig in-vm-access-control-profile-version create -g {rg}  --gallery-name {gallery} --profile-name {profile2} --profile-version 1.0.0 --default-access Allow --mode Audit"
        ).get_output_in_json()

        self.kwargs.update({"wireserver_profileid": wireseriver_prifile["id"], "imds_profileid": imds_prifile["id"]})
        self.cmd(
            "vm update -g {rg} -n {vm1} --wire-server-profile-id {wireserver_profileid}",
            checks=[
                self.check(
        ...
        self.cmd(
            "vm update -g {rg} -n {vm1} --imds-profile-id {imds_profileid}",
            checks=[
                self.check(
                    "securityProfile.proxyAgentSettings.wireServer.inVmAccessControlProfileReferenceId",
                    "{wireserver_profileid}",
                ...
                self.check(
                    "securityProfile.proxyAgentSettings.imds.inVmAccessControlProfileReferenceId", "{imds_profileid}"
                ),
            ],
        )

        self.cmd(
            "vmss create -g {rg} -n {vmss1} --image Win2022Datacenter --nsg {nsg1} --enable-proxy-agent --wire-server-mode Audit --imds-mode Audit --vm-sku Standard_D2s_v3 --orchestration-mode Flexible --admin-password Password001!",
            checks=[
                self.check("vmss.virtualMachineProfile.securityProfile.proxyAgentSettings.enabled", True),
                self.check("vmss.virtualMachineProfile.securityProfile.proxyAgentSettings.wireServer.mode", "Audit"),
                self.check("vmss.virtualMachineProfile.securityProfile.proxyAgentSettings.imds.mode", "Audit"),
        ...
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --wire-server-profile-id {wireserver_profileid}",
            checks=[
                self.check(
                    "virtualMachineProfile.securityProfile.proxyAgentSettings.wireServer.inVmAccessControlProfileReferenceId",
                    "{wireserver_profileid}",
                )
            ],
        )
        self.cmd(
            "vmss update -g {rg} -n {vmss1} --imds-profile-id {imds_profileid}",
            checks=[
                self.check(
                    "virtualMachineProfile.securityProfile.proxyAgentSettings.imds.inVmAccessControlProfileReferenceId",
                    "{imds_profileid}",
                ),
                self.check(
                    "virtualMachineProfile.securityProfile.proxyAgentSettings.wireServer.inVmAccessControlProfileReferenceId",
...
class DiskHibernationScenarioTest(ScenarioTest):
    ...
    def test_disk_hibernation(self):
        self.cmd(
            ...
            checks=[self.check("supportsHibernation", True)],
        )
    ...
    def test_disk_data_access_auth_mode(self):
        self.kwargs.update(
            {"disk": self.create_random_name("disk-", 10), "disk1": self.create_random_name("disk-", 10)}
        )
        self.cmd(
            "disk create -g {rg} -n {disk} --size-gb 10 --data-access-auth-mode AzureActiveDirectory",
            checks=[self.check("dataAccessAuthMode", "AzureActiveDirectory")],
        )
        self.cmd(
            "disk create -g {rg} -n {disk1} --size-gb 10 --data-access-auth-mode None",
            checks=[self.check("dataAccessAuthMode", "None")],
        )
...
class VMCreateCountScenarioTest(ScenarioTest):
    ...
    def test_vm_create_count(self, resource_group):
        ...
        self.cmd(
            "az network vnet subnet create -g {rg} --vnet-name vnet -n subnet1 --address-prefixes 10.0.0.0/24 --default-outbound false"
        )
        self.cmd(
            "az network vnet subnet create -g {rg} --vnet-name vnet -n subnet2 --address-prefixes 10.0.1.0/24 --default-outbound false"
        )
        self.cmd(
            "vm create -g {rg} -n vma --image Canonical:UbuntuServer:18.04-LTS:latest --count 3 --subnet {subnet} --vnet-name {vnet} --nsg-rule None "
        ...
        self.cmd(
            "vm create -g {rg} -n vmb --image Canonical:UbuntuServer:18.04-LTS:latest --count 3 --subnet {subnet} --vnet-name {vnet} --nsg-rule None --generate-ssh-keys --admin-username vmtest"
        )
        self.cmd("vm list -g {rg}", checks=[self.check("length(@)", 6)])
...
class VMListFilterScenarioTest(ScenarioTest):
    ...
    def test_vm_list_filter(self, resource_group):
        self.kwargs.update({"vmss_flex": self.create_random_name("vmss", 10)})

        self.cmd(
            "vmss create -g {rg} -n {vmss_flex} --admin-username vmtest --platform-fault-domain-count 2 --image Canonical:UbuntuServer:18.04-LTS:latest --instance-count 2"
        )
        vmss_id = self.cmd("vmss show -g {rg} -n {vmss_flex}").get_output_in_json()["id"]
        self.kwargs.update({"vmss_id": vmss_id})
        time.sleep(600)
        self.cmd("vm list -g {rg} --vmss {vmss_id}", checks=[self.check("length(@)", 2)])
        self.cmd("vm list --vmss {vmss_id}", checks=[self.check("length(@)", 2)])
        self.cmd("vm list -g {rg} --vmss {vmss_flex}", checks=[self.check("length(@)", 2)])
...
class ExtendedLocation(ScenarioTest):
    ...
    def test_vm_extended_location(self, resource_group):
        self.cmd(
            "vm create -g {rg} -n vm --image Canonical:UbuntuServer:18.04-LTS:latest --nsg-rule None --generate-ssh-keys --admin-username vmtest "
    ...
    def test_vmss_extended_location(self, resource_group):
        self.cmd(
            "vmss create -g {rg} -n vmss --image Canonical:UbuntuServer:18.04-LTS:latest --generate-ssh-keys --admin-username vmtest "
...
class DiskZRSScenarioTest(ScenarioTest):
    ...
    def test_disk_zrs(self, resource_group):
        ...
        self.cmd(
            "disk create -g {rg} -n {disk} --size-gb 1024 --sku StandardSSD_ZRS --max-shares 3",
            checks=[self.check("sku.name", "StandardSSD_ZRS")],
        )
        self.cmd("disk update -g {rg} -n {disk} --sku Premium_ZRS", checks=[self.check("sku.name", "Premium_ZRS")])
        self.cmd(
            "vm create -g {rg} -n {vm} --image Canonical:UbuntuServer:20_04-lts-gen2:latest --zone 1 --attach-data-disks {disk} "
...
class CapacityReservationScenarioTest(ScenarioTest):
    ...
    def test_capacity_reservation(self, resource_group):
        ...
        self.kwargs["reservation_group_id1"] = self.cmd(
            "capacity reservation group create -n {reservation_group} -g {rg} --tags key=val --zones 1 2",
            checks=[
        ...
        self.cmd(
            "capacity reservation group update -n {reservation_group} -g {rg} --tags key=val key1=val1",
            checks=[self.check("name", "{reservation_group}"), self.check("tags", {"key": "val", "key1": "val1"})],
        )

        self.cmd(
            "capacity reservation group show -n {reservation_group} -g {rg}",
            checks=[
                self.check("name", "{reservation_group}"),
                self.check("zones", ["1", "2"]),
        ...
        self.cmd(
            "capacity reservation group list -g {rg} --query \"[?name=='{reservation_group}']\" ",
            checks=[
        ...
        self.cmd(
            "capacity reservation list -c {reservation_group} -g {rg} --query \"[?name=='{reservation_name}']\" ",
            checks=[
        ...
        self.cmd(
            "capacity reservation group show -n {reservation_group} -g {rg} --instance-view",
            checks=[
                self.check("name", "{reservation_group}"),
                self.check("zones", ["1", "2"]),
                self.check("tags", {"key": "val", "key1": "val1"}),
        ...
        self.kwargs["vm_id"] = self.cmd(
            ...
            checks=[
                self.check("provisioningState", "Succeeded"),
                self.check(
                    "capacityReservation.capacityReservationGroup.id", "{reservation_group_id1}", case_sensitive=False
                ),
            ],
        ).get_output_in_json()["id"]
        ...
        self.cmd(
            ...
            checks=[
                ...
                self.check(
                    "vmss.virtualMachineProfile.capacityReservation.capacityReservationGroup.id",
                    "{reservation_group_id1}",
                    case_sensitive=False,
                ),
            ],
        )

        self.kwargs["vmss_id"] = self.cmd(
            "vmss show -g {rg} -n {vmss} ",
            checks=[
                self.check("provisioningState", "Succeeded"),
                self.check(
                    "virtualMachineProfile.capacityReservation.capacityReservationGroup.id",
                    "{reservation_group_id1}",
                    case_sensitive=False,
                ),
            ],
        ...
        self.kwargs["reservation_group_id2"] = self.cmd(
            "capacity reservation group create -n {reservation_group2} -g {rg} --tags key=val --zones 1 2",
            checks=[
        ...
        self.cmd(
            ...
            checks=[
                self.check("provisioningState", "Succeeded"),
                self.check(
                    "capacityReservation.capacityReservationGroup.id", "{reservation_group_id2}", case_sensitive=False
                ),
            ],
        )
    ...
    def test_capacity_reservation_operations(self, resource_group):
        ...
        self.cmd(
            "capacity reservation create -c {reservation_group} -n {reservation_name} -g {rg} "
        ...
        self.cmd(
            "capacity reservation update -c {reservation_group} -n {reservation_name} -g {rg} "
        ...
        self.cmd(
            "capacity reservation show -c {reservation_group} -n {reservation_name} -g {rg}",
            checks=[
                self.check("name", "{reservation_name}"),
                self.check("sku.name", "{sku}"),
                self.check("sku.capacity", 6),
    ...
    def test_capacity_reservation_sharing_profile(self, resource_group):
        ...
        self.cmd(
            "capacity reservation group create -n {reservation_group_1} -g {rg} --sharing-profile {sub_id} subscriptions/00000000-0000-0000-0000-000000000000 -l westEurope"
        )
        ...
        self.cmd(
            "capacity reservation group create -n {reservation_group_2} -g {rg} --sharing-profile {sub_id} 00000000-0000-0000-0000-000000000000 -l westEurope"
        )
...
class VMVMSSAddApplicationTestScenario(ScenarioTest):
    ...
    def test_vm_add_application_empty_version_ids(self, resource_group):
        ...
        # Prepare VM
        self.cmd(
            "vm create -l eastus -g {rg} -n {vm} --image Win2022Datacenter --admin-username clitest1234 --admin-password Test123456789# "
    ...
    def test_vm_add_application(self, resource_group):
        ...
        # Prepare VM
        self.cmd(
            "vm create -l eastus -g {rg} -n {vm} --image Win2022Datacenter --admin-username clitest1234 "
    ...
    def test_vm_add_application_with_config_override(self, resource_group):
        ...
        message = (
            "usage error: --app-config-overrides should have the same number of items as --application-version-ids"
        )
        with self.assertRaisesRegex(ArgumentUsageError, message):
            self.cmd(
    ...
    def test_vmss_add_application_empty_version_ids(self, resource_group):
        ...
        # Prepare VMSS
        self.cmd(
            "vmss create -l eastus -g {rg} -n {vmss} --authentication-type password --admin-username admin123 "
    ...
    def test_vmss_add_application_with_order_application(self, resource_group):
        ...
        # Prepare VMSS
        self.cmd(
            "vmss create -l eastus -g {rg} -n {vmss} --authentication-type password --admin-username admin123 --admin-password PasswordPassword1! --image Win2022Datacenter --orchestration-mode Flexible"
        )
    ...
    def test_vmss_add_application_with_config_override(self, resource_group):
        ...
        self.cmd(
            ...
            checks=[
                self.check(
                    "virtualMachineProfile.applicationProfile.galleryApplications[0].configurationReference", "null"
                ),
                self.check(
                    "virtualMachineProfile.applicationProfile.galleryApplications[1].configurationReference", "{config}"
                ),
            ],
        )
...
class DiskRPTestScenario(ScenarioTest):
    ...
    def test_public_network_access(self, resource_group):
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{disk1}"), self.check("publicNetworkAccess", "Enabled")],
        )
        self.cmd("disk update --public-network-access Disabled -n {disk1} -g {rg}")
        self.cmd(
            "disk show -g {rg} -n {disk1}",
            checks=[self.check("name", "{disk1}"), self.check("publicNetworkAccess", "Disabled")],
        )
        self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{disk2}"), self.check("publicNetworkAccess", "Disabled")],
        )
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{disk3}"), self.check("publicNetworkAccess", "Enabled")],
        )

        self.cmd("snapshot create --public-network-access Enabled --size-gb 5 -n {snapshot1} -g {rg}")
        self.cmd(
            "snapshot show -g {rg} -n {snapshot1}",
            checks=[self.check("name", "{snapshot1}"), self.check("publicNetworkAccess", "Enabled")],
        )
        self.cmd("snapshot update --public-network-access Disabled -n {snapshot1} -g {rg}")
        self.cmd(
            "snapshot show -g {rg} -n {snapshot1}",
            checks=[self.check("name", "{snapshot1}"), self.check("publicNetworkAccess", "Disabled")],
        )
        self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{snapshot2}"), self.check("publicNetworkAccess", "Disabled")],
        )
        self.cmd("disk-access create -g {rg} -n diskaccess")
        self.cmd(
        ...
        self.cmd(
            ...
            checks=[self.check("name", "{snapshot3}"), self.check("publicNetworkAccess", "Enabled")],
        )
    ...
    def test_accelerated_network(self, resource_group):
        ...
        self.cmd(
            "disk show -g {rg} -n {disk1}",
            checks=[self.check("name", "{disk1}"), self.check("supportedCapabilities.acceleratedNetwork", True)],
        )
        self.cmd("disk update --accelerated-network false -n {disk1} -g {rg}")
        self.cmd(
            "disk show -g {rg} -n {disk1}",
            checks=[self.check("name", "{disk1}"), self.check("supportedCapabilities.acceleratedNetwork", False)],
        )
        self.cmd("disk create --size-gb 5 -n {disk2} -g {rg}")
        self.cmd(
            "disk show -g {rg} -n {disk2}",
            checks=[self.check("name", "{disk2}"), self.check("supportedCapabilities", None)],
        )
        ...
        self.cmd(
            "snapshot show -g {rg} -n {snapshot1}",
            checks=[self.check("name", "{snapshot1}"), self.check("supportedCapabilities.acceleratedNetwork", True)],
        )
        self.cmd("snapshot update --accelerated-network false -n {snapshot1} -g {rg}")
        self.cmd(
            "snapshot show -g {rg} -n {snapshot1}",
            checks=[self.check("name", "{snapshot1}"), self.check("supportedCapabilities.acceleratedNetwork", False)],
        )
        self.cmd("snapshot create --size-gb 5 -n {snapshot2} -g {rg}")
        self.cmd(
            "snapshot show -g {rg} -n {snapshot2}",
            checks=[self.check("name", "{snapshot2}"), self.check("supportedCapabilities", None)],
        )
        self.cmd("snapshot update --accelerated-network true -n {snapshot2} -g {rg}")
        self.cmd(
            "snapshot show -g {rg} -n {snapshot2}",
            checks=[self.check("name", "{snapshot2}"), self.check("supportedCapabilities.acceleratedNetwork", True)],
        )
    ...
    def test_completion_percent(self, resource_group):
        ...
        self.cmd("snapshot show -g {rg2} -n snapb", checks=[self.check_pattern("completionPercent", r"\d?.\d?")])
    ...
    def test_snapshot_incremental(self, resource_group):
        ...
        self.cmd(
            "disk create -n {disk} -g {rg} --security-type Standard --size-gb 10",
            checks=[
        ...
        self.cmd(
            "snapshot create -n {snapshot} -g {rg} --incremental true --source {disk}",
            checks=[self.check("creationData.createOption", "Copy")],
        )
        self.cmd(
            "snapshot create -g {rg} -n {snapshot1} --source {snapshot} --incremental true -l eastus2euap",
            checks=[self.check("creationData.createOption", "CopyStart")],
        )
    ...
    @ResourceGroupPreparer(
        name_prefix="cli_test_snapshot_create_with_bandwidth_copy_speed2_", location="centraluseuap", key="rg2"
    )
    def test_snapshot_create_with_bandwidth_copy_speed(self, resource_group):
        ...
        # Create a snapshot A from a empty disk with createOption as "Copy"
        self.kwargs["source"] = self.cmd(
            "snapshot create -g {rg} -n {snapshot} --source {disk} -l eastus2euap --incremental true"
        ).get_output_in_json()["id"]
        # Create a snapshot B in different region with createOption as "CopyStart" with snapshot A as source
        self.cmd(
            "snapshot create -g {rg2} -n {snapshot1} --copy-start true --incremental true --source {source} -l centraluseuap --bandwidth-copy-speed Enhanced",
    ...
    def test_snapshot_ultra_ssd(self, resource_group):
        ...
        disk_info = self.cmd(
            "disk create -g {rg} -n {disk} --size-gb 4 --sku UltraSSD_LRS",
            checks=[self.check("sku.name", "UltraSSD_LRS")],
        ).get_output_in_json()
        self.kwargs.update({"disk_id": disk_info["id"]})
        self.cmd(
            "snapshot create -n {snapshot} -g {rg} --incremental true --source {disk}",
            checks=[self.check("creationData.sourceResourceId", "{disk_id}")],
        )
    ...
    def test_disk_snapshot_hyper_v_generation_default_to_null(self, resource_group):
        ...
        self.cmd("disk create -g {rg} -n {disk} --size-gb 4 ", checks=[self.check("hyperVGeneration", None)])
        self.cmd("snapshot create -n {snapshot} -g {rg} --source {disk}", checks=[self.check("hyperVGeneration", None)])
...
class RestorePointScenarioTest(ScenarioTest):
    ...
    def test_restore_point(self, resource_group):
        ...
        vm = self.cmd(
            "vm create -n {vm_name} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest --admin-username vmtest "
            "--admin-password testPassword0 --nsg-rule NONE --tag EnableCrashConsistentRestorePoint=True --subnet {subnet} --vnet-name {vnet}"
        ).get_output_in_json()
        ...
        self.cmd(
            "restore-point show -g {rg} -n {point_name} --collection-name {collection_name}",
            checks=[
        ...
        self.cmd(
            "restore-point collection show -g {rg} --collection-name {collection_name} --restore-points",
            checks=[
                self.check("location", "eastus2euap"),
                self.check("name", "{collection_name}"),
    ...
    def test_restore_point_collection(self, resource_group):
        ...
        vm = self.cmd(
            "vm create -n {vm_name} -g {rg} --image Canonical:UbuntuServer:18.04-LTS:latest "
            "--admin-username vmtest --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        ).get_output_in_json()
        ...
        self.cmd(
            "restore-point collection update -g {rg} --collection-name {collection_name} --tags tag=test",
            checks=[self.check("tags", {"tag": "test"})],
        )

        self.cmd(
            "restore-point collection show -g {rg} --collection-name {collection_name}",
            checks=[
                self.check("location", "westus"),
                self.check("name", "{collection_name}"),
                self.check("resourceGroup", "{rg}"),
        ...
        self.cmd(
            "restore-point collection list -g {rg}",
            checks=[
        ...
        self.cmd("restore-point collection list-all", checks=[self.check("type(@)", "array")])
    ...
    def test_copy_vm_restore_point(self, resource_group):
        ...
        vm = self.cmd(
            "vm create -n {vm_name} -g {rg} --image ubuntu2204 --admin-username vmtest --subnet {subnet} --vnet-name {vnet} --nsg-rule NONE"
        ).get_output_in_json()
        ...
        self.kwargs.update({"collection_id": collection["id"]})
        ...
        self.cmd(
            ...
            checks=[self.check("sourceRestorePoint.id", "{point_id}")],
        )
    ...
    def test_restore_point_encryption_local(self, resource_group):
        ...
        self.cmd(
            "keyvault create -g {rg} -n {vault1} --enabled-for-disk-encryption true --enable-purge-protection --enable-rbac-authorization false"
        )
        ...
        des1 = self.cmd(
            "disk-encryption-set create -g {rg} -n {des1} --key-url {kid} --source-vault {vault1} --encryption-type EncryptionAtRestWithPlatformAndCustomerKeys"
        ).get_output_in_json()
        des2 = self.cmd(
            "disk-encryption-set create -g {rg} -n {des2} --key-url {kid} --source-vault {vault1} --encryption-type EncryptionAtRestWithPlatformAndCustomerKeys"
        ).get_output_in_json()

        vm = self.cmd(
            "vm create -g {rg} -n {vm_name} --size Standard_B2ms --attach-os-disk {os_disk} --attach-data-disk {data_disk1} {data_disk2} --os-type linux --nsg-rule NONE"
        ).get_output_in_json()
        self.cmd("vm deallocate -g {rg} -n {vm_name}")
        self.kwargs.update(
            {
        ...
        self.cmd(
            "keyvault set-policy --name {vault1} -g {rg} --key-permissions wrapkey unwrapkey get --object-id {des1_obj_id}"
        )
        self.cmd(
            "keyvault set-policy --name {vault1} -g {rg} --key-permissions wrapkey unwrapkey get --object-id {des2_obj_id}"
        )

        self.cmd("restore-point collection create -g {rg} --collection-name {collection_name} --source-id {vm_id}")
        self.cmd(
            "restore-point create -g {rg} -n {point_name1} --collection-name {collection_name} --source-os-resource {os_disk_id} "
            "--os-restore-point-encryption-set {des1_id} --source-data-disk-resource {data_disk1_id} {data_disk2_id} "
            "--data-disk-restore-point-encryption-set {des1_id} {des2_id}",
            checks=[
    ...
    def test_restore_point_encryption_remote(self, resource_group):
        ...
        vm = self.cmd(
            "vm create -g {rg} -n {vm_name} --attach-os-disk {os_disk} --attach-data-disk {data_disk1} {data_disk2} --os-type linux --size Standard_B2ms --nsg-rule NONE"
        ).get_output_in_json()
        self.cmd("vm deallocate -g {rg} -n {vm_name}")
        self.kwargs.update(
            {
        ...
        source_collection = self.cmd(
            "restore-point collection create -g {rg} --collection-name {collection_name} --source-id {vm_id}"
        ).get_output_in_json()
        source_point = self.cmd(
            "restore-point create -g {rg} -n {point_name1} --collection-name {collection_name} --source-os-resource {os_disk_id} "
            "--os-restore-point-encryption-set {des1_id} --source-data-disk-resource {data_disk1_id} {data_disk2_id} "
            "--data-disk-restore-point-encryption-set {des1_id} {des2_id}",
            checks=[
        ...
        self.cmd(
            "keyvault create -g {rg} -n {remote_vault1} --enabled-for-disk-encryption true --enable-purge-protection -l CentralUSEUAP --enable-rbac-authorization false"
        )
        remote_key1 = self.cmd(
            "keyvault key create -n {remote_key1} --vault {remote_vault1} --protection software"
        ).get_output_in_json()
        self.kwargs.update({"remote_kid": remote_key1["key"]["kid"]})
        remote_des1 = self.cmd(
            "disk-encryption-set create -g {rg} -n {remote_des1} --key-url {remote_kid} --source-vault {remote_vault1} -l CentralUSEUAP"
        ).get_output_in_json()
        remote_des2 = self.cmd(
            "disk-encryption-set create -g {rg} -n {remote_des2} --key-url {remote_kid} --source-vault {remote_vault1} -l CentralUSEUAP"
        ).get_output_in_json()
        self.kwargs.update(
            {
                ...
                "remote_os_disk_id": source_point["sourceMetadata"]["storageProfile"]["osDisk"]["diskRestorePoint"][
                    "id"
                ],
                "remote_data_disk_id1": source_point["sourceMetadata"]["storageProfile"]["dataDisks"][0][
                    "diskRestorePoint"
                ]["id"],
                "remote_data_disk_id2": source_point["sourceMetadata"]["storageProfile"]["dataDisks"][1][
                    "diskRestorePoint"
                ]["id"],
            }
        )
        self.cmd(
            "keyvault set-policy --name {remote_vault1} -g {rg} --key-permissions wrapkey unwrapkey get --object-id {remote_des1_obj_id}"
        )
        self.cmd(
            "keyvault set-policy --name {remote_vault1} -g {rg} --key-permissions wrapkey unwrapkey get --object-id {remote_des2_obj_id}"
        )

        self.cmd(
            "restore-point collection create -g {rg} --collection-name {remote_collection_name} --source-id {source_collection_id} -l CentralUSEUAP"
        )
        self.cmd(
            "restore-point create -g {rg} -n {remote_point_name1} --collection-name {remote_collection_name} --source-restore-point {source_point_id} "
            ...
            checks=[
                self.check(
                    "sourceMetadata.storageProfile.osDisk.diskRestorePoint.sourceDiskRestorePoint.id",
                    "{remote_os_disk_id}",
                ),
                self.check(
                    "sourceMetadata.storageProfile.dataDisks[0].diskRestorePoint.sourceDiskRestorePoint.id",
                    "{remote_data_disk_id1}",
                ),
                self.check(
                    "sourceMetadata.storageProfile.dataDisks[1].diskRestorePoint.sourceDiskRestorePoint.id",
                    "{remote_data_disk_id2}",
                ),
            ],
        )
...
class ArchitectureScenarioTest(ScenarioTest):
    ...
    def test_architecture(self, resource_group):
        ...
        self.cmd(
            ...
            checks=[self.check("architecture", "x64")],
        )

        self.cmd(
            "disk create -g {rg} -n {disk_name} --architecture x64 --size-gb 20",
            checks=[self.check("supportedCapabilities.architecture", "x64")],
        )
        self.cmd(
            "disk update -g {rg} -n {disk_name} --architecture Arm64",
            checks=[self.check("supportedCapabilities.architecture", "Arm64")],
        )

        self.cmd(
            "snapshot create -n {snap_name} -g {rg} --size-gb 1 --architecture x64",
            checks=[self.check("supportedCapabilities.architecture", "x64")],
        )
        self.cmd(
            "snapshot update -n {snap_name} -g {rg} --architecture Arm64",
            checks=[self.check("supportedCapabilities.architecture", "Arm64")],
        )
    ...
    def test_sig_image_definition_default_value(self, resource_group):
        ...
        self.cmd(
            "sig image-definition create -g {rg} --gallery-name {gallery1} --gallery-image-definition {sig1} "
        ...
        self.cmd("sig create -g {rg} --gallery-name {gallery2}")
        self.cmd(
            'sig image-definition create -g {rg} --gallery-name {gallery2} --gallery-image-definition image --os-type linux -p publisher1 -f offer1 -s sku1 --features "IsAcceleratedNetworkSupported=true"',
            checks=[
                self.check("features[0].name", "IsAcceleratedNetworkSupported"),
                self.check("features[0].value", "true", False),
        ...
        self.cmd(
            "sig image-definition create --resource-group {rg} --gallery-name {gallery3} --gallery-image-definition suse_liberty_linux_8_byos_gen1 --offer suse_liberty_linux_8_byos --os-type Linux --publisher suse --sku gen1 --architecture x64 --os-state Generalized --hyper-v-generation V1",
            checks=[
                self.check("features", None),
                self.check("hyperVGeneration", "V1"),
            ],
