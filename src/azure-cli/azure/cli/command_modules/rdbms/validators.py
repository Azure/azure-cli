...
def pg_storage_validator(ns):
    if ns.storage_mb:
        if str(ns.storage_mb) in ['32', '64', '128', '256', '512', '1024', '2048', '4096', '8192', '16384']:
            ...
            raise CLIError(
                'Incorrect value for --storage-size : Allowed values(in GB) : {32, 64, 128, 256, 512, 1024, '
...
def tier_validator(ns):
    if ns.tier:
        ...
        if re.match(r"^Standard_B\d", ns.sku_name) and ns.tier != 'Burstable':
            raise CLIError(
                'SKU and tier are not matching. SKU "{}" is in "Burstable" tier. Specify "--tier Burstable" argument to'
                ' deploy with the SKU'.format(
                    ns.sku_name))
        elif re.match(r"^Standard_D\d+s_v3$", ns.sku_name) and ns.tier != 'GeneralPurpose':
            raise CLIError(
                'SKU and tier are not matching. SKU "{}" is in "General Purpose" tier. Specify "--tier GeneralPurpose"'
                ' argument to deploy with the SKU'.format(
                    ns.sku_name))
        elif re.match(r"^Standard_E\d+s_v3$", ns.sku_name) and ns.tier != 'MemoryOptimized':
            raise CLIError(
                'SKU and tier are not matching. SKU "{}" is in "Memory Optimized" tier. Specify "--tier '
                'MemoryOptimized" argument to deploy with the SKU'.format(
                    ns.sku_name))
...
def pg_sku_name_validator(ns):
    if ns.sku_name:
        if len(ns.sku_name.split('_')) != 3:
            raise CLIError(
                'Incorrect value for --sku-name. Follow the convention Standard_{VM name}. Examples: Standard_D4s_v3')
    ...
    if ns.sku_name:
        if len(ns.sku_name.split('_')) == 2:
            ...
            if server_type != 'Standard' or cpu not in ['B1ms', 'B2s', 'B1s']:
                raise CLIError(
                    'Incorrect value for --sku-name. Follow the convention Standard_{VM name}. Examples: '
                    'Standard_D4s_v3 Standard_E8s_v3 Standard_B1ms')
        elif len(ns.sku_name.split('_')) == 3:
            server_type, cpu, server_v3 = ns.sku_name.split('_')
            if server_type != 'Standard' or server_v3 != 'v3':
                raise CLIError(
                    'Incorrect value for --sku-name. Follow the convention Standard_{VM name}. Examples: '
                    'Standard_D4s_v3 Standard_B1ms')
            if not re.match(r"^(D|E)\d+s$", cpu) or (
                    re.match(r"^(D|E)\d+s$", cpu) and int(cpu[1:-1]) not in [2, 4, 8, 16, 32, 48, 64]):
                raise CLIError(
                    'Incorrect value for --sku-name. Follow the convention Standard_{VM name}. Examples: '
                    'Standard_D4s_v3 Standard_E8s_v3 Standard_B1ms. \nAllowed number of vCores: 2, 4, 8, 16, 32, '
                    '48, 64')
...
def maintenance_window_validator(ns):
    ...
    if ns.maintenance_window:
        ...
        if len(parsed_input) < 1 or len(parsed_input) > 3:
            raise CLIError(
                'Incorrect value for --maintenance-window. Enter <Day>:<Hour>:<Minute>. Example: "Mon:8:30" to '
                'schedule on Monday, 8:30 UTC')
        elif len(parsed_input) >= 1 and parsed_input[0] not in options:
            raise CLIError(
                'Incorrect value for --maintenance-window. The first value means the scheduled day in a week or '
                'can be "Disabled" to reset maintenance window.'
                '. Allowed values: {"Sun","Mon","Tue","Wed","Thu","Fri","Sat"}')
        elif len(parsed_input) >= 2 and (
                not parsed_input[1].isdigit() or int(parsed_input[1]) < 0 or int(parsed_input[1]) > 23):
            raise CLIError(
                'Incorrect value for --maintenance-window. The second number means the scheduled hour in the '
                'scheduled day. Allowed values: {0, 1, ... 23}')
        elif len(parsed_input) >= 3 and (
                not parsed_input[2].isdigit() or int(parsed_input[2]) < 0 or int(parsed_input[2]) > 59):
            raise CLIError(
                'Incorrect value for --maintenance-window. The third number means the scheduled minute in the '
                'scheduled hour. Allowed values: {0, 1, ... 59}')
...
def _validate_ranges_in_ip(ip):
    parsed_ip = ip.split('.')
    if len(parsed_ip) == 4 and _valid_range(int(parsed_ip[0])) and _valid_range(int(parsed_ip[1])) and _valid_range(
            int(parsed_ip[2])) and _valid_range(int(parsed_ip[3])):
        return True
    else:
        return False
