...
class AmsLiveOutputTests(ScenarioTest):
    ...
    def test_live_output_show(self, storage_account_for_create):
        ...
        self.cmd('az ams live output create -a {amsname} -n {liveOutputName} -g {rg} --asset-name {assetName} --live-event-name {liveEventName} --archive-window-length {archiveWindowLength} --manifest-name {manifestName}', checks=[
            self.check('archiveWindowLength', '0:00:02'),
            self.check('assetName', '{assetName}'),
            self.check('manifestName', '{manifestName}'),
            self.check('name', '{liveOutputName}'),
