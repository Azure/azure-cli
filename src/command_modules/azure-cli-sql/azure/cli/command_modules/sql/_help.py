...
long-summary: If the policy is being enabled, `--storage-account` or both `--storage-endpoint`
    and `--storage-key` must be specified.
examples:
-   name: Enable by storage account name.
    text: az sql db audit-policy update -g mygroup -s myserver -n mydb --state Enabled
...
-   name: Remove an audit action by list index.
    text: az sql db audit-policy update -g mygroup -s myserver -n mydb --remove auditActionsAndGroups
        0
...
long-summary: A full list of performance level options can be seen by executing `az
    sql db list-editions -a -o table -l LOCATION`. The copy destination database must
    have the same edition as the source database, but you can change the edition after
    the copy has completed.
examples:
-   name: Create a database with performance level S0 as a copy of an existing Standard
        database.
    text: az sql db copy -g mygroup -s myserver -n originalDb --dest-name newDb --service-objective
        S0
-   name: Create a database with GeneralPurpose edition, Gen4 hardware, and 1 vcore
        as a copy of an existing GeneralPurpose database.
    text: az sql db copy -g mygroup -s myserver -n originalDb --dest-name newDb -f
        Gen4 -c 1
"""
...
long-summary: A full list of performance level options can be seen by executing `az
    sql db list-editions -a -o table -l LOCATION`.
examples:
-   name: Create a Standard S0 database.
    text: az sql db create -g mygroup -s myserver -n mydb --service-objective S0
-   name: Create a database with GeneralPurpose edition, Gen4 hardware and 1 vcore
    text: az sql db create -g mygroup -s myserver -n mydb -e GeneralPurpose -f Gen4
        -c 1
...
-   name: Create a database.
    text: az sql db create --server myserver --name mydb --collation {collation} --elastic-pool
        {elastic-pool} --resource-group mygroup
    crafted: 'True'
"""
...
-   name: Delete a database.
    text: az sql db delete --server {server} --resource-group MyResourceGroup --yes  --name
        MyAzureSQLDatabase
    crafted: 'True'
"""
...
-   name: Export a database to a bacpac.
    text: az sql db export --storage-key-type SharedAccessKey --server {server} --storage-uri
        {storage-uri} --resource-group MyResourceGroup --admin-user {admin-user} --name
        MyAzureSQLDatabase --storage-key {storage-key} --admin-password {admin-password}
    crafted: 'True'
"""
...
examples:
-   name: Returns database usages.
    text: az sql db list-usages --server {server} --resource-group MyResourceGroup
        --name MyAzureSQLDatabase
    crafted: 'True'
-   name: List databases a server or elastic pool.
    text: az sql db list --output json --server {server} --query [0] --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
long-summary: Includes available service objectives and storage limits. In order to
    reduce verbosity, settings to intentionally reduce storage limits are hidden by
    default.
examples:
...
-   name: Show available max database sizes for P1 service objective
    text: az sql db list-editions -l westus --service-objective P1 --show-details
        max-size
"""
...
long-summary: A full list of performance level options can be seen by executing `az
    sql db list-editions -a -o table -l LOCATION`. The secondary database must have
    the same edition as the primary database.
examples:
-   name: Create a database with performance level S0 as a secondary replica of an
        existing Standard database.
    text: az sql db replica create -g mygroup -s myserver -n originalDb --partner-server
        newDb --service-objective S0
-   name: Create a database with GeneralPurpose edition, Gen4 hardware, and 1 vcore
        as a secondary replica of an existing GeneralPurpose database
    text: az sql db replica create -g mygroup -s myserver -n originalDb --partner-server
        newDb -f Gen4 -c 1
"""
...
type: command
short-summary: Set the primary replica database by failing over from the current primary
    replica database.
"""
...
examples:
-   name: Get the details for a database.
    text: az sql db show --server {server} --name MyAzureSQLDatabase --resource-group
        MyResourceGroup
    crafted: 'True'
"""
...
long-summary: If the policy is being enabled, storage_account or both storage_endpoint
    and storage_account_access_key must be specified.
examples:
-   name: Enable by storage account name.
    text: |
...
-   name: Create a Standard 100 DTU elastic pool.
    text: az sql elastic-pool create -g mygroup -s myserver -n mydb -e Standard -c
        100
-   name: Create an elastic pool with GeneralPurpose edition, Gen4 hardware and 1
        vcore.
    text: az sql elastic-pool create -g mygroup -s myserver -n mydb -e GeneralPurpose
        -f Gen4 -c 1
"""
...
-   name: Show available max sizes for elastic pools with at least 100 DTUs in the
        West US region.
...
examples:
-   name: Cancel an operation.
    text: az sql elastic-pool op cancel -g mygroup -s myserver -ep myelasticpool -n
...
type: command
short-summary: Set the primary of the failover group by failing over all databases
    from the current primary server.
"""
...
-   name: Create a managed instance with specified parameters and with identity
    text: az sql mi create -g mygroup -n myinstance -l mylocation -i -u myusername
        -p mypassword --license-type LicenseIncluded --subnet /subscriptions/{SubID}/resourceGroups/{ResourceGroup}/providers/Microsoft.Network/virtualNetworks/{VNETName}/subnets/{SubnetName}
        --capacity 8 --storage 32GB --edition GeneralPurpose --family Gen4
-   name: Create a managed instance with minimal set of parameters
    text: az sql mi create -g mygroup -n myinstance -l mylocation -i -u myusername
        -p mypassword --subnet /subscriptions/{SubID}/resourceGroups/{ResourceGroup}/providers/Microsoft.Network/virtualNetworks/{VNETName}/subnets/{SubnetName}
"""
...
-   name: Updates a mi with specified parameters and with identity
    text: az sql mi update -g mygroup -n myinstance -i -p mypassword --license-type
        mylicensetype --capacity vcorecapacity --storage storagesize
-   name: Update a managed instance.
    text: az sql mi update --capacity vcorecapacity --name myinstance --resource-group
        mygroup
    crafted: 'True'
"""
...
-   name: Create a managed database with specified collation
    text: az sql midb create -g mygroup --mi myinstance -n mymanageddb --collation
...
-   name: Restore a managed database using Point in time restore
    text: az sql midb restore -g mygroup --mi myinstance -n mymanageddb --dest-name
        targetmidb --time "2018-05-20T05:34:22"
"""
...
-   name: Create a server.
    text: az sql server create -l westus -g mygroup -n myserver -u myadminuser -p
        myadminpassword
-   name: Create a server.
    text: az sql server create --admin-password myadminpassword --admin-user myadminuser
        --resource-group mygroup --location westus --name myserver
    crafted: 'True'
"""
...
-   name: Create a firewall rule
    text: az sql server firewall-rule create -g mygroup -s myserver -n myrule --start-ip-address
...
-   name: Create a firewall rule.
    text: az sql server firewall-rule create --server myserver --start-ip-address
        1.2.3.4 --name myrule --end-ip-address 5.6.7.8 --resource-group mygroup
    crafted: 'True'
"""
...
-   name: List a server's firewall rules.
    text: az sql server firewall-rule list --server myserver --resource-group mygroup
    crafted: 'True'
"""
...
-   name: Shows the details for a firewall rule.
    text: az sql server firewall-rule show --server myserver --name myrule --resource-group
        mygroup
    crafted: 'True'
"""
...
-   name: Update a firewall rule
    text: az sql server firewall-rule update -g mygroup -s myserver -n myrule --start-ip-address
        9.8.7.6 --end-ip-address 5.4.3.2
-   name: Update a firewall rule.
    text: az sql server firewall-rule update --server myserver --start-ip-address
        9.8.7.6 --name myrule --end-ip-address 5.4.3.2 --resource-group mygroup
    crafted: 'True'
"""
...
-   name: List available servers.
    text: az sql server list --output json --query [0] --resource-group mygroup
    crafted: 'True'
"""
...
-   name: Create a vnet rule by providing the vnet and subnet name. The subnet id
        is created by taking the resource group name and subscription id of the SQL
        server.
