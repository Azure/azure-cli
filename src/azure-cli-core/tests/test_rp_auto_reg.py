    ...
    def setUp(self):
        if self.in_recording:
            ...
            if result.lower().strip() != 'unregistered':
                ...
                time.sleep(30)  # a bit random but like to ensure unregister went through
        return super().setUp()

    @ResourceGroupPreparer()
    def test_rp_auto_register(self, resource_group, resource_group_location):
        if self.in_recording:  # should go away once #3027 gets fixed
            self.core_test(resource_group, resource_group_location)
        else:
            with mock.patch('time.sleep'):
                self.core_test(resource_group, resource_group_location)

    def core_test(self, resource_group, resource_group_location):
