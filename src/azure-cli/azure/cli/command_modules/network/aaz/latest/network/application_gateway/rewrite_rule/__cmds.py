...
class Create(AAZCommand):
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        _args_schema.request_header_configurations = AAZListArg(
            options=["--request-header-configurations"],
            help="Request Header Actions in the Action Set.",
        )
        _args_schema.response_header_configurations = AAZListArg(
        ...
        _args_schema.sequence = AAZIntArg(
            options=["--sequence"],
            help="Determine the execution order of the rule in the rule set.",
        )

        request_header_configurations = cls._args_schema.request_header_configurations
        ...
        response_header_configurations = cls._args_schema.response_header_configurations
        ...
        # define Arg Group "URL Configuration"

        _args_schema = cls._args_schema
        _args_schema.modified_path = AAZStrArg(
            options=["--modified-path"],
            arg_group="URL Configuration",
            help="URL path for url rewrite.",
        )
        _args_schema.modified_query_string = AAZStrArg(
            options=["--modified-query-string"],
            arg_group="URL Configuration",
            help="Query string for url rewrite.",
        )
        _args_schema.enable_reroute = AAZBoolArg(
            options=["--enable-reroute"],
            arg_group="URL Configuration",
            help="If set as true, it will re-evaluate the url path map provided in path based request routing rules using modified path.",
        )
        return cls._args_schema
    ...
    class InstanceCreateByJson(AAZJsonInstanceCreateOperation):
        ...
        def _create_instance(self):
            ...
            _builder.set_prop("actionSet", AAZObjectType)
            _builder.set_prop("conditions", AAZListType, ".conditions")
            _builder.set_prop("name", AAZStrType, ".name")
            _builder.set_prop("ruleSequence", AAZIntType, ".sequence")

            action_set = _builder.get(".actionSet")
            if action_set is not None:
                ...
                action_set.set_prop("urlConfiguration", AAZObjectType)

            request_header_configurations = _builder.get(".actionSet.requestHeaderConfigurations")
            ...
            response_header_configurations = _builder.get(".actionSet.responseHeaderConfigurations")
            ...
            url_configuration = _builder.get(".actionSet.urlConfiguration")
            if url_configuration is not None:
                ...
                url_configuration.set_prop("reroute", AAZBoolType, ".enable_reroute")

            conditions = _builder.get(".conditions")
            if conditions is not None:
                conditions.set_elements(AAZObjectType, ".")

            _elements = _builder.get(".conditions[]")
...
class List(AAZCommand):
    ...
    class SubresourceSelector(AAZJsonSelector):
        ...
        def _get(self):
            ...
            idx = next(filters)[0]
            result = result[idx]
            return result.properties.rewriteRules
        ...
        def _set(self, value):
            ...
            idx = next(filters)[0]
            result = result[idx]
            result.properties.rewriteRules = value
            return
...
class ListRequestHeaders(AAZCommand):
    """List all available request headers.

    :example: List all available request headers.
    ...
    def _execute_operations(self):
        self.pre_operations()
        self.ApplicationGatewaysListAvailableRequestHeaders(ctx=self.ctx)()
        self.post_operations()
    ...
    class ApplicationGatewaysListAvailableRequestHeaders(AAZHttpOperation):
        CLIENT_TYPE = "MgmtClient"
...
class Show(AAZCommand):
    ...
    :example: Get the details of a rewrite rule.
...
@register_command(
    "network application-gateway rewrite-rule wait",
)
...
@link_helper(
    __package__,
    ("_build_schema_application_gateway_header_configuration_update", _Helper),
...
__all__ = ["Create","Delete","List","ListRequestHeaders","ListResponseHeaders","Show","Update","Wait","__CMDGroup"]
