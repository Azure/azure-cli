...
class DiskEncryptionSetTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_test_os_disk_security_encryption_type', location='CentralUSEUAP')
    @KeyVaultPreparer(name_prefix='vault4-', name_len=20, key='vault', location='westcentralus', additional_params='--enable-purge-protection')
    def test_os_disk_security_encryption_type(self, resource_group):
        self.kwargs.update({
        ...
        self.cmd('vm show -n {vm1} -g {rg}', checks=[
            ...
            self.check('storageProfile.osDisk.managedDisk.diskEncryptionSet.id', '{des1_id}')
        ])
