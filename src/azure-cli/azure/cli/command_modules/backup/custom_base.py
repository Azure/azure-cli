...
def unregister_container(cmd, client, vault_name, resource_group_name, container_name, backup_management_type=None):
    ...
    if container is None:
        raise ResourceNotFoundError("Could not find the container '{}'. Please check the container name and "
                                    "backup management type.".format(container_name))
    if isinstance(container, list):
        raise InvalidArgumentValueError("Multiple containers with the name '{}' found. Please provide the "
                                        "native container name instead of a friendly name.".format(container_name))
