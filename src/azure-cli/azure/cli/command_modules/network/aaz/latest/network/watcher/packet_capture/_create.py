...
class Create(AAZCommand):
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        storage_location.file_path = AAZStrArg(
            options=["file-path"],
            help="A valid local path on the targeting VM. Must include the name of the capture file (*.cap). For linux virtual machine it must start with /var/captures. Required if no storage ID is provided, otherwise optional.",
        )
        storage_location.storage_id = AAZStrArg(
            options=["storage-id"],
            help="The ID of the storage account to save the packet capture session. Required if no local file path is provided.",
        )
