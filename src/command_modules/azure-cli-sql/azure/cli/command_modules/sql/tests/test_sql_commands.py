...
class SqlServerMgmtScenarioTest(ResourceGroupVCRTestBase):
    ...
    def __init__(self, test_method):
        ...
        self.sql_server_names = ['cli-sql-12345-a', 'cli-sql-12345-b']
    ...
    def body(self):
        ...
        self.cmd('sql server show -g {} --server-name {}'.format(rg, self.sql_server_names[0]), checks=[
            JMESPathCheck('name', self.sql_server_names[0]),
            JMESPathCheck('resourceGroup', rg),
            JMESPathCheck('administratorLogin', user)])

        # test delete sql server
        self.cmd('sql server delete -g {} --server-name {}'.format(rg, self.sql_server_names[0]), checks=NoneCheck())
        self.cmd('sql server delete -g {} --server-name {}'.format(rg, self.sql_server_names[1]), checks=NoneCheck())
