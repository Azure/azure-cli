...
def managed_registry_validation(registry_name, resource_group_name=None, message=None):
    ...
    if not arm_resource.sku or arm_resource.sku.name not in MANAGED_REGISTRY_SKU:
...
def premium_registry_validation(registry_name, resource_group_name=None, message=None):
    ...
    if not arm_resource.sku or arm_resource.sku.name != SkuName.managed_premium.value:
...
def validate_sku_update(current_sku, sku_parameter):
    ...
    if isinstance(sku_parameter, dict):
        if 'name' not in sku_parameter or sku_parameter['name'] not in MANAGED_REGISTRY_SKU:
            _invalid_sku_update()
        if current_sku == SkuName.classic.value:
            raise CLIError("Updating SKU from Classic to {} is not supported yet.".format(
                sku_parameter['name']))
