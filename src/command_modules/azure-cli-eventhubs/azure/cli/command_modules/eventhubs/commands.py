...
# Namespace Region
    ...
    with self.command_group('eventhubs namespace', eh_namespace_util, client_factory=namespaces_mgmt_client_factory, exception_handler=response_exception_handler) as g:
    ...
    with self.command_group('eventhubs namespace authorization-rule', eh_namespace_util, client_factory=namespaces_mgmt_client_factory, exception_handler=response_exception_handler) as g:
...
# EventHub Region
    with self.command_group('eventhubs eventhub', eh_event_hub_util, client_factory=event_hub_mgmt_client_factory, exception_handler=response_exception_handler) as g:
    ...
    with self.command_group('eventhubs eventhub authorization-rule', eh_event_hub_util, client_factory=event_hub_mgmt_client_factory, exception_handler=response_exception_handler) as g:
        g.command('create', 'create_or_update_authorization_rule')
        g.show_command('show', 'get_authorization_rule')
        g.command('list', 'list_authorization_rules', exception_handler=empty_on_404)
        g.command('keys list', 'list_keys')
...
# ConsumerGroup Region
    with self.command_group('eventhubs eventhub consumer-group', eh_consumer_groups_util, client_factory=consumer_groups_mgmt_client_factory, exception_handler=response_exception_handler) as g:
...
# DisasterRecoveryConfigs Region
    with self.command_group('eventhubs georecovery-alias', eh_geodr_util, client_factory=disaster_recovery_mgmt_client_factory, exception_handler=response_exception_handler) as g:
    ...
    with self.command_group('eventhubs georecovery-alias authorization-rule', eh_geodr_util, client_factory=disaster_recovery_mgmt_client_factory, exception_handler=response_exception_handler) as g:
        g.command('list', 'list_authorization_rules', exception_handler=empty_on_404)
        g.show_command('show', 'get_authorization_rule')
        g.command('keys list', 'list_keys')
