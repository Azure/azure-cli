...
def load_arguments(self, _):
    ...
    for scope in ['identity federated-credential create', 'identity federated-credential update']:
        with self.argument_context(scope) as c:
            ...
            c.argument('claims_matching_expression_value', help='A claims matching expression that is evaluated against incoming tokens for access token requests. Cannot be used with --subject.')
            c.argument('claims_matching_expression_version', help='Version of the claims matching expression language. Required when using --claims-matching-expression-value.')
