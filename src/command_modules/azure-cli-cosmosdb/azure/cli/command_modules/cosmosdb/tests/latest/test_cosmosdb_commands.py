...
class CosmosDBTests(ScenarioTest):
    ...
    def test_locations_database_accounts(self, resource_group):
        ...
        # account1 = self.cmd('az cosmosdb create -n {acc1} -g {rg_name} --locations {write_location}=0 {read_location}=1').get_output_in_json()
        # assert len(account1['properties']['locations']) == 2
        # locations = {}
        # for loc in account1['properties']['locations']:
        #     locations[loc['failoverPriority']] = loc['locationName']
        # assert locations[0] == "East US"
        # assert locations[1] == "West US"

        account1 = self.cmd('az cosmosdb create -n {acc} -g {rg} --location {write_location} --locations {write_location}=0 {read_location}=1').get_output_in_json()
        print(account1)
        ...
        account2 = self.cmd('az cosmosdb failover-priority-change -n {acc} -g {rg} --failover-policies {read_location}=0 {write_location}=1').get_output_in_json()
        print(account2)
        assert len(account2['locations']) == 2
        locations = {}
        for loc in account2['locations']:
            locations[loc['failoverPriority']] = loc['locationName']
        assert locations[0] == "West US"
        assert locations[1] == "East US"
