...
class IoTDpsTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(parameter_name='group_name', parameter_name_for_location='group_location', location='eastus')
    ...
    def test_dps_access_policy_lifecycle(self, group_name, group_location):
        ...
        self.cmd('az iot dps access-policy update -g {} --dps-name {} -n {} -r {}'.format(group_name, dps_name, policy_name, new_right), checks=[
            self.check('keyName', policy_name),
    ...
    @ResourceGroupPreparer(parameter_name='group_name', parameter_name_for_location='group_location', location='eastus')
    def test_dps_allocation_policy_lifecycle(self, group_name, group_location):
        new_policy = '"GeoLatency"\n'
        
        # Set up a provisioning service
        dps_name = self._create_test_dps(group_name, group_location)      

        # Get allocation policy
        self.cmd('az iot dps allocation-policy show -g {} --dps-name {}'.format(group_name, dps_name))
        
        # Update allocation policy
        output = self.cmd('az iot dps allocation-policy update -g {} --dps-name {} -p {}'.format(group_name, dps_name, new_policy))
        self.assertEqual(output.output, new_policy)
    ...
    def test_dps_linked_hub_lifecycle(self, group_name, group_location):              
        ...
        self.cmd('az iot dps linked-hub show --dps-name {} -g {} -n {}'.format(dps_name, group_name, hub_host_name), checks=[
        ...
        self.cmd('az iot dps linked-hub update --dps-name {} -g {} -n {} --allocation-weight {} --apply-allocation-policy {}'
        ...
        self.cmd('az iot dps linked-hub delete --dps-name {} -g {} -n {}'.format(dps_name, group_name, hub_host_name))
    ...
    def test_dps_certificate_lifecycle(self, group_name, group_location):              
        ...
        # Create certificate
        self.cmd('az iot dps certificate create --dps-name {} -g {} --name {} -p {}'.format(dps_name, group_name, cert_name, cert_file), checks=[
        ...
        etag =  self.cmd('az iot dps certificate update --dps-name {} -g {} --name {} -p {} --etag {}'
            .format(dps_name, group_name, cert_name, cert_file, etag), checks=[
            self.check('name', cert_name),
            self.check('properties.isVerified', False)
        ]).get_output_in_json()['etag']
        ...
        output = self.cmd('az iot dps certificate generate-verification-code --dps-name {} -g {} -n {} --etag {}'
            .format(dps_name, group_name, cert_name, etag), checks=[
            self.check('name', cert_name),
            self.check('properties.isVerified', False)
        ]).get_output_in_json()
