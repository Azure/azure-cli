...
examples:
  - name: Create a federated identity credential using subject identifier
    text: |
        az identity federated-credential create --fed-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --subject 'system:serviceaccount:ns:svcaccount' --audiences 'api://AzureADTokenExchange'
  - name: Create a federated identity credential using claims matching expression (long form)
    text: |
        az identity federated-credential create --fed-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --cme-value "claims['sub'] startswith 'repo:contoso-org/contoso-repo:ref:refs/heads/'" --cme-version 1 --audiences 'api://AzureADTokenExchange'
  - name: Create a federated identity credential using claims matching expression (short form)
    text: |
        az identity federated-credential create -f $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' -v "claims['sub'] startswith 'repo:contoso-org/contoso-repo:ref:refs/heads/'" -e 1 --audiences 'api://AzureADTokenExchange'
"""
...
examples:
  - name: Update a federated identity credential using subject identifier
    text: |
        az identity federated-credential update --fed-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --subject 'system:serviceaccount:ns:svcaccount' --audiences 'api://AzureADTokenExchange'
  - name: Update a federated identity credential using claims matching expression (long form)
    text: |
        az identity federated-credential update --fed-name $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' --cme-value "claims['sub'] startswith 'repo:contoso-org/contoso-repo:ref:refs/heads/'" --cme-version 1 --audiences 'api://AzureADTokenExchange'
  - name: Update a federated identity credential using claims matching expression (short form)
    text: |
        az identity federated-credential update -f $ficId --identity-name $uaId --resource-group $rg --issuer 'https://aks.azure.com/issuerGUID' -v "claims['sub'] startswith 'repo:contoso-org/contoso-repo:ref:refs/heads/'" -e 1 --audiences 'api://AzureADTokenExchange'
"""
...
examples:
  - name: Delete a federated identity credential using long form
    text: |
        az identity federated-credential delete --fed-name myFicName --identity-name myIdentityName --resource-group myResourceGroup
  - name: Delete a federated identity credential using short form
    text: |
        az identity federated-credential delete -f myFicName --identity-name myIdentityName --resource-group myResourceGroup
"""
...
examples:
  - name: Show a federated identity credential using long form
    text: |
        az identity federated-credential show --fed-name myFicName --identity-name myIdentityName --resource-group myResourceGroup
  - name: Show a federated identity credential using short form
    text: |
        az identity federated-credential show -f myFicName --identity-name myIdentityName --resource-group myResourceGroup
"""
