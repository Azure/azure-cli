...
class StorageAzcopyTests(StorageScenarioMixin, LiveScenarioTest):
    ...
    def test_storage_blob_azcopy_sync_oauth(self, resource_group, storage_account, test_dir):
    ...
    def test_storage_blob_azcopy_remove(self, resource_group, storage_account_info, test_dir):
        ...
        self.oauth_cmd('storage remove -c {} --include-path apple --account-name {} --include-pattern "file*" --exclude-pattern "file_1*" --recursive'.format(
            container, storage_account))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
    ...
    def test_storage_azcopy_blob_url(self, resource_group, first_account, second_account, test_dir):
        ...
        self.oauth_cmd('storage account show -n {}'.format(second_account), checks=[
        ...
        self.oauth_cmd('storage container list --account-name {}'
        ...
        self.oauth_cmd('disk create -n {} -g {} --for-upload --upload-size-bytes 20972032'
                 .format(diskname, resource_group))
        sasURL = self.oauth_cmd(
        ...
        self.oauth_cmd('storage copy -s "{}" -d "{}" --blob-type PageBlob'
    ...
    def test_storage_azcopy_blob_account(self, resource_group, first_account, second_account, test_dir):
        ...
        # Upload entire directory
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-account-name {} --destination-container {} --recursive'
        ...
        local_folder = self.create_temp_dir()
        # Download a single file
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --source-blob {} --destination-local-path "{}"'
                 .format(first_account, first_container, 'readme', local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download entire directory
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --source-blob {} --destination-local-path "{}" --recursive'
                 .format(first_account, first_container, 'apple/', local_folder))
        self.assertEqual(1, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(11, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download a set of files
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --include-path {} --include-pattern {} --destination-local-path "{}" --recursive'
        ...
        # Copy an entire storage account data to another blob account
        self.oauth_cmd('storage copy --source-account-name {} --destination-account-name {} --recursive --preserve-s2s-access-tier false'
    ...
    def test_storage_azcopy_blob_source_with_credential(self, resource_group, first_account, second_account, test_dir):
        ...
        first_connection_string = self.oauth_cmd('storage account show-connection-string -n {} -o tsv'.format(
            first_account)).output.strip()
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-container {} --recursive --connection-string {}'
        ...
        local_folder = self.create_temp_dir()
        self.oauth_cmd('storage copy --source-account-name {} --source-account-key {} --source-container {} --source-blob {} '
        ...
        first_sas_token = self.oauth_cmd(
        ...
        self.oauth_cmd('storage copy --source-account-name {} --source-sas {} --source-container {} --include-path {} '
        ...
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --source-blob {} --source-sas {} '
    ...
    def test_storage_azcopy_file_url(self, resource_group, storage_account_info, test_dir):
        ...
        import os
        # Upload a single file
        self.oauth_cmd('storage copy -s "{}" -d "{}"'
                 .format(os.path.join(test_dir, 'readme'), share_url))
        self.oauth_cmd('storage file list -s {} --account-name {}'
                 .format(share, storage_account), checks=JMESPathCheck('length(@)', 1))

        # Upload entire directory
        self.oauth_cmd('storage copy -s "{}" -d "{}" --recursive'
                 .format(os.path.join(test_dir, 'apple'), share_url))
        self.oauth_cmd('storage file list -s {} --account-name {}'.format(
    ...
    def test_storage_azcopy_file_account(self, resource_group, storage_account_info, test_dir):
        ...
        # Upload entire directory
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-account-name {} --destination-share {} --recursive'
        ...
        local_folder = self.create_temp_dir()
        # Download a single file
        self.oauth_cmd('storage copy --source-account-name {} --source-share {} --source-file-path {} --destination-local-path "{}"'
                 .format(storage_account, share, 'readme', local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download entire directory
        self.oauth_cmd('storage copy --source-account-name {} --source-share {} --source-file-path {} --destination-local-path "{}" --recursive'
                 .format(storage_account, share, 'apple', local_folder))
        self.assertEqual(1, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(11, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download a set of files
        self.oauth_cmd('storage copy --source-account-name {} --source-share {} --include-path "apple" --include-pattern file* --destination-local-path "{}" --recursive'
