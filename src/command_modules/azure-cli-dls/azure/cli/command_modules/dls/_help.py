...
-   name: --default-group
    type: string
    short-summary: Name of the default group to give permissions to for freshly created
...
type: command
short-summary: Enable the use of Azure Key Vault for encryption of a Data Lake Store
    account.
"""
...
-   name: Update the access control list for a file or folder.
    text: az dls fs access set-entry --account {account} --path {path} --acl-spec
...
type: command
short-summary: Set the owner information for a file or folder in a Data Lake Store
    account.
parameters:
-   name: --owner
    type: string
    short-summary: The user Azure Active Directory object ID or user principal name
        to set as the owner.
-   name: --group
    type: string
    short-summary: The group Azure Active Directory object ID or user principal name
        to set as the owning group.
"""
...
-   name: --permission
    type: int
    short-summary: The octal representation of the permissions for user, group and
        mask.
example:
-   name: Set full permissions for a user, read-execute permissions for a group, and
...
-   name: Set the permissions for a file or folder in a Data Lake Store account.
    text: az dls fs access set-permission --account {account} --path {path} --permission
        {permission}
    crafted: 'True'
-   name: Set the permissions for a file or folder in a Data Lake Store account.
    text: az dls fs access set-permission --account {account} --path {path} --permission
...
type: command
short-summary: Download a file or folder from a Data Lake Store account to the local
    machine.
parameters:
-   name: --source-path
    type: string
    short-summary: The full path in the Data Lake Store filesystem to download the
        file or folder from.
...
-   name: --thread-count
    type: int
    short-summary: 'Parallelism of the download. Default: The number of cores in the
        local machine.'
-   name: --chunk-size
    type: int
    short-summary: Size of a chunk, in bytes.
    long-summary: Large files are split into chunks. Files smaller than this size
        will always be transferred in a single thread.
-   name: --buffer-size
    type: int
    short-summary: Size of the transfer buffer, in bytes.
    long-summary: A buffer cannot be bigger than a chunk and cannot be smaller than
        a block.
-   name: --block-size
    type: int
    short-summary: Size of a block, in bytes.
    long-summary: Within each chunk, a smaller block is written for each API call.
        A block cannot be bigger than a chunk and must be bigger than a buffer.
examples:
-   name: Download a file or folder from a Data Lake Store account to the local machine.
    text: 'az dls fs download --account {account} --source-path {source-path} --output
        json --destination-path {destination-path} --overwrite '
    crafted: 'True'
-   name: Download a file or folder from a Data Lake Store account to the local machine.
    text: 'az dls fs download --account {account} --source-path {source-path} --output
        json --destination-path {destination-path} --overwrite '
...
-   name: --source-paths
    type: list
    short-summary: The space-separated list of files in the Data Lake Store account
        to join.
-   name: --destination-path
    type: string
...
-   name: --length
    ...
    long-summary: If not specified, attempts to preview the full file. If the file
        is > 1MB `--force` must be specified.
-   name: --offset
    type: long
...
-   name: --destination-path
    type: string
    short-summary: The full path in the Data Lake Store filesystem to upload the file
        or folder to.
-   name: --thread-count
    type: int
    short-summary: 'Parallelism of the upload. Default: The number of cores in the
        local machine.'
-   name: --chunk-size
    type: int
    short-summary: Size of a chunk, in bytes.
...
-   name: Upload a file or folder to a Data Lake Store account.
    text: 'az dls fs upload --account {account} --source-path {source-path} --destination-path
