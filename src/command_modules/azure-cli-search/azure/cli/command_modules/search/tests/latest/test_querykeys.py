...
class AzureSearchQueryKeysTests(ScenarioTest):
    ...
    def test_querykeys_create_delete_list(self, resource_group):
        self.kwargs.update({
            'sku_name': 'standard',
        ...
        _querykeys = self.cmd('az search querykeys list -n {name} -g {rg}').get_output_in_json()
        self.assertTrue(len(_querykeys) == 1)
        self.assertTrue(_querykeys[0]['name'] is None)
        self.assertTrue(len(_querykeys[0]['key']) > 1)

        _querykey_1 = self.cmd('az search querykeys create -n {name} -g {rg} --key-name {key_name_1}',
        ...
        _querykey_2 = self.cmd('az search querykeys create -n {name} -g {rg} --key-name {key_name_2}',
        ...
        self.cmd('az search querykeys delete -n {name} -g {rg} --key {key_value_1}')

        _querykeys = self.cmd('az search querykeys list -n {name} -g {rg}').get_output_in_json()
        self.assertFalse(_querykey_1['name'] in [x['key'] for x in _querykeys])

        self.cmd('az search querykeys delete -n {name} -g {rg} --key {key_value_2}')
