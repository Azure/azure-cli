...
class Delete(AAZCommand):
    ...
    :example: Remove non-compliance messages from a policy assignment that contain a specific message and a specific policy definition reference ID
    ...
    class SubresourceSelector(AAZJsonSelector):
        ...
        def _get(self):
            ...
            result = result.properties.nonComplianceMessages
            filters = enumerate(result)
            filters = filter(
                lambda e: e[1].message == self.ctx.args.message,
                filters
            )
            filters = filter(
                lambda e: e[1].policyDefinitionReferenceId == self.ctx.args.policy_definition_reference_id,
                filters
            )
            idx = next(filters)[0]
            return result[idx]
    ...
    class PolicyAssignmentsGet(AAZHttpOperation):
        ...
        def _build_schema_on_200(cls):
            ...
            _DeleteHelper._build_schema_policy_assignment_read(cls._schema_on_200)
    ...
    class PolicyAssignmentsCreate(AAZHttpOperation):
        ...
        def _build_schema_on_201(cls):
            ...
            _DeleteHelper._build_schema_policy_assignment_read(cls._schema_on_201)
