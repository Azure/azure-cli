...
def _update_ssl_binding(cmd, resource_group_name, name, certificate_thumbprint, ssl_type, hostname, slot=None):
    ...
    if not found_cert:
        ...
        for webapp_cert in webapp_certs:
            if webapp_cert.thumbprint == certificate_thumbprint:
                found_cert = webapp_cert
    # search for a cert that matches in the subscription
    if not found_cert:
        sub_certs = client.certificates.list()
        found_cert = next([c for c in sub_certs if c.thumbprint == certificate_thumbprint], None)
    if found_cert:
        if not hostname:
