...
from azure.cli.command_modules.vm.azure_stack._vm_utils import get_key_vault_base_url, \
    create_data_plane_keyvault_key_client
from azure.cli.command_modules.vm.azure_stack.custom import set_vm, _compute_client_factory, _is_linux_os
...
def decrypt_vmss(cmd, resource_group_name, vmss_name, volume_type=None, force=False):
    ...
    ade_extension = [x for x in extensions if
                     x.type_properties_type.lower() == extension['name'].lower() and x.publisher.lower() == extension[
                         'publisher'].lower()]  # pylint: disable=line-too-long
