...
def load_arguments(self, _):  # pylint: disable=too-many-locals, too-many-statements
    ...
    with self.argument_context('storage copy') as c:
        ...
        for item in ['destination', 'source']:
            ...
            c.argument('{}_container_name'.format(item), arg_group='Copy {}'.format(item),
                       help='Container name of copy {} storage account'.format(item))
            c.argument('{}_blob_name'.format(item), arg_group='Copy {}'.format(item),
                       help='Blob name in blob container of copy {} storage account'.format(item))
            c.argument('{}_share_name'.format(item), arg_group='Copy {}'.format(item),
        ...
        c.argument('check-md5', help='Specify how strictly MD5 hashes should be validated when downloading.\
                   Only available when downloading. Available options: NoCheck, LogOnly, FailIfDifferent, \
                   FailIfDifferentOrMissing. (default "FailIfDifferent")')
