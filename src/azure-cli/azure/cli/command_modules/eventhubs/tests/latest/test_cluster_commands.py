...
class EHNamespaceCURDScenarioTest(ScenarioTest):
    ...
    def test_eh_cluster(self):
        ...
        # Get Cluster
        getresponse = self.cmd('eventhubs cluster show --resource-group test-migration1 --name {clustername}',
                               checks=[self.check('sku.capacity', self.kwargs['capacity'])]).get_output_in_json()

        self.kwargs.update({'clusterid': getresponse['id']})

        # Create Namespace in cluster
        self.cmd('eventhubs namespace create --resource-group test-migration1 --name {namespacename} --location westus --tags {tags} --sku {sku} --cluster-arm-id {clusterid}')

        # Get namespaces created in the cluster
        listnsclusterresult = self.cmd('eventhubs cluster namespace list --resource-group test-migration1 --name {clustername}').output
        self.assertGreater(len(listnsclusterresult), 0)

        # update cluster
        self.cmd('eventhubs cluster update --resource-group test-migration1 --name {clustername} --tags {tags2}',
                 checks=[self.check('tags', {'tag2': 'value2'})])

        # Get cluster created in the resourcegroup
        listclusterresult = self.cmd('eventhubs cluster list --resource-group test-migration1').output
        self.assertGreater(len(listclusterresult), 0)

        # Delete cluster
        # commented as the cluster can be deleted only after 4 hours
        #self.cmd('eventhubs cluster delete --resource-group test-migration1 --name {clustername} --yes')
    ...
    def test_eh_self_serve_cluster(self):
        ...
        # Create Cluster
        self.cmd(
            'eventhubs cluster create --resource-group test-migration1 --name {clustername} --location {loc} --supports-scaling --capacity 2')

        # Get Cluster
        getresponse = self.cmd('eventhubs cluster show --resource-group test-migration1 --name {clustername}').get_output_in_json()
        ...
        # Create Namespace in cluster
        namespace = self.cmd(
                'eventhubs namespace create --resource-group test-migration1 --name {namespacename} --location {loc} --sku {sku} --cluster-arm-id {clusterid}').get_output_in_json()

        self.assertIsNotNone(namespace)

        self.cmd('eventhubs cluster update --resource-group test-migration1 --name {clustername} --capacity 1')
        ...
        listnsclusterresult = self.cmd(
            'eventhubs cluster namespace list --resource-group test-migration1 --name {clustername}').output
        self.assertGreater(len(listnsclusterresult), 0)
