...
from knack.prompting import prompt_pass, NoTTYException, prompt, prompt_y_n
...
class AKSCreateDecorator:
    def __init__(self, cmd, client, resource_type, raw_parameters):
        self.cmd = cmd
        self.client = client
        self.resource_type = resource_type
        self.context = AKSCreateContext()
        self.models = AKSCreateModels(
            cmd=self.cmd, resource_type=self.resource_type
        )
    ...
    def check_params(self):
        ...
        if (
            ...
            and self.param.load_balancer_sku == "basic"
        ):
            raise CLIError(
    ...
    def set_up_agent_pool_profiles(self):
        ...
        if self.param.node_osdisk_size:
            agent_pool_profile.os_disk_size_gb = int(
                self.param.node_osdisk_size
            )
            ...
            ssh_config = self.models.ContainerServiceSshConfiguration(
                public_keys=[
                    self.models.ContainerServiceSshPublicKey(
                        key_data=self.param.ssh_key_value
                    )
                ]
            )
    ...
    def set_up_windows_profile(self):
        ...
        if (
            ...
            or self.param.windows_admin_password
        ):
            ...
            if self.param.windows_admin_username is None:
                try:
                    self.param.windows_admin_username = prompt(
    ...
    def set_up_service_principal_profile(self):
        ...
        if not (
            ...
            and not self.param.service_principal
            and not self.param.client_secret
        ):
            principal_obj = _ensure_aks_service_principal(
                self.cmd.cli_ctx,
                service_principal=self.param.service_principal,
                client_secret=self.param.client_secret,
                subscription_id=self.context.subscription_id,
                dns_name_prefix=self.param.dns_name_prefix,
                fqdn_subdomain=self.param.fqdn_subdomain,
        ...
        if (
            ...
            and not self.param.skip_subnet_role_assignment
            and not subnet_role_assignment_exists(
                self.cmd.cli_ctx, self.param.vnet_subnet_id
            )
        ):
            ...
            if (
                service_principal_profile is None
                and not self.param.assign_identity
            ):
                msg = (
    ...
    def check_outbound_type(self):
        self.param.outbound_type = _set_outbound_type(
            self.param.outbound_type,
            self.param.vnet_subnet_id,
            self.param.load_balancer_sku,
            self.param.load_balancer_profile,
        )
    ...
    def set_up_network_profile(self):
        ...
        if any(
            [
                self.param.network_plugin,
                self.param.pod_cidr,
                self.param.service_cidr,
                self.param.dns_service_ip,
                self.param.docker_bridge_address,
                self.param.network_policy,
            ]
        ):
            ...
            if (
                self.param.load_balancer_sku.lower() == "standard"
                or self.context.load_balancer_profile
            ):
                network_profile = self.models.ContainerServiceNetworkProfile(
    ...
    def set_up_aad_profile(self):
        ...
        if self.param.enable_aad:
            if any(
                [
                    self.param.aad_client_app_id,
                    self.param.aad_server_app_id,
                    self.param.aad_server_app_secret,
                ]
            ):
                raise CLIError(
            ...
            if self.param.aad_admin_group_object_ids is not None:
                raise CLIError(
                    '"--aad-admin-group-object-ids" can only be used together with "--enable-aad"'
                )
    ...
    def set_up_api_server_access_profile(self):
        ...
        if (
            ...
            and self.param.load_balancer_sku.lower() != "standard"
        ):
            raise CLIError(
        ...
        if (
            self.param.api_server_authorized_ip_ranges
            or self.param.enable_private_cluster
        ):
    ...
    def set_up_identity(self):
        ...
        if (
            ...
            and self.param.assign_identity
        ):
            raise ArgumentUsageError(
    ...
    def set_up_identity_profile(self):
        ...
        if self.param.assign_kubelet_identity:
            ...
            kubelet_identity = _get_user_assigned_identity(
                self.cmd.cli_ctx, self.param.assign_kubelet_identity
            )
            identity_profile = {
                # pylint: disable=line-too-long
            ...
            cluster_identity_object_id = _get_user_assigned_identity_object_id(
                self.cmd.cli_ctx, self.param.assign_identity
            )
    ...
    def set_up_mc_with_params(self):
        ...
        self.mc.cluster_autoscaler_profile = (
    ...
    def set_up_private_dns(self):
        ...
        if self.param.private_dns_zone:
            ...
            if (
                self.param.private_dns_zone.lower()
                != CONST_PRIVATE_DNS_ZONE_SYSTEM
                and self.param.private_dns_zone.lower()
