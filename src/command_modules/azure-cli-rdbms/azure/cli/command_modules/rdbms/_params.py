...
def load_arguments(self, _):    # pylint: disable=too-many-statements
    ...
    for scope in ['mysql server vnet-rule', 'postgres server vnet-rule']:
        with self.argument_context(scope) as c:
            ...
            for scope1 in ['postgres server vnet-rule create', 'postrgres server vnet-rule update', 'mysql server vnet-rule create', 'mysql server vnet-rule update']:
                with self.argument_context(scope1) as d:
                    d.extra('vnet_name', options_list=['--vnet-name'], help='The virtual network name', validator=validate_subnet)
