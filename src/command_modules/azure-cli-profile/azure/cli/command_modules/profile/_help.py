...
examples:
-   name: Get a token for utilities to access Azure.
...
examples:
-   name: Set a subscription to be the current active subscription.
...
long-summary: If the subscription isn't specified, shows the details of the default
    subscription.
"""
...
-   name: Log in interactively.
    text: >
        az login
-   name: Log in with user name and password. This doesn't work with Microsoft accounts
        or accounts that have two-factor authentication enabled.
    text: >
        az login -u johndoe@contoso.com -p VerySecret
-   name: Log in with a service principal using client secret.
    text: >
        az login --service-principal -u http://azure-cli-2016-08-05-14-31-15 -p VerySecret
...
-   name: Log in using a VM's system assigned identity
    text: >
        az login --identity
-   name: Log in using a VM's user assigned identity. Client or object ids of the
        service identity also work
    text: >
