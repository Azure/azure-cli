...
-   name: --resource-group -g
    type: string
    short-summary: >
        Name of the resource group under which the action groups are being listed.
        If it is omitted, all the action groups under
...
-   name: --remove-action -r
    short-summary: Remove receivers from the action group. Accept space-separated
...
type: command
short-summary: Add action groups to this activity log alert. It can also be used to
    overwrite existing webhook properties of particular action groups.
parameters:
-   name: --name -n
...
-   name: --webhook-properties -w
    short-summary: >
        Space-separated webhook properties in 'key[=value]' format. These properties
        will be associated with
        the action groups added in this command.
    long-summary: >
        For any webhook receiver in these action group, these data are appended to
        the webhook payload.
        To attach different webhook properties to different action groups, add the
        action groups in separate update-action commands.
-   name: --strict
    short-summary: Fails the command if an action group to be added will change existing
        webhook properties.
examples:
...
-   name: Remove webhook properties from an existing action group.
    text: |
        az monitor activity-log alert action-group add -n {AlertName} -g {ResourceGroup} \\
          -a /subscriptions/{SubID}/resourceGroups/{ResourceGroup}/providers/microsoft.insights/actionGroups/{ActionGroup}
-   name: Add new action groups but prevent the command from accidently overwrite
...
long-summary: This command will create a default activity log with one condition which
    compares if the activities logs 'category' field equals to 'ServiceHealth'. The
    newly created activity log alert does not have any action groups attached to it.
parameters:
-   name: --name -n
    short-summary: Name of the activity log alerts
-   name: --scope -s
    ...
    long-summary: >
        The alert will only apply to activity logs with resourceIDs that fall under
...
-   name: --condition -c
    short-summary: The condition that will cause the alert to activate. The format
    ...
    long-summary: >
        ...
        'resourceProvider', 'status', 'subStatus', 'resourceType', or anything beginning
        with 'properties.'.
-   name: --action-group -a
    short-summary: >
        Add an action group. Accepts space-separated action group identifiers. The
        identifier can be the action group's name
        or its resource ID.
-   name: --webhook-properties -w
    short-summary: >
        Space-separated webhook properties in 'key[=value]' format. These properties
        are associated with the action groups
        added in this command.
    long-summary: >
        For any webhook receiver in these action group, this data is appended to the
        webhook payload. To attach different webhook
...
-   name: Create an alert with condition about error level service health log.
    text: >
        az monitor activity-log alert create -n {AlertName} -g {ResourceGroup}   --condition
        category=ServiceHealth and level=Error
-   name: Create an alert with an action group and specify webhook properties.
    text: >
        az monitor activity-log alert create -n {AlertName} -g {ResourceGroup}   -a
        /subscriptions/{SubID}/resourceGroups/{ResourceGroup}/providers/microsoft.insights/actionGroups/{ActionGroup}   -w
        usage=test owner=jane
-   name: Create an alert which is initially disabled.
    text: >
        az monitor activity-log alert create -n {AlertName} -g {ResourceGroup} --disable
-   name: Create a default activity log alert.
    text: az monitor activity-log alert create --condition {condition} --resource-group
        {ResourceGroup} --action-group {action-group} --name {AlertName}
    crafted: 'True'
-   name: Create a default activity log alert.
    text: az monitor activity-log alert create --condition {condition} --resource-group
        {ResourceGroup} --action-group {action-group} --name {AlertName}
...
parameters:
-   name: --resource-group -g
    short-summary: Name of the resource group under which the activity log alerts
        are being listed. If it is omitted, all the activity log alerts under the
        current subscription are listed.
examples:
-   name: List activity log alerts under a resource group or the current subscription.
    text: az monitor activity-log alert list --query [0]
    crafted: 'True'
-   name: List activity log alerts under a resource group or the current subscription.
    text: az monitor activity-log alert list --query [0]
    crafted: true
...
-   name: --description
    short-summary: A description of this activity log alert.
-   name: --condition -c
    short-summary: The conditional expression that will cause the alert to activate.
...
-   name: Disable an alert
    text: >
        az monitor activity-log alert update -n {AlertName} -g {ResourceGroup} --enable
        false
"""
...
-   name: --caller
    short-summary: Caller to query for, such as an e-mail address or service principal
        ID.
-   name: --status
    short-summary: >
...
-   name: --offset
    ...
    long-summary: >
        Can be used with either --start-time or --end-time. If used with --start-time,
        then
        the end time will be calculated by adding the offset. If used with --end-time
        (default), then
        the start time will be calculated by subtracting the offset. If --start-time
        and --end-time are
...
-   name: List and query activity log events.
    text: az monitor activity-log list --query [0] --start-time {start-time} --resource-group
        {ResourceGroup}
    crafted: 'True'
-   name: List and query activity log events.
    text: az monitor activity-log list --query [0] --start-time {start-time} --resource-group
...
-   name: --condition
    ...
    long-summary: >
        The form of a condition is "METRIC {>,>=,<,<=} THRESHOLD {avg,min,max,total,last}
        PERIOD".
        Values for METRIC and appropriate THRESHOLD values can be obtained from `az
        monitor metric` commands,
...
-   name: Create a high CPU usage alert on a VM with email and webhook actions.
    text: |
        az monitor alert create -n rule1 -g {ResourceGroup} --target {VirtualMachineID}     --condition "Percentage CPU } 90 avg 5m"     --action email bob@contoso.com ann@contoso.com --email-service-owners     --action webhook https://www.contoso.com/alerts?type=HighCPU     --action webhook https://alerts.contoso.com apiKey={APIKey} type=HighCPU
-   name: Create a classic metric-based alert rule.
    text: az monitor alert create --action {action} --condition "Percentage CPU }
        90 avg 5m" --resource-group {ResourceGroup} --target {VirtualMachineID} --email-service-owners
        false --name rule1 --target-type {target-type}
    crafted: 'True'
-   name: Create a classic metric-based alert rule.
    text: az monitor alert create --action {action} --condition "Percentage CPU }
        90 avg 5m" --resource-group {ResourceGroup} --target {VirtualMachineID} --email-service-owners
...
examples:
-   name: List alert rules in a resource group.
...
-   name: --period
    short-summary: >
        Time span over which to apply --aggregation, in nDnHnMnS shorthand or full
        ISO8601 format.
"""
...
-   name: --action -a
    ...
    long-summary: |
        ...
        Multiple actions can be specified by using more than one `--action` argument.
examples:
-   name: Create autoscale settings to scale between 2 and 5 instances (3 as default).
        Email the administrator when scaling occurs.
    text: |
...
-   name: --recurrence -r
    short-summary: When the profile recurs. If omitted, a fixed (non-recurring) profile
        is created.
    long-summary: |
        Usage:     --recurrence {week} [ARG ARG ...]
        Weekly:    --recurrence week Sat Sun
-   name: --start
    short-summary: When the autoscale profile begins. Format depends on the type of
        profile.
    long-summary: |
        Fixed:  --start yyyy-mm-dd [hh:mm:ss]
        Weekly: [--start hh:mm]
-   name: --end
    short-summary: When the autoscale profile ends. Format depends on the type of
        profile.
    long-summary: |
        Fixed:  --end yyyy-mm-dd [hh:mm:ss]
        Weekly: [--end hh:mm]
examples:
-   name: Create a fixed date profile, inheriting the default scaling rules but changing
        the capacity.
    text: |
...
-   name: Create a recurring weekend profile, inheriting the default scaling rules
        but changing the capacity.
    text: |
        az monitor autoscale create -g {myrg} --resource {resource-id} --min-count 2 --count 3 \\
          --max-count 5
...
-   name: --condition
    ...
    long-summary: >
        The form of a condition is "METRIC {==,!=,>,>=,<,<=} THRESHOLD {avg,min,max,total,count}
        PERIOD".
        Values for METRIC and appropriate THRESHOLD values can be obtained from the
        `az monitor metric` command.
...
-   name: --timegrain
    ...
    long-summary: >
        The form of the timegrain is {avg,min,max,sum} VALUE. Values can be obtained
        from the `az monitor metric` command.
        Format of VALUE is "##h##m##s".
examples:
-   name: Scale to 5 instances when the CPU Percentage across instances is greater
        than 75 averaged over 10 minutes.
    text: |
        az monitor autoscale rule create -g {myrg} --autoscale-name {myvmss} \\
          --scale to 5 --condition "Percentage CPU } 75 avg 10m"
-   name: Scale up 2 instances when the CPU Percentage across instances is greater
        than 75 averaged over 5 minutes.
    text: |
        az monitor autoscale rule create -g {myrg} --autoscale-name {myvmss} \\
          --scale out 2 --condition "Percentage CPU } 75 avg 5m"
-   name: Scale down 50% when the CPU Percentage across instances is less than 25
...
-   name: --resource-group -g
    type: string
    short-summary: Name of the resource group for the Log Analytics and Storage Account
        when the name of the service instead of a full resource ID is given.
-   name: --logs
    type: string
    short-summary: JSON encoded list of logs settings. Use '@{file}' to load from
        a file.
-   name: --metrics
    type: string
    short-summary: JSON encoded list of metric settings. Use '@{file}' to load from
        a file.
...
-   name: --workspace
    type: string
    short-summary: Name or ID of the Log Analytics workspace to send diagnostic logs
        to.
-   name: --event-hub
    type: string
    short-summary: >
        Name or ID an event hub. If none is specified, the default event hub will
        be selected.
-   name: --event-hub-rule
...
-   name: Create diagnostic settings for the specified resource.
    text: az monitor diagnostic-settings create --event-hub {event-hub} --resource
        {resource} --logs  --event-hub-rule {event-hub-rule} --name MyDiagnosticSettings
    crafted: 'True'
-   name: Create diagnostic settings for the specified resource.
    text: az monitor diagnostic-settings create --event-hub {event-hub} --resource
        {resource} --logs  --event-hub-rule {event-hub-rule} --name MyDiagnosticSettings
...
-   name: --locations
    short-summary: Space-separated list of regions for which Activity Log events should
        be stored.
-   name: --categories
    short-summary: Space-separated categories of the logs. These categories are created
        as is convenient to the user. Some values are Write, Delete, and/or Action.
-   name: --storage-account-id
    short-summary: The resource id of the storage account to which you would like
        to send the Activity Log.
-   name: --service-bus-rule-id
    short-summary: The service bus rule ID of the service bus namespace in which you
        would like to have Event Hubs created for streaming the Activity Log. The
        rule ID is of the format '{service bus resource ID}/authorizationrules/{key
        name}'.
-   name: --days
    short-summary: The number of days for the retention in days. A value of 0 will
...
parameters:
-   name: --action -a
    short-summary: Add an action group and optional webhook properties to fire when
...
-   name: Create a high CPU usage alert on a VM with email and webhook actions.
    text: |
        az monitor metrics alert create -n alert1 -g {ResourceGroup} --scopes {VirtualMachineID}     --condition "avg Percentage CPU } 90" --window-size 5m --evaluation-frequency 1m     --action {actionGroupId} apiKey={APIKey} type=HighCPU --description "High CPU"
-   name: Create an alert when a storage account shows a high number of slow transactions,
        using multi-dimensional filters.
    text: |
        az monitor metrics alert create -g {ResourceGroup} -n alert1 --scopes {StorageAccountId}     --description "Storage Slow Transactions"     --condition "total transactions } 5 where ResponseType includes Success"     --condition "avg SuccessE2ELatency } 250 where ApiName includes GetBlob or PutBlob"
-   name: Create a metric-based alert rule.
    text: 'az monitor metrics alert create --action {action} --condition "avg Percentage
        CPU } 90" --description "High CPU" --resource-group {ResourceGroup} --name
        alert1 --scopes {VirtualMachineID} --window-size ##h##m##s --evaluation-frequency
        ##h##m##s'
    crafted: 'True'
-   name: Create a metric-based alert rule.
    text: 'az monitor metrics alert create --action {action} --condition "avg Percentage
...
-   name: --filter
    short-summary: A string used to reduce the set of metric data returned. eg. "BlobType
        eq '*'"
...
-   name: --dimension
    short-summary: The list of dimensions (space-separated) the metrics are queried
        into.
-   name: --namespace
...
-   name: List success E2E latency of a storage account and split the data series
        based on API name
    text: >
        az monitor metrics list --resource {ResourceName} --metric SuccessE2ELatency                         --dimension
        ApiName
-   name: List success E2E latency of a storage account and split the data series
        based on both API name and geo type
    text: >
        az monitor metrics list --resource {ResourceName} --metric SuccessE2ELatency                         --dimension
        ApiName GeoType
-   name: List success E2E latency of a storage account and split the data series
        based on both API name and geo type using "--filter" parameter
    text: >
        az monitor metrics list --resource {ResourceName} --metric SuccessE2ELatency                         --filter
        "ApiName eq '*' and GeoType eq '*'"
-   name: List success E2E latency of a storage account and split the data series
        based on both API name and geo type. Limits the api name to 'DeleteContainer'
    text: >
        az monitor metrics list --resource {ResourceName} --metric SuccessE2ELatency                         --filter
        "ApiName eq 'DeleteContainer' and GeoType eq '*'"
-   name: List transactions of a storage account per day since 2017-01-01
    text: >
        az monitor metrics list --resource {ResourceName} --metric Transactions                         --start-time
        2017-01-01T00:00:00Z                         --interval PT24H
-   name: List the metadata values for a storage account under transaction metric's
        api name dimension since 2017
    text: >
        az monitor metrics list --resource {ResourceName} --metric Transactions                         --filter
        "ApiName eq '*'"                         --start-time 2017-01-01T00:00:00Z
-   name: List the metric values for a resource.
    text: az monitor metrics list --resource-namespace {resource-namespace} --interval
        {interval} --resource {ResourceName} --start-time {start-time} --resource-group
        MyResourceGroup --output json --resource-type {resource-type} --metrics "Percentage
        CPU" --end-time {end-time} --aggregation Average
    crafted: 'True'
-   name: List the metric values for a resource.
    text: az monitor metrics list --resource-namespace {resource-namespace} --interval
        {interval} --resource {ResourceName} --start-time {start-time} --resource-group
...
examples:
-   name: Lists the metric definitions for the resource.
