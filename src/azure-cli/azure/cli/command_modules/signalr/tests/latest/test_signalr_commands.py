...
class AzureSignalRServiceScenarioTest(ScenarioTest):
    ...
    def test_signalr_commands(self, resource_group):
        ...
        update_enable_message_logs = 'False'
        ...
        self.kwargs.update({
            ...
            'enable_message_logs': enable_message_logs,
            'service_mode': service_mode,
            'allowed_origins': ' '.join(allowed_origins),
            'updated_sku': updated_sku,
            'updated_tags': '{}={}'.format(tags_key, updated_tags_val),
            'update_service_mode': update_service_mode,
            'update_enable_message_logs': update_enable_message_logs,
            'added_allowed_origins': ' '.join(added_allowed_origins),
            'default_action': "Deny",
        })

        # Test create
        self.cmd('az signalr create -n {signalr_name} -g {rg} --sku {sku} --unit-count {unit_count} -l {location} --tags {tags} --service-mode {service_mode} --enable-message-logs {enable_message_logs} --allowed-origins {allowed_origins} --default-action {default_action}',
                 checks=[
                     ...
                     self.check('features[2].value', '{enable_message_logs}'),
        ...
        self.cmd('az signalr list -g {rg}', checks=[
            ...
            self.check('[0].features[2].value', '{enable_message_logs}'),
        ...
        self.cmd('az signalr update -n {signalr_name} -g {rg} --sku {updated_sku} --tags {updated_tags} --service-mode {update_service_mode} --enable-message-logs {update_enable_message_logs}',
                 checks=[
                     ...
                     self.check('features[1].value', '{update_enable_message_logs}'),
                     self.check('cors.allowedOrigins', allowed_origins),
                     self.exists('hostName'),
                     self.exists('publicPort'),
                     self.exists('serverPort')
