...
def load_arguments(self, _):  # pylint: disable=too-many-locals, too-many-statements, too-many-lines, too-many-branches, line-too-long
    ...
    for item in ['stats', 'exists', 'metadata show', 'metadata update', 'delete']:
    ...
    with self.argument_context('storage queue create') as c:
        c.extra('fail_on_exist', help='Specify whether to throw an exception if the queue already exists.')
        c.extra('metadata', help='Metadata in space-separated key=value pairs. This overwrites any existing metadata.')
        c.extra('timeout', help='Request timeout in seconds. Apply to each call to the service.', type=int)

    with self.argument_context('storage queue delete') as c:
        c.extra('fail_not_exist', help='Specify whether to throw an exception if the queue doesn\'t exist.')
