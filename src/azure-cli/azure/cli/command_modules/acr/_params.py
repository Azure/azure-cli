...
def load_arguments(self, _):  # pylint: disable=too-many-statements
    ...
    with self.argument_context('acr run') as c:
        ...
        c.argument('log_template', options_list=['--log-template'], help="The repository and tag template for run log artifact using the format: 'log/repo:tag' (e.g., 'acr/logs:{{.Run.ID}}')", is_preview=True)
    ...
    with self.argument_context('acr task') as c:
        ...
        c.argument('log_template', options_list=['--log-template'], help="The repository and tag template for run log artifact using the format: 'log/repo:tag' (e.g., 'acr/logs:{{.Run.ID}}')", is_preview=True)
        c.argument('is_system_task', options_list=['--is-system-task'], help="Indicates whether the task resource is a system task. The name of the task must be 'quicktask'", action='store_true', is_preview=True)
