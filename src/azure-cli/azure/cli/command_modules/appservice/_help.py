...
type: group
short-summary: Troubleshoot a web app's runtime state.
long-summary: >
    Inspect runtime state and recent startup attempts for a Linux web app.
    Useful when an app is returning HTTP 5xx or otherwise behaving
    unexpectedly. Backed by KuduLite endpoints that aggregate the last 24
    hours of startup logs across all instances.
"""
...
long-summary: |
    ...
    Use --instance to scope both to a single worker. Use `-o json` to get the
    underlying structured payload.
examples:
  - name: Show status for all instances of a web app
  ...
  - name: Get the raw structured payload
    text: az webapp troubleshoot status --name MyWebApp --resource-group MyResourceGroup --output json
"""
