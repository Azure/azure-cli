...
class JsonCTemplatePolicy(SansIOHTTPPolicy):
    ...
    # Obtain the template data and then splice it with other properties into valid JSON format
    def on_request(self, request):
        ...
        if modified_data.get('properties', {}).get('template'):
            ...
            # The template is embedded directly as a JSON object (not as an escaped string) to avoid deserialization issues on the service side.
            # The key "template" must be properly quoted to produce valid JSON that all Azure environments (including custom clouds) can accept.
