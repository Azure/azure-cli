...
def load_command_table(self, _):
    ...
    with self.command_group('acr helm', acr_helm_util,
        ...
        g.command('delete', 'acr_helm_delete',
                            deprecate_info=self.deprecate(redirect="Helm v3 commands",
                                                          message_func=_helm_deprecate_message))
        g.command('push', 'acr_helm_push', deprecate_info=self.deprecate(redirect="Helm v3 commands",
                                                          message_func=_helm_deprecate_message))
        g.command('repo add', 'acr_helm_repo_add', deprecate_info=self.deprecate(redirect="Helm v3 commands",
                                                          message_func=_helm_deprecate_message))
        g.command('install-cli', 'acr_helm_install_cli', is_preview=True, deprecate_info=self.deprecate(redirect="Helm v3 commands",
                                                          message_func=_helm_deprecate_message))
