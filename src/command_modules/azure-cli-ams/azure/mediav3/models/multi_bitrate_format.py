# --------------------------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.
# --------------------------------------------------------------------------------------------
# pylint: skip-file
# coding=utf-8
# --------------------------------------------------------------------------
# Code generated by Microsoft (R) AutoRest Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is
# regenerated.
# --------------------------------------------------------------------------

from .format import Format


class MultiBitrateFormat(Format):
    """GOP aligned multi bitrate output.

    You probably want to use the sub-classes and not this class directly. Known
    sub-classes are: CmafFormat, MultiBitrateMp4Format, TransportStreamFormat

    :param filename_pattern: Gets or sets the pattern of the filename to use
     excluding the extension. REVIEW: List "macros" that can be used and give
     examples.
    :type filename_pattern: str
    :param odatatype: Constant filled by server.
    :type odatatype: str
    :param manifest_filename: Gets or sets the pattern of the manifest file
     name to use excluding the extension. REVIEW: List "macros" that can be
     used and give examples.
    :type manifest_filename: str
    :param output_files: Gets the list of output files.
    :type output_files: list[~accounts.models.OutputFile]
    """

    _validation = {
        'odatatype': {'required': True},
    }

    _attribute_map = {
        'filename_pattern': {'key': 'filenamePattern', 'type': 'str'},
        'odatatype': {'key': '@odata\\.type', 'type': 'str'},
        'manifest_filename': {'key': 'manifestFilename', 'type': 'str'},
        'output_files': {'key': 'outputFiles', 'type': '[OutputFile]'},
    }

    _subtype_map = {
        'odatatype': {'#Microsoft.Media.CmafFormat': 'CmafFormat', '#Microsoft.Media.MultiBitrateMp4Format': 'MultiBitrateMp4Format', '#Microsoft.Media.TransportStreamFormat': 'TransportStreamFormat'}
    }

    def __init__(self, filename_pattern=None, manifest_filename=None, output_files=None):
        super(MultiBitrateFormat, self).__init__(filename_pattern=filename_pattern)
        self.manifest_filename = manifest_filename
        self.output_files = output_files
        self.odatatype = '#Microsoft.Media.MultiBitrateFormat'
