...
class ContainerappIdentityTests(ScenarioTest):
    ...
    def test_containerapp_identity_system(self, resource_group):
        ...
        self.cmd('containerapp env create -g {} -n {} --logs-workspace-id {} --logs-workspace-key {}'.format(resource_group,
                 env_name, logs_workspace_id, logs_workspace_key))
...
class ContainerappIngressTests(ScenarioTest):
    ...
    def test_containerapp_custom_domains_e2e(self, resource_group):
        ...
        self.cmd("appservice domain create -g {} --hostname {} --contact-info=@'{}' --accept-terms".format(resource_group,
                 zone_name, contacts)).get_output_in_json()
        self.cmd('network dns record-set txt add-record -g {} -z {} -n {} -v {}'.format(resource_group,
                 zone_name, txt_name_1, verification_id)).get_output_in_json()
        self.cmd('network dns record-set txt add-record -g {} -z {} -n {} -v {}'.format(resource_group,
                 zone_name, txt_name_2, verification_id)).get_output_in_json()
        ...
        self.cmd('containerapp hostname bind -g {} -n {} --hostname {} --thumbprint {}'.format(resource_group,
                 ca_name, hostname_2, cert_thumbprint), expect_failure=True)
        ...
        self.cmd('containerapp hostname delete -g {} -n {} --hostname {} -l "{}" --yes'.format(resource_group,
                 ca_name, hostname_1, "eastus2"), expect_failure=True)
        ...
        self.cmd('containerapp hostname bind -g {} -n {} --hostname {} --certificate {}'.format(resource_group,
                 ca_name, hostname_1, cert_name), expect_failure=True)
    ...
    def test_containerapp_tcp_ingress(self, resource_group):
        ...
        sub_id = self.cmd(
            f"az network vnet subnet create --address-prefixes '14.0.0.0/23' -n sub -g {resource_group} --vnet-name {vnet}").get_output_in_json()["id"]
...
class ContainerappEnvStorageTests(ScenarioTest):
    ...
    def test_containerapp_env_storage(self, resource_group):
        ...
        storage_keys = self.cmd('az storage account keys list -g {} -n {}'.format(resource_group,
                                storage_name)).get_output_in_json()[0]
...
class ContainerappScaleTests(ScenarioTest):
    ...
    def test_containerapp_create_with_vnet_yaml(self, resource_group):
        ...
        sub_id = self.cmd(
            f"network vnet subnet create --address-prefixes '14.0.0.0/23' -n sub -g {resource_group} --vnet-name {vnet}").get_output_in_json()["id"]
