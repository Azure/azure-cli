...
class ListWithCustomerId(AAZCommand):
    """List all Azure credits for a customer. The API is only supported for Microsoft Partner  Agreements (MPA) billing accounts.
    """

    _aaz_info = {
        "version": "2023-11-01",
        "resources": [
            ["mgmt-plane", "/providers/microsoft.billing/billingaccounts/{}/customers/{}/providers/microsoft.consumption/lots", "2023-11-01"],
        ]
    }
    ...
    def _build_arguments_schema(cls, *args, **kwargs):
        ...
        _args_schema.filter = AAZStrArg(
            options=["--filter"],
            help="May be used to filter the lots by Status, Source etc. The filter supports 'eq', 'lt', 'gt', 'le', 'ge', and 'and'. Tag filter is a key value pair string where key and value is separated by a colon (:).",
        )
        return cls._args_schema

    def _execute_operations(self):
        self.pre_operations()
        self.LotsListByCustomer(ctx=self.ctx)()
    ...
    class LotsListByCustomer(AAZHttpOperation):
        ...
        def url_parameters(self):
            parameters = {
                ...
                **self.serialize_url_param(
                    "customerId", self.ctx.args.customer_id,
                    required=True,
                ),
            }
            return parameters
        ...
        def _build_schema_on_200(cls):
            ...
            _ListWithCustomerIdHelper._build_schema_amount_read(properties.closed_balance)
            properties.closed_balance_in_billing_currency = AAZObjectType(
                serialized_name="closedBalanceInBillingCurrency",
            )
            _ListWithCustomerIdHelper._build_schema_amount_with_exchange_rate_read(properties.closed_balance_in_billing_currency)
            ...
            _ListWithCustomerIdHelper._build_schema_amount_read(properties.original_amount)
            properties.original_amount_in_billing_currency = AAZObjectType(
                serialized_name="originalAmountInBillingCurrency",
            )
            _ListWithCustomerIdHelper._build_schema_amount_with_exchange_rate_read(properties.original_amount_in_billing_currency)
            ...
            _ListWithCustomerIdHelper._build_schema_amount_read(properties.used_amount)
...
class _ListWithCustomerIdHelper:
    """Helper class for ListWithCustomerId"""
