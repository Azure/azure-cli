...
def load_arguments(self, _):
    ...
    with self.argument_context('migrate local replication init') as c:
        ...
        c.argument('project_name',
                   project_name_type,
                   required=True,
                   help='Specifies the name of the Azure Migrate project to be used '
    ...
    with self.argument_context('migrate local replication new') as c:
        ...
        c.argument('machine_index',
                   ...
                   type=int,
                   help='Specifies the index (1-based) of the discovered server from the list. '
        ...
        c.argument('disk_to_include',
                   options_list=['--disk-to-include'],
                   nargs='+',
                   help='Specifies the disks on the source server to be included for replication. '
                        'Space-separated list of disk IDs.')
        c.argument('nic_to_include',
                   options_list=['--nic-to-include'],
                   nargs='+',
                   help='Specifies the NICs on the source server to be included for replication. '
                        'Space-separated list of NIC IDs.')
        c.argument('target_resource_group_id',
                   options_list=['--target-resource-group-id', '--target-rg-id'],
                   help='Specifies the target resource group ARM ID where the migrated VM '
        ...
        c.argument('target_vm_name',
                   ...
                   required=True)
        c.argument('os_disk_id',
                   options_list=['--os-disk-id'],
                   help='Specifies the operating system disk for the source server to be migrated.')
        c.argument('source_appliance_name',
                   options_list=['--source-appliance-name'],
                   help='Specifies the source appliance name for the AzLocal scenario.',
                   required=True)
