...
def create_assign_sp_to_mediaservice(cmd, client, account_name, resource_group_name, sp_name=None,
    ...
    from azure.cli.command_modules.role.custom import (create_service_principal_for_rbac, create_role_assignment,
                                                       show_application, list_role_assignments)
    ...
    if aad_sps:
        ...
        app = show_application(graph_client.applications, aad_sps[0].app_id)

        _update_password_credentials(graph_client, app.object_id, sp_password, years)

    else:
        create_sp_result = create_service_principal_for_rbac(cmd, name=sp_name,
                                                             password=sp_password, skip_assignment=True)
    ...
    if assignments:
        if not list(filter(lambda x: x['properties']['roleDefinitionName'] == role, assignments)):
            create_role_assignment(cmd, role, assignee=app_id, scope=ams.id)
    else:
        create_role_assignment(cmd, role, assignee=app_id, scope=ams.id)
...
def _update_password_credentials(client, app_object_id, sp_password, years):
    ...
    app_creds.append(PasswordCredential(start_date=start_date, end_date=end_date,
                                        key_id=str(uuid.uuid4()), value=sp_password))
