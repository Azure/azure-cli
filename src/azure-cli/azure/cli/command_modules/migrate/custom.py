...
def get_discovered_server(cmd,
    ...
    from azure.cli.command_modules.migrate._get_discovered_server_helpers import (
    ...
    # Build the base URI
    base_uri = build_base_uri(subscription_id, resource_group_name, project_name,
    ...
    request_uri = f"{cmd.cli_ctx.cloud.endpoints.resource_manager}{base_uri}?{'&'.join(query_params)}"
...
def initialize_replication_infrastructure(cmd,
    ...
    """
    from azure.cli.core.commands.client_factory import get_subscription_id
    from azure.cli.command_modules.migrate._initialize_replication_infrastructure_helpers import (
...
def new_local_server_replication(cmd,
    ...
    from azure.cli.command_modules.migrate._new_local_server_replication_helpers import (
    ...
    is_dynamic_ram_enabled, is_power_user_mode = validate_required_parameters(
        machine_id,
        target_storage_path_id,
        target_resource_group_id,
        target_vm_name,
    ...
    try:
        site_type, site_name, machine_name, run_as_account_id, instance_type = validate_ARM_id_formats(
            machine_id,
            target_storage_path_id,
            target_resource_group_id,
            target_virtual_switch_id,
        ...
        if site_type == SiteTypes.HyperVSites.value:
            ...
            run_as_account_id, machine, site_object, instance_type = process_site_type_vmware(cmd,
                             rg_uri,
                             site_name,
                             machine_name,
                             subscription_id,
        ...
        source_fabric, fabric_instance_type, instance_type, all_fabrics = process_source_fabric(
            cmd,
            rg_uri,
            app_map,
            source_appliance_name,
