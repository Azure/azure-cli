...
canipull_image = "mcr.microsoft.com/aks/canipull:0.0.1-alpha"
...
def load_arguments(self, _):
    ...
    with self.argument_context("aks create") as c:
        ...
        c.argument(
            "api_server_authorized_ip_ranges", type=str, validator=validate_ip_ranges
        )
    ...
    with self.argument_context("aks enable-addons") as c:
        ...
        c.argument(
            ...
            help="Name of an existing subnet to use with the virtual-node add-on.",
        )
        c.argument(
            "appgw_name", options_list=["--appgw-name"], arg_group="Application Gateway"
        )
    ...
    for scope in ["aks", "acs kubernetes", "acs dcos"]:
        with self.argument_context("{} install-cli".format(scope)) as c:
            ...
            c.argument(
                "base_src_url", help="Base download source URL for kubectl releases."
            )
            c.argument(
                "kubelogin_version",
    ...
    with self.argument_context("aks check-acr") as c:
        c.argument("acr", help="The FQDN of the ACR.")
    ...
    with self.argument_context("aks use-dev-spaces") as c:
        ...
        c.argument(
            "endpoint_type",
            get_enum_type(["Public", "Private", "None"], default="Public"),
    ...
    with self.argument_context("openshift") as c:
        ...
        c.argument(
            "compute_count", options_list=["--compute-count", "-c"], type=int, default=4
        )
        c.argument("tags", tags_type)
    ...
    with self.argument_context("openshift create") as c:
        ...
        c.argument(
            "customer_admin_group_id", options_list=["--customer-admin-group-id"]
        )
