...
class ContainerAppWorkloadProfilesTest(ScenarioTest):
    ...
    def test_containerapp_env_workload_profiles_e2e(self, resource_group):
        ...
        profiles = self.cmd(
            "az containerapp env workload-profile list -g {} -n {}".format(resource_group, env)).get_output_in_json()
    ...
    def test_containerapp_env_workload_profiles_delete(self, resource_group):
        ...
        sub_id = self.cmd("az network vnet subnet create --address-prefixes '14.0.0.0/22' --delegations Microsoft.App/environments -n sub -g {} --vnet-name {}".format(
            resource_group, vnet)).get_output_in_json()["id"]

        self.cmd('containerapp env create -g {} -n {} -s {} --location {}  --logs-destination none --enable-workload-profiles'.format(
            resource_group, env, sub_id, location))
    ...
    def test_containerapp_create_with_workloadprofile_yaml(self, resource_group):
        ...
        self.cmd("az containerapp env workload-profile add -g {} -n {} --workload-profile-name {} --workload-profile-type E16 --min-nodes 1 --max-nodes 3".format(
            resource_group, env, workload_profile_name))
        ...
        self.cmd(
            f'containerapp create -n {app} -g {resource_group} --environment {env} --yaml {containerapp_file_name_01}')

        self.assertContainerappProperties(containerapp_env, resource_group, app,
                                          workload_profile_name, revision_01, "10.0", "5Gi")
        ...
        self.assertContainerappProperties(containerapp_env, resource_group, app,
                                          "Consumption", revision_02, "0.5", "1Gi")
