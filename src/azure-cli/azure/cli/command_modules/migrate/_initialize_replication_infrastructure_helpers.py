...
def get_and_validate_resource_group(cmd, subscription_id, resource_group_name):
...
def _store_appliance_site_mapping(app_map, appliance_name, site_id):
    """Store appliance name to site ID mapping in both lowercase and original case."""
...
def _process_v3_dict_map(app_map, app_map_v3):
    ...
    for appliance_name_key, site_info in app_map_v3.items():
        if isinstance(site_info, dict) and 'SiteId' in site_info:
            _store_appliance_site_mapping(app_map, appliance_name_key, site_info['SiteId'])
...
def _process_v3_list_item(app_map, item):
    ...
    if 'ApplianceName' in item and 'SiteId' in item:
        _store_appliance_site_mapping(app_map, item['ApplianceName'], item['SiteId'])
        return

    # Or it might be a single key-value pair
    for key, value in item.items():
        if isinstance(value, dict) and 'SiteId' in value:
            _store_appliance_site_mapping(app_map, key, value['SiteId'])
...
def parse_appliance_mappings(discovery_solution):
    ...
    if 'applianceNameToSiteIdMapV2' in extended_details:
        try:
            ...
            get_logger(__name__).warning("Failed to parse applianceNameToSiteIdMapV2: %s", str(e))
    ...
    if 'applianceNameToSiteIdMapV3' in extended_details:
        try:
            ...
            get_logger(__name__).warning("Failed to parse applianceNameToSiteIdMapV3: %s", str(e))
...
def validate_and_get_site_ids(app_map, source_appliance_name, target_appliance_name):
...
def determine_instance_types(source_site_id, target_site_id, source_appliance_name, target_appliance_name):
...
def find_fabric(all_fabrics, appliance_name, fabric_instance_type, amh_solution, is_source=True):
    ...
    if not fabric:
        ...
        error_msg = f"Couldn't find connected {appliance_type_label} appliance '{appliance_name}'.\n"

        if fabric_candidates:
            error_msg += f"Found {len(fabric_candidates)} fabric(s) with "
            ...
            elif fabric_instance_type == FabricInstanceTypes.HyperVInstance.value:
            ...
            if all_fabrics:
                ...
                for fab in all_fabrics:
                    ...
                    custom_props = props.get('customProperties', {})
                    error_msg += f"  - {fab.get('name')} (type: {custom_props.get('instanceType')})\n"

        raise CLIError(error_msg)

    return fabric

def get_fabric_agent(cmd, replication_fabrics_uri, fabric, appliance_name, fabric_instance_type):
    ...
    if not dra:
        raise CLIError(f"The appliance '{appliance_name}' is in a disconnected state.")

    return dra

def setup_replication_policy(cmd, rg_uri, replication_vault_name, instance_type):
...
def setup_cache_storage_account(cmd, rg_uri, amh_solution, cache_storage_account_id,
...
def verify_storage_account_network_settings(cmd, rg_uri, cache_storage_account):
...
def get_all_fabrics(cmd, rg_uri, resource_group_name, source_appliance_name,
...
def _get_role_name(role_def_id):
    ...
    return "Contributor" if role_def_id == RoleDefinitionIds.ContributorId else "Storage Blob Data Contributor"

def _assign_role_to_principal(auth_client, storage_account_id, subscription_id,
                               principal_id, role_def_id, principal_type_name):
    ...
    if not has_role:
        ...
        print(f"  ✓ Created {role_name} role for {principal_type_name} {principal_id[:8]}...")
        return f"{principal_id[:8]} - {role_name}", False
    print(f"  ✓ {role_name} role already exists for {principal_type_name} {principal_id[:8]}")
    return f"{principal_id[:8]} - {role_name} (existing)", True

def _verify_role_assignments(auth_client, storage_account_id, expected_principal_ids):
...
def grant_storage_permissions(cmd, storage_account_id, source_dra, target_dra,
                              replication_vault, subscription_id):
    """Grant role assignments for DRAs and vault identity to storage account."""
    logger = get_logger(__name__)
    ...
    expected_principal_ids = [source_dra_object_id, target_dra_object_id, vault_identity_id]
    _verify_role_assignments(auth_client, storage_account_id, expected_principal_ids)

def update_amh_solution_storage(cmd, project_uri, amh_solution, storage_account_id):
    """Update AMH solution with storage account ID if needed."""
    amh_solution_uri = f"{project_uri}/solutions/Servers-Migration-ServerMigration_DataReplication"
...
def get_or_check_existing_extension(cmd, extension_uri, replication_extension_name,
                                     storage_account_id, _pass_thru):
...
def verify_extension_prerequisites(cmd, rg_uri, replication_vault_name, instance_type,
                                   storage_account_id, amh_solution_uri,
                                   source_fabric_id, target_fabric_id):
...
def build_extension_body(instance_type, source_fabric_id, target_fabric_id, storage_account_id):
...
def _wait_for_extension_creation(cmd, extension_uri):
    ...
    for i in range(20):
        ...
        try:
            ...
            replication_extension = get_resource_by_id(cmd, extension_uri, api_version)
            if replication_extension:
...
def _handle_extension_creation_error(cmd, extension_uri, create_error):
    ...
    if not replication_extension:
        raise CLIError(f"Failed to create replication extension: {str(create_error)}") from create_error
...
def setup_replication_extension(cmd, rg_uri, replication_vault_name, source_fabric,
    ...
    replication_extension, is_complete = get_or_check_existing_extension(
        cmd, extension_uri, replication_extension_name, storage_account_id, pass_thru
    )
    ...
    verify_extension_prerequisites(
        ...
        storage_account_id, amh_solution_uri, source_fabric_id, target_fabric_id
    )
    ...
    if not replication_extension:
        ...
        extension_body = build_extension_body(
            instance_type, source_fabric_id, target_fabric_id, storage_account_id
        )
...
def setup_project_and_solutions(cmd, subscription_id, resource_group_name, project_name):
    """Setup and retrieve project and solutions."""
    rg_uri = get_and_validate_resource_group(cmd, subscription_id, resource_group_name)
    ...
    return rg_uri, project_uri, migrate_project, amh_solution, discovery_solution

def setup_appliances_and_types(discovery_solution, source_appliance_name, target_appliance_name):
    ...
    instance_type, fabric_instance_type = determine_instance_types(
        source_site_id, target_site_id, source_appliance_name, target_appliance_name
    )
    return source_site_id, instance_type, fabric_instance_type

def setup_fabrics_and_dras(cmd, rg_uri, resource_group_name, source_appliance_name,
    ...
    source_fabric = find_fabric(all_fabrics, source_appliance_name, fabric_instance_type,
                                amh_solution, is_source=True)
    target_fabric_instance_type = FabricInstanceTypes.AzLocalInstance.value
    target_fabric = find_fabric(all_fabrics, target_appliance_name, target_fabric_instance_type,
                                amh_solution, is_source=False)

    source_dra = get_fabric_agent(cmd, replication_fabrics_uri, source_fabric,
                                  source_appliance_name, fabric_instance_type)
    target_dra = get_fabric_agent(cmd, replication_fabrics_uri, target_fabric,
                                  target_appliance_name, target_fabric_instance_type)

    return source_fabric, target_fabric, source_dra, target_dra

def setup_storage_and_permissions(cmd, rg_uri, amh_solution, cache_storage_account_id,
                                  source_site_id, source_appliance_name, migrate_project,
                                  project_name, source_dra, target_dra, replication_vault,
                                  subscription_id):
    ...
    storage_account_id = cache_storage_account['id']
    verify_storage_account_network_settings(cmd, rg_uri, cache_storage_account)
    grant_storage_permissions(cmd, storage_account_id, source_dra, target_dra,
                             replication_vault, subscription_id)

    return storage_account_id

def initialize_infrastructure_components(cmd, rg_uri, project_uri, amh_solution,
                                         replication_vault_name, instance_type,
                                         migrate_project, project_name,
                                         cache_storage_account_id, source_site_id,
                                         source_appliance_name, source_dra, target_dra,
                                         replication_vault, subscription_id):
    """Initialize policy, storage, and AMH solution."""
    setup_replication_policy(cmd, rg_uri, replication_vault_name, instance_type)
    ...
    amh_solution_uri = update_amh_solution_storage(cmd, project_uri, amh_solution, storage_account_id)

    return storage_account_id, amh_solution_uri

def execute_replication_infrastructure_setup(cmd, subscription_id, resource_group_name,
                                             project_name, source_appliance_name,
                                             target_appliance_name, cache_storage_account_id,
                                             pass_thru):
    """Execute the complete replication infrastructure setup workflow."""
    # Setup project and solutions
    rg_uri, project_uri, migrate_project, amh_solution, discovery_solution = setup_project_and_solutions(
        cmd, subscription_id, resource_group_name, project_name
    )

    # Get and setup replication vault
    replication_vault, replication_vault_name = get_and_setup_replication_vault(
        cmd, amh_solution, rg_uri
    )

    # Setup appliances and determine types
    source_site_id, instance_type, fabric_instance_type = setup_appliances_and_types(
        discovery_solution, source_appliance_name, target_appliance_name
    )

    # Setup fabrics and DRAs
    source_fabric, target_fabric, source_dra, target_dra = setup_fabrics_and_dras(
        cmd, rg_uri, resource_group_name, source_appliance_name,
        target_appliance_name, project_name, fabric_instance_type, amh_solution
    )

    # Initialize policy, storage, and AMH solution
    storage_account_id, amh_solution_uri = initialize_infrastructure_components(
        cmd, rg_uri, project_uri, amh_solution, replication_vault_name,
        instance_type, migrate_project, project_name, cache_storage_account_id,
        source_site_id, source_appliance_name, source_dra, target_dra,
        replication_vault, subscription_id
    )
