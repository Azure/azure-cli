...
def page_blob_service_factory(cli_ctx, kwargs):
    PageBlobService = cli_ctx.cloud.get_sdk(ResourceType.DATA_STORAGE,
...
def blob_data_service_factory(cli_ctx, kwargs):
    BlockBlobService = cli_ctx.cloud.get_sdk(ResourceType.DATA_STORAGE, 'blob#BlockBlobService')
...
def table_data_service_factory(cli_ctx, kwargs):
    TableService = cli_ctx.cloud.get_sdk(ResourceType.DATA_STORAGE, 'table#TableService')
    return generic_data_service_factory(
        TableService,
        kwargs.pop('account_name', None),
        kwargs.pop('account_key', None),
...
def queue_data_service_factory(cli_ctx, kwargs):
    QueueService = cli_ctx.cloud.get_sdk(ResourceType.DATA_STORAGE, 'queue#QueueService')
    return generic_data_service_factory(
        QueueService,
        kwargs.pop('account_name', None),
        kwargs.pop('account_key', None),
...
def cloud_storage_account_service_factory(cli_ctx, kwargs):
    CloudStorageAccount = cli_ctx.cloud.get_sdk(ResourceType.DATA_STORAGE, '#CloudStorageAccount')
...
def multi_service_properties_factory(cli_ctx, kwargs):
    ...
    BaseBlobService, FileService, TableService, QueueService, = \
        cli_ctx.cloud.get_sdk(ResourceType.DATA_STORAGE,
