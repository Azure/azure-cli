...
class AppConfigSnapshotRefScenarioTest(ScenarioTest):
    ...
    def test_azconfig_kv_set_snapshot_reference(self):
        ...
        self.cmd('appconfig kv set-snapshot-reference --endpoint {endpoint} --auth-mode login --key {key} --label {label} --snapshot-name {snapshot_name} -y',
        ...
        result = self.cmd('appconfig kv show --endpoint {endpoint} --auth-mode login --key {key} --label {label}').get_output_in_json()
        ...
        self.cmd('appconfig kv set-snapshot-reference --endpoint {endpoint} --auth-mode login --key {key} --snapshot-name {snapshot_name} --tags env=prod team=config -y',
                 checks=[self.check('contentType', SnapshotReferenceConstants.SNAPSHOT_REFERENCE_CONTENT_TYPE),
    ...
    def test_azconfig_kv_list_resolve_snapshot_references(self):
        ...
        self.cmd('appconfig kv set --endpoint {endpoint} --auth-mode login --key Color --value red -y')
        self.cmd('appconfig kv set --endpoint {endpoint} --auth-mode login --key Size --value large -y')
        ...
        self.cmd('appconfig snapshot create --endpoint {endpoint} --auth-mode login -s {snapshot_name} --filters {filter} --retention-period {retention_period}')

        production_ref_key = "ProductionConfigRef"
        self.kwargs.update({'key': production_ref_key})
        self.cmd('appconfig kv set-snapshot-reference --endpoint {endpoint} --auth-mode login --key {key} --snapshot-name {snapshot_name} -y')

        # List without resolve returns the snapshot reference itself
        result = self.cmd('appconfig kv list --endpoint {endpoint} --auth-mode login --key {key}').get_output_in_json()
        assert len(result) == 1
        ...
        result = self.cmd('appconfig kv list --endpoint {endpoint} --auth-mode login --key {key} --resolve-snapshot-references').get_output_in_json()
        keys = [kv['key'] for kv in result]
        assert 'Color' in keys
        assert 'Size' in keys

        # Non-reference key-values are returned alongside resolved snapshot key-values
        self.cmd('appconfig kv set --endpoint {endpoint} --auth-mode login --key Shape --value circle -y')
        result = self.cmd('appconfig kv list --endpoint {endpoint} --auth-mode login --resolve-snapshot-references').get_output_in_json()
        ...
        self.cmd('appconfig kv set --endpoint {endpoint} --auth-mode login --key Color --value green -y')
        result = self.cmd('appconfig kv list --endpoint {endpoint} --auth-mode login --resolve-snapshot-references').get_output_in_json()
        ...
        self.cmd('appconfig kv set --endpoint {endpoint} --auth-mode login --key Color --value blue --label staging -y')
        ...
        self.cmd('appconfig kv set-snapshot-reference --endpoint {endpoint} --auth-mode login --key {key} --snapshot-name nonexistent-snapshot -y')
        result = self.cmd('appconfig kv list --endpoint {endpoint} --auth-mode login --resolve-snapshot-references').get_output_in_json()
        result_keys = [kv['key'] for kv in result]
