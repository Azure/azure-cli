...
helps['webapp up'] = """
type: command
short-summary: Create a webapp and deploy code from a local workspace to the app. The command needs to run from the folder where the code is present. Supports running the command in preview mode using --dryrun parameter. Current supports includes Node (detected by package.json), Python (detected by requirements.txt), .NET Core and ASP.NET (detected by *.csproj), staticHtml (detected by *.html). Node, Python apps are created as Linux apps. .Net Core, ASP.NET and static HTML apps are created as Windows apps. If command is run from an empty folder, an empty windows web app is created.
examples:
