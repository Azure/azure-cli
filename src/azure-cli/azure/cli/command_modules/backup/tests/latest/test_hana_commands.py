...
class BackupTests(ScenarioTest, unittest.TestCase):
    ...
    def test_backup_wl_hana_snapshot(self):
        ...
        policy_json = self.cmd('backup policy show --subscription "{sub}" -g "{rg}" -v "{vault}" -n "{existing_policy_name}"').get_output_in_json()
        ...
        self.cmd('backup policy create --backup-management-type "{bmt}" --workload-type "{wt}" --subscription "{sub}" -g "{rg}" -v "{vault}" -n "{policy_name}" --policy "{existing_policy}"')
        self.cmd('backup policy delete --subscription "{sub}" -g "{rg}" -v "{vault}" -n "{policy_name}"')
