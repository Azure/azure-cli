...
class WebpubsubScenarioTest(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_test_webpubsub')
    def test_webpubsub(self, resource_group):
        ...
        self.cmd('webpubsub create -g {rg} -n {name} --tags foo=doo', checks=[
            self.check('tags.foo', 'doo'),
            self.check('name', '{name}')
        ])
        self.cmd('webpubsub update -g {rg} -n {name} --tags foo=boo', checks=[
            self.check('tags.foo', 'boo')
        ])
        count = len(self.cmd('webpubsub list').get_output_in_json())
        self.cmd('webpubsub show - {rg} -n {name}', checks=[
            self.check('name', '{name}'),
