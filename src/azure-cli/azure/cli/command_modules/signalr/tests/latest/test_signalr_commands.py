...
class AzureSignalRServiceScenarioTest(ScenarioTest):
    ...
    def test_signalr_commands(self, resource_group):
        ...
        self.kwargs.update({
            ...
            'update_enable_messaging_logs': update_enable_messaging_logs,
            'added_allowed_origins': ' '.join(added_allowed_origins),
            'default_action': "Deny",
        })

        # Test create
        self.cmd('az signalr create -n {signalr_name} -g {rg} --sku {sku} --unit-count {unit_count} -l {location} --tags {tags} --service-mode {service_mode} --enable-messaging-logs {enable_messaging_logs} --allowed-origins {allowed_origins} --default-action {default_action}',
                 checks=[
                     ...
                     self.check('features[2].value', '{enable_messaging_logs}'),
        ...
        self.cmd('az signalr list -g {rg}', checks=[
            ...
            self.check('[0].features[2].value', '{enable_messaging_logs}'),
        ...
        # Test update
        self.cmd('az signalr update -n {signalr_name} -g {rg} --sku {updated_sku} --tags {updated_tags} --service-mode {update_service_mode} --enable-messaging-logs {update_enable_messaging_logs}',
                 checks=[
                     self.check('name', '{signalr_name}'),
                     self.check('location', '{location}'),
                     self.check('provisioningState', 'Succeeded'),
                     ...
                     self.check('features[1].value', '{update_enable_messaging_logs}'),
                     self.check('cors.allowedOrigins', allowed_origins),
                     self.exists('hostName'),
                     self.exists('publicPort'),
                     self.exists('serverPort')
