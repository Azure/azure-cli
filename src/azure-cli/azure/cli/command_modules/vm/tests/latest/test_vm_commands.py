...
class MSIScenarioTest(ScenarioTest):
    ...
    def test_vm_explicit_msi(self, resource_group):
        ...
        result = self.cmd('vm create -g {rg} -n vm2 --image Debian11 --assign-identity {emsi} '
                          '--generate-ssh-keys --admin-username {user} --subnet {subnet} --vnet-name {vnet} '
                          '--nsg-rule NONE --size Standard_B2ms', checks=[
            self.check('identity.role', None),
            self.check('identity.scope', None),
        ]).get_output_in_json()
        ...
        result = self.cmd('vm create -g {rg} -n {vm} --image Debian11 --assign-identity {emsi} [system] '
                          '--role reader --scope {scope} --generate-ssh-keys --admin-username {user} --subnet {subnet} '
                          '--vnet-name {vnet} --nsg-rule NONE --size Standard_B2ms').get_output_in_json()
        emsis = [x.lower() for x in result['identity']['userAssignedIdentities'].keys()]
        self.assertEqual(emsis, [emsi_result['id'].lower()])
