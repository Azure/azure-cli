...
type: group
short-summary: Manage event subscriptions for an Event Grid topic or for an Azure
    resource.
"""

helps["eventgrid event-subscription create"] = """
type: command
short-summary: Create a new event subscription for an Event Grid topic or for an Azure
    resource.
examples:
-   name: Create a new event subscription for an Event Grid topic, using default filters.
    text: |
        az eventgrid event-subscription create -g rg1 --topic-name topic1 --name es1     --endpoint https://contoso.azurewebsites.net/api/f1?code=code
-   name: Create a new event subscription for a subscription, using default filters.
    text: |
        az eventgrid event-subscription create --name es2     --endpoint https://contoso.azurewebsites.net/api/f1?code=code
-   name: Create a new event subscription for a resource group, using default filters.
    text: |
        az eventgrid event-subscription create -g rg1 --name es3     --endpoint https://contoso.azurewebsites.net/api/f1?code=code
...
-   name: Create a new event subscription for a subscription, with a filter specifying
        a subject prefix.
    text: |
        az eventgrid event-subscription create --name es4     --endpoint https://contoso.azurewebsites.net/api/f1?code=code     --subject-begins-with mysubject_prefix
-   name: Create a new event subscription for a resource group, with a filter specifying
        a subject suffix.
    text: |
        az eventgrid event-subscription create -g rg2 --name es5     --endpoint https://contoso.azurewebsites.net/api/f1?code=code     --subject-ends-with mysubject_suffix
-   name: Create a new event subscription for a subscription, using default filters,
        and an EventHub as a destination.
    text: |
        az eventgrid event-subscription create --name es2 --endpoint-type eventhub     --endpoint /subscriptions/55f3dcd4-cac7-43b4-990b-a139d62a1eb2/resourceGroups/TestRG/providers/Microsoft.EventHub/namespaces/ContosoNamespace/eventhubs/EH1
-   name: Create a new event subscription for an Event Grid topic or for an Azure
        resource.
    text: az eventgrid event-subscription create --max-delivery-attempts {max-delivery-attempts}
        --event-delivery-schema cloudeventv01schema --endpoint https://contoso.azurewebsites.net/api/f1?code=code
        --name es2
    crafted: 'True'
"""
...
-   name: List all event subscriptions for a topic-type in a specific location (under
...
-   name: List all regional event subscriptions in a specific location (under the
        currently selected Azure subscription).
    text: |
        az eventgrid event-subscription list --location westus2
-   name: List all event subscriptions in a specific location under a specified resource
        group.
    text: |
        az eventgrid event-subscription list --location westus2 --resource-group kalstest
-   name: List all global event subscriptions (under the currently selected Azure
        subscription).
    text: |
        az eventgrid event-subscription list
-   name: List all global event subscriptions under the currently selected resource
        group.
    text: |
...
-   name: Get the details of an event subscription.
    text: az eventgrid event-subscription show --name es1 --include-full-endpoint-url
        false --topic-name topic1 --resource-group rg1
    crafted: 'True'
"""
...
-   name: Update an event subscription for a subscription to specify a new subject-ends-with
        filter.
    text: |
        az eventgrid event-subscription update --name es2 --subject-ends-with .jpg
-   name: Update an event subscription for a resource group to specify a new endpoint
        and a new subject-ends-with filter.
    text: |
        az eventgrid event-subscription update -g rg1 --name es3  --subject-ends-with .png \\
            --endpoint https://contoso.azurewebsites.net/api/f1?code=code
-   name: Update an event subscription for a storage account to specify a new list
        of included event types.
    text: |
