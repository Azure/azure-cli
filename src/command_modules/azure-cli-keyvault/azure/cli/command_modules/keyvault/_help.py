...
examples:
-   name: Create a self-signed certificate with the default policy and add it to a
        virtual machine.
    text: |
...
-   name: Create a Key Vault certificate.
    text: az keyvault certificate create --disabled false --policy {policy} --vault-name
        MyVault --name MyCertificate
    crafted: 'True'
-   name: Create a Key Vault certificate.
    text: az keyvault certificate create --disabled false --policy {policy} --vault-name
        MyVault --name MyCertificate
...
examples:
-   name: Create a service principal with a certificate, add the certificate to Key
        Vault and provision a VM with that certificate.
    text: |
...
long-summary: Default permissions are created for the current user or service principal
    unless the `--no-self-perms` flag is specified.
examples:
-   name: Create a key vault.
    text: az keyvault create --resource-group MyResourceGroup --location westus2 --name
        MyKeyVault
    crafted: 'True'
-   name: Create a key vault.
    text: az keyvault create --resource-group MyResourceGroup --location westus2 --name
...
-   name: Add a sas-definition for a blob sas-token
    text: >4
        ...
        $sastoken = az storage blob generate-sas --account-name storageacct --account-key
        00000000 \\ -c container1 -n blob1 --https-only --permissions rw

        $url = az storage blob url --account-name storageacct -c container1 -n blob1


        az keyvault storage sas-definition create --vault-name vault --account-name
        storageacct   \\ -n rwblobaccess --validity-period P2D --sas-type service
        --template-uri $url?$sastoken
"""
