...
class EHNamespaceCURDScenarioTest(ScenarioTest):
    ...
    def test_eh_eventhub(self, resource_group):
        self.kwargs.update({
            ...
            'newmessageretentionindays': 7,
            'partitioncount': 4
        })

        # Create Eventhub
        self.cmd(
            'eventhubs eventhub create --resource-group {rg} --namespace-name {namespacename} --name {eventhubname} --partition-count {partitioncount} --message-retention {messageretentionindays}',
        ...
        # update Eventhub
        self.cmd(
            'eventhubs eventhub update --resource-group {rg} --namespace-name {namespacename} --name {eventhubname} --partition-count {partitioncount} --message-retention {newmessageretentionindays}',
            checks=[self.check('name', self.kwargs['eventhubname'])])
    ...
    def test_eh_namespace(self, resource_group):
        ...
        # Update Namespace
        self.cmd(
            'eventhubs namespace update --resource-group {rg} --name {namespacename} --tags {tags}',
            checks=[self.check('sku.name', self.kwargs['sku'])])
