...
class BatchMgmtScenarioTests(ScenarioTest):  # pylint: disable=too-many-instance-attributes
    ...
    def test_batch_account_cmd(self, resource_group):
        ...
        self.cmd('keyvault create -g {} -n {} -l {} --enabled-for-deployment true '
                 ...
                 ' true'.format(resource_group, keyvault_name, byos_location)) \
                 .assert_with_checks([JMESPathCheck('name', keyvault_name),
        ...
        self.cmd('batch account create -g {} -n {} -l {} --keyvault {}'.
                 format(resource_group, byos_account_name, byos_location, keyvault_name)) \
                 .assert_with_checks([JMESPathCheck('name', byos_account_name),
                                      JMESPathCheck('location', byos_location),
                                      JMESPathCheck('resourceGroup', resource_group)])

        self.cmd('batch account set -g {} -n {} --storage-account {}'.
                 format(resource_group, account_name, storage_account_name)) \
                 .assert_with_checks([JMESPathCheck('name', account_name),
                                      JMESPathCheck('location', location),
                                      JMESPathCheck('resourceGroup', resource_group)])

        self.cmd('batch account show -g {} -n {}'.format(resource_group, account_name)) \
                 .assert_with_checks([JMESPathCheck('name', account_name),
                                      JMESPathCheck('location', location),
                                      JMESPathCheck('resourceGroup', resource_group),
        ...
        keys = self.cmd('batch account keys list -g {} -n {}'
                        ...
                        .assert_with_checks([JMESPathCheck('primary != null', True),
    ...
    def test_batch_application_cmd(self, resource_group):
        ...
        result = self.cmd('storage account create -g {} -n {} -l {} --sku Standard_LRS'.format(
            resource_group, storage_account_name, location)) \
                .assert_with_checks([
                    JMESPathCheck('name', storage_account_name),
                    JMESPathCheck('location', location),
                    JMESPathCheck('resourceGroup', resource_group)])
        ...
        self.cmd('batch application create -g {} -n {} --application-id {} --allow-updates'.
                 format(resource_group, account_name, application_name)).assert_with_checks([
                     JMESPathCheck('id', application_name),
                     JMESPathCheck('allowUpdates', True)])

        self.cmd('batch application list -g {} -n {}'.format(resource_group, account_name)) \
        ...
        self.cmd('batch application show -g {} -n {} --application-id {}'
                 .format(resource_group, account_name, application_name)) \
                 .assert_with_checks([JMESPathCheck('id', application_name),
