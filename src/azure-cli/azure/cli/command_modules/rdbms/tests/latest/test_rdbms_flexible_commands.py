...
class FlexibleServerMgmtScenarioTest(ScenarioTest):
    ...
    def _remove_server(self, database_engine, resource_group_name, server_name):
        if server_name:
            self.cmd('{} flexible-server delete -g {} -n {} --force'.format(database_engine, resource_group_name,
                                                                            server_name))
    ...
    def _test_flexible_server_mgmt(self, database_engine, resource_group):
        ...
        self.cmd('{} flexible-server show -g {} -n {}'
                 .format(database_engine, generated_resource_group_name, generated_server_name),
        ...
        self.cmd('{} flexible-server update -g {} -n {} --tier {} --sku-name {} --admin-password {}'
                 .format(database_engine, resource_group, server_name, tier, sku_name, admin_password),
                 checks=updated_list_checks)
...
class FlexibleServerLocalContextScenarioTest(LocalContextScenarioTest):
    ...
    def _test_flexible_server_local_context(self, database_engine, resource_group):
        ...
        self.cmd('{} flexible-server create --name {} --admin-user {} --admin-password {} --version {} --storage-size {} \
                --sku-name {} --tier {}'.format(database_engine, server_name, admin_user, admin_password, version,
                                                storage_size, sku_name, tier))
...
class FlexibleServerProxyResourceMgmtScenarioTest(ScenarioTest):
    ...
    def _test_firewall_rule_mgmt(self, database_engine, resource_group):
        ...
        self.cmd('{} flexible-server create -g {} --name {} -l {} -tier {} -sku_name {}'.format(database_engine,
                                                                                                resource_group,
        ...
        self.cmd('{} flexible-server firewall-rule create -g {} -s {} --name {} '
                 ...
                 .format(database_engine, resource_group, server_name, firewall_rule_name, start_ip_address,
                         end_ip_address),
        ...
        self.cmd('{} flexible-server firewall-rule create -g {} -s {} --name {} '
                 '--start-ip-address {} --end-ip-address {} '
                 .format(database_engine, resource_group, server_name, new_firewall_rule_name, start_ip_address,
                         end_ip_address),
                 checks=firewall_rule_checks)
    ...
    def _test_parameter_mgmt(self, database_engine, resource_group):
        ...
        self.cmd('{} flexible-server create -g {} --name {} -l {}'.format(database_engine, resource_group, server_name,
                                                                          location))

        # list
        self.cmd('{} flexible-server parameter list -g {} -s {}'.format(database_engine, resource_group, server_name),
                 checks=[JMESPathCheck('type(@)', 'array')])
        ...
        self.cmd('{} flexible-server parameter show --name {} -g {} -s {}'.format(database_engine, parameter_name,
                                                                                  resource_group, server_name),
                 checks=[JMESPathCheck('defaultValue', default_value),
                         JMESPathCheck('source', source)])
        # set
        source = 'user-override'
        self.cmd('{} flexible-server parameter set --name {} -v {} --source {} -s {} -g {}'.format(database_engine,
                                                                                                   parameter_name,
                                                                                                   value, source,
                                                                                                   server_name,
                                                                                                   resource_group),
