...
class StorageAzcopyTests(StorageScenarioMixin, LiveScenarioTest):
    ...
    @StorageTestFilesPreparer()
    def test_storage_azcopy_blob_url_oauth(self, resource_group, first_account, second_account, test_dir):
        import os
        ...
        # test validation
        self.oauth_cmd('storage copy -s "{}" -d "{}" --destination-container test'
                       .format(os.path.join(test_dir, 'readme'), first_container_url), expect_failure=True)

        # Upload a single file
        content_type = "application/json"
        self.oauth_cmd('storage copy -s "{}" -d "{}" --content-type {}'.format(
            os.path.join(test_dir, 'readme'), first_container_url, content_type))
        self.oauth_cmd('storage blob list -c {} --account-name {}', first_container, first_account) \
            .assert_with_checks(JMESPathCheck('length(@)', 1))
        self.oauth_cmd('storage blob show -n {} -c {} --account-name {}', 'readme', first_container, first_account) \
            .assert_with_checks(JMESPathCheck('properties.contentSettings.contentType', content_type))

        # Upload entire directory
        self.oauth_cmd('storage copy -s "{}" -d "{}" --recursive'.format(
            os.path.join(test_dir, 'apple'), first_container_url))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            first_container, first_account), checks=JMESPathCheck('length(@)', 11))
        ...
        local_folder = self.create_temp_dir()
        # Download a single file
        self.oauth_cmd('storage copy -s "{}" -d "{}"'.format(
            '{}/readme'.format(first_container_url), local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))
        ...
        # Download a set of files
        self.oauth_cmd('storage copy -s "{}" --include-path "apple" --include-pattern file* -d "{}" --recursive'.format(
            first_container_url, local_folder))
        self.assertEqual(3, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(21, sum(len(f) for r, d, f in os.walk(local_folder)))
        ...
        self.oauth_cmd('storage copy -s "{}" -d "{}" --preserve-s2s-access-tier false'.format(
            '{}/readme'.format(first_container_url), second_container_url))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            second_container, second_account), checks=JMESPathCheck('length(@)', 1))

        # Copy an entire directory from blob virtual directory to another blob virtual directory
        self.oauth_cmd('storage copy -s "{}" -d "{}" --recursive --preserve-s2s-access-tier false'.format(
            '{}/apple'.format(first_container_url), second_container_url))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            second_container, second_account), checks=JMESPathCheck('length(@)', 11))
        ...
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            first_container, second_account), checks=JMESPathCheck('length(@)', 21))
    ...
    def test_storage_azcopy_blob_account_oauth(self, resource_group, first_account, second_account, test_dir):
        ...
        # Upload a single file
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-account-name {} --destination-container {}'
                       .format(os.path.join(test_dir, 'readme'), first_account, first_container))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            first_container, first_account), checks=JMESPathCheck('length(@)', 1))

        # Upload entire directory
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-account-name {} --destination-container {}'
                       ' --recursive'.format(os.path.join(test_dir, 'apple'), first_account, first_container))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            first_container, first_account), checks=JMESPathCheck('length(@)', 11))

        # Upload a set of files
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-account-name {} --destination-container {}'
                       ' --recursive'.format(os.path.join(test_dir, 'butter/file_*'), first_account, first_container))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            first_container, first_account), checks=JMESPathCheck('length(@)', 21))
        ...
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-account-name {} --destination-container {} '
                       '--follow-symlinks'.format(symlink, first_account, first_container))
        self.oauth_cmd('storage blob list -c {} --account-name {}'.format(
            first_container, first_account), checks=JMESPathCheck('length(@)', 22))

        local_folder = self.create_temp_dir()
        # Download a single file
        self.oauth_cmd(
            'storage copy --source-account-name {} --source-container {} --source-blob {} --destination-local-path "{}"'
            .format(first_account, first_container, 'readme', local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download entire directory
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --source-blob {} '
                       '--destination-local-path "{}" --recursive'.format(
            first_account, first_container, 'apple/', local_folder))
        self.assertEqual(1, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(11, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download a set of files
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --include-path {} '
                       '--include-pattern {} --destination-local-path "{}" --recursive'.format(
            first_account, first_container, 'apple', 'file*', local_folder))
        self.assertEqual(3, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(21, sum(len(f) for r, d, f in os.walk(local_folder)))
        ...
        # Copy an entire storage account data to another blob account
        self.oauth_cmd('storage copy --source-account-name {} --destination-account-name {} --recursive '
                       '--preserve-s2s-access-tier false'.format(
    ...
    @StorageAccountPreparer(parameter_name='second_account', sku='Premium_LRS', kind='BlockBlobStorage',
                            allow_shared_key_access=False)
    @StorageTestFilesPreparer()
    def test_storage_azcopy_blob_source_with_credential_oauth(self, resource_group, first_account, second_account,
                                                              test_dir):
        first_container = self.create_random_name(prefix='container', length=24)
        second_container = self.create_random_name(prefix='container', length=24)
        self.oauth_cmd('storage container create -n {} --account-name {}', first_container, first_account)
        ...
        import os
        # Upload a single file with destination account key
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-container {} --account-name {}',
                       os.path.join(test_dir, 'readme'), first_container, first_account)
        self.oauth_cmd('storage blob list -c {} --account-name {}', first_container, first_account)\
            .assert_with_checks(JMESPathCheck('length(@)', 1))
        ...
        first_connection_string = self.cmd('storage account show-connection-string -n {} -o tsv'.format(
            first_account)).output.strip()
        self.oauth_cmd('storage copy --source-local-path "{}" --destination-container {} --recursive '
                       '--connection-string {} --account-name {}'.format(
            os.path.join(test_dir, 'apple'), first_container, first_connection_string, first_account))
        self.oauth_cmd('storage blob list -c {} --account-name {}', first_container, first_account) \
            .assert_with_checks(JMESPathCheck('length(@)', 11))

        # Download a single file with source
        local_folder = self.create_temp_dir()
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --source-blob {} '
                       '--destination-local-path "{}"'.format(first_account, first_container, 'readme', local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download entire directory with source connection string
        self.oauth_cmd('storage copy --source-connection-string {} --source-container {} --source-blob {} '
                       '--destination-local-path "{}" --recursive'.format(first_connection_string, first_container,
                                                                          'apple/', local_folder))
        self.assertEqual(1, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(11, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download a set of files with source
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --include-path {} '
                       '--include-pattern {} --destination-local-path "{}" --recursive'.format(
            first_account, first_container, 'apple', 'file*', local_folder))
        self.assertEqual(3, sum(len(d) for r, d, f in os.walk(local_folder)))
        ...
        # Copy a single blob to another single blob
        self.oauth_cmd('storage copy --source-account-name {} --source-container {} --source-blob {} '
                       '--account-name {} --destination-container {} '
                       '--preserve-s2s-access-tier false'.format(
            first_account, first_container, 'readme', second_account, second_container))
        self.oauth_cmd('storage blob list -c {} --account-name {}', second_container, second_account).assert_with_checks(
            JMESPathCheck('length(@)', 1))

        # Copy an entire storage account data to another blob account with account key
        self.oauth_cmd('storage copy --source-account-name {} --recursive --preserve-s2s-access-tier false '
                       '--account-name {}', first_account, second_account)
        self.oauth_cmd('storage container list --account-name {}', second_account).assert_with_checks(
            JMESPathCheck('length(@)', 2))
        self.oauth_cmd('storage blob list -c {} --account-name {}', first_container, second_account) \
            .assert_with_checks(JMESPathCheck('length(@)', 11))

    @ResourceGroupPreparer()
    @StorageAccountPreparer()
    @StorageTestFilesPreparer()
    def test_storage_azcopy_file_url_oauth(self, resource_group, storage_account_info, test_dir):
        storage_account, _ = storage_account_info
        share = self.create_share(storage_account_info)
        ...
        import os
        # Upload a single file
        self.oauth_cmd('storage copy -s "{}" -d "{}" --cap-mbps 1.0'.format(
            os.path.join(test_dir, 'readme'), share_url))
        self.oauth_cmd('storage file list -s {} --account-name {} --enable-file-backup-request-intent'.format(
            share, storage_account), checks=JMESPathCheck('length(@)', 1))

        # Upload entire directory
        self.oauth_cmd('storage copy -s "{}" -d "{}" --recursive'.format(
            os.path.join(test_dir, 'apple'), share_url))
        self.oauth_cmd('storage file list -s {} --account-name {} --enable-file-backup-request-intent'.format(
            share, storage_account), checks=JMESPathCheck('length(@)', 2))

        # Upload a set of files
        self.oauth_cmd('storage copy -s "{}" -d "{}" --recursive'.format(
            os.path.join(test_dir, 'butter/file_*'), share_url))
        self.oauth_cmd('storage file list -s {} --account-name {} --enable-file-backup-request-intent'.format(
            share, storage_account), checks=JMESPathCheck('length(@)', 12))
        ...
        # Download a single file
        self.oauth_cmd('storage copy -s "{}" -d "{}"'.format(
            '{}/readme'.format(share_url), local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))
    ...
    @StorageAccountPreparer()
    @StorageTestFilesPreparer()
    def test_storage_azcopy_file_account_oauth(self, resource_group, storage_account_info, test_dir):
        storage_account, _ = storage_account_info
        share = self.create_share(storage_account_info)
        ...
        # Upload a single file
        self.oauth_cmd(
            'storage copy --source-local-path "{}" --destination-account-name {} --destination-share {} --cap-mbps 1.0'
            .format(os.path.join(test_dir, 'readme'), storage_account, share))
        self.oauth_cmd('storage file list -s {} --account-name {} --enable-file-backup-request-intent'.format(
            share, storage_account), checks=JMESPathCheck('length(@)', 1))
        ...
        # Upload entire directory
        self.oauth_cmd(
            'storage copy --source-local-path "{}" --destination-account-name {} --destination-share {} --recursive'
            .format(os.path.join(test_dir, 'apple'), storage_account, share))
        self.oauth_cmd('storage file list -s {} --account-name {} --enable-file-backup-request-intent'.format(
            share, storage_account), checks=JMESPathCheck('length(@)', 2))
        ...
        local_folder = self.create_temp_dir()
        # Download a single file
        self.oauth_cmd('storage copy --source-account-name {} --source-share {} --source-file-path {} '
                       '--destination-local-path "{}"'.format(
            storage_account, share, 'readme', local_folder))
        self.assertEqual(1, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download entire directory
        self.oauth_cmd('storage copy --source-account-name {} --source-share {} --source-file-path {} '
                       '--destination-local-path "{}" --recursive'.format(
            storage_account, share, 'apple', local_folder))
        self.assertEqual(1, sum(len(d) for r, d, f in os.walk(local_folder)))
        self.assertEqual(11, sum(len(f) for r, d, f in os.walk(local_folder)))

        # Download a set of files
        self.oauth_cmd('storage copy --source-account-name {} --source-share {} --include-path "apple"'
                       ' --include-pattern file* --destination-local-path "{}" --recursive'.format(
