...
def _install_k8_shares(resource_group, dns_prefix):
    ...
    try:
        storage_command = "az storage account keys list -n {0} -g {1}".format(
            config_storage, resource_group)
    ...
    try:
        wks_storage_command = "az storage account keys list -n {0} -g {1}".format(
            workspace_storage, resource_group)
    ...
    _execute_command("az storage share create -n 'cfgs' --account-name {0} --account-key {1}".format(
        config_storage, config_storage_key))

    # Ensure 'mindaro' share exists in configStorage
    _execute_command("az storage share create -n 'mindaro' --account-name {0} --account-key {1}".format(
        workspace_storage, workspace_storage_key))
