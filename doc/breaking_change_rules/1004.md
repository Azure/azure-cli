...
**Description**: Checks whether a new property of a cmd is removed from the previous cli version. 

**Analysis**: Not all configured properties removed from a cmd is considered as breaking change. The properties listed in CMD_PROPERTY_REMOVE_BREAK_LIST below are thought to influence or interrupt current workflow and be breaking changes.
For now, removed properties in cmd level do not cause breaking workflows.
