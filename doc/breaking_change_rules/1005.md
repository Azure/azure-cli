...
**Analysis**: Not all configured properties updated for a cmd is considered as breaking change. The properties listed in CMD_PROPERTY_UPDATE_BREAK_LIST below are thought to influence or interrupt current workflow and be breaking changes.
For now, updated properties in cmd level do not cause breaking workflows.
