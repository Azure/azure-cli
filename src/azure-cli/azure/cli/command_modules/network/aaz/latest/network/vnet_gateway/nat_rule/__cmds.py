...
@register_command(
    "network vnet-gateway nat-rule list",
    is_preview=True,
)
class List(AAZCommand):
    """List nat rule for a virtual network gateway.

    :example: List nat rule
    ...
    class SubresourceSelector(AAZJsonSelector):
        ...
        def _get(self):
            result = self.ctx.vars.instance
            return result.properties.natRules

        def _set(self, value):
            result = self.ctx.vars.instance
            result.properties.natRules = value
            return
...
@register_command(
    "network vnet-gateway nat-rule wait",
)
