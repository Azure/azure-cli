...
helm_bc_msg = 'In November 2020, Helm 2 reached end of life. ' \
              ...
              'Starting January 21st, 2025 the CLI command `az acr helm push` will be retired to ' \
              'prevent pushing new Helm charts to legacy Helm repositories.\n' \
              'Starting March 30th, 2025 the CLI command group `az acr helm` will be retired, ' \
              'ending all legacy Helm repository capabilities in Azure Container Registry.\n' \
              'All Helm charts not stored as an OCI artifact ' \
              'will be deleted from Azure Container Registry on March 30th, 2025.\n' \
...
register_command_group_deprecate(command='acr helm', redirect='Helm v3 commands', message=helm_bc_msg,
