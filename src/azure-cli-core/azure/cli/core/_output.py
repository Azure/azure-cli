...
class AzOutputProducer(knack.output.OutputProducer):
    ...
    def out(self, obj, formatter=None, out_file=None):
        ...
        if binary_stream is None:
            knack.output.logger.warning(
                "TSV output stream does not expose a binary buffer; falling back to default writer."
            )
            return super().out(obj, formatter=formatter, out_file=out_file)
        ...
        try:
            ...
            knack.output.logger.warning(
                "Unable to encode TSV output with %s encoding. Unsupported characters are discarded.",
                encoding
            )
