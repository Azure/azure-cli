...
def _get_client_factory(name, kwargs):
    factory = kwargs.get('client_factory', kwargs.get('command_type').settings['client_factory'])
    if not factory:
        raise CLIError("command authoring error: cmd '{}' does not have a client_factory.".format(name))
    return factory
