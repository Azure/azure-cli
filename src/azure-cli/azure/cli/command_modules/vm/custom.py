...
def _assign_identity(resource, identity, external_identities=None, enable_local_identity=None, command_args={}):
    if resource.get('identity', {}).get('type', None) == IdentityType.SYSTEM_ASSIGNED_USER_ASSIGNED.value:
        ...
        identity_types = IdentityType.SYSTEM_ASSIGNED_USER_ASSIGNED.value
    elif resource.get('identity', {}).get('type', None) == IdentityType.USER_ASSIGNED.value and enable_local_identity:
        identity_types = IdentityType.SYSTEM_ASSIGNED_USER_ASSIGNED.value
    elif external_identities and enable_local_identity:
        identity_types = IdentityType.SYSTEM_ASSIGNED_USER_ASSIGNED.value
    elif external_identities:
    ...
    for key in resource.get('identity', {}).get('userAssignedIdentities', {}).keys():
        command_args['mi_user_assigned'].append(key)
...
def assign_sig_identity(cmd, resource_group_name, gallery_name, assign_identity=None, identity_role=None,
    ...
    def getter():
        ...
        return SigShow(cli_ctx=cmd.cli_ctx)(command_args=command_args)

    def setter(sig, external_identities=external_identities):
        new_command_args = _assign_identity(sig, identity, external_identities, enable_local_identity, command_args)
        from .aaz.latest.sig.identity import Assign as SigIdentityAssign
        update_sig_identity = SigIdentityAssign(cli_ctx=cmd.cli_ctx)(command_args=new_command_args)
        LongRunningOperation(cmd.cli_ctx)(update_sig_identity)
        return update_sig_identity.result()
...
def remove_sig_identity(cmd, resource_group_name, gallery_name, identities=None):
    ...
    return SigIdentityRemove(cli_ctx=cmd.cli_ctx)(command_args=command_args)
