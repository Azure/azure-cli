...
class DiskRPTestScenario(ScenarioTest):
    ...
    @ResourceGroupPreparer(name_prefix='cli_disk_with_elastic_san', location='westus2')
    @AllowLargeResponse(size_kb=99999)
    def test_disk_with_elastic_san(self, resource_group):
        self.kwargs.update({
            'disk': self.create_random_name('disk', 10),
            'snapshot': self.create_random_name('snap', 10),
        ...
        disk_info = self.cmd('disk create -g {rg} -n {disk} --elastic-san-resource-id {snapshot_id}', checks=[
