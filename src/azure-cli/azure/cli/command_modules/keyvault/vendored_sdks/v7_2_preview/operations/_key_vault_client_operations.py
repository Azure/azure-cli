...
class KeyVaultClientOperationsMixin(object):
    ...
    def full_backup(self, vault_base_url, storage_resource_uri, token, cls=None, **kwargs):
        ...
        if storage_resource_uri is not None or token is not None:
            azure_storage_blob_container_uri = models.SASTokenParameter(storage_resource_uri=storage_resource_uri, token=token)
    ...
    def full_backup_status(self, vault_base_url, job_id, cls=None, **kwargs):
    ...
    def full_restore_operation_method(self, vault_base_url, sas_token_parameters, folder_to_restore, cls=None, **kwargs):
        ...
        if sas_token_parameters is not None or folder_to_restore is not None:
            restore_blob_details = models.RestoreOperationParameters(sas_token_parameters=sas_token_parameters, folder_to_restore=folder_to_restore)

        # Construct URL
        url = self.full_restore_operation_method.metadata['url']
        path_format_arguments = {
            'vaultBaseUrl': self._serialize.url("vault_base_url", vault_base_url, 'str', skip_quote=True)
        }
        url = self._client.format_url(url, **path_format_arguments)
        ...
        return deserialized
    full_restore_operation_method.metadata = {'url': '/restore'}

    def full_restore_status(self, vault_base_url, job_id, cls=None, **kwargs):
        """Returns the status of full restore operation.

        :param vault_base_url: The vault name, for example
         https://myvault.vault.azure.net.
        :type vault_base_url: str
        :param job_id: The Job Id returned part of the full restore operation
        :type job_id: str
        :param callable cls: A custom type or function that will be passed the
         direct response
        :return: FullRestoreOperation or the result of cls(response)
        ...
        error_map = kwargs.pop('error_map', None)
        # Construct URL
        url = self.full_restore_status.metadata['url']
        path_format_arguments = {
            'vaultBaseUrl': self._serialize.url("vault_base_url", vault_base_url, 'str', skip_quote=True),
            'jobId': self._serialize.url("job_id", job_id, 'str')
        }
        ...
        return deserialized
    full_restore_status.metadata = {'url': '/restore/{jobId}/pending'}
