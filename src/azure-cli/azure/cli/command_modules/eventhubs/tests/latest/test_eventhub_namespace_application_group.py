...
class EHNamespaceAppl(ScenarioTest):
    ...
    def test_eventhub_app_group(self, resource_group):
        ...
        AppGroup = self.cmd(
            'eventhubs namespace application-group application-group-policy add --resource-group {rg} --namespace-name {namespacename} '
            '--name {appgroup1} '
        ...
        AppGroup = self.cmd(
            'eventhubs namespace application-group application-group-policy remove --resource-group {rg} --namespace-name {namespacename} '
            '--name {appgroup1} '
            '--throttling-policy-config name=policy4 rate-limit-threshold=19000 metric-id=OutgoingBytes').get_output_in_json()
        ...
        self.assertEqual(len(list_of_app_groups), 0)
