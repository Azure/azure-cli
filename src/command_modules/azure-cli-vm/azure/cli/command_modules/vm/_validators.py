...
def process_vmss_create_namespace(cmd, namespace):
    ...
    if namespace.vm_sku is None:
        ...
        if cmd.cli_ctx.cloud.name != AZURE_US_GOV_CLOUD.name:
            logger.warning('In a future release, the default vm size will be changed from "Standard_D1_v2" to "Standard_DS1_v2".'
                           ' You can use "--vm-sku" argument to maintain the same size')
