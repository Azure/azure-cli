...
from azure.cli.core.azclierror import UnrecognizedArgumentError, ValidationError
...
def validate_resync_quotarule(cmd, namespace):
    ...
    volume = netAppVolumeClient.get(namespace.resource_group_name, namespace.account_name, namespace.pool_name, namespace.volume_name)
    ...
    if volume.data_protection is not None and volume.data_protection.replication is not None:
        ...
        if endpointType is not None and endpointType == 'Src':
            logger.debug("ANF validate_resync_quotarule: Get dataprotection volume for volume %s RemoteVolumeResourceId %s:", namespace.volume_name, remoteVolumeResourceId)
            if is_valid_resource_id(remoteVolumeResourceId):
                ...
                try:
                    ...
                    if destVol is not None:
                        destVolumeQuotaRules = netAppVolumeQuotaRuleClient.list_by_volume(destGroupName, destAccountName, destPoolName, destVolumeName)
                        rules_list = list(destVolumeQuotaRules)
                        if not rules_list:
                            logger.debug("ANF validate_resync_quotarule: There are %s quota rules for volume:", len(rules_list))
                        else:
                            logger.debug("ANF validate_resync_quotarule: There are %s quota rules for this volume:", len(rules_list))
                            logger.warning("\nIf any quota rules exists on destination volume they will be overwritten with source volume's quota rules.")
                except:
                    raise ValidationError("\nDestination volume not found check remote_volume_resource_id: '{}'.".format(remoteVolumeResourceId))
            else:
                logger.warning("ANF validate_resync_quotarule: The remote_volume_resource_id %s is not a valid resourceId:", remoteVolumeResourceId)
    else:
