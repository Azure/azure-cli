...
# file paths to the built packages
built_packages = []
...
def build_package(path_to_package):
    ...
    cmd_success = exec_command('python setup.py sdist', cwd=path_to_package)
    if not cmd_success:
        print_heading('Error building {}!'.format(path_to_package), f=sys.stderr)
        sys.exit(1)
    p = os.path.join(path_to_package, 'dist')
    files_in_p = [f for f in os.listdir(p) if os.path.isfile(os.path.join(p, f))]
    built_packages.append(os.path.join(p, files_in_p[0]))
    print_heading('Built {}'.format(path_to_package))

def install_package(path_to_package, package_name, package_dir):
    print_heading('Installing {}'.format(path_to_package))
    cmd = 'python -m pip install {} --find-links file://{}'.format(package_name, package_dir)
...
build_package(PATH_TO_AZURE_CLI)
build_package(PATH_TO_AZURE_CLI_CORE)
for name, fullpath in all_command_modules:
    build_package(fullpath)

# tmp dir to store all the built packages
built_packages_dir = tempfile.mkdtemp()
# move all built packages to this dir
for package in built_packages:
    os.rename(package, os.path.join(built_packages_dir, os.path.basename(package)))

# STEP 3:: Install the CLI and dependencies
