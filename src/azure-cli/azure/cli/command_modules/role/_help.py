...
long-summary: |-
    When you create a service principal using this command, the output includes credentials that you must protect. Be sure that you do not include these credentials in your code or check the credentials into your source control. As an alternative, consider using managed identities (https://docs.microsoft.com/azure/active-directory/managed-identities-azure-resources/overview) to avoid the need to use credentials. 
    By default, `az ad sp create-for-rbac` assigns the Contributor role to the service principal at the subscription scope. To reduce your risk of a compromised service principal, use --skip-assignment to create the service principal and assign a more specific role and narrow the scope to a resource or resource group with `az role assignment create`. See https://docs.microsoft.com/azure/role-based-access-control/role-assignments-steps for more information.
parameters:
  - name: --name -n
