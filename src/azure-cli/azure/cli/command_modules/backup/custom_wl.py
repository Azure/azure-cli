...
def register_wl_container(cmd, client, vault_name, resource_group_name, workload_type, resource_id, container_type):
    ...
    for container in containers:
        if cust_help._get_resource_id(container.properties.container_id) == cust_help._get_resource_id(resource_id):
...
def list_protectable_items(cmd, client, resource_group_name, vault_name, workload_type, container_name=None,
    ...
    if container_name:
        ...
        if cust_help._is_native_name(container_name):
            ...
            container = show_wl_container(cmd, container_name, resource_group_name, vault_name)
...
def enable_protection_for_azure_wl(cmd, client, resource_group_name, vault_name, policy_name, protectable_item):
    ...
    policy_client = protection_policies_cf(cmd.cli_ctx)
    policy_object = show_wl_policy(policy_client, resource_group_name, vault_name, policy_name)[0]
    cust_help._validate_policy(policy_object)
...
def show_recovery_config(cmd, client, resource_group_name, vault_name, restore_mode, container_name, item_name, rp_name,
    ...
    if rp_name == 'DefaultRangeRecoveryPoint':
        recovery_points = list_wl_recovery_points(cmd, client, resource_group_name, vault_name, container_name, 
                                                  item_name, item_type, None, None, True)
        ...
        recovery_point = show_wl_recovery_point(cmd, client, resource_group_name, vault_name, container_name, 
