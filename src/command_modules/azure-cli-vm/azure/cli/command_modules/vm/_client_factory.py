...
def cf_vm_create(_):
    ...
    return get_mgmt_service_client(VMCreateClient).vm
...
def cf_ni(_):
    ...
    # is fixed.
    return get_mgmt_service_client(NetworkManagementClient, api_version='2016-03-30').network_interfaces

def cf_avail_set_create(_):
    from azure.cli.core.commands.client_factory import get_mgmt_service_client
    from azure.cli.command_modules.vm.mgmt_avail_set.lib import AvailSetCreationClient as AvailSetCreateClient
...
def cf_acs(_):
    return _compute_client_factory().container_service
