...
def initialize_replication_infrastructure(cmd,
    ...
    try:
        ...
        if not replication_extension:
            ...
            # Wait a bit to ensure previous operations have completed
            time.sleep(30)
            ...
            # Build the extension body with minimal required properties
            extension_body = {
                "properties": {
                    "customProperties": {
                        "instanceType": instance_type,
            ...
            try:
                ...
                if not replication_extension:
                    # Try alternative approach without optional properties
                    if "InvalidProperty" in error_str or "unknown property" in error_str.lower():
                        print("\nTrying simplified extension body...")
                        ...
                        try:
                            ...
                            print("Extension created with simplified body")
                            time.sleep(60)
                        except Exception as simple_error:
                            raise CLIError(f"Failed to create replication extension. Error: {str(simple_error)}")
                    else:
