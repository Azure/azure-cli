...
class BackupTests(ScenarioTest, unittest.TestCase):
    ...
    @AFSPolicyPreparer()
    @AFSItemPreparer()
    def test_afs_reconfigure_protection(self, resource_group, source_vault, target_vault, storage_account, afs_name, policy_name, item_name):
        """Test reconfiguring AFS backup protection from one vault to another."""
        self.kwargs.update({
            'rg': resource_group,
            ...
            'afs': afs_name,
            'policy': policy_name,
            'new_policy': 'DefaultAFSPolicy',  # Use default AFS policy in target vault
            'type': "AzureStorage"
        })

        # Trigger backup to create recovery points
        self.kwargs['retain_date'] = (datetime.utcnow() + timedelta(days=30)).strftime('%d-%m-%Y')
        self.kwargs['job'] = self.cmd('backup protection backup-now -g {rg} -v {source_vault} -c {container} -i {item} --retain-until {retain_date} --backup-management-type AzureStorage --query name').get_output_in_json()
        self.cmd('backup job wait -g {rg} -v {source_vault} -n {job}')

        # Verify item is protected in source vault
        self.cmd('backup item show -g {rg} -v {source_vault} -c {container} -i {item} --backup-management-type AzureStorage', checks=[
            self.check('properties.friendlyName', '{item}'),
            self.check('properties.protectionState', 'Protected')
        ])

        # Create default policy in target vault (AFS doesn't automatically have DefaultPolicy)
        default_policy_json = self.cmd('backup policy show -g {rg} -v {source_vault} -n {policy}').get_output_in_json()
        default_policy_json['name'] = self.kwargs['new_policy']
        self.kwargs['default_policy_json'] = json.dumps(default_policy_json)
        
        self.cmd("backup policy create -g {rg} -v {target_vault} --policy '{default_policy_json}' --name {new_policy} --backup-management-type AzureStorage")

        # Reconfigure protection to target vault without retaining recovery points
        reconfigure_job = self.cmd('backup protection reconfigure -g {rg} -v {source_vault} -c {container} -i {item} --backup-management-type AzureStorage --new-vault-name {target_vault} --new-vault-resource-group {rg} --new-policy-name {new_policy}', checks=[
            self.check("properties.operation", "ConfigureBackup"),
            self.check("resourceGroup", '{rg}')
        ]).get_output_in_json()
        ...
        self.cmd('backup item show -g {rg} -v {target_vault} -c {container} -i {item} --backup-management-type AzureStorage', checks=[
            self.check('properties.friendlyName', '{item}'),
            self.check('properties.protectionState', 'Protected')
        ])
        ...
        try:
            source_item = self.cmd('backup item show -g {rg} -v {source_vault} -c {container} -i {item} --backup-management-type AzureStorage').get_output_in_json()
        ...
        self.cmd('backup protection disable -g {rg} -v {target_vault} -c {container} -i {item} --backup-management-type AzureStorage --delete-backup-data true --yes')