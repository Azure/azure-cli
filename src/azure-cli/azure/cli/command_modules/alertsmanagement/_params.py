...
def load_arguments(self, _):
    ...
    custom_time_range_arg_type = CLIArgumentType(options_list=['--custom-time-range'],
                                                 help='Supported format - <start-time>/<end-time> where time is in ISO-8601 format',
    ...
    with self.argument_context('alertsmanagement alert list-summary') as c:
        c.argument('groupby', options_list=['--group-by'], required=True,
                   help='Supported values – severity, alertState, monitorCondition, monitorService, signalType, alertRule')
        c.argument('include_smart_groups_count', options_list=['--include-smart-groups-count'],
                   required=False,
                   help='Supported values - true or false')
        c.argument('target_resource', target_resource_arg_type, required=False)
        c.argument('target_resource_type', target_resource_type_arg_type, required=False)
        c.argument('target_resource_group', options_list=['-target-resource-group'], required=False,
                   help='Filter by resource group')
        c.argument('monitor_service', monitor_service_arg_type, required=False)
        c.argument('monitor_condition', monitor_condition_arg_type, required=False)
        c.argument('severity', severity_arg_type, required=False)
    ...
    with self.argument_context('alertsmanagement action-rule set') as c:
        ...
        c.argument('severity_condition', options_list=['--severity-condition'], required=False, help='Expected format - {<operation>:<comma separated list of values>} For eg. Equals:Sev0,Sev1')
        c.argument('monitor_service_condition', options_list=['--monitor-service-condition'], required=False, help='Expected format - {<operation>:<comma separated list of values>} For eg. Equals:Platform,Log Analytics')
        c.argument('monitor_condition', monitor_condition_arg_type, required=False)
        c.argument('target_resource_type_condition', options_list=['--target-resource-type-condition'], required=False, help='Expected format - {<operation>:<comma separated list of values>} For eg. Contains:Virtual Machines,Storage Account')
        c.argument('alert_ruleId_condition', options_list=['--alert-ruleId-condition'], required=False, help='Expected format - {<operation>:<comma separated list of values>} For eg. Equals:ARM_ID_1,ARM_ID_2')
